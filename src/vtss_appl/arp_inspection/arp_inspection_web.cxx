...
static i32 handler_config_arp_inspection(CYG_HTTPD_STATE *p)
{
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        if (cyg_httpd_form_varable_long_int(p, "arp_inspection_mode", &mode)) {
            if (arp_inspection_mgmt_conf_mode_set(&mode) != VTSS_RC_OK) {
        ...
        while (port_iter_getnext(&pit)) {
            ...
            (void) cyg_httpd_form_varable_long_int(p, search_str, &mode);
            port_mode_conf.log_type[pit.iport] = (arp_inspection_log_type_t)mode;


        }
        if (arp_inspection_mgmt_conf_port_mode_set(sid, &port_mode_conf) != VTSS_RC_OK) {
        ...
        if ((var_string = cyg_httpd_form_varable_find(p, "translateFlag")) != NULL) {
            ...
            if (var_value == 1) {
                if (arp_inspection_mgmt_conf_translate_dynamic_into_static() < VTSS_RC_OK) {
        ...
        */
        if (arp_inspection_mgmt_conf_mode_get(&mode) == VTSS_RC_OK) {
            ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%d,", mode);
            (void)cyg_httpd_write_chunked(p->outbuffer, ct);

            if (arp_inspection_mgmt_conf_port_mode_get(sid, &port_mode_conf) == VTSS_RC_OK) {
...
static i32 handler_config_arp_inspection_static_table(CYG_HTTPD_STATE *p)
{
    vtss_isid_t             sid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        for (entry_index = 1; entry_index <= ARP_INSPECTION_MAX_ENTRY_CNT; entry_index++) {
            ...
            if (cyg_httpd_form_varable_long_int(p, search_str, &entry.port_no) &&
                ...
                if (arp_inspection_mgmt_conf_static_entry_set(&entry) != VTSS_RC_OK) {
        ...
        */
        ...
        /* add the other entries which are stored into dynamic table */
        entry_cnt = entry_cnt + arp_inspection_mgmt_conf_dynamic_entry_count_get();
...
static i32 handler_stat_dynamic_arp_inspection(CYG_HTTPD_STATE *p)
{
    vtss_isid_t                     sid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    int                             ct;
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        */
        ...
        search_entry = entry;
        if (arp_inspection_mgmt_conf_port_mode_get(sid, &port_mode_conf) == VTSS_RC_OK) {
            while (arp_inspection_mgmt_conf_dynamic_entry_get(&search_entry, TRUE) == VTSS_RC_OK) {
...
static i32 handler_config_arp_inspection_vlan(CYG_HTTPD_STATE *p)
{
...
static i32 handler_config_arp_inspection_dynamic_table(CYG_HTTPD_STATE *p)
{
    vtss_isid_t             sid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    int                     ct;
    int                     entry_cnt = 0;
...
static size_t ARP_INSPECTION_info_lib_config_js(char **base_ptr, char **cur_ptr, size_t *length)
{
    char buff[WEB_BUF_LEN];

    (void) snprintf(buff, WEB_BUF_LEN,
                    "var configArpInspectionMaxRules = %u;\n"
                    , ARP_INSPECTION_MAX_ENTRY_CNT
                   );
...
web_lib_config_js_tab_entry(ARP_INSPECTION_info_lib_config_js);
