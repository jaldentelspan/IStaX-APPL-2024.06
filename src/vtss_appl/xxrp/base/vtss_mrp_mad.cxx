...
mesa_rc vtss_mrp_mad_create_port(vtss_mrp_t *appl, vtss_mrp_mad_t **mad_ports, u32 l2port, vtss::VlanList &vls)
{
    ...
    for (machine_index = 0; machine_index < (appl->max_mad_index + 1); machine_index++) {
        is_attr_registered = FALSE;
#ifdef VTSS_SW_OPTION_MVRP
        if (reg_admin_status[machine_index + 1] == VLAN_REGISTRATION_TYPE_FIXED) {
...
#ifdef VTSS_SW_OPTION_MVRP
    for (machine_index = 0; machine_index < (appl->max_mad_index + 1); machine_index++) {
        (void)vtss_mrp_reg_admin_status_set(VTSS_MRP_APPL_MVRP, l2port, machine_index, reg_admin_status[machine_index + 1]);
    }
#endif
