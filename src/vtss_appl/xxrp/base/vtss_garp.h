...
#undef GARP_MODIFIED_PROTOCOL_1
#undef GARP_MODIFIED_PROTOCOL_2

typedef size_t garp_typevalue_t;
typedef u8     garp_attribute_type_t;

#define GARP_NO_GIP_CONTEXTS  N_L2_MSTI_MAX
...
struct garp_gid_instance {
    ...
    vtss_tick_count_t leave_timeout;

    struct PN_participant leave_list;
    struct PN_participant vlan_membership_update;
 ...
 */
typedef struct garp_participant {
    ...
    struct PN_participant gip[GARP_NO_GIP_CONTEXTS + 1];
    u8                    gip_count;
    BOOL   port_msti_enabled[GARP_NO_GIP_CONTEXTS];
    //    u8     trace_gip_change[GARP_NO_GIP_CONTEXTS]; // 0=initial, i.e. always detect as change, 1=out, 2=in
...
#define IS_PARTICIPANT_IN_TXPDU_QUEUE(p) (p->transmitPDU_list.next ? TRUE : FALSE)
    ...
    vtss_tick_count_t     GARP_transmitPDU_timeout;
    struct PN_participant transmitPDU_list;


    vtss_tick_count_t     leaveAll_timeout;
    ...
    u32 leaveall_mask[0x1000 / 0x20];
#endif
} garp_participant_t;



typedef enum garp_attribute_event {
...
#define GVRP_SEND_JOIN_ON_PORT_ITSELF (1UL<<0)
        u32 flags;
#endif
    } iter__gip;
    ...
    struct {
        struct PN_participant *add;
        struct PN_participant *del;
    } vlan_membership;

    // --- From this point on, the structure is not set to zero in the
    //     constructor.

#define GARP_SCALE_UNIT (8)

    struct _timer_context {
        vtss_tick_count_t actual;
        vtss_tick_count_t scale; // 1= scale=2^GARP_SCALE_UNIT
        vtss_tick_count_t offset;
        u32               time;
...
typedef enum garp_applicant_state {
...
} garp_applicant_state_t;

extern const char *const applicant_state_name(enum garp_applicant_state S);
...
extern const char *const registrar_event_name(enum garp_registrar_event E);

// (2.2) --- Registrar SM events (IEEE802.1D-2004, table 12.4)
typedef enum garp_registrar_state {
...
} garp_registrar_state_t;

extern const char *const registrar_state_name(enum garp_registrar_state S);
...
extern const char *const leaveall_event_name(enum garp_leaveall_event S);

// (3.2) --- LeaveAll SM events (IEEE802.1D-2004, table 12.5)
typedef enum garp_leaveall_state {
    garp_leaveall_state__Active,
    garp_leaveall_state__Passive,
    garp_leaveall_state__MAX
} garp_leaveall_state_t;

extern const char *const leaveall_state_name(enum garp_leaveall_state S);
