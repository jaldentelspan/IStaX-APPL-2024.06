...
extern mesa_vid_t vtss_get_vid(struct garp_gid_instance *gid);
...
#if 1
#define T_SM_LOG(fmt, ...)  do { if ( 0 != vtss_gvrp_trace_vid && vtss_get_vid(gid) == vtss_gvrp_trace_vid) {printf("%s: " fmt "\n", __func__, ##__VA_ARGS__); } } while(0)
#else
#define T_SM_LOG(fmt, ...) do {} while(0)
#endif

mesa_vid_t vtss_gvrp_trace_vid = 0;
...
#undef ENABLE_GARPTT_CRITD
#ifdef ENABLE_GARPTT_CRITD
static critd_t                  garp_crit;                     // Critical section for GARP state machine
#define GARP_STATE_CRIT_ENTER() critd_enter(&garp_crit, __FILE__, __LINE__)
#define GARP_STATE_CRIT_EXIT()  critd_exit (&garp_crit, __FILE__, __LINE__)
#else
#define GARP_STATE_CRIT_ENTER() {}
#define GARP_STATE_CRIT_EXIT()  {}
#endif
...
static int applicant_action_state_change;
...
/*lint -sem(applicant_action__LeaveAll_VO, thread_protected) */
static void applicant_action__LeaveAll_VO(ACTION_SIGNATUR)
{
    // --- (ZyXEL) Applicant is in VO and got LeaveAll.
    //     If Registrar SM is in state IN or LV, then
    //     follow the standard. Else stay in VO
    if ( garp_registrar_state__IN == gid->registrar_state ||
         garp_registrar_state__LV == gid->registrar_state ) {

        // Follow standard, i.e., do nothing here.

    } else {

        // The Applicant is in VO and Regisrar is not in IN or LV,
        // so stay in VO.

        GARP_STATE_CRIT_ENTER();
        applicant_action_state_change = 0;
        GARP_STATE_CRIT_EXIT();
...
#ifdef GARP_MODIFIED_PROTOCOL_1
        APPLICANT_SM_ELEMENT(LO, LeaveAll_VO),
#else
        APPLICANT_SM_ELEMENT(LO, NoAction),
#endif
...
static void registrar_action__StartLeaveTimer(ACTION_SIGNATUR)
{
    ...
    vtss_garp_reinsert_list(&a->GARP_timer_list_leave, &gid->leave_list);
}
...
#ifdef GARP_MODIFIED_PROTOCOL_1
static int test_leaveAll_dependency(int a, int r)
{
    return ( (a == garp_applicant_state__VO || a == garp_applicant_state__VP ) &&
             (r != garp_registrar_state__IN && r != garp_registrar_state__LV) ) ? 1 : 0;
}
#endif
...
// (4.2) --- Top handler
static mesa_rc vtss_garp_gidtt_event_handler3(struct garp_application  *a,
                                              ...
                                              const struct garp_gid_SM_events  *events)
{
    ...
    if (events->applicant < garp_applicant_event__MAX) {
        int  applicant_old_state;

        T_SM_LOG("(1) --- Applicant:");

        applicant_old_state = gid->applicant_state;

        // (1.4) --- Call action
        GARP_STATE_CRIT_ENTER();
        applicant_action_state_change = 1;
        GARP_STATE_CRIT_EXIT();
        garp_applicant_SM[gid->applicant_state][events->applicant].action(a, p, gid);

        // (1.5) --- Change state
        GARP_STATE_CRIT_ENTER();
        if (applicant_action_state_change) {
            gid->applicant_state = garp_applicant_SM[gid->applicant_state][events->applicant].next_state;
        }
        GARP_STATE_CRIT_EXIT();

        T_SM_LOG("(1.3) --- Applicant State change: %s-(%s)->%s, port=%d, vid=%d",
                 applicant_state_name((enum garp_applicant_state)applicant_old_state),
                 applicant_event_name((enum garp_applicant_event)events->applicant),
                 applicant_state_name((enum garp_applicant_state)gid->applicant_state),
                 p->gid_index, vtss_get_vid(gid));


        // (1.1) --- Check if transmitPDU list shall be updated for this SM
        if (change_transmitPDU_list(applicant_old_state, gid->applicant_state)) {

            // Add to or remove from the transmitPDU list
            if (testA(applicant_old_state)) {
                T_SM_LOG("(2.1) --- Add to txPDU list");
                APPLICANT_ADD_TO_LIST(a, p, gid, transmitPDU);
            } else {
                T_SM_LOG("(2.2) --- Remove to txPDU list");
        ...
        if (change_applicant_VO_LO(applicant_old_state, gid->applicant_state)) {
            if (testA3(applicant_old_state)) {
                // Moved out of {VO,LO} set
                a->ref_count(p, gid, 1);
                T_SM_LOG("(2.3) --- Moved out of {VO,LO} set => Ref count up");
            } else {
                // Moved into {VO,LO} set
                a->ref_count(p, gid, -1);
                T_SM_LOG("(2.4) --- Moved into {VO,LO} set => Ref count down");
            }
        }
    ...
    if (events->registrar < garp_registrar_event__MAX) {
        ...
        T_SM_LOG("(3.1) --- Registrar State change: %s-(%s)->%s, port=%d, vid=%d",
                 registrar_state_name((enum garp_registrar_state)gid->registrar_state),
                 registrar_event_name((enum garp_registrar_event)events->registrar),
                 registrar_state_name((enum garp_registrar_state)garp_registrar_SM[gid->registrar_state][events->registrar].next_state),
                 p->gid_index, vtss_get_vid(gid));
        ...
        if (change_registrar_MT(gid->registrar_state, garp_registrar_SM[gid->registrar_state][events->registrar].next_state)) {
            if (testR2(gid->registrar_state)) {
                // Moved out of {MT} set
                a->ref_count(p, gid, 1);
                T_SM_LOG("(3.2) --- Moved out of {MT} set => Ref count up");
            } else {
                // Moved into {MT} set
                a->ref_count(p, gid, -1);
                T_SM_LOG("(3.3) --- Moved into {MT} set => Ref count down");
            }
        }
...
#ifdef GARP_MODIFIED_PROTOCOL_1
    // (4) --- Calculate flag that indicate whether LeaveAll event will have any effect
    //         on this Applicant, Registrar pair.
    if (gid != (struct garp_gid_instance *)1) {
        if (test_leaveAll_dependency(gid->applicant_state, gid->registrar_state)) {
            T_SM_LOG("(4) --- Set LeaveAll mask for (port, vid)=(%d, %d)", p->gid_index, vtss_get_vid(gid));
            p->leaveall_mask[vtss_get_vid(gid) >> 5] |= (1UL << (vtss_get_vid(gid) & 0x1f) );
        } else {
            T_SM_LOG("(4) --- Clear LeaveAll mask for (port, vid)=(%d, %d)", p->gid_index, vtss_get_vid(gid));
            p->leaveall_mask[vtss_get_vid(gid) >> 5] &= ~(1UL << (vtss_get_vid(gid) & 0x1f) );
        }
    }
#endif

    // (5) --- Handle leaveall SM */
    if (events->leaveall < garp_leaveall_event__MAX) {

        T_SM_LOG("(5.1) --- LeaveAll State change: %s-(%s)->%s", leaveall_state_name((enum garp_leaveall_state)p->leaveall_state),
                 leaveall_event_name((enum garp_leaveall_event)events->leaveall),
                 leaveall_state_name((enum garp_leaveall_state)garp_leaveall_SM[p->leaveall_state][events->leaveall].next_state));
...
mesa_rc vtss_garp_gidtt_event_handler2(struct garp_application *a,
...
mesa_rc vtss_garp_gidtt_event_handler(struct garp_application *a,
                                      struct garp_participant  *participant,
                                      struct garp_gid_instance *gid,
                                      enum garp_all_event event)
{
...
mesa_rc vtss_garp_gidtt_rx_pdu_event_handler(struct garp_application *a,
                                             struct garp_participant *p,
                                             struct garp_gid_instance *gid,
                                             enum garp_attribute_event attribute_event)
{
    static const enum garp_all_event map[garp_attribute_event__MAX] = {
 ...
 */
mesa_rc vtss_gid_join_request(struct garp_application  *a,
                              struct garp_participant  *p,
 ...
 */
mesa_rc vtss_gid_leave_request(struct garp_application *a,
                               struct garp_participant *p,
                               garp_attribute_type_t    type,
                               void                    *value)
{
...
{
    static const enum garp_all_event M[3] = {
        garp_event__Normal,
        garp_event__Fixed,
 ...
 */
static vtss_tick_count_t map_lt2rt(struct garp_application *a, enum timer_context tc, vtss_tick_count_t lt)
{
    vtss_tick_count_t rt;

    if (!a->GARP_timer[tc].scale) {
        T_E("a->GARP_timer[%d].scale=" VPRI64u, tc, a->GARP_timer[tc].scale);
    }
...
{
    mesa_rc rc;
    vtss_tick_count_t t, X, tick_start = 0, next_time = 0;
    ...
    T_N("Enter GARP timer tick");
    tick_start = vtss_current_time();
    do {
        ...
        if ((pn = a->GARP_timer_list_leave)) {
            ...
            do {
                ...
                if (t - gid->leave_timeout < GARP_POSITIVE_U32) {
                    ...
                    //?tf?
                    a->ref_count(p, gid, +1);
                    rc = vtss_garp_gidtt_event_handler2(a, p, gid, garp_event__leavetimer);
                    p->done_gid(p, gid);
                    if (rc) {
                        T_E("rc=%d", rc);
                    }
        ...
        if (a->GARP_timer_list_transmitPDU) {
            ...
            is_next_time_set = 1;
            T_N("(4.1) --- Calculate when next time tick/transmitPDU next_time=" VPRI64u"", next_time);

        }

        t = vtss_current_time();
        T_N1("E.4 t=" VPRI64u", next_time=" VPRI64u", next_time-t=" VPRI64u"", t, next_time, next_time - t);
        ...
        if (a->GARP_timer_list_leaveAll) {
            ...
            if (!is_next_time_set || next_time - X < GARP_POSITIVE_U32)  {
                next_time = X;
                is_next_time_set = 1;
                T_N("(4.2) --- Calculate when next time tick/leaveall next_time=" VPRI64u"", next_time);
            }
        }
        ...
        if (a->GARP_timer_list_leave) {
            ...
            if (!is_next_time_set || next_time - X < GARP_POSITIVE_U32) {
                next_time = X;
                is_next_time_set = 1;
                T_N("(4.3) --- Calculate when next time tick/leave next_time=" VPRI64u"", next_time);
            }
        ...
        T_N("E.5 D= %d, t=" VPRI64u, D, t);

        /* If the CPU is too stressed, it is possible that we stay in this loop
         * indefinitely. This loop is processing all the events in the list that
         * are due already, i.e. their timers had already expired when we entered
         * the loop. When this is finished, the delay until the next timer expires
         * is calculated and returned to the timer thread.
         * Now, if the processing takes more time than the time to the next timer
         * expiration (hence the delay is a negative value) this loop will keep
         * processing events from the list until we have a positive delay.
         * After a given CPU stress threshold, the processing time of events will
         * always be longer than the time to the next timer expiration and therefore
         * we will stay in the loop indefinitely since we can never catch up.
         * So, if we detect that we have stayed in the loop for too long, we
         * forcibly exit the loop and leave events unprocessed in order to unlock
         * the mutex for other threads. We then return a delay of 1 tick, so that
         * we return into processing as soon as other pending threads have been
         * served. The "too long" threshold is a percentage of the default maximum
         * lock time of a mutex, 'CRITD_MAX_LOCK_TIME_DEFAULT'.
         *
         * Furthermore, if the CPU stress persists we can expect the GVRP protocol to
         * stop operating correctly because it cannot provide timely response to its
         * partners. The user should be notified of this condition, thus we also emit
         * proper messages to both 'syslog' and CLI traces.
         * When in this condition, it might be possible to reduce the CPU load that
         * GVRP itself is creating by increasing the configured timer timeout values,
         * e.g. the 'LeaveAll' timer timeout can be increased from the default of 10sec
         * to a max of 50sec. That will reduce the GVRP events generated, decrease the
         * CPU load and hopefully be enough to allow the protocol to catch up on event
         * processing.
         */
        T_N("Has spent " VPRI64u" msec in the loop", VTSS_OS_TICK2MSEC(vtss_current_time() - tick_start));
        if (VTSS_OS_TICK2MSEC(vtss_current_time() - tick_start) > (CRITD_MAX_LOCK_TIME_DEFAULT * 1000 * 9 / 10)) {
#ifdef VTSS_SW_OPTION_SYSLOG
            S_W("GVRP: Protocol is not operating properly due to high CPU load. "
                "Try increasing the configured GVRP timer timeouts to reduce the load towards the CPU.");
#endif
            T_W("GVRP: Protocol is not operating properly due to high CPU load. "
                "Try increasing the configured GVRP timer timeouts to reduce the load towards the CPU.");
            D = 1;
            break;
        }
    ...
    T_N("Next timer at  D=%u. Processing of events took " VPRI64u" msecs", D, VTSS_OS_TICK2MSEC(vtss_current_time() - tick_start));
...
static void vtss_garp_change_clock_scale(struct garp_application *a, enum timer_context tc, vtss_tick_count_t scale, u32 time)
{
    vtss_tick_count_t t, t2;

    t = vtss_current_time();

    t2 = ( ( (t - a->GARP_timer[tc].actual) * a->GARP_timer[tc].scale ) >> GARP_SCALE_UNIT) + a->GARP_timer[tc].offset;

    T_N1("Current: actual=" VPRI64u", offset=" VPRI64u", scale=" VPRI64u"", a->GARP_timer[tc].actual, a->GARP_timer[tc].offset, a->GARP_timer[tc].scale);
    ...
    T_N1("New    : actual=" VPRI64u", offset=" VPRI64u", scale=" VPRI64u"", a->GARP_timer[tc].actual, a->GARP_timer[tc].offset, a->GARP_timer[tc].scale);

}


// (6.2) --- Set the timeout value 'new_time' for timer 'tc' within application 'a'.
mesa_rc vtss_garp_set_timer(struct garp_application *a, enum timer_context tc, u32 new_time, int full)
{
    vtss_tick_count_t scale;
...
#ifdef ENABLE_GARPTT_CRITD
    critd_init(&garp_crit, "garptt", VTSS_MODULE_ID_XXRP, CRITD_TYPE_MUTEX);
#endif
}

const char *const registrar_state_name(enum garp_registrar_state S)
{
    static const char *const N[] = {"MT", "IN", "LV",
                                    "Fixed", "Forbidden"
                                   };
    return N[S];
}

const char *const leaveall_state_name(enum garp_leaveall_state S)
{
    static const char *const N[] = {"Active ", "Passive"};
    return N[S];
}

const char *const applicant_state_name(enum garp_applicant_state S)
{
    static const char *const N[] = {"VO", "VA", "AA", "QA", "LA", "VP", "AP", "QP", "AO", "QO", "LO"};
    return N[S];
}

const char *const applicant_event_name(enum garp_applicant_event E)
{
    static const char *const N[] = {"txPDU", "rJoinIn", "rJoinEmpty", "rEmpty", "rLeaveIn", "rLeaveEmpty", "LeaveAll", "ReqJoin", "ReqLeave", "Initialize"};
    return N[E];
}

const char *const registrar_event_name(enum garp_registrar_event E)
{
    static const char *const N[] = {"rJoinIn", "rJoinEmpty", "rEmpty", "rLeaveIn", "rLeaveEmpty", "LeaveAll", "leavetimer", "Initialize",
                                    "Normal", "Fixed", "Forbidden"
                                   };
    return N[E];
}

const char *const leaveall_event_name(enum garp_leaveall_event E)
{
    static const char *const N[] = {"txPDU", "LeaveAll", "leavealltimer"};
    return N[E];
}
...
// (8.5) --- less function used in list function (8.2).
...
#if 0
// (8.6) --- Remove element 'p' in the end of the list 'P'
...
// (8.7) --- Element 'p' is in either list 'P' or list 'Q' or in no list. Move it to 'P'.
int vtss_garp_move_list(struct PN_participant **P, struct PN_participant **Q, struct PN_participant *p)
{
    int rc = 1;

    // --- It is not necessary to check if 'p' is in a list
    //     but since this is expected to be the common case
    //     it might just save us some work.
    if ( p->next ) {
        if (*P == p) {
            vtss_garp_remove_list(P, p);
        } else {
            vtss_garp_remove_list(Q, p);
        }
        rc = 0;
    }

    if (p->next || p->prev) {
        T_E("%p(%p, %p), *P = %p, *Q = %p", p, p->next, p->prev, *P, *Q);
        p->next = p->prev = NULL;
    }
