...
static i32 handler_config_dhcp_snooping(CYG_HTTPD_STATE *p)
{
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        if (dhcp_snooping_mgmt_conf_get(&conf) != VTSS_RC_OK ||
            dhcp_snooping_mgmt_port_conf_get(sid, &port_conf) != VTSS_RC_OK) {
        ...
        if (vtss_memcmp(port_newconf, port_conf) != 0) {
...
static i32 handler_stat_dhcp_snooping_statistics(CYG_HTTPD_STATE *p)
{
    vtss_isid_t             sid = web_retrieve_request_sid(p); /* Includes USID = ISID */
...
static i32 handler_stat_dynamic_dhcp_snooping(CYG_HTTPD_STATE *p)
{
    int                                 ct;
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        */
        search_entry = entry;
        if (!dyn_get_next_entry && search_entry.vid != 0) {
            search_entry.vid--;
        }
        do {
