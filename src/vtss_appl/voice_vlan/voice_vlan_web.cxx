...
static i32 handler_conf_voice_vlan(CYG_HTTPD_STATE *p)
{
    ...
    if (voice_vlan_mgmt_conf_get(&voice_vlan_conf) != VTSS_RC_OK) {
        return -1;
    }
    if (voice_vlan_mgmt_port_conf_get(sid, &voice_vlan_port_conf) != VTSS_RC_OK) {
        return -1;
    }
#ifdef VTSS_SW_OPTION_LLDP
    if ((sid != VTSS_ISID_START) || (lldp_mgmt_conf_get(&lldp_conf[0]) != VTSS_RC_OK)) {
        return -1;
    }
#endif
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        if ((rc = VOICE_VLAN_is_valid_voice_vid(voice_vlan_newconf.vid)) != VTSS_RC_OK) {
        ...
        if (memcmp(&voice_vlan_newconf, &voice_vlan_conf, sizeof(voice_vlan_newconf)) != 0) {
            ...
            if ((rc = voice_vlan_mgmt_conf_set(&voice_vlan_newconf)) != VTSS_RC_OK) {
...
#ifdef VTSS_SW_OPTION_LLDP
                    && (lldp_conf[pit.iport].admin_states == VTSS_APPL_LLDP_DISABLED || lldp_conf[pit.iport].admin_states == VTSS_APPL_LLDP_ENABLED_TX_ONLY)
#endif
                   ) {
                    char err[64];
        ...
        if (vtss_memcmp(voice_vlan_port_newconf, voice_vlan_port_conf) != 0) {
            T_D("Calling voice_vlan_mgmt_port_conf_set()");
            if ((rc = voice_vlan_mgmt_port_conf_set(sid, &voice_vlan_port_newconf)) != VTSS_RC_OK) {
                if (rc == VOICE_VLAN_ERROR_IS_CONFLICT_WITH_LLDP) {
                    char err[64];
                    sprintf(err, "The LLDP feature should be enabled first");
                    send_custom_error(p, err, err, strlen(err));
        ...
        */
...
#ifdef VTSS_SW_OPTION_MVR
        {
            // What is this good for? Previously, the first MVR VLAN was put in this
            // one, so let's do the same with the new implementation of MVR.
            vtss_appl_ipmc_lib_vlan_key_t vlan_key;
            vlan_key.is_mvr  = true; // Get an MVR VLAN
            vlan_key.is_ipv4 = true; // Doesn't matter
            vlan_key.vid     = 0;    // Get first MVR VLAN.
            if (vtss_appl_ipmc_lib_vlan_itr(&vlan_key, &vlan_key, true /* stay in key.is_mvr and key.is_ipv4 */) != VTSS_RC_OK) {
                vlan_key.vid = 0;
            }

            ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "%u,", vlan_key.vid);
        }
#else
        ct = snprintf(p->outbuffer, sizeof(p->outbuffer), "0,");
#endif
        (void)cyg_httpd_write_chunked(p->outbuffer, ct);
        while (port_iter_getnext(&pit)) {
            if (vlan_mgmt_port_conf_get(sid, pit.iport, &vlan_port_conf, VTSS_APPL_VLAN_USER_VOICE_VLAN, TRUE) != VTSS_RC_OK) {
                continue;
            }
#ifdef VTSS_SW_OPTION_LLDP
...
#ifdef VTSS_SW_OPTION_LLDP
                          (lldp_conf[pit.iport].admin_states == VTSS_APPL_LLDP_ENABLED_RX_TX || lldp_conf[pit.iport].admin_states == VTSS_APPL_LLDP_ENABLED_RX_ONLY) ? 1 : 0,
#endif
...
static i32 handler_conf_voice_vlan_oui(CYG_HTTPD_STATE *p)
{
    int                     ct, idx;
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        for (idx = 1; idx <= VOICE_VLAN_OUI_ENTRIES_CNT; idx++) {
            ...
            if (cyg_httpd_form_varable_oui(p, search_str, entry.oui_addr)) {
                ...
                entry.valid = 1;
                if (voice_vlan_oui_entry_add(&entry) != VTSS_RC_OK) {
