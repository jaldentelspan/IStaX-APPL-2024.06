...
#define FIELD_U8(NAME)                                                         \
    u8 NAME() const {                                                          \
        return *(lower_layer_.payload() + offset_ ## NAME);                    \
    }                                                                          \
    void NAME(u8 v) {                                                          \
        *(lower_layer_.payload() + offset_ ## NAME) = v;                       \
    }

#define FIELD_U16(NAME)                                                        \
    u16 NAME() const {                                                         \
        return read_uint16(lower_layer_.payload() + offset_ ## NAME);          \
    }                                                                          \
    void NAME(u16 v) {                                                         \
        write_uint16(v, lower_layer_.payload() + offset_ ## NAME);             \
    }

#define FIELD_MAC(NAME)                                                        \
    Mac_t NAME() const {                                                       \
        return Mac_t(lower_layer_.payload() + offset_ ## NAME);                \
    }                                                                          \
    void NAME(const Mac_t& v) {                                                \
        v.copy_to(lower_layer_.payload() + offset_ ## NAME);                   \
    }

#define FIELD_IPV4(NAME)                                                       \
    IPv4_t NAME() const {                                                      \
        return IPv4_t(read_uint32(lower_layer_.payload() + offset_ ## NAME));  \
    }                                                                          \
    void NAME(const IPv4_t& v) {                                               \
        write_uint32(v.as_int(), lower_layer_.payload() + offset_ ## NAME);    \
    }
...
inline u16 inet_chksum(u32 sum, const u16 * buf, int length)
{
    while (length > 1) {
...
template<class LL>
struct ArpFrame : public StackableFrameAdaptor<ArpFrame<LL>> {
    typedef typename meta::add_const<LL>::type const_ll_t;
    typedef StackableFrameAdaptor<ArpFrame<LL>> BASE;
    typedef typename StackableFrameAdaptor<ArpFrame<LL>>::iterator iterator;
    typedef typename StackableFrameAdaptor<ArpFrame<LL>>::const_iterator const_iterator;

    static constexpr uint32_t operation_request = 1;
    static constexpr uint32_t operation_reply = 2;

    static constexpr uint32_t offset_hardware_type = 0;
    static constexpr uint32_t offset_protocol_type = 2;
    static constexpr uint32_t offset_hardware_addr_length = 4;
    static constexpr uint32_t offset_protocol_addr_length = 5;
    static constexpr uint32_t offset_operation = 6;
    static constexpr uint32_t offset_sender_hardware_address = 8;
    static constexpr uint32_t offset_sender_protocol_address = 14;
    static constexpr uint32_t offset_target_hardware_address = 18;
    static constexpr uint32_t offset_target_protocol_address = 24;
    static constexpr uint32_t HEADER_SIZE = 28;

    ArpFrame(LL & ll) : lower_layer_(ll) { }
    unsigned header_size() const { return HEADER_SIZE; }
    unsigned payload_length() const { return 0; }
    void payload_length(unsigned l) { }
    unsigned total_length() const { return HEADER_SIZE; }

    FIELD_U16(hardware_type);
    FIELD_U16(protocol_type);
    FIELD_U8(hardware_addr_length);
    FIELD_U8(protocol_addr_length);
    FIELD_U16(operation);
    FIELD_MAC(sender_hardware_address);
    FIELD_IPV4(sender_protocol_address);
    FIELD_MAC(target_hardware_address);
    FIELD_IPV4(target_protocol_address);

    void operation_request_set() { operation(operation_request); }
    bool operation_request_get() const {
        return operation() == operation_request;
    }

    void operation_reply_set() { operation(operation_reply); }
    bool operation_reply_get() const {
        return operation() == operation_reply;
    }

    bool check() const {
        if (BASE::max_size() < total_length()) return false;
        if (hardware_type() != 1) return false;
        if (protocol_type() != 0x0800) return false;
        if (hardware_addr_length() != 6) return false;
        if (protocol_addr_length() != 4) return false;
...
struct IpFrame : public StackableFrameAdaptor<IpFrame<LL> >
{
    ...
    bool is_simple() const {
        // In DHCP the renew messages are unicast messages exchanged directly between client
        //  and server. The ack messages sent from a linux ISC DHCP server have the
        //  don't fragment bit set.
        return (((*(lower_layer_.payload() + 6)) == 0 || 
                 (*(lower_layer_.payload() + 6)) == 0x40) && // don't fragment
    ...
    bool check() const {
        ...
        // We can not expect this to be exact match as padding may occure
    ...
    u32 udp_checksum_contribution() const {
        ...
        sum += *p++; sum += *p++; // src
        sum += *p++; sum += *p++; // dst
        sum += htons(protocol() + total_length() - header_size());
        return sum;
    }
...
std::ostream& operator<<(std::ostream& o, const EthernetFrame<T>& e) {
    ...
    o << "dst:" << e.dst() << " src:" << e.src() << " type:0x" <<
        hex << setw(4) << setfill('0') << e.etype() << " length:" <<
...
std::ostream& operator<<(std::ostream& o, const IpFrame<T>& i) {
    ...
    o << "IPv4> dst:" << i.dst() << " src:" << i.src() << " proto:" <<
        (int) i.protocol() << " length:" << i.payload_length();
    return o;
}
...
std::ostream& operator<<(std::ostream& o, const UdpFrame<T>& i) {
    using namespace std;
    o << "UDP> src:" << i.src() << " dst:" << i.dst() << " length:" <<
        i.payload_length();
    return o;
}
#endif
