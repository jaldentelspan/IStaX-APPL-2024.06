...
static inline Mac_t build_mac_broadcast()
{
    u8 tmp [] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
    return Mac_t(tmp);
}

inline Mac_t build_mac_zero()
{
    u8 tmp [] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
    return Mac_t(tmp);
}
...
enum Code {
    ...
    CODE_VENDOR_SPECIFIC_INFORMATION             = 43,
...
template <u8 CODE>
struct StringBase : public Base<CODE, 257>
{
    StringBase() {
        Base<CODE, 257>::data_[1] = 0;
    }

    bool set(const char *data, size_t length) {
        return set((const u8 *)data, length);
    }

    bool set(const u8 *data, size_t length) {
        u8 cnt = 0;

        if (length + 2 > 257) { //2^8-1=255 is the max; cause len field of option is 8-bit wide
            return false;
        }

        for (cnt = 0; cnt < length; ) {
            Base<CODE, 257>::data_[cnt + 2] = *data++;
            ++cnt;
        }
        Base<CODE, 257>::data_[1] = cnt;

        return true;
    }

    u16 length() const {
        return Base<CODE, 257>::data_[1] + 2;
    }

    const u8 * data() const {
        return Base<CODE, 257>::data_ + 2;
    }

    size_t size() const {
        return Base<CODE, 257>::data_[1];
    }
};
...
struct VendorSpecificInformation :
    public StringBase<CODE_VENDOR_SPECIFIC_INFORMATION> {};
...
//       | 67  |  n  |  c1 |  c2 |  c3 | ...
//       +-----+-----+-----+-----+-----+---
struct BootfileName : public StringBase<CODE_BOOTFILE_NAME> { };
...
//    |  60 |  n  |  i1 |  i2 | ...
//    +-----+-----+-----+-----+---
struct VendorClassIdentifier :
    public StringBase<CODE_VENDOR_CLASS_IDENTIFIER> { };
