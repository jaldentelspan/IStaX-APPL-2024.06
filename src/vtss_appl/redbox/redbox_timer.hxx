...
#ifndef _REDBOX_TIMER_HXX_
#define _REDBOX_TIMER_HXX_

#include "main_types.h"

struct redbox_timer_s;
typedef void (*redbox_timer_callback_t)(struct redbox_timer_s &timer, const void *context);

typedef struct redbox_timer_s {
    // Function to call upon timeout
    redbox_timer_callback_t callback;

    // User-defined context passed back in the callback
    const void *context;

    // A name for this timer (only used in trace output and 'debug redbox timers')
    const char *name;
    ...
    struct redbox_timer_s *prev;

    // Points to next active timer. NULL if last or this timer is not active.
    struct redbox_timer_s *next;
} redbox_timer_t;

// User operations.
// RedBox mutex must be taken upon calls to these functions, and it is already
// taken upon callback.
