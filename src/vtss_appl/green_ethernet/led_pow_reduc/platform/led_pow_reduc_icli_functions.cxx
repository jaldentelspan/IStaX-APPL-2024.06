...
void led_pow_reduc_icli_led_interval(u32 session_id, icli_unsigned_range_t *led_interval_list, u8 intensity,  BOOL no)
{
  ...
  for (element_index = 0; element_index < cnt; element_index++) {
    ...
    if (min == max) {
      ICLI_PRINTF("Please specify both start and end time. If the interval is all 24 hours the interval shall be specified as 0-24.\n");
      return;
    }
...
void led_pow_reduc_icli_on_event(u32 session_id, BOOL has_link_change, u16 v_0_to_65535, BOOL has_error, BOOL no)
{
  ...
  mesa_rc rc;
  if ((rc = led_pow_reduc_mgmt_set_switch_conf(&led_local_conf)) != VTSS_RC_OK) {
    T_E("%s", error_txt(rc));
  }
}

#ifdef VTSS_SW_OPTION_ICFG
static mesa_rc led_pow_reduc_icfg_print_led_intensity(u8 start_index, u8 end_index, u8 intensity, vtss_icfg_query_result_t *result)
{
...
static mesa_rc led_pow_reduc_global_conf(const vtss_icfg_query_request_t *req,
