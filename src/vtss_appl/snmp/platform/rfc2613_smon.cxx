...
{
    return (smonVlanStatsControlTable_entry_t *)vtss_free_list_malloc(&smon_vlan_counter_free_list);
}
...
static i32 vlan_stats_ctrl_entry_compare_func(void *elm1, void *elm2)
{
    smonVlanStatsControlTable_entry_t *in_list = (smonVlanStatsControlTable_entry_t *)elm1;
    smonVlanStatsControlTable_entry_t *new_entry = (smonVlanStatsControlTable_entry_t *)elm2;
...
{
    return (portCopyTable_entry_t *)vtss_free_list_malloc(&smon_port_copy_free_list);
}
...
static i32 portCpy_entry_compare_func(void *elm1, void *elm2)
{
    portCopyTable_entry_t *in_list = (portCopyTable_entry_t *)elm1;
    portCopyTable_entry_t *new_entry = (portCopyTable_entry_t *)elm2;
...
//static BOOL is_rmirror_mode(void)
//{
//    rmirror_switch_conf_t conf;
//    if (rmirror_mgmt_is_rmirror_enabled()) {
//        (void) rmirror_mgmt_conf_get( &conf);
//        if (conf.type != RMIRROR_SWITCH_TYPE_DEFAULT ) {
//            return TRUE;
//        }
//    }
//    return FALSE;
...
//#ifdef  VTSS_SW_OPTION_RMIRROR
//        if ( FALSE == is_rmirror_mode()) {
//            /* portCopy capability is all 0s if RMIRROR is enabled */
...
static BOOL apply_smonVlanStatsControlTableEntry (smonVlanStatsControlTable_entry_t *table_entry)
{
    ...
    (void) mesa_vlan_counters_clear(NULL, datasource_2_vlanid(table_entry->smonVlanStatsControlDataSource, table_entry->smonVlanStatsControlDataSource_len));
    if (snmp_mgmt_smon_stat_entry_set(&stored_entry) != VTSS_RC_OK) {
        return FALSE;
    }
    return TRUE;
}
...
static BOOL deactive_smonVlanStatsControlTableEntry (smonVlanStatsControlTable_entry_t *table_entry)
{
    ...
    stored_entry.ctrl_index = table_entry->smonVlanStatsControlIndex;
    if (snmp_mgmt_smon_stat_entry_get(&stored_entry, FALSE) != VTSS_RC_OK) {
        return TRUE;
    } else {
                              ...
                              WriteMethod **write_method)
{
    ...
    *write_method = NULL;
    ...
    if (!fast_cap(VTSS_APPL_CAP_VLAN_COUNTERS)) {
        return NULL;
    } else if ((rc = parse_smonVlanStatsControlTable(name, length, exact, &table_entry)) < 0) {
        return NULL;
    } else if (rc > 0) { /* getfirst */
...
static BOOL get_smonVlanIdStatsTableEntryByKey (smonVlanIdStatsTable_entry_t *entry)
{
    ...
    if (mesa_vlan_counters_get(NULL, entry->smonVlanIdStatsId, &counter) != VTSS_RC_OK) {
        return FALSE;
    }

#define XFER_CTRS(first, last, total)                                 \
    do {                                                              \
        vtss_snmp_u64_to_counter64(&entry->first##HC##last, (total)); \
        entry->first##last           = entry->first##HC##last.low;    \
        entry->first##Overflow##last = entry->first##HC##last.high;   \
    } while (0)

    XFER_CTRS(smonVlanIdStatsTotal,  Pkts,   counter.rx_vlan_unicast.frames   + counter.rx_vlan_multicast.frames + counter.rx_vlan_broadcast.frames);
    XFER_CTRS(smonVlanIdStatsNUcast, Pkts,   counter.rx_vlan_multicast.frames + counter.rx_vlan_broadcast.frames);
    XFER_CTRS(smonVlanIdStatsTotal,  Octets, counter.rx_vlan_unicast.bytes    + counter.rx_vlan_multicast.bytes  + counter.rx_vlan_broadcast.bytes);
    XFER_CTRS(smonVlanIdStatsNUcast, Octets, counter.rx_vlan_multicast.bytes  + counter.rx_vlan_broadcast.bytes);
                         ...
                         WriteMethod **write_method)
{
    ...
    *write_method = NULL;
    ...
    if (!fast_cap(VTSS_APPL_CAP_VLAN_COUNTERS)) {
        return NULL;
    } else if ((rc = parse_smonVlanIdStatsTable(name, length, exact, &table_entry)) < 0) {
        return NULL;
    } else if (rc > 0) { /* getfirst */
...
void smon_create_stat_default_entry(void)
{
    ...
    while (snmp_mgmt_smon_stat_entry_get(&entry, TRUE) == VTSS_RC_OK) {
                              ...
                              WriteMethod **write_method)
{
    ...
    *write_method = NULL;
    ...
    if (!fast_cap(VTSS_APPL_CAP_VLAN_COUNTERS)) {
        return NULL;
    } else if ((rc = parse_smonPrioStatsControlTable(name, length, exact, &table_entry)) < 0) {
        return NULL;
    } else if (rc > 0) { /* getfirst */
                       ...
                       WriteMethod **write_method)
{
    ...
    *write_method = NULL;
    ...
    if (!fast_cap(VTSS_APPL_CAP_VLAN_COUNTERS)) {
        return NULL;
    } else if ((rc = parse_smonPrioStatsTable(name, length, exact, &table_entry)) < 0) {
        return NULL;
    } else if (rc > 0) { /* getfirst */
...
/* get the disabled  mirror session */
static BOOL get_portCopy_config(u16 *const session_id, BOOL *const enable,
                                vtss::PortListStackable *const source_pls_rx,
                                vtss::PortListStackable *const source_pls_tx,
                                vtss::PortListStackable *const destination_pls)
{
    u16 *ptr = NULL, tmp_session;
    vtss_appl_mirror_session_entry_t tmp_config, config, *pcfg = NULL;

    memset(&tmp_config, 0, sizeof(tmp_config));
    memset(&config, 0, sizeof(config));

    while (VTSS_RC_OK == vtss_appl_mirror_session_entry_itr(ptr, &tmp_session)) {
        (void) vtss_appl_mirror_session_entry_get(tmp_session, &config);
        if (!ptr) {
            ptr = &tmp_session;
        }

        if (config.type == VTSS_APPL_MIRROR_SESSION_TYPE_RMIRROR_SOURCE && config.enable) {
            return FALSE;
        } else if (config.type == VTSS_APPL_MIRROR_SESSION_TYPE_MIRROR) {
            if (config.enable) {
                pcfg = &config;
                goto got_it;
            }
...
got_it:
        ...
        if (source_pls_rx) {
            *source_pls_rx      = (vtss::PortListStackable)pcfg->source_port_list_rx;
        }
        if (source_pls_tx) {
            *source_pls_tx      = (vtss::PortListStackable)pcfg->source_port_list_tx;
        }
        if (destination_pls) {
            *destination_pls    = (vtss::PortListStackable)pcfg->destination_port_list;
        }
    }

    return pcfg ? TRUE : FALSE;
}

static BOOL get_next_portCopyTableEntryInModule(portCopyTable_entry_t *entry)
{
    long                                portCopyDirection = COPY_END, next_portCopyDirection = COPY_END;
    vtss_appl_port_status_t             port_status;
    BOOL                                enable, dest_found = FALSE, src_link = FALSE, dest_link = FALSE;
    vtss::PortListStackable             source_pls_rx,
         source_pls_tx,
         destination_pls;
    vtss_ifindex_t                      *ifindex = NULL, buf,
                                         dest_ifindex(VTSS_IFINDEX_NONE), source_ifindex, next_source_ifindex, source_key, dest_key;
    vtss_ifindex_elm_t                  ife;

    T_D("key is (%lu, %lu)", entry->portCopySource, entry->portCopyDest);
    if (FALSE == get_portCopy_config(NULL, &enable, &source_pls_rx, &source_pls_tx, &destination_pls)) {
        return FALSE;
    }

    source_key = vtss_ifindex_cast_from_u32_0(entry->portCopySource);
    dest_key = vtss_ifindex_cast_from_u32_0(entry->portCopyDest);

    while (VTSS_RC_OK == vtss_ifindex_getnext_by_type(ifindex, &buf, VTSS_IFINDEX_GETNEXT_PORTS)) {
        if (!ifindex) {
            ifindex = &buf;
        }

        (void) vtss_ifindex_decompose(buf, &ife);

        if (buf == source_key) {
            BOOL rx_dir = source_pls_rx.get(ife.isid, ife.ordinal), tx_dir = source_pls_tx.get(ife.isid, ife.ordinal);

            if (rx_dir && tx_dir) {
                portCopyDirection = COPY_BOTH;
            } else if (rx_dir) {
                portCopyDirection = COPY_RX_ONLY;
            } else if (tx_dir) {
                portCopyDirection = COPY_TX_ONLY;
            } else {
                portCopyDirection = COPY_END;
            }

            if (portCopyDirection != COPY_END) {
                T_D("GET: %u/%u is %s source", ife.isid, ife.ordinal,
                    portCopyDirection == COPY_BOTH ? "BOTH" :
                    portCopyDirection == COPY_RX_ONLY ? "RX_ONLY" : "TX_ONLY");
                source_ifindex = buf;
            }
        } else if (buf > source_key && next_portCopyDirection == COPY_END) {
            BOOL rx_dir = source_pls_rx.get(ife.isid, ife.ordinal), tx_dir = source_pls_tx.get(ife.isid, ife.ordinal);

            if (rx_dir && tx_dir) {
                next_portCopyDirection = COPY_BOTH;
            } else if (rx_dir) {
                next_portCopyDirection = COPY_RX_ONLY;
            } else if (tx_dir) {
                next_portCopyDirection = COPY_TX_ONLY;
            } else {
                next_portCopyDirection = COPY_END;
            }

            if (next_portCopyDirection != COPY_END) {
                T_D("NEXT: %u/%u is %s source", ife.isid, ife.ordinal,
                    next_portCopyDirection == COPY_BOTH ? "BOTH" :
                    next_portCopyDirection == COPY_RX_ONLY ? "RX_ONLY" : "TX_ONLY");
                next_source_ifindex = buf;
            }

        }

        if (destination_pls.get(ife.isid, ife.ordinal) && !dest_found) {
            T_D("%u/%u is destination", ife.isid, ife.ordinal);
            dest_ifindex = buf;
            dest_found = TRUE;
        }

        if (next_portCopyDirection != COPY_END && dest_found) {
            break;
        }
    }
    ...
    if (portCopyDirection != COPY_END && dest_key < dest_ifindex) {
        entry->portCopySource = vtss_ifindex_cast_to_u32(source_ifindex);
        entry->portCopyDirection = portCopyDirection;
        entry->portCopyDest = vtss_ifindex_cast_to_u32(dest_ifindex);
        (void)vtss_appl_port_status_get(dest_ifindex, &port_status);
        dest_link = port_status.link;
        ifindex = &source_ifindex;
    } else if (next_portCopyDirection != COPY_END) {
        entry->portCopySource = vtss_ifindex_cast_to_u32(next_source_ifindex);
        entry->portCopyDirection = next_portCopyDirection;
        entry->portCopyDest = vtss_ifindex_cast_to_u32(dest_ifindex);
        (void)vtss_appl_port_status_get(dest_ifindex, &port_status);
        dest_link = port_status.link;
        ifindex = &next_source_ifindex;
    } else {
        return FALSE;
    }

    (void)vtss_appl_port_status_get(*ifindex, &port_status);
    src_link = port_status.link;

    entry->portCopyStatus = (src_link && dest_link) ? 1 : 3;
...
static BOOL get_next_portCopyTableEntryByKey(portCopyTable_entry_t *entry)
{
    ...
    T_D("%sFOUND in Module, %sFOUND in tmp", rc_mod ? "" : "NOT ", rc_tmp ? "" : "NOT ");
...
static BOOL apply_portCopyTableEntry(portCopyTable_entry_t *table_entry)
{
    ...
    vtss::PortListStackable             &source_pls_rx = (vtss::PortListStackable &)config.source_port_list_rx,
                                         &source_pls_tx = (vtss::PortListStackable &)config.source_port_list_tx,
                                          &destination_pls = (vtss::PortListStackable &)config.destination_port_list;
    vtss_ifindex_t                      *ifindex = NULL, buf;
    vtss_ifindex_elm_t                  ife;
    BOOL                                found = FALSE;

    T_D("%lu.%lu.%lu", table_entry->portCopyDest, table_entry->portCopySource, table_entry->portCopyDirection);

    (void) vtss_appl_mirror_session_entry_get(session_id, &config);
    ...
    while (VTSS_RC_OK == vtss_ifindex_getnext_by_type(ifindex, &buf, VTSS_IFINDEX_GETNEXT_PORTS)) {
        ...
        if (destination_pls.get(ife.isid, ife.ordinal)) {
            found = TRUE;
            break;
        }
    }
    ...
    if (found && (ife.isid != info.isid || ife.ordinal != info.if_id)) {
        T_D("found another dest(%d/%d)", ife.isid, ife.ordinal);
        return FALSE;
    }

    if (!found) {
        destination_pls.set(info.isid, info.if_id);
    }
    ...
    case COPY_BOTH:
        source_pls_rx.set(info.isid, info.if_id);
        source_pls_tx.set(info.isid, info.if_id);
        break;
    case COPY_RX_ONLY:
        source_pls_rx.set(info.isid, info.if_id);
        source_pls_tx.clear(info.isid, info.if_id);
        break;
    case COPY_TX_ONLY:
        source_pls_rx.clear(info.isid, info.if_id);
        source_pls_tx.set(info.isid, info.if_id);
        break;
    default:
        ;
    ...
    return vtss_appl_mirror_session_entry_set(session_id, &config) ? FALSE : TRUE;
}
...
static BOOL deactive_portCopyTableEntry (portCopyTable_entry_t *table_entry)
{
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        while (port_iter_getnext(&pit)) {
            if (source_pls_rx.get(sit.isid, pit.iport) == TRUE || source_pls_tx.get(sit.isid, pit.iport) == TRUE) {
                found = TRUE;
                break;
            }
        }
    ...
    if (found == FALSE) {
        destination_pls.clear_all();
        config.enable = FALSE;
    }

    (void) vtss_appl_mirror_session_entry_set(session_id, &config);
    return TRUE;
}
                                 ...
                                 size_t   name_len)
{
    ...
    case COMMIT: {
        ...
        if ( VTSS_RC_OK != rc_get) {
            ...
            (void) rfc2579_2_rowEntry((rfc2579_status_t)table_entry.smonVlanStatsControlStatus, &cur_state);
        }

        params_valid = smonVlanStatsControlTable_entry_valid(&table_entry);
        if ( FALSE == rowStatus_nextState(cur_state, (rowEntry_action_t)set_value, params_valid, vlan_stats_caps, &next_state, &rc)) {
                        ...
                        size_t   name_len)
{
    ...
    case COMMIT: {
        ...
        if (FALSE == get_portCopy_config(&session_id, NULL, NULL, NULL, NULL)) {
            return SNMP_ERR_COMMITFAILED;
        }
                     ...
                     size_t   name_len)
{
    ...
    case COMMIT: {
        ...
        if ( get_portCopyTableEntry(&table_entry, FALSE)) {
            ...
            (void) rfc2579_2_rowEntry((rfc2579_status_t)table_entry.portCopyStatus, &cur_state);
        }

        if ( FALSE == rowStatus_nextState(cur_state, (rowEntry_action_t)set_value, TRUE, portCpy_caps, &next_state, &rc)) {
        ...
        if (cur_state == ROWENTRY_STATE_NOT_EXISTENT) {
            if ((next_state == ROWENTRY_STATE_NOT_READY || next_state == ROWENTRY_STATE_ACTIVE) &&
                FALSE == get_portCopy_config(&session_id, NULL, NULL, NULL, NULL)) {
                return SNMP_ERR_COMMITFAILED;
            }
