...
#define DOT1QSTATICUNICASTSTATUS        22
    {DOT1QSTATICUNICASTSTATUS, ASN_INTEGER, RONLY, var_dot1qStaticUnicastTable, 5, {1, 3, 1, 1, 4}},
...
#define DOT1QSTATICMULTICASTSTATUS      25
    {DOT1QSTATICMULTICASTSTATUS, ASN_INTEGER, RONLY, var_dot1qStaticMulticastTable, 5, {1, 3, 2, 1, 5}},
...
{
    scalar_entry->dot1qMaxVlanId = VTSS_APPL_VLAN_ID_MAX;  // Maximum VLAN ID we can configure
...
static int get_next_dot1qFdbId(mesa_vid_t *vid)
{
    vtss_appl_vlan_entry_t vlan_entry;

    if (vtss_appl_vlan_get(VTSS_ISID_GLOBAL, *vid, &vlan_entry, TRUE, VTSS_APPL_VLAN_USER_ALL) != VTSS_RC_OK) {
...
getfirst_dot1qFdbTableEntry(dot1qFdbTable_entry_t *table_entry)
{
    ...
    if (get_next_dot1qFdbId (&vid) != VTSS_RC_OK) {
        return VTSS_RC_ERROR;
    }

    table_entry->dot1qFdbId = vid;
    table_entry->dot1qFdbDynamicCount = 0;
    (void)switch_iter_init(&sit, VTSS_ISID_GLOBAL, SWITCH_ITER_SORT_ORDER_ISID);
    while (switch_iter_getnext(&sit)) {
        if (mac_mgmt_table_vlan_stats_get(sit.isid, vid, &stats) != VTSS_RC_OK) {
            return VTSS_RC_ERROR;
        }
...
{
    mesa_vid_t          vid = getnext ? table_entry->dot1qFdbId : table_entry->dot1qFdbId - 1;
    switch_iter_t       sit;
    mac_table_stats_t   stats;
...
void get_1qTpFdbPort_by_port( vtss_isid_t isid, mesa_port_no_t iport, long *qTpFdbPort)
{
...
getfirst_dot1qTpFdbTableEntry(dot1qTpFdbTable_entry_t *table_entry)
{
    ...
    if (mac_mgmt_stack_get_next(&vid_mac_in, &mac_entry, &mac_type, TRUE) == VTSS_RC_OK) {
        ...
        if (*length > op_pos) {
            *cp++ = (u_char) * op++;
            ...
            do {
                ...
                *(cp--) = 0xff;
                zero_cnt++;
            } while (cp >= (u_char *)table_entry->dot1qTpGroupAddress);
            if (zero_cnt == (*length - orig_op_pos + 1)) {
                memset(table_entry->dot1qTpGroupAddress, 0, sizeof(table_entry->dot1qTpGroupAddress));
                //return 1;
            }
            return 0;
        }
                              ...
                              dot1qStaticUnicastTable_entry_t *table_entry)
{
    ...
    if (*length > op_pos) {
        ...
        /* Because 0 is allowed by dot1qStaticUnicastReceivePort,
           get_dot1qStaticUnicastTableEntry() don't know dot1qStaticUnicastReceivePort is 0 or not assigned,
           the only way is get previous for the other keys
        */
        table_entry->dot1qStaticUnicastReceivePort = 0xffffffff;
        len = 6;
        cp = (u_char *) table_entry->dot1qStaticUnicastAddress + 6;
        while (len-- > 0) {
            if (*(--cp)) {
                (*cp)--;
                return 0;
        ...
        if (!table_entry->dot1qFdbId) {
            return -1;
        }
        table_entry->dot1qFdbId--;
    }
                                ...
                                dot1qStaticMulticastTable_entry_t *table_entry)
{
        ...
        if (*length > op_pos) {
            *cp++ = (u_char) * op++;
            ...
            do {
                ...
                *(cp--) = 0xff;
                zero_cnt++;
            } while (cp >= (u_char *)table_entry->dot1qStaticMulticastAddress);
            if (zero_cnt == (*length - orig_op_pos + 1)) {
                memset(table_entry->dot1qStaticMulticastAddress, 0, sizeof(table_entry->dot1qStaticMulticastAddress));
                //return 1;
            }
            return 0;
        }
    ...
    if (*length > op_pos) {
        ...
        /* Because 0 is allowed by dot1qStaticMulticastReceivePort,
           get_dot1qStaticMulticastTableEntry() don't know dot1qStaticMulticastReceivePort is 0 or not assigned,
           the only way is get previous for the other keys
        */
        table_entry->dot1qStaticMulticastReceivePort = 0xffffffff;
        len = 6;
        cp = (u_char *) table_entry->dot1qStaticMulticastAddress + 6;
        while (len-- > 0) {
            if (*(--cp)) {
                (*cp)--;
                return 0;
        ...
        if (!table_entry->dot1qVlanIndex) {
            return -1;
        }
        table_entry->dot1qVlanIndex--;
    }
...
_get_dot1qVlanCurrentTableEntry(dot1qVlanCurrentTable_entry_t *table_entry, BOOL getnext)
{
    ...
    if (vtss_appl_vlan_get(VTSS_ISID_GLOBAL, vlan_entry.vid, &vlan_entry, getnext, VTSS_APPL_VLAN_USER_ALL) != VTSS_RC_OK) {
        return VTSS_RC_ERROR;
    }
    ...
    (void)switch_iter_init(&sit, VTSS_ISID_GLOBAL, SWITCH_ITER_SORT_ORDER_ISID);
    while (switch_iter_getnext(&sit)) {
        if (vtss_appl_vlan_get(sit.isid, vlan_entry.vid, &vlan_entry, FALSE, VTSS_APPL_VLAN_USER_ALL) != VTSS_RC_OK) {
            continue;
        }
        ...
        while (port_iter_getnext(&pit)) {
            ...
            if (vlan_mgmt_port_conf_get(sit.isid, pit.iport, &port_conf, VTSS_APPL_VLAN_USER_ALL, TRUE) == VTSS_RC_OK) {
                VTSS_PORTLIST_BF_SET(table_entry->dot1qVlanCurrentUntaggedPorts, dot1port_start + pit.iport, vlan_mgmt_vid_gets_tagged(&port_conf.hybrid, vlan_entry.vid) ? 0 : 1);
            }
        }
    }
...
_get_dot1qVlanStaticTableEntry(dot1qVlanStaticTable_entry_t *table_entry, BOOL getnext)
{
    ...
    if (getnext) {
        ...
        for (vid = table_entry->dot1qVlanIndex + 1; vid <= VTSS_APPL_VLAN_ID_MAX; vid++) {
            if (VTSS_BF_GET(access_vids, vid)) {
                found = TRUE;
                table_entry->dot1qVlanIndex = vid;
                break;
            }
        }
        ...
        if (!VTSS_BF_GET(access_vids, table_entry->dot1qVlanIndex)) {
            return -1;
        }
    }
    ...
    if ((rc = vtss_appl_vlan_name_get(vlan_entry.vid, table_entry->dot1qVlanStaticName, NULL)) != VTSS_RC_OK) {
        return rc;
    }
    table_entry->dot1qVlanStaticName_len = strlen(table_entry->dot1qVlanStaticName);

    (void)switch_iter_init(&sit, VTSS_ISID_GLOBAL, SWITCH_ITER_SORT_ORDER_ISID);
    while (switch_iter_getnext(&sit)) {
        if (vtss_appl_vlan_get(sit.isid, vlan_entry.vid, &vlan_entry, FALSE, VTSS_APPL_VLAN_USER_STATIC) != VTSS_RC_OK) {
        ...
        while (port_iter_getnext(&pit)) {
            ...
            if (vlan_mgmt_port_conf_get(sit.isid, pit.iport, &port_conf, VTSS_APPL_VLAN_USER_STATIC, TRUE) == VTSS_RC_OK) {
                VTSS_PORTLIST_BF_SET(table_entry->dot1qVlanStaticUntaggedPorts, dot1port_start + pit.iport, vlan_mgmt_vid_gets_tagged(&port_conf.hybrid, vlan_entry.vid) ? 0 : 1);
            }
        }
    }
...
set_dot1qVlanStaticTableEntry(dot1qVlanStaticTable_entry_t *table_entry)
{
    ...
    if (table_entry->dot1qVlanStaticRowStatus == 4) {
        ...
        vlan_name[0] = '\0';
        (void)vlan_mgmt_vlan_del(VTSS_ISID_GLOBAL, table_entry->dot1qVlanIndex, VTSS_APPL_VLAN_USER_STATIC);
        ...
        while (switch_iter_getnext(&sit)) {
            ...
            strncpy(vlan_name, table_entry->dot1qVlanStaticName, sizeof(vlan_name) - 1);
            vlan_name[MIN(table_entry->dot1qVlanStaticName_len, sizeof(vlan_name) - 1)] = '\0';
            if (VTSS_RC_OK != (rc = vtss_appl_vlan_name_set(vlan_entry.vid, vlan_name))) {
                return -1;
            }
        }
...
getfirst_dot1qPortVlanTableEntry(dot1qPortVlanTable_entry_t *table_entry)
{
    ...
    if (vlan_mgmt_port_conf_get(info.isid, info.if_id, &port_conf, VTSS_APPL_VLAN_USER_STATIC, TRUE) == VTSS_RC_OK) {
        ...
        table_entry->dot1qPvid = port_conf.hybrid.pvid;
...
static int set_dot1qPortVlanTableEntry(dot1qPortVlanTable_entry_t *table_entry)
{
    ...
    port_conf.hybrid.pvid           = table_entry->dot1qPvid;
    port_conf.hybrid.untagged_vid   = table_entry->dot1qPvid;
    port_conf.hybrid.frame_type     = (table_entry->dot1qPortAcceptableFrameTypes == 1) ? MESA_VLAN_FRAME_ALL : MESA_VLAN_FRAME_TAGGED;
    port_conf.hybrid.ingress_filter = (table_entry->dot1qPortIngressFiltering == 1) ? TRUE : FALSE;
                ...
                size_t   name_len)
{
    ...
    case RESERVE1: {
        ...
        if (set_value < VTSS_APPL_VLAN_ID_MIN || set_value > VTSS_APPL_VLAN_ID_MAX) {
