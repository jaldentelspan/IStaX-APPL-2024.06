...
int get_available_radiusAuthServerExtTableIndex(int if_num)
{
    ...
    for (i = if_num; i <= VTSS_RADIUS_NUMBER_OF_SERVERS; i++) {
        ...
        if ((rc != VTSS_RC_OK) || (radius_auth_client_server_mib.state != VTSS_RADIUS_SERVER_STATE_READY)) {
            continue;
        } else {
            break;
        }
                             ...
                             size_t *var_len, WriteMethod **write_method)
{
    ...
    case RADIUSAUTHCLIENTEXTROUNDTRIPTIME: {
        // table_entry.radiusAuthClientExtRoundTripTime is measured in 1/100 of a second
        // (centisecond) which is also what we need to return, so no conversions required here.
        ulong_ret = table_entry.radiusAuthClientExtRoundTripTime;
        return (u_char *)&ulong_ret;
    }
    ...
    case RADIUSAUTHCLIENTCOUNTERDISCONTINUITY: {
        // table_entry.radiusAuthClientCounterDiscontinuity is measured in 1/100 of a second
        // (centisecond) which is also what we need to return, so no conversions required here.
        ulong_ret = table_entry.radiusAuthClientCounterDiscontinuity;
        return (u_char *)&ulong_ret;
    }
    default:
        DEBUGMSGTL(("snmpd", "unknown sub-id %d in var_radiusAuthServerExtTable\n", vp->magic));
    }
