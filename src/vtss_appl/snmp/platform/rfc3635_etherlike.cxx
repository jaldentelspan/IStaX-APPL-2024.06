...
static BOOL update_dot3StatsTable_entry(vtss_isid_t isid, mesa_port_no_t port_idx, dot3StatsTable_entry_t *table_entry_p)
{
    ...
    if (port_status.link) {
        table_entry_p->dot3StatsDuplexStatus           = port_status.fdx ? 3 : 2;
    } else if (port_conf.speed == MESA_SPEED_AUTO) {
...
BOOL get_dot3StatsTable_entry(iftable_info_t *table_info, dot3StatsTable_entry_t *table_entry_p)
{
    ...
    T_D("table_entry_p->dot3StatsIndex = %lu (%s)", table_entry_p->dot3StatsIndex,
        table_info->type == IFTABLE_IFINDEX_TYPE_PORT ? "IFTABLE_IFINDEX_TYPE_PORT" :
        table_info->type == IFTABLE_IFINDEX_TYPE_LLAG ? "IFTABLE_IFINDEX_TYPE_LLAG" :
        table_info->type == IFTABLE_IFINDEX_TYPE_GLAG ? "IFTABLE_IFINDEX_TYPE_GLAG" :
        table_info->type == IFTABLE_IFINDEX_TYPE_VLAN ? "IFTABLE_IFINDEX_TYPE_VLAN" : "OTHERS");
    ...
    case IFTABLE_IFINDEX_TYPE_GLAG:
        ...
        while (switch_iter_getnext(&sit)) {
            ...
            if (aggr_mgmt_port_members_get(sit.isid, table_info->if_id, &aggr_members, FALSE) != VTSS_RC_OK) {
                continue;
            }
    ...
    if (table_entry_p->dot3StatsEtherChipSet_len == 0) {
        return FALSE;
    }
                   ...
                   size_t *var_len, WriteMethod **write_method)
{
    ...
    T_D("enter: table_info.ifIndex = %lu, %s, idx_num = %d", table_info.ifIndex, exact ? "GET" : "GET-NEXT", idx_num);
    ...
    if (!exact && (FALSE == ifIndex_get_next_by_type(&table_info, if_types) ||
