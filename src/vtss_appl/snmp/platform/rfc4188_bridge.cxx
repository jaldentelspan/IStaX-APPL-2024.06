...
#include <vtss/appl/port.h>   // vtss_appl_port_statistics_get()
...
get_dot1dBaseScalar(dot1dBase_scalar_t *scalar_entry)
{
    ...
    bcopy(board_conf.mac_address.addr, scalar_entry->dot1dBaseBridgeAddress, 6);
...
static BOOL update_dot1dBasePortTable_entry(vtss_isid_t isid, mesa_port_no_t port_idx, dot1dBasePortTable_entry_t *table_entry_p)
{
...
getfirst_dot1dBasePortTableEntry(dot1dBasePortTable_entry_t *table_entry)
{
    ...
    ifIndex_info.type = (ifIndex_type_t)dot1Port_info.type;
...
get_dot1dBasePortTableEntry(dot1dBasePortTable_entry_t *table_entry, BOOL getnext)
{
    ...
    case DOT1PORT_TYPE_LLAG:
        ...
        if ((aggr_mgmt_port_members_get(dot1Port_info.isid, aggr_no, &aggr_members, FALSE) != VTSS_RC_OK)
#ifdef VTSS_SW_OPTION_LACP
            && (aggr_mgmt_lacp_members_get(dot1Port_info.isid, aggr_no, &aggr_members, FALSE) != VTSS_RC_OK)
            ...
            WriteMethod **write_method)
{
    ...
    dot1dTp_scalar_entry.dot1dTpAgingTime = 0; // Satisfy compiler
...
static int get_next_dot1dTpFdbTableEntry_by_mac (mesa_vid_mac_t *vid_mac)
{
    ...
    memset(&vid_mac_in,      0, sizeof(mesa_vid_mac_t));
...
next_vid:
    while (vtss_appl_vlan_get(VTSS_ISID_GLOBAL, vlan_mgmt_entry.vid, &vlan_mgmt_entry, TRUE, VTSS_APPL_VLAN_USER_ALL) == VTSS_RC_OK) {
        ...
        if (mac_mgmt_stack_get_next(&vid_mac_in, &buf, &mac_type, TRUE) != VTSS_RC_OK || vid_mac_in.vid != buf.vid_mac.vid ) {
            T_D("next vid");
            goto next_vid;
        }
...
static int get_dot1dTpFdbTableEntry_by_mac (mesa_vid_mac_t *vid_mac)
{
    mesa_vid_mac_t buf;
...
void get_TpFdbPort_by_port( vtss_isid_t isid, mesa_port_no_t iport, long *tpFdbPort)
{
...
getfirst_dot1dTpFdbTableEntry(dot1dTpFdbTable_entry_t *table_entry)
{
    ...
    if (mac_mgmt_stack_get_next(&vid_mac_in, &mac_entry, &mac_type, FALSE) == VTSS_RC_OK) {
        ...
        if (*length > op_pos) {
            *cp++ = (u_char) * op++;
            ...
            do {
                ...
                *(cp--) = 0xff;
                zero_cnt++;
            } while (cp >= (u_char *)table_entry->dot1dTpFdbAddress);
            if (zero_cnt == (*length - orig_op_pos + 1)) {
                memset(table_entry->dot1dTpFdbAddress, 0, sizeof(table_entry->dot1dTpFdbAddress));
                //return 1;
            }
            return 0;
        }
...
static BOOL update_dot1dTpPortTable_entry(vtss_isid_t isid, mesa_port_no_t port_idx, dot1dTpPortTable_entry_t *table_entry_p)
{
    mesa_port_counters_t  counters;
