...
#include "misc_api.h"  //mibContextTable_register
...
static CapArray<unsigned short, VTSS_APPL_CAP_ISID_CNT, MEBA_CAP_BOARD_PORT_MAP_COUNT> old_actor_admin_key;
static CapArray<unsigned short, VTSS_APPL_CAP_ISID_CNT, MEBA_CAP_BOARD_PORT_MAP_COUNT> now_actor_admin_key;
static CapArray<unsigned char,  VTSS_APPL_CAP_ISID_CNT, MEBA_CAP_BOARD_PORT_MAP_COUNT> old_actor_admin_state;
...
char *iftable_ifindex_port_list2txt(vtss_common_bool_t *list, char *buf)
{
    ...
    p = buf;
    *p = '\0';
    for (i = 0; i < VTSS_LACP_MAX_PORTS_; i++) {
        member = list[i];
        if ((member && (count == 0 || i == VTSS_LACP_MAX_PORTS_)) || (!member && count > 1)) {
...
BOOL get_dot3adAggTable_entry(int table_index, dot3adAggTable_entry_t *table_entry_p)
{
    ...
    for (aid = VTSS_PORT_NO_START; aid < (VTSS_LACP_MAX_AGGR_ + VTSS_PORT_NO_START); aid++) {
    ...
    if (aid >= VTSS_LACP_MAX_AGGR_) { // Not found LACP LAG
        return FALSE;
    }
    ...
    if (lacp_mgmt_system_conf_get(&lacp_system_config) != VTSS_RC_OK) {
        return FALSE;
    }
    ...
    for (l2port = VTSS_PORT_NO_START; l2port < (VTSS_LACP_MAX_PORTS_ + VTSS_PORT_NO_START); l2port++) {
        if (lacp_aggregatorstatus.port_list[l2port - VTSS_PORT_NO_START]) {
            ...
            if (lacp_mgmt_port_status_get(l2port, &lacp_portstatus) != VTSS_RC_OK) {
                return FALSE;
            }
...
static void DOT3ADAGG_PortList_BF_size_get(u_long *port_count_p)
{
...
BOOL get_dot3adAggPortListTable_entry(int table_index, dot3adAggPortListTable_entry_t *table_entry_p)
{
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        while (port_iter_getnext(&pit)) {
            if (lacp_aggregatorstatus.port_list[pit.iport]) {
                VTSS_PORTLIST_BF_SET(table_entry_p->dot3adAggPortListPorts, dot1port_start + pit.iport, 1);
            } else {
                VTSS_PORTLIST_BF_SET(table_entry_p->dot3adAggPortListPorts, dot1port_start + pit.iport, 0);
            }
        }
    }
    DOT3ADAGG_PortList_BF_size_get( (u_long *)&table_entry_p->dot3adAggPortListPorts_len);
...
BOOL get_dot3adAggPortTable_entry(int table_index, dot3adAggPortTable_entry_t *table_entry_p)
{
    ...
    if (lacp_mgmt_port_conf_get(table_info.isid, table_info.if_id, &lacp_port_config) != VTSS_RC_OK) {
        return FALSE;
    }
    ...
    /*  1) LACP_Activity is encoded in bit 0. This flag indicates the Activity control value with regard to
            this link. Active LACP is encoded as a 1; Passive LACP is encoded as a 0.
        2) LACP_Timeout is encoded in bit 1. This flag indicates the Timeout control value with regard to
            this link. Short Timeout is encoded as a 1; Long Timeout is encoded as a 0.
        3) Aggregation is encoded in bit 2. If TRUE (encoded as a 1), this flag indicates that the System
            considers this link to be Aggregatable; i.e., a potential candidate for aggregation. If FALSE
            (encoded as a 0), the link is considered to be Individual; i.e., this link can be operated only as an
            individual link.
        (see 43.4.2.2 in IEEE Std 802.3, 2000 Edition)
    */
    table_entry_p->dot3adAggPortActorAdminState |= lacp_port_config.active_or_passive ? 0x80 : 0x00;
    table_entry_p->dot3adAggPortActorAdminState |= lacp_port_config.xmit_mode ? 0x40 : 0x00;
    table_entry_p->dot3adAggPortActorAdminState |= lacp_port_config.enable_lacp ? 0x20 : 0x00;
    T_D("dot3adAggPortActorAdminState on %d is 0x%02x(%s%s%s)",
        iport2uport(table_info.if_id), table_entry_p->dot3adAggPortActorAdminState,
        lacp_port_config.active_or_passive ? "LACP_active " : "",
        lacp_port_config.xmit_mode ? "Timeout " : "",
        lacp_port_config.enable_lacp ? "Aggregation" : ""
       );

    if (lacp_mgmt_port_status_get(L2PORT2PORT(table_info.isid, table_info.if_id), &lacp_portstatus) != VTSS_RC_OK) {
        return FALSE;
    }
    if (aggID_ifindex == 0) {
...
#define DOT3ADAGGPORTACTOROPERKEY                           17
    {DOT3ADAGGPORTACTOROPERKEY, ASN_INTEGER, RONLY, var_dot3adAggPortTable, 4, {2, 1, 1, 5}},
...
init_lagMIBObjects(void)
{
     ...
     */
    old_actor_admin_key.clear();
    old_actor_admin_state.clear();
}
                   ...
                   int exact, size_t *var_len, WriteMethod **write_method)
{
    ...
    *write_method = 0; /* assume it isnt writable for the time being */
    ...
    if (table_index == IFTABLE_IFINDEX_END) {
        return NULL;
    }
                           ...
                           size_t *var_len, WriteMethod **write_method)
{
    ...
    case DOT3ADAGGPORTLISTPORTS: {
        *var_len = table_entry.dot3adAggPortListPorts_len;
        memcpy(string, (char *) table_entry.dot3adAggPortListPorts, *var_len);
        string[*var_len] = '\0';
    ...
    *write_method = 0; /* assume it isnt writable for the time being */
    ...
    case DOT3ADAGGPORTACTORADMINSTATE: {
        *write_method = write_dot3adAggPortActorAdminState;
        uchar_ret = table_entry.dot3adAggPortActorAdminState;
        *var_len = sizeof(table_entry.dot3adAggPortActorAdminState);
                                 ...
                                 oid *name, size_t name_len)
{
    ...
    if (lacp_mgmt_port_conf_set(table_info.isid, table_info.if_id, &lacp_port_config) != VTSS_RC_OK) {
        return SNMP_ERR_RESOURCEUNAVAILABLE;
    }
                                   ...
                                   oid *name, size_t name_len)
{
    ...
    */
    ...
    T_D("*var_val = 0x%02x", *var_val);
    lacp_port_config.active_or_passive = (*var_val & 0x80) ? VTSS_LACP_ACTMODE_ACTIVE : VTSS_LACP_ACTMODE_PASSIVE;
    lacp_port_config.xmit_mode = (*var_val & 0x40) ? VTSS_LACP_FSMODE_FAST : VTSS_LACP_FSMODE_SLOW;
    lacp_port_config.enable_lacp = (*var_val & 0x20) ? TRUE : FALSE;
