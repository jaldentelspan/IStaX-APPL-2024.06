...
#include "port_api.h" /* port_cap_get() */
...
CapArray<u_long, VTSS_APPL_CAP_ISID_CNT, MEBA_CAP_BOARD_PORT_MAP_COUNT> linkdown_counter;
...
BOOL get_ifMauTable_entry(int table_index, ifMauTable_entry_t *table_entry_p)
{
    ...
    if (vtss_ifindex_from_port(table_info.isid, table_info.if_id, &port_ifindex) != VTSS_RC_OK) {
        return FALSE;
    };

    switch (table_info.type) {
    case IFTABLE_IFINDEX_TYPE_PORT:
        if (vtss_appl_port_conf_get(port_ifindex, &port_conf) != VTSS_RC_OK) {
            return FALSE;
        }
        if (vtss_appl_port_status_get(port_ifindex, &port_status) != VTSS_RC_OK) {
            return FALSE;
        }
        ...
        if (port_status.link) {
            ...
            } else if (port_status.speed == MESA_SPEED_100M) {
                if (port_status.fdx) {
                    mau_type_oid [8] = 16;
                } else {
                    mau_type_oid [8] = 15;
                }
            } else if (port_status.speed == MESA_SPEED_10M) {
                if (port_status.fdx) {
                    mau_type_oid [8] = 11;
                } else {
                    mau_type_oid [8] = 5;
        ...
        if (port_conf.speed == MESA_SPEED_1G) {
            if (port_conf.fdx) {
                if (port_status.fiber) {
...
BOOL next_ifMauAutoNegTable_entry(int table_index)
{
    iftable_info_t          table_info;
    vtss_appl_port_conf_t   port_conf;
    vtss_appl_port_status_t port_status;
    ...
    if (vtss_ifindex_from_port(table_info.isid, table_info.if_id, &ifindex) != VTSS_RC_OK) {
        return FALSE;
    };
    ...
    case IFTABLE_IFINDEX_TYPE_PORT:
        ...
        if (port_cap_get(table_info.if_id, &cap) != VTSS_RC_OK) {
            return FALSE;
        }

        /* Get port capability; when meets 10G/SFP ports it returns TRUE to get next*/
        if (cap & (MEBA_PORT_CAP_10G_FDX | MEBA_PORT_CAP_SFP_ONLY)) {
            return TRUE;
        } else if ((cap & MEBA_PORT_CAP_SPEED_DUAL_ANY_FIBER)) { /* dual media and now in fiber mode */
            if ( MESA_PORT_INTERFACE_SGMII != port_status.mac_if && MESA_PORT_INTERFACE_QSGMII != port_status.mac_if ) {
                return TRUE;
            }
        }
...
BOOL get_ifMauAutoNegTable_entry(int table_index, ifMauAutoNegTable_entry_t *table_entry_p)
{
    ...
    case IFTABLE_IFINDEX_TYPE_PORT:
        ...
        if ( misc_debug_phy_read(table_info.isid, table_info.if_id, 0x1, 0, &staus_reg, 0, 0) != VTSS_RC_OK) {
            return FALSE;
        }

        if ( misc_debug_phy_read(table_info.isid, table_info.if_id, 0x4, 0, &autoneg_adver, 0, 0) != VTSS_RC_OK) {
            return FALSE;
        }

        if ( misc_debug_phy_read(table_info.isid, table_info.if_id, 0x5, 0, &autoneg_partner, 0, 0) != VTSS_RC_OK ) {
            return FALSE;
        }

        if ( misc_debug_phy_read(table_info.isid, table_info.if_id, 0x9, 0, &thoundbaseT_control, 0, 0) != VTSS_RC_OK ) {
            return FALSE;
        }

        if ( misc_debug_phy_read(table_info.isid, table_info.if_id, 0xA, 0, &thoundbaseT_status, 0, 0) != VTSS_RC_OK ) {
            return FALSE;
        }
        ...
        if (caps & MEBA_PORT_CAP_FLOW_CTRL) {
                       ...
                       u_char *statP, oid *name, size_t name_len)
{
    ...
    case ACTION: {
        ...
        case IFTABLE_IFINDEX_TYPE_PORT:
            ...
            if (vtss_appl_port_conf_set(ifindex, &port_conf) != VTSS_RC_OK) {
                return SNMP_ERR_RESOURCEUNAVAILABLE;
            }
                      ...
                      size_t *var_len, WriteMethod **write_method)
{
    ...
    *var_len = sizeof(ulong_ret);   /* assume an integer and change later if not */
    ...
    if (exact) {
        ...
        /* AW, 01/29/2016 13:33, Bugzilla#19971; see explanation below */
        if (next_ifMauAutoNegTable_entry(table_index)) {
            return NULL;
        }
        ...
        do {
            table_index = get_available_ifTableIndex(mau_ifIndex);
            mau_ifIndex = table_index + 1;
            /* CP, 08/21/2013 13:57, Bugzilla#11153, walk MAU MIB will cause error messages */
            /* AW, 01/29/2016 13:33, Bugzilla#19971, Illegal miim_controller error messages */
            /*
              misc_debug_phy_read() can *NOT* read/write 10G/SFP ports, so skip those
              ports completely from the MIB.

              This fix is an enhancement to the one by CP which avoided using
              misc_debug_phy_read and returned 0 on read. This however exposed a number of
              read-write entries which could not be written. This now removes 10G/SFP ports
              completely from the ifMauAutoNegTable.

              If misc_debug_phy_read() is fixed then this check should be
              removed.
            */
        } while (next_ifMauAutoNegTable_entry(table_index));
        if (!get_ifMauAutoNegTable_entry(table_index, &table_entry) ) {
            return NULL;
        }
                                    ...
                                    oid *name, size_t name_len)
{
    ...
    case ACTION: {
        ...
        if (port_cap_get(table_info.if_id, &cap) == VTSS_RC_OK && (cap & MEBA_PORT_CAP_1G_PHY)) {
            if ( misc_debug_phy_write(table_info.isid, table_info.if_id, 0x4, 0, autoneg_adver, 0, 0) != VTSS_RC_OK ) {
                return SNMP_ERR_RESOURCEUNAVAILABLE;
            }
            if ( misc_debug_phy_write(table_info.isid, table_info.if_id, 0x9, 0, thoundbaseT_control, 0, 0) != VTSS_RC_OK ) {
                return SNMP_ERR_RESOURCEUNAVAILABLE;
            }
        }
        break;
