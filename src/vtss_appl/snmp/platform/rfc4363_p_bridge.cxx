...
static BOOL update_dot1dTpHCPortTable_entry(vtss_isid_t isid, mesa_port_no_t port_idx, dot1dTpHCPortTable_entry_t *table_entry_p)
{
    ...
    vtss_snmp_u64_to_counter64(&table_entry_p->dot1dTpHCPortInFrames,   counters.rmon.rx_etherStatsPkts);
    vtss_snmp_u64_to_counter64(&table_entry_p->dot1dTpHCPortOutFrames,  counters.rmon.tx_etherStatsPkts);
    vtss_snmp_u64_to_counter64(&table_entry_p->dot1dTpHCPortInDiscards, counters.if_group.ifInDiscards);
                            ...
                            BOOL getnext)
{
    ...
    case DOT1PORT_TYPE_LLAG:
        ...
        if ((aggr_mgmt_port_members_get(table_info.isid, aggr_no, &aggr_members, FALSE) != VTSS_RC_OK)
#ifdef VTSS_SW_OPTION_LACP
            && (aggr_mgmt_lacp_members_get(table_info.isid, aggr_no, &aggr_members, FALSE) != VTSS_RC_OK)
#endif
           ) {
            return VTSS_RC_OK;
        }

        for (port_idx = VTSS_PORT_NO_START; port_idx < VTSS_PORT_NO_START + port_count_max(); port_idx++) {
...
static CapArray<BOOL, VTSS_APPL_CAP_ISID_END, MEBA_CAP_BOARD_PORT_MAP_COUNT> dscp_class_enable;
...
static void get_init_dscp(void)
{
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        while (port_iter_getnext(&pit)) {
            ...
            if ( VTSS_RC_OK != (rc = vtss_appl_qos_port_conf_get(sit.isid, pit.iport, &conf)) ) {
                T_E("%s", error_txt(rc));
                continue;
            }
            *ptr = conf.port.trust_dscp;
...
get_dot1dExtBaseScalar(dot1dExtBase_scalar_t *scalar_entry)
{
    ...
    scalar_entry->dot1dTrafficClassesEnabled = (rc == VTSS_RC_OK ? (conf.port.trust_tag ? 1 : 2) : 2);
    return VTSS_RC_OK;
}
...
set_dot1dExtBaseScalar(dot1dExtBase_scalar_t *scalar_entry)
{
    ...
    while (switch_iter_getnext(&sit)) {
        ...
        while (port_iter_getnext(&pit)) {
            ...
            ptr = &dscp_class_enable[sit.isid][pit.iport];
            if ( conf.port.trust_tag != tmp ) {
                if ( TRUE == tmp ) {
                    conf.port.default_cos = conf.tag_cos_map[conf.port.default_pcp][0].cos;
                    *ptr = conf.port.trust_dscp;
                    conf.port.trust_dscp = FALSE;
                } else {
                    conf.port.default_cos = 0;
                    conf.port.trust_dscp = *ptr;
                }
                conf.port.tag_remark_mode = VTSS_APPL_QOS_TAG_REMARK_MODE_CLASSIFIED;
                conf.port.trust_tag = tmp;
                if ( VTSS_RC_OK != (rc = vtss_appl_qos_port_conf_set(sit.isid, pit.iport, &conf)) ) {
                    T_E("%s", error_txt(rc));
                }
            }
        }
...
static BOOL get_dot1dPortPriorityTableEntryByDot1Port(dot1Port_info_t *table_info, dot1dPortPriorityTable_entry_t *table_entry, vtss_appl_qos_port_conf_t *conf )
{
    mesa_rc                   rc;
    vtss_appl_qos_port_conf_t tmp;

    if ( VTSS_RC_OK != (rc = vtss_appl_qos_port_conf_get(table_info->isid, table_info->if_id, &tmp)) ) {
        T_E("%s", error_txt(rc));
        return FALSE;
    }
    ...
    if ( table_entry ) {
        table_entry->dot1dBasePort = table_info->dot1port;
        table_entry->dot1dPortDefaultUserPriority = ( long ) tmp.port.default_pcp;
    }
                                ...
                                table_entry)
{
    ...
    conf.port.default_pcp = table_entry->dot1dPortDefaultUserPriority;
    conf.port.default_dei = 0;
    conf.port.default_cos = conf.tag_cos_map[conf.port.default_pcp][0].cos;

    if ( (rc = vtss_appl_qos_port_conf_set(table_info.isid, table_info.if_id, &conf)) != VTSS_RC_OK ) {
        T_E("%s", error_txt(rc));
        return VTSS_RC_ERROR;
    }
...
static BOOL get_dot1dTrafficClassTableEntryByDot1Port(dot1Port_info_t *table_info, dot1dTrafficClassTable_entry_t *table_entry, vtss_appl_qos_port_conf_t *conf )
{
    mesa_rc                   rc;
    vtss_appl_qos_port_conf_t tmp_conf;

    if ( VTSS_RC_OK != (rc = vtss_appl_qos_port_conf_get(table_info->isid, table_info->if_id, &tmp_conf)) ) {
        T_E("%s", error_txt(rc));
        return FALSE;
    }

    table_entry->dot1dTrafficClass = tmp_conf.tag_cos_map[table_entry->dot1dTrafficClassPriority][0].cos;
    if ( conf ) {
        *conf = tmp_conf;
    }
    return TRUE;
                                ...
                                table_entry)
{
    ...
    conf.tag_cos_map[table_entry->dot1dTrafficClassPriority][0].cos = table_entry->dot1dTrafficClass;
    conf.tag_cos_map[table_entry->dot1dTrafficClassPriority][1].cos = table_entry->dot1dTrafficClass;

    if ( table_entry->dot1dTrafficClassPriority == conf.port.default_pcp) {
        conf.port.default_cos = table_entry->dot1dTrafficClass;
    }
