...
int get_available_radiusAccServerExtTableIndex(int if_num)
{
    ...
    for (i = if_num; i <= VTSS_RADIUS_NUMBER_OF_SERVERS; i++) {
        ...
        if ((rc != VTSS_RC_OK) || (radius_acct_client_server_mib.state != VTSS_RADIUS_SERVER_STATE_READY)) {
            continue;
        } else {
            break;
        }
                            ...
                            size_t *var_len, WriteMethod **write_method)
{
    ...
    case RADIUSACCCLIENTEXTROUNDTRIPTIME: {
        // table_entry.radiusAccClientExtRoundTripTime is measured in 1/100 of a second
        // (centisecond) which is also what we need to return, so no conversions required here.
        ulong_ret = table_entry.radiusAccClientExtRoundTripTime;
        return (u_char *)&ulong_ret;
    }
    ...
    case RADIUSACCCLIENTCOUNTERDISCONTINUITY: {
        // table_entry.radiusAccClientCounterDiscontinuity is measured in 1/100 of a second
        // (centisecond) which is also what we need to return, so no conversions required here.
        ulong_ret = table_entry.radiusAccClientCounterDiscontinuity;
        return (u_char *)&ulong_ret;
    }
    default:
        DEBUGMSGTL(("snmpd",
