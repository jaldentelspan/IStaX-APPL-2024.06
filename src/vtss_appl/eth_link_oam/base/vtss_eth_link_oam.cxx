...
u32 vtss_eth_link_oam_default_set()
{
    ...
    vtss_clear(link_oam_conf.port_conf);
...
u32 vtss_eth_link_oam_mgmt_port_conf_init(const u32 port_no)
{
    ...
    do {
        ...
        message = (vtss_eth_link_oam_message_t *)vtss_eth_link_oam_malloc(sizeof(vtss_eth_link_oam_message_t));
        if (message != NULL) {
...
u32 vtss_eth_link_oam_mgmt_port_control_conf_set (u32 port_no, const vtss_eth_link_oam_control_t conf)
{
    ...
    do {
        ...
        if ( (conf != VTSS_APPL_ETH_LINK_OAM_CONTROL_DISABLE) &&
             (conf != VTSS_APPL_ETH_LINK_OAM_CONTROL_ENABLE)
           ) {
            rc = VTSS_ETH_LINK_OAM_RC_INVALID_PARAMETER;
            break;
        }
        if (GET_MGMT_PORT_CONTROL(port_no) == conf) {
            ...
            if (!conf) {
                // Link-oam being disabled on port. Send dying gasp.
                vtss_eth_link_oam_crit_data_unlock();
...
u32 vtss_eth_link_oam_mgmt_port_mode_conf_set (const u32 port_no, const vtss_eth_link_oam_mode_t conf, const BOOL init_flag)
{
    ...
    do {
        ...
        if ( (conf != VTSS_APPL_ETH_LINK_OAM_MODE_PASSIVE) && (conf != VTSS_APPL_ETH_LINK_OAM_MODE_ACTIVE)) {
            rc = VTSS_ETH_LINK_OAM_RC_INVALID_PARAMETER;
            break;
        }
...
u32 vtss_eth_link_oam_mgmt_port_mib_retrieval_conf_set (const u32  port_no, const BOOL conf)
{
    ...
    do {
        ...
        if (GET_MGMT_PORT_MIB_RETRIEVAL_SUPPORT(port_no) == conf) {
            rc = VTSS_ETH_LINK_OAM_RC_ALREADY_CONFIGURED;
            break;
        } else {
            SET_MGMT_PORT_MIB_RETRIEVAL_SUPPORT(port_no, conf);
            break;
        }
    } while (VTSS_ETH_LINK_OAM_NULL);
    ...
    if (rc == VTSS_ETH_LINK_OAM_RC_OK) {
        ...
        if (message != NULL) {
            message->event_on_port = port_no;
            message->event_code = VTSS_ETH_LINK_OAM_MGMT_PORT_MIB_RETRIEVAL_CONF_EVENT;
...
u32 vtss_eth_link_oam_mgmt_port_mib_retrieval_conf_get (const u32  port_no, BOOL *conf)
{
    ...
    do {
        ...
        *conf = GET_MGMT_PORT_MIB_RETRIEVAL_SUPPORT(port_no);
        break;
    } while (VTSS_ETH_LINK_OAM_NULL);
...
u32 vtss_eth_link_oam_mgmt_port_remote_loopback_oper_conf_set (const u32  port_no, const BOOL conf)
{
    ...
    do {
        ...
        if (mode_conf == VTSS_APPL_ETH_LINK_OAM_MODE_PASSIVE) {
            rc = VTSS_ETH_LINK_OAM_RC_NOT_SUPPORTED;
            break;
        }
    ...
    if ( rc == VTSS_ETH_LINK_OAM_RC_OK) {
        if (vtss_eth_link_oam_rlb_opr_lock() == FALSE) {
            if (conf == TRUE) {
                (void)vtss_eth_link_oam_client_port_state_conf_set(port_no, VTSS_ETH_LINK_OAM_MUX_FWD_STATE, VTSS_ETH_LINK_OAM_PARSER_FWD_STATE, TRUE, FALSE);
            }
...
/* Retrieves the Port link monitoring support configuration                    */
...
/* Retrieves the Port Error Frame Event configuration                          */
...
/* Retrieves the symbol period event window threshold                      */
...
/* Retrieves the Port symbole period event error threshold configuration    */
...
/* Retrieves the Port's symbole period event rx symbols count threshold        */
...
/* Retrieves the Port Error Frame Second Summary Event configuration          */
