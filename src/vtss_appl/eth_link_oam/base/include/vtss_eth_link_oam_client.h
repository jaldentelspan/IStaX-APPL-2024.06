...
#define VTSS_ETH_LINK_OAM_SLEEP_TIME 1000 /* milliseconds */
...
typedef struct eth_link_oam_client_conf {
    ...
    void                                 (*oam_eth_link_cb)(char *resp);
    char                                  *buf;
    u32                                   buf_size;
...
typedef struct eth_link_oam_client_oper_conf {
    CapArray<vtss_eth_link_oam_port_client_conf_t, MEBA_CAP_BOARD_PORT_MAP_COUNT> port_conf;
...
typedef enum {
    ...
    VTSS_ETH_LINK_OAM_MGMT_PORT_MIB_RETRIEVAL_CONF_EVENT,
...
/* oam_code:   OAM code to be supported by OAM client                         */
/* Retrieves the OAM Control support for the specified OAM code               */
