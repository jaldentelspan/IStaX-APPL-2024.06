...
static i32 handler_config_oam_ports(CYG_HTTPD_STATE *p)
{
        ...
        if (port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_RC_OK) {
            while (port_iter_getnext(&pit)) {
                if (eth_link_oam_mgmt_port_conf_get(isid, pit.iport, &conf) == VTSS_RC_OK ) {
                    vtss_eth_link_oam_conf_t new_conf, *old = &conf;
                    BOOL                     oper_lb = FALSE, current_oper_lb = FALSE;
                    memset(&new_conf, 0, sizeof(new_conf));
                    new_conf = *old;
                    new_conf.oam_control = (vtss_eth_link_oam_control_t)cyg_httpd_form_variable_check_fmt(p, "enable_%d", pit.uport);
                    new_conf.oam_remote_loop_back_support = cyg_httpd_form_variable_check_fmt(p, "loop_%d", pit.uport);
                    if (cyg_httpd_form_variable_int_fmt(p, &val, "role_%d", pit.uport)) {
                        new_conf.oam_mode = (vtss_eth_link_oam_mode_t)val;
                    }
                    new_conf.oam_link_monitoring_support = cyg_httpd_form_variable_check_fmt(p, "monitor_%d", pit.uport);
                    new_conf.oam_mib_retrieval_support = cyg_httpd_form_variable_check_fmt(p, "mib_%d", pit.uport);
                    ...
                    if ((memcmp(&new_conf, old, sizeof(*old)) != 0)) {
                        *old = new_conf;
                        ...
                        if ( rc == VTSS_RC_OK || rc == ETH_LINK_OAM_RC_ALREADY_CONFIGURED) {
                            rc = eth_link_oam_mgmt_port_mib_retrieval_conf_set(isid, pit.iport, conf.oam_mib_retrieval_support);
                        }
...
static i32 handler_stat_link_oam_statistics(CYG_HTTPD_STATE *p)
{
    vtss_isid_t isid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    int                    ct;
    port_iter_t            pit;
...
static i32 handler_stat_oam_port_status(CYG_HTTPD_STATE *p)
{
    vtss_isid_t                                 isid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    port_iter_t                                 pit;
    int                                         ct = 0;
    ...
    char                                        oui_buf[80] = {0}, remote_oui_buf[80] = {0};
    int char_cnt                                = 0;
    char                                        temp_remote_mac[80] = {0};
    u8                                          remote_mac[6] = {0};
    ...
    if (port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_RC_OK) {
        while (port_iter_getnext(&pit)) {
            ...
            if (eth_link_oam_client_port_local_info_get (isid, pit.iport, &local_info) == VTSS_RC_OK) {
                ...
                char_cnt = snprintf(oui_buf, sizeof(oui_buf), "%02x-%02x-%02x", local_info.oui[0], local_info.oui[1], local_info.oui[2]);
                if (char_cnt <= 0) {
                    T_D("Failed to do snprintf\n");
                }
            } else {
            ...
            if (eth_link_oam_client_port_remote_info_get(isid, pit.iport, &remote_info) == VTSS_RC_OK) {
                ...
                char_cnt = snprintf(remote_oui_buf, sizeof(remote_oui_buf), "%02x-%02x-%02x", remote_info.oui[0], remote_info.oui[1], remote_info.oui[2]);
                if (char_cnt <= 0) {
                    T_D("Failed to do snprintf\n");
                }
            } else {
            ...
            if (eth_link_oam_control_layer_port_pdu_control_status_get(isid, pit.iport,
                ...
                strncpy(buf, pdu_tx_control_to_str(tx_control), sizeof(buf));
            }
            ...
            if (eth_link_oam_client_port_remote_mac_addr_info_get(isid, pit.iport,
                                                                  (u8 *)remote_mac) != VTSS_RC_OK) {
                rc_peer = FALSE;
            } else {
                ct = snprintf(temp_remote_mac, sizeof(temp_remote_mac), "%02x:%02x:%02x:%02x:%02x:%02x",
            ...
            if (rc == TRUE && rc_peer == FALSE) {
                ct = snprintf(p->outbuffer, sizeof(p->outbuffer),
                              ...
                              IS_CONF_ACTIVE(local_info.oam_conf, VTSS_ETH_LINK_OAM_CONF_VARIABLE_RETRIEVAL_SUPPORT) ? "Enabled" : "Disabled",
                              mtu_size,
                              mux_state_to_str((vtss_eth_link_oam_mux_state_t)((local_info.state & 4) >> 2)),
                              parser_state_to_str((vtss_eth_link_oam_parser_state_t)(local_info.state & 3)),
                              oui_buf,
                              temp,
                              buf,
                              disc_buf,
                ...
                ct = snprintf(p->outbuffer, sizeof(p->outbuffer),
                              ...
                              IS_CONF_ACTIVE(remote_info.oam_conf, VTSS_ETH_LINK_OAM_CONF_VARIABLE_RETRIEVAL_SUPPORT) ? "Enabled" : "Disabled",
                              remote_mtu_size,
                              mux_state_to_str((vtss_eth_link_oam_mux_state_t)((remote_info.state & 4) >> 2)),
                              parser_state_to_str((vtss_eth_link_oam_parser_state_t)(remote_info.state & 3)),
                              remote_oui_buf,
                              remote_temp,
                              temp_remote_mac
                             );
...
static i32 handler_diag_mib_retrieve(CYG_HTTPD_STATE *p)
{
    vtss_isid_t  isid = 0;
    const char   *err = NULL;
    BOOL         opr_lock = FALSE;
    static char  buf[VTSS_ETH_LINK_OAM_RESPONSE_BUF] = {0};
    mesa_rc      rc;
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        if (!port_rc) {
            ...
            vtss_eth_link_oam_mgmt_client_register_cb(uport2iport(port), vtss_eth_link_oam_send_response_to_cli, (char *)buf, VTSS_ETH_LINK_OAM_RESPONSE_BUF);
        }
        if (!memcmp(sel, "1", len)) {
            rc = eth_link_oam_mgmt_port_mib_retrieval_oper_set(isid, uport2iport(port), 1);
        } else {
            rc = eth_link_oam_mgmt_port_mib_retrieval_oper_set(isid, uport2iport(port), 2);
        }
        if (rc == VTSS_RC_INV_STATE) {
            ...
            opr_lock = vtss_eth_link_oam_mib_retrieval_opr_lock();
            if (opr_lock == FALSE) {
        ...
        cyg_httpd_start_chunked("html");
        cyg_httpd_write_chunked(buf, VTSS_ETH_LINK_OAM_RESPONSE_BUF);
        cyg_httpd_end_chunked();
    }
...
static i32 handler_config_link_oam_events(CYG_HTTPD_STATE *p)
{
    vtss_isid_t isid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    int                    ct;
    port_iter_t            pit;
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        if (port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_RC_OK) {
            while (port_iter_getnext(&pit)) {
                ...
                ct = snprintf(p->outbuffer, sizeof(p->outbuffer),
                              "%d/%u/%u/" VPRI64u"/" VPRI64u"/%u/%u|",/*/" VPRI64u"/%d/%d/" VPRI64u"|*/
                              pit.uport,
                              error_frame_window,
...
static i32 handler_stat_link_oam_link_status(CYG_HTTPD_STATE *p)
{
    vtss_isid_t                  isid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    port_iter_t                  pit;
    mesa_rc                      rc;
    ...
    if (port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_NPI) == VTSS_RC_OK) {
        while (port_iter_getnext(&pit)) {
            ...
            ct = snprintf(p->outbuffer, sizeof(p->outbuffer),
                          "%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/" VPRI64u"/" VPRI64u"/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/" VPRI64u"/" VPRI64u"/%u/%u/%u/%u/" VPRI64u"/" VPRI64u"/" VPRI64u"/" VPRI64u"/" VPRI64u"/" VPRI64u"/" VPRI64u"/" VPRI64u"/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u/%u|",
                          pit.uport,
                          seq_num,
