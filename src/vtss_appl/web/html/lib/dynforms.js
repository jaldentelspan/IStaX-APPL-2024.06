...
function addTextCell(tr, value, style, colspan, name)
{
    var cell = addTypedTextCell(tr, "td", value, style, colspan);
    if (name) {
        cell.id = cell.name = name;
    }
    return cell;
}

function addTextHeaderCell(tr, value, style, colspan, rowspan, width, name)
{
    var cell = addTypedTextCell(tr, "th", value, style, colspan, rowspan);
...
function addCheckBoxCell(tr, value, style, name, onchange)
{
    var td = CreateTd(style);
...
function addSelectCell(tr, oT, oV, value, style, name, width, colspan, disabled, onchange, oD)
{
    var td = CreateTd(style);
    ...
    for (x=0; x < oT.length; x++) {
        ...
        if (oD && oD[x]) {
            optionItem.setAttribute("disabled", oD[x]); // disable option in drop down menu
        }
...
function addInputCell(tr, value, style, name, size, maxsize, width, colspan, disabled, onchange)
{
    var td = CreateTd(style);
    var field = document.createElement('input');
    field.id = field.name = name;
...
function addImageCell(tr, style, src, text, name, onclick)
{
    var td = CreateTd(style);
    var field = document.createElement('img');
    field.src = src;
...
function addButtonCell(tr, value, style, name, onclick)
{
    var td = CreateTd(style);
    var field = document.createElement('input');
    field.id = field.name = name;
 ...
 * @tbody: [Mandatory], table body.
 * @frag:  [Mandatory], fragment element.
 */
function updateTable(tbody, frag)
{
    // Get reference element
    var my_tbody = document.getElementById(tbody);
    clearChildNodes(my_tbody);
    my_tbody.appendChild(frag);
}

/* Find paraemt element by tag name.
 * @elem:     [Mandatory], current element.
 * @tag_name: [Mandatory], find the paraemt element with this tag name.
 */
function findParentElem(elem, tag_name)
{
    // Loopup until parent element is found
    tag_name = tag_name.toUpperCase();
    while (elem && elem.tagName !== tag_name) {
        elem = elem.parentNode;
    }

    return elem; // Return found element or null
}

/* Busy loop sleep
 * @milliseconds: [Mandatory], sleep time, 1 second = 1000 milliseconds.
 */
function milliSleep(milli_seconds)
{
    var start = new Date().getTime();
    while (1) {
        if ((new Date().getTime() - start) > milli_seconds) {
            break;
        }
    }
}
...
/* Get CSS style
 * @selector: [sel_txt], CSS selector.
 * @property: [Optional], CSS property.
 */
function getCssStyle(sel_txt, property)
{
    for (var idx = 0; idx < document.styleSheets.length; ++idx) {
        ...
        if (sheet) {
            ...
            if (rule) {
                var i = rule.length;
                while (i--) {
                    if (rule[i].selectorText && rule[i].selectorText.toLowerCase() === sel_txt.toLowerCase()) {
                        return property ? (rule[i].style[property]) : rule[i].style;
                    }
                }
            }
        }
...
function getSelectedOptElement(elem_id) {
    var fld = $(elem_id);
    for (var i = 0; i < fld.options.length; i++) {
        if (fld.options[i].selected) {
            return fld.options[i];
        }
    }
}
