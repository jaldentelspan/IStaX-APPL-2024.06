...
)
{
    ...
    case DHCP_SERVER_MESSAGE_OPTION_CODE_RELAY_AGENT:
        if ( b_same ) {
            /* check length */
            if ( pcode == DHCP_SERVER_MESSAGE_OPTION_CODE_CLIENT_ID || pcode == DHCP_SERVER_MESSAGE_OPTION_CODE_RELAY_AGENT) {
                min = 2;
            } else {
                min = 1;
            }
            ...
            if ( len < min ) {
                T_D("incorrect length %u for %s\n", len,
                    ...
                    (pcode == DHCP_SERVER_MESSAGE_OPTION_CODE_NETBIOS_SCOPE)   ? "NETBIOS_SCOPE"   :
                    "RELAY_AGENT" );
                return FALSE;
            }
    ...
    case DHCP_SERVER_MESSAGE_OPTION_CODE_OPTION_OVERLOAD:
        if ( b_same ) {
            /* check length */
            if ( len != 1 ) {
                T_D("incorrect length %u for OPTION_OVERLOAD\n", len);
                return FALSE;
            }
