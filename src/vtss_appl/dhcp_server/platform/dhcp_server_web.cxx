...
)
{
    ...
    const char                  *delete_prefix = "excluded_delete_";
    const char                  *low_prefix    = "excluded_low_";
    const char                  *high_prefix   = "excluded_high_";
...
)
{
    ...
    const char              *delete_prefix = "pool_delete_";
    const char              *add_prefix    = "pool_add_";
    char                    id_str[_ID_STR_LEN + 1];
    char                    encoded_string[3 * VTSS_APPL_DHCP_SERVER_POOL_NAME_LEN + 1];
    u32                     i;
    ...
    while ( dhcp_server_pool_get_next(&pool) == VTSS_RC_OK ) {
        // WebStaX escape pool name
        memset(encoded_string, 0, sizeof(encoded_string));
        _webstax_escape(pool.pool_name, encoded_string);
        ...
        if ( cyg_httpd_form_varable_find(p, id_str) ) {
            ...
            // normal escape pool name
            memset(encoded_string, 0, sizeof(encoded_string));
            _normal_escape(pool.pool_name, encoded_string);
            ...
            if ( cyg_httpd_form_varable_find(p, id_str) ) {
                ...
                if ( cyg_httpd_form_varable_find(p, id_str) ) {
                    ...
                    memset(id_str, 0, sizeof(id_str));
                    sprintf(id_str, "%s%s", delete_prefix, pool.pool_name);
    ...
    for ( i = 1; i <= DHCP_SERVER_POOL_MAX_CNT; i++ ) {
        ...
        // create new pool
        if ( dhcp_server_pool_set(&pool) != VTSS_RC_OK ) {
            T_D("dhcp_server_pool_set( %s )\n", pool.pool_name);
        }
    }
}
 ...
 */
static i32 handler_config_dhcp_server_mode(
    IN CYG_HTTPD_STATE  *p
)
{
    int             i;
    ...
    if ( p->method == CYG_HTTPD_METHOD_POST ) {
        ...
        if ( dhcp_server_enable_set((BOOL)i) != VTSS_RC_OK ) {
            T_D("Fail to set global mode, %d\n", i);
        }

        /* VLAN Enabled */
        for (vid = 1; vid < VTSS_VIDS; vid++) {
            if (dhcp_server_vlan_enable_get(vid, &b_enable) == VTSS_RC_OK) {
                (void) dhcp_server_vlan_enable_set((mesa_vid_t) vid, cyg_httpd_form_variable_check_fmt(p, "vlan_mode_%d", vid));
            }
        }
        ...
        // VLAN Enabled
        // Format: vlan:enabled(0 or 1),vlan:enabled, ... e.g.: 1:0,3:1,...
        b_first = TRUE;
        for (vid = 1; vid < VTSS_VIDS; vid++) {
            if (dhcp_server_vlan_enable_get(vid, &b_enable) == VTSS_RC_OK) {
                if (b_first == FALSE) { // Omit the comma before the first entry.
                    __STRING_GET(",");
                }
                __UINT_GET(vid);
                __STRING_GET(":");
 ...
 */
static i32 handler_config_dhcp_server_excluded(
    IN CYG_HTTPD_STATE  *p
)
{
    int                         sn;
 ...
 */
static i32 handler_config_dhcp_server_pool(CYG_HTTPD_STATE *p)
{
    int                         sn;
    char                        tmp_buf[80 + 1];
    BOOL                        b_first;
...
#define __VENDOR_POST(_n_) \
    ...
    if ( strlen(class_id) ) { \
        strcpy(pool.class_info[_n_].class_id, class_id); \
    } \
    if ( strlen(specific_str) ) { \
        if ( _hexval_get(specific_str, specific_info, &specific_info_len) ) { \
            pool.class_info[_n_].specific_info_len = specific_info_len; \
            memcpy(pool.class_info[_n_].specific_info, specific_info, VTSS_APPL_DHCP_SERVER_VENDOR_SPECIFIC_INFO_LEN); \
        } \
    }

#define __SERVER_GET(_f_) \
    for ( i = 0; i < (VTSS_APPL_DHCP_SERVER_SERVER_MAX_CNT - 1); i++ ) { \
        __IP_GET( pool._f_[i] ); \
        __STRING_GET("/"); \
    } \
    __IP_GET( pool._f_[i] ); \
 ...
 */
static i32 handler_config_dhcp_server_pool_config(CYG_HTTPD_STATE *p)
{
    const char          *var_string;
    size_t              len;
    dhcp_server_pool_t  pool;
    char                encoded_string[3 * VTSS_APPL_DHCP_SERVER_DOMAIN_NAME_LEN + 1];
    int                 i, j, k;
    mesa_ipv4_t         ip[VTSS_APPL_DHCP_SERVER_SERVER_MAX_CNT];
    char                class_id[VTSS_APPL_DHCP_SERVER_VENDOR_CLASS_ID_LEN + 1];
    char                specific_str[2 + 2 * VTSS_APPL_DHCP_SERVER_VENDOR_SPECIFIC_INFO_LEN + 1];
    ...
    if ( p->method == CYG_HTTPD_METHOD_POST ) {
        ...
        case VTSS_APPL_DHCP_SERVER_CLIENT_IDENTIFIER_TYPE_NAME:
            __STRING_POST( "pool_client_id_value", pool.client_identifier.u.name );
            break;
        ...
        case VTSS_APPL_DHCP_SERVER_CLIENT_IDENTIFIER_TYPE_NAME:
            __STRING_GET( pool.client_identifier.u.name );
            break;
 ...
 */
static i32 handler_stat_dhcp_server(CYG_HTTPD_STATE *p)
{
    ...
    if ( dhcp_server_statistics_get(&stats) != VTSS_RC_OK ) {
        T_D("dhcp_server_statistics_get()\n");
        (void)cyg_httpd_end_chunked();
        return -1;
    }
 ...
 *      web handler for statistics page.
 */
static i32 handler_stat_dhcp_server_binding(CYG_HTTPD_STATE *p)
{
    ...
    if ( cyg_httpd_form_varable_find(p, "clear_automatic") ) {
        dhcp_server_binding_clear_by_type( VTSS_APPL_DHCP_SERVER_BINDING_TYPE_AUTOMATIC );
    } else if ( cyg_httpd_form_varable_find(p, "clear_manual") ) {
        dhcp_server_binding_clear_by_type( VTSS_APPL_DHCP_SERVER_BINDING_TYPE_MANUAL );
    } else if ( cyg_httpd_form_varable_find(p, "clear_expired") ) {
        dhcp_server_binding_clear_by_type( VTSS_APPL_DHCP_SERVER_BINDING_TYPE_EXPIRED );
    ...
    while ( dhcp_server_binding_get_next(&binding) == VTSS_RC_OK ) {
        if ( b_first ) {
            b_first = FALSE;
        } else {
            __STRING_GET("|");
        ...
        // pool name
        __STRING_GET( binding.pool->pool_name );
        __STRING_GET(",");
 ...
 *      web handler for statistics page.
 */
static i32 handler_stat_dhcp_server_binding_data(CYG_HTTPD_STATE *p)
{
    dhcp_server_binding_t   binding;
    BOOL                    b_first;
    int                     sn;
    char                    tmp_buf[80 + 1];
    mesa_ipv4_t             binding_ip;
    char                    encoded_string[3 * VTSS_APPL_DHCP_SERVER_CLIENT_IDENTIFIER_NAME_LEN + 1];
    ...
    if ( dhcp_server_binding_get(&binding) != VTSS_RC_OK ) {
        (void)cyg_httpd_end_chunked();
        return -1;
    }
    ...
    case VTSS_APPL_DHCP_SERVER_CLIENT_IDENTIFIER_TYPE_NAME:
        ...
        // encode NAME to avoid ','
        memset(encoded_string, 0, sizeof(encoded_string));
        _all_escape(binding.identifier.u.name, encoded_string);
        __STRING_GET(encoded_string);
        break;
    case VTSS_APPL_DHCP_SERVER_CLIENT_IDENTIFIER_TYPE_MAC:
        __STRING_GET("MAC");
 ...
 *      web handler for statistics page.
 */
static i32 handler_stat_dhcp_server_declined(CYG_HTTPD_STATE *p)
{
    int                         sn;
