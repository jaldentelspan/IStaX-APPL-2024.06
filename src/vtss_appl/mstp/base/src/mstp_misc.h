...
static inline BOOL valid_port(uint max, uint portnum)
{
    return (portnum > 0 && portnum <= max);
}

static inline mstp_port_t *get_port(const mstp_bridge_t *mstp, uint portnum)
{
    return mstp->ist.portmap[portnum - 1];
}

static inline mstp_port_t *get_tport(const mstp_tree_t *tree, uint portnum)
{
    return tree->portmap[portnum - 1];
}

static inline mstp_tree_t *mstp_get_instance(mstp_bridge_t *mstp, u8 msti)
{
    return mstp->trees[msti];
}

static inline mstp_tree_t *mstp_get_tree_by_id(mstp_bridge_t *mstp, u16 MSTID)
{
    uint i;
...
static inline u32 calc_pathcost(u32 adminPathCost, u32 linkspeed)
{
    u32 pc;
...
static inline mstp_port_param_t *vtss_mstp_get_port_block(mstp_bridge_t *mstp, uint portnum)
{
...
static inline mstp_msti_port_param_t *vtss_mstp_get_msti_port_block(mstp_bridge_t *mstp, uint msti, uint portnum)
{
    uint ix;

    if (msti >= N_MSTI_MAX || !valid_port(mstp->n_ports, portnum)) {
        return NULL;
    }

    ix = (mstp->n_ports * msti) + (portnum - 1);
