 ...
 */
static u8 role2flags(PortRole_t role)
{
...
/**
 * vector2s() - Convert a Priority Vector to a display string
 */
static const char *vector2s(char *str, size_t sz, const PriorityVector_t *vec)
{
    char root[32], regRoot[32], desg[32];

    (void)vtss_mstp_bridge2str(root,    sizeof(root),    (const uint8_t *)&vec->rootBridgeId);
    (void)vtss_mstp_bridge2str(regRoot, sizeof(regRoot), (const uint8_t *)&vec->regRootBridgeId);
    (void)vtss_mstp_bridge2str(desg,    sizeof(desg),    (const uint8_t *)&vec->DesignatedBridgeId);
    snprintf(str, sz,
 ...
 */
static void pdu_set_times(mstp_bpdu_t *pdu, const mstp_port_t *port)
{
...
static void log_prio(PriorityVector_t const *prio, const char *id, uint port_no, u8 msti)
{
    ...
    T_I("%s priority %d[%d]: %s-[%lu]-%s", id, port_no, msti, root, unal_ntohl(prio->extRootPathCost.bytes), regroot);
}
#endif

static const mstp_bpdu_mstirec_t *getMstiRecord(const mstp_cistport_t *cist, u16 MSTID)
{
    uint i;
    u16  recMSTID;
    ...
    for (i = 0; i < cist->rcvBpdu.mstp_records; i++) {
        ...
        recMSTID = ((pRec->regional_root_bridge_id.bridgeIdPrio[0] & 0x0F) << 8) + pRec->regional_root_bridge_id.bridgeIdPrio[1];
        if (recMSTID == MSTID) {
            return pRec;
        }
    }
 ...
 */
static void updateMsgInfo(const mstp_cistport_t *cist, mstp_port_t *port)
{
    u16 MSTID;
 ...
 */
static u8 encode_flags(mstp_port_t *port)
{
 ...
 */
static void encode_msti_record(mstp_tree_t *tree, mstp_port_t *port, mstp_bpdu_mstirec_t *rec)
{
    /* 14.6.1 a) */
...
*/
static BOOL calculateMasterFlag(const mstp_tree_t *tree, const mstp_port_t *msti)
{
 ...
 */
static BOOL priorityIsBetter(PriorityVector_t const *prio, mstp_port_t const *port)
{
 ...
 */
static BOOL priorityIsBetterEq(PriorityVector_t const *prio, mstp_port_t const *port)
{
    return memcmp(prio, &port->portPriority, sizeof(PriorityVector_t)) <= 0;
}
 ...
 */
static BOOL priorityIsEqual(PriorityVector_t const *prio, mstp_port_t const *port)
{
    return memcmp(prio, &port->portPriority, sizeof(PriorityVector_t)) == 0;
}
 ...
 */
static BOOL priorityIsWorseEq(PriorityVector_t const *prio, mstp_port_t const *port)
{
    return memcmp(prio, &port->portPriority, sizeof(PriorityVector_t)) >= 0;
}
 ...
 */
static BOOL priorityIsWorse(PriorityVector_t const *prio, mstp_port_t const *port)
{
    return memcmp(prio, &port->portPriority, sizeof(PriorityVector_t)) > 0;
}
 ...
 */
static BOOL priorityIsSuperior(PriorityVector_t const *prio, mstp_port_t const *port)
{
    ...
    if (!superior) {
         ...
         * port).
         */
        if (memcmp(prio->DesignatedBridgeId.bridgeAddress, port->portPriority.DesignatedBridgeId.bridgeAddress, 6)      == 0 &&
            memcmp(prio->DesignatedPortId.bytes,           port->portPriority.DesignatedPortId.bytes, sizeof(PortId_t)) == 0) {
 ...
 */
static void send_bpdu(const mstp_port_t *port, mstp_bpdu_t *pdu, size_t len)
{
...
static u32 TotalXmits(const mstp_cistport_t *cist)
{
...
/** doBpduFiltering()
 */
static BOOL doBpduFiltering(const mstp_cistport_t *cist, const mstp_port_t *port)
{
    if (!portEnabled(cist)) { /* This os *non*-standard - but makes sense */
        return TRUE;
    }

    if (bpduFiltering(cist) && TotalXmits(cist) >= 3) {
...
RcvdInfo_t rcvInfo(mstp_port_t *port)
{
    ...
    T_N("Port %d[%d] - role %s, return %s", port->port_no, port->msti, rstp_portrole2str(role), rstp_rcvinfo2str(info));
    T_N("Port %d[%d] - portPrio %s",        port->port_no, port->msti, vector2s(str, sizeof(str), &port->portPriority));
    T_N("Port %d[%d] - msgPrio  %s",        port->port_no, port->msti, vector2s(str, sizeof(str), &port->msgPriority));
 ...
 * FALSE otherwise.
 */
static BOOL sameRootAsCist(const mstp_port_t *port)
{
    BOOL        ret    = FALSE;
    ...
    const PriorityVector_t *msgPrio = &port->cistport->rcvBpdu.priority;
    const PriorityVector_t *prtPrio = &cport->portPriority;
...
void recordDispute(mstp_port_t *port)
{
    ...
    T_I("Port %d[%d] - learning %s",      port->port_no, port->msti, (flags & MSTP_BPDU_FLAG_LEARNING) ? "ON" : "OFF");
    T_I("Port %d[%d] - Disputed:Port %s", port->port_no, port->msti, vector2s(str, sizeof(str), &port->portPriority));
    T_I("Port %d[%d] - Disputed:Mesg %s", port->port_no, port->msti, vector2s(str, sizeof(str), &port->msgPriority));
...
void recordMastered(mstp_port_t *port)
{
    ...
    if (isCistPort(port)) {
         ...
         */
        port->mastered = (cist->operPointToPointMAC && (getBpduFlags(port) & MSTP_BPDU_FLAG_MASTER));
    }
}
...
void recordProposal(mstp_port_t *port)
{
    ...
    if (isCistPort(port)) {
         ...
         */
        if (role == DesignatedPort && (flags & MSTP_BPDU_FLAG_PROPOSAL)) {
            port->proposed = TRUE;
        }
...
void setTcFlags(mstp_port_t *port)
{
    ...
    T_D("Port %d[%d] - TC %d Flags 0x%02x", port->port_no, port->msti, !!(flags & MSTP_BPDU_FLAG_TC), flags);
...
void setTcPropTree(const mstp_port_t *iport)
{
    mstp_tree_t *tree;
    mstp_port_t *port;

    T_D("Port %d[%d] restrictedTcn %d", iport->port_no, iport->msti, iport->cistport->restrictedTcn);
 ...
 */
static void syncMaster(mstp_bridge_t *mstp)
{
    mstp_port_t     *port;
    mstp_cistport_t *cist;
...
void txMstp(mstp_port_t *port)
{
    ...
    if (pdu.protocolVersionIdentifier == MSTP_PROTOCOL_VERSION_MSTP) {
        ...
        ForAllMstiTrees(mstp, tree, {
            msti = get_tport(tree, port->port_no);
            if (msti)
            {
        ...
        dynparts = nrecords * sizeof(mstp_bpdu_mstirec_t);
        v3len = /* Static parts */ sizeof(pdu.mstp) + dynparts;
 ...
 *
 */
void updtRolesTree(mstp_tree_t *tree)
{
    PriorityVector_t rootPriority = tree->BridgePriority; /* Default to myself */
    PortId_t         rootPortId;
    mstp_port_t      *port, *root = NULL, *cport;
    mstp_cistport_t  *cist;
    PortRole_t       oldrole;
    ...
    if (tree->msti == MSTID_CIST &&
        struct_cmp(tree->rootPriority.regRootBridgeId, !=, rootPriority.regRootBridgeId) &&
    ...
    ForAllPorts(tree, port) {
        ...
        if (tree->msti == MSTID_CIST) {
            if (!port->cistport->sendRSTP) {
                port->designatedPriority.regRootBridgeId = tree->BridgeIdentifier;
            }
        }
...
timer FwdDelay(const mstp_port_t *port)
{
    ...
    cport = get_port(port->cistport->bridge, port->port_no);
 ...
 */
BOOL allSynced(const struct mstp_tree *tree, const mstp_port_t *iport)
{
    mstp_port_t *port;
