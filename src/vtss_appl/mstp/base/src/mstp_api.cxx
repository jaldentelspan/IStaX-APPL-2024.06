...
static void strncpyz(char *dst, const char *src, size_t maxlen)
{
    (void)strncpy(dst, src, maxlen);
    dst[maxlen - 1] = '\0'; /* Explicit null terminated/truncated */
}

static void dispose_ports(mstp_tree_t *tree)
{
    mstp_port_t *port;
...
static void initialize_port(mstp_port_t *port, mstp_tree_t *tree, mstp_cistport_t *cist, uint portnum)
{
    ...
    port->tree = tree;
    port->cistport = cist;
    port->role = UnknownPort;    // For meaningful initial transition
...
static mstp_port_t *add_tport(mstp_tree_t *tree, mstp_cistport_t *cist, uint portnum)
{
    mstp_port_t *tport;

    if ((tport = (mstp_port_t *)zmalloc(sizeof(*tport))) == NULL) {
        return NULL;
    }
...
static int vtss_mstp_mac2str(char *buffer, size_t size, const u8 *mac)
{
...
static void mstp_run_stm_tree(mstp_tree_t *tree, uint *transitions)
{
    size_t      i;
    mstp_port_t *port;
    ...
    for (i = 0; i < ARR_SZ(tree_stms); i++) {
        ...
        if ((new_state = stm->run(tree, transitions, tree->state[i])) != tree->state[i]) {
            T_D("Run - bridge %d stm %s, state %s => %s", tree->msti,  stm->name, stm->statename(tree->state[i]), stm->statename(new_state));
    ...
    ForAllPorts(tree, port) {
        ...
        for (i = 0; i < ARR_SZ(msti_stms); i++) {
            ...
            if ((new_state = stm->run(port, transitions, port->state[i])) != port->state[i]) {
                T_D("Run - MSTI %d[%d] stm %s, state %s => %s", port->port_no, tree->msti, stm->name, stm->statename(port->state[i]), stm->statename(new_state));
...
static void mstp_run_stm(mstp_bridge_t *mstp)
{
    uint            transitions, loops, i;
...
static void cist_stm_begin(mstp_port_t *port)
{
    mstp_cistport_t *cist;
    size_t          i;
    ...
    for (i = 0; i < ARR_SZ(cist_stms); i++) {
        ...
        T_D("Begin - cistport %d stm %s, state %s", port->port_no, stm->name, stm->statename(cist->state[i]));
    }
}

static void port_stm_begin(mstp_port_t *port)
{
    size_t i;
    ...
    for (i = 0; i < ARR_SZ(msti_stms); i++) {
        ...
        T_D("Begin - tport %d[%d] stm %s, state %s", port->port_no, port->msti, stm->name, stm->statename(port->state[i]));
    }
}

static void tree_stm_begin(mstp_tree_t *tree)
{
    size_t i;
...
static void bridge_stm_begin(mstp_bridge_t *bridge)
{
    mstp_port_t *port;
...
static void mstp_init_tree(mstp_bridge_t *bridge, u8 msti, u16 MSTID, mstp_tree_t *tree, const mstp_macaddr_t *bridge_id)
{
    tree->msti = msti;
    tree->bridge = bridge;
 ...
 * used internally.
 */
static BOOL vtss_mstp_create_msti(mstp_bridge_t *mstp, uint msti, u16 MSTID)
{
    mstp_tree_t *tree;
    ...
    if ((MSTID == MSTID_CIST || MSTID > MSTID_MAX) ||
        ...
        (tree = (mstp_tree_t *)zmalloc(sizeof(*tree))) == NULL) {
        return FALSE;    /* Entry taken/no room/no memory */
    }

    /* Add portmap */
    tree->portmap = (mstp_port_t **)zmalloc(mstp->n_ports * sizeof(mstp_port_t *));
    if (tree->portmap) {
 ...
 * used internally.
 */
static BOOL vtss_mstp_delete_msti(mstp_bridge_t *mstp, uint msti)
{
    mstp_tree_t *tree;
...
static BOOL apply_mapping(mstp_bridge_t *mstp, mstp_map_t *map)
{
    uint                   i;
    BOOL                   in_use[N_MSTI_MAX];
    vtss_appl_mstp_mstid_t msti;
    mstp_port_t            *port;
    mstp_tree_t            *tree;
    ...
    for (i = MSTI_MSTI1; i < N_MSTI_MAX; i++) {
        if (in_use[i] && mstpVersion(mstp)) {
            if (!mstp->trees[i] && !vtss_mstp_create_msti(mstp, i, i)) {
...
mstp_bridge_t *_vtss_mstp_create_bridge(const mstp_macaddr_t *bridge_id, uint n_ports)
{
    ...
    mstp_port_param_t      *cparam;
    mstp_msti_port_param_t *mparam;

    if ((bridge = (mstp_bridge_t *)zmalloc(sizeof(*bridge))) != NULL &&
        (bridge->ist.portmap = (mstp_port_t **)zmalloc(n_ports * sizeof(mstp_port_t *))) != NULL &&
        (bridge->conf.cist = (mstp_port_param_t *)zmalloc(n_ports * sizeof(mstp_port_param_t))) != NULL &&
        (bridge->conf.msti = (mstp_msti_port_param_t *)zmalloc(N_MSTI_MAX * n_ports * sizeof(mstp_msti_port_param_t))) != NULL) {
        ...
        for (port = 1; port <= n_ports; port++) {
            cparam = vtss_mstp_get_port_block(bridge, port);
            ...
            cparam->restrictedRole = FALSE;
            cparam->restrictedTcn = FALSE;
            cparam->bpduGuard = FALSE;

            for (msti = 0; msti < N_MSTI_MAX; msti++) {
                mparam = vtss_mstp_get_msti_port_block(bridge, msti, port);
        ...
        (void)vtss_mstp_mac2str((char *)bridge->cfgid.cfgid_name, sizeof(bridge->cfgid.cfgid_name), bridge_id->mac);
...
mesa_rc _vtss_mstp_set_mapping(mstp_bridge_t *mstp, mstp_map_t *map)
{
    ...
    return rc ? VTSS_RC_OK : VTSS_RC_ERROR;
}
...
mesa_rc _vtss_mstp_get_bridge_status(mstp_bridge_t *mstp, uint msti, mstp_bridge_status_t *status)
{
     ...
     * non-zero.
     */
    status->timeSinceTopologyChange = (tree->lastTopologyChange ? vtss_mstp_current_time() - tree->lastTopologyChange : MSTP_TIMESINCE_NEVER);
    ...
    status->forceVersion = (mstp_forceversion_t) ForceProtocolVersion(mstp);
...
{
    mstp_port_t *port;
    BOOL        mustInit = FALSE, mustUpdateTimes = FALSE, resetTxCount = FALSE;  /* 17.13 paragraph 4 */
    Times_t     BridgeTimes;

    memset(&BridgeTimes, 0, sizeof(BridgeTimes)); // Assure that all bytes including padding bytes
...
mesa_rc _vtss_mstp_set_bridge_priority(mstp_bridge_t *mstp, uint msti, u8 bridgePriority)
{
    ...
    tree = mstp_get_instance(mstp, msti);
    if (!tree) {
        return VTSS_RC_ERROR;
    }
...
mesa_rc _vtss_mstp_add_port(mstp_bridge_t *mstp, uint portnum)
{
    ...
    if (!valid_port(mstp->n_ports, portnum) ||
        ...
        (port = (mstp_port_t *)zmalloc(sizeof(mstp_port_t) + sizeof(mstp_cistport_t))) == NULL) {
        return VTSS_RC_ERROR;
    }
    ...
    /* Configure */
    cparam = vtss_mstp_get_port_block(mstp, portnum);
    VTSS_ASSERT(cparam != NULL);
    ...
    for (i = 0; i < ARR_SZ(cist_stms); i++) {
        const port_stm_t *stm = cist_stms[i];
        cist->state[i] = stm->begin(port);
        T_D("Begin - port %d stm %s, state %s", port->port_no, stm->name, stm->statename(cist->state[i]));
    }
...
{
    return (valid_port(mstp->n_ports, portnum) && get_port(mstp, portnum) != NULL) ? VTSS_RC_OK : VTSS_RC_ERROR;
}
...
mesa_rc _vtss_mstp_get_port_status(mstp_bridge_t *mstp, uint msti, uint portnum, mstp_port_status_t *status)
{
    ...
    /* b) State */
    status->state = (portEnabled(cist) ? (port->forwarding ? PORTSTATE_FORWARDING : (port->learning ? PORTSTATE_LEARNING : PORTSTATE_DISCARDING)) : PORTSTATE_DISABLED);
...
mesa_rc _vtss_mstp_set_port_parameters(mstp_bridge_t *mstp, uint portnum, const mstp_port_param_t *param)
{
    ...
    *cparam = *param;
    ...
    if ((port = get_port(mstp, portnum)) == NULL) {
        return VTSS_RC_OK;    /* Port not active, just return */
    }
...
mesa_rc _vtss_mstp_set_msti_port_parameters(mstp_bridge_t *mstp, uint msti, uint portnum, const mstp_msti_port_param_t *param)
{
    ...
    *mparam = *param;
    ...
    if ((port->portId.bytes[0] & 0xf0) != port->adminPortPriority) {
        T_I("Port %d priority change %d -> %d", portnum, port->portId.bytes[0] & 0xf0, port->adminPortPriority);
...
#define dec(x)  do { if(x) { x--; } } while(0)
    ...
    if (mstp->ist.portlist) {
        ...
        ForAllCistPorts(mstp, port) {
             ...
             */
            if (cist->stpInconsistent && cist->errorRecoveryWhile) {
                if (--cist->errorRecoveryWhile == 0) {
                    vtss_mstp_log("STP inconsistent port %s recovered by timeout", port->port_no);
                    cist->stpInconsistent = FALSE;
                }
            }
...
static void getTimes(mstp_cistport_t *cist, const mstp_bpdu_t *bpdu)
{
...
static void getStpPrio(mstp_cistport_t *cist, const mstp_bpdu_t *bpdu)
{
...
static void getMstpPrio(mstp_cistport_t *cist, const mstp_bpdu_t *bpdu)
{
    cist->rcvBpdu.priority.rootBridgeId = bpdu->rootBridgeId;
    cist->rcvBpdu.priority.extRootPathCost = bpdu->rootPathCost;
    cist->rcvBpdu.priority.regRootBridgeId = bpdu->DesignatedBridgeId;
...
void _vtss_mstp_rx(const mstp_bridge_t *mstp, uint portnum, const void *buffer, size_t size)
{
    ...
    if (!valid_port(mstp->n_ports, portnum) || (port = get_port(mstp, portnum)) == NULL) {
        return;
    }
    ...
    if (size > (14 + 3) &&       /* MAC + LLC present*/
         ...
         */
        if (!(bpdu->protocolIdentifier[0] == 0 && bpdu->protocolIdentifier[1] == 0)) {
        ...
        if (decode_as != BPDU_ILL) {
            if (bpduGuard(cist)) {
                vtss_mstp_log("STP inconsistent port %s disabled (BPDU Guard)", portnum);
...
{
    return snprintf((char *)buffer, size,
