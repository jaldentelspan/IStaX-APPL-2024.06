...
static i32 handler_config_time_dst(CYG_HTTPD_STATE *p)
{
    int             ct, var_value;
    ...
    size_t        len;
    int           tz, offset_hours, offset_minutes;
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        if (cyg_httpd_form_varable_int(p, "time_zone", &tz)) {
            if (tz == 99999) { // Manual setting
                if (cyg_httpd_form_varable_int(p, "time_offset_hours", &offset_hours) &&
                    cyg_httpd_form_varable_int(p, "time_offset_minutes", &offset_minutes)) {
                    newconf.tz = offset_hours * 600;
                    if (offset_hours >= 0) {
                        newconf.tz += offset_minutes * 10;
                    } else {
                        newconf.tz -= offset_minutes * 10;
                    }
                    newconf.tz_offset = newconf.tz / 10;
                }
            } else {
                newconf.tz = tz;
        ...
        if (len > 0) {
            ...
            T_D("POST: acronym = %s.", newconf.tz_acronym);
        } else {
            strcpy(newconf.tz_acronym, "");
            newconf.tz_acronym[0] = '\0';
            T_D("POST: acronym = null string.");
        }

        /******************** daylight saving time configuration ********************/
        // mode
        if (cyg_httpd_form_varable_int(p, "mode", &var_value)) {
            newconf.dst_mode = (vtss_appl_clock_summertime_mode_t)var_value;
        }
        // week_s
        ...
        */
        ...
        if (strlen(conf.tz_acronym)) {
            ...
            T_D("GET: acronym = %s(%s).", conf.tz_acronym, encoded_string);
        } else {
            T_D("GET: acronym = null string.");
        }
