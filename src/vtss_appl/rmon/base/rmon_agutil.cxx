...
AGUTIL_advance_index_name(struct variable *vp, oid *name,
                          size_t *length, size_t key_cnt, int exact)
{
    int             result;

    if (exact) {
        /* Do a simplest length check here. */
        if (*length != vp->namelen + key_cnt) {
            return -1;
        }
        return 0;
    }
...
{
    /* According to RFC1213, the unit of sysUpTime is hundrendths of second */
    return msg_uptime_get(VTSS_ISID_LOCAL) * 100;
}
...
SYSTEM_get_eth_statistics(VAR_OID_T *data_source, ETH_STATS_T *where)
{
    ...
    agent_get_Rmon_ethernet_statistics(where->ifIndex, 1,       /* exact */
                                       where);
#else                           /* OPTICALL_ACESS */
    static ETH_STATS_T prev = { -1u, -1u };
    static time_t   ifLastRead = 0;
    time_t          curr_time;
    u_char          need_to_read;
    u_long          rc;

    where->ifIndex = vtss_ifindex_cast_from_u32_0(data_source->objid[data_source->length - 1]);
    ...
    need_to_read = vtss_ifindex_cast_to_u32(prev.ifIndex);
    memcpy(&prev, where, sizeof(ETH_STATS_T));
    prev.ifIndex = vtss_ifindex_cast_from_u32_0(need_to_read);
...
{
    dbg_mem_cnt++;
    return rmon_callout_malloc(size);
}
