...
#else
#  define AGMALLOC(X)   rmon_callout_malloc(X)
#  define AGFREE(X) { rmon_callout_free(X); X = NULL; }
#  define AGSTRDUP(X)   rmon_callout_strdup(X)
#endif
...
#else
typedef vtss_var_oid_t VAR_OID_T;
#endif
...
#else
typedef vtss_eth_stats_t ETH_STATS_T;
#endif /* Added by SGZ end     */
#endif

/* peter, 2007/8, add SNMP ag_trace switch */
//#define SNMP_AGTRACE_ENABLE
