...
#define MAX_CREATION_TIME   300 //60
...
rowapi_delete(RMON_ENTRY_T *eold)
{
    ...
    if (eold->timer_id) {
        ROWAPI_alarm_unregister(eold->timer_id);
    }
...
rowapi_too_long_creation_callback(unsigned int clientreg, void *clientarg)
{
    ...
    if (RMON1_ENTRY_VALID != eptr->status) {
        ...
        /* Since this alarm is registered by single event, the alarm will be unregistered automatically */
        eptr->timer_id = 0;
        rowapi_delete(eptr);
    }
}
...
rowapi_deactivate(TABLE_DEFINTION_T *table_ptr, RMON_ENTRY_T *eptr)
{
    ...
    eptr->timer_id = ROWAPI_alarm_register(MAX_CREATION_TIME, 0,
...
rowapi_activate(TABLE_DEFINTION_T *table_ptr, RMON_ENTRY_T *eptr)
{
    ...
    ROWAPI_alarm_unregister(eptr->timer_id);
    eptr->timer_id = 0;
...
ROWAPI_new(TABLE_DEFINTION_T *table_ptr, u_long ctrl_index)
{
    ...
    enew->timer_id = ROWAPI_alarm_register(MAX_CREATION_TIME, 0,
                                           rowapi_too_long_creation_callback,
                                           enew);
                  ...
                  SCROLLER_T * (*extract_scroller) (void *body))
{
    ...
    table_ptr->extract_scroller = extract_scroller;
                           ...
                           void *entry_ptr, size_t entry_size)
{
    ...
    if (vp) {
        ...
        if (0 != AGUTIL_advance_index_name(vp, name, length, 1, exact)) {
                         ...
                         TABLE_DEFINTION_T *table_ptr, size_t entry_size)
{
    ...
    if (action == *prev_action) {
        //return ROW_ERR_NOERROR;    /* I want to process it only once ! */
    }
    *prev_action = action;
                ...
                int (*data_destructor) (struct data_scroller *, void *))
{
    ...
    scrlr->first_data_ptr = 0;
    scrlr->last_data_ptr = 0;
    scrlr->current_data_ptr = NULL;
...
{
    NEXTED_PTR_T *data_ptr = (NEXTED_PTR_T *)delete_me;
    NEXTED_PTR_T *tmp;
    ...
    if (data_ptr == scrlr->first_data_ptr) {
        ...
        for (tmp = (NEXTED_PTR_T *)scrlr->first_data_ptr; tmp; tmp = (NEXTED_PTR_T *)tmp->next) {
...
realloc_number_of_data(SCROLLER_T *scrlr, long dlong)
{
    ...
    if (dlong > 0) {
        for (; dlong; dlong--, prev = (NEXTED_PTR_T *)bptr, scrlr->data_created++) {
        ...
        if (scrlr->last_data_ptr) {
            scrlr->last_data_ptr->next = (NEXTED_PTR_T *)first;
        } else {
...
ROWDATAAPI_descructor(SCROLLER_T *scrlr)
{
    ...
    for (bptr = (NEXTED_PTR_T *)scrlr->first_data_ptr; bptr; bptr = (NEXTED_PTR_T *)next) {
        next = bptr->next;
        if (scrlr->data_destructor) {
...
ROWDATAAPI_locate_new_data(SCROLLER_T *scrlr)
{
    ...
    if (!scrlr->current_data_ptr) {     /* there was wrap */
        bptr = (NEXTED_PTR_T *)scrlr->first_data_ptr;
        if (!bptr) {
        ...
        bptr = (NEXTED_PTR_T *)scrlr->current_data_ptr;
                            ...
                            size_t data_size, void *entry_ptr)
{
    ...
    if (vp) {
#endif
        if (0 != AGUTIL_advance_index_name(vp, name, length, 2, exact)) {
            ag_trace("cannot advance_index_name");
            return NULL;
        }
        ...
        ctrl_indx = name[*length - 2];
        data_index = name[*length - 1];
    }
#endif
        ...
        if (hdr) {
            ...
            if (!bptr) {        /* travel to next row */
                ...
                for (hdr = hdr->next; hdr; hdr = hdr->next) {
                    ...
                    for (bptr = (NEXTED_PTR_T *)scrlr->first_data_ptr; bptr;
                         bptr = (NEXTED_PTR_T *)bptr->next) {
                        if (bptr->data_index) {
                            break;
                        }
                    }
            ...
            if (bptr) {         /* set new index */
                ...
                if (vp) {
                    ...
                    *length = vp->namelen + 2;
                    ...
                    name[*length - 2] = hdr->ctrl_index;
                    name[*length - 1] = bptr->data_index;
                }
#endif
            } else {
                hdr = NULL;
...
int     ROWAPI_commit_status(oid *name,
...
{
    int rc = 0;
    rc =  ROWAPI_do_another_action(name, tbl_first_index_begin, ACTION,
                                   prev_action, table_ptr, entry_size);

    if (rc) {
        ROWAPI_do_another_action(name,
    ...
    rc =  ROWAPI_do_another_action(name, tbl_first_index_begin, COMMIT,
                                   prev_action, table_ptr, entry_size);
...
void ROWAPI_alarm_unregister(unsigned int clientreg)
{
    rmon_timer_unregister(clientreg);
}

unsigned int ROWAPI_alarm_register(unsigned int when, unsigned int flags,
                                   ROWAPIAlarmCallback *thecallback,
                                   void *clientarg)
{
    return rmon_timer_register(when, flags,
                               thecallback,
                               clientarg);
}
