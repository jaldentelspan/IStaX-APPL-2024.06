...
#if VTSS_APPL_SFLOW_RECEIVER_CNT != 1 || VTSS_APPL_SFLOW_INSTANCE_CNT != 1
...
/******************************************************************************/
static void SFLOW_web_err_msg_compose(CYG_HTTPD_STATE *p, int errors, mesa_rc rc)
{
    if (errors) {
...
/******************************************************************************/
static i32 SFLOW_web_handler_config(CYG_HTTPD_STATE *p)
{
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        ...
        if (inbuf && cgi_unescape(inbuf, ipstr, len, sizeof(ipstr))) {
            if (mgmt_txt2ipv4_ext(ipstr, &agent_cfg.agent_ip_addr.addr.ipv4, NULL, FALSE, TRUE, TRUE, TRUE) == VTSS_RC_OK) {
                agent_cfg.agent_ip_addr.type = MESA_IP_TYPE_IPV4;
            }
#ifdef VTSS_SW_OPTION_IPV6
            else if (mgmt_txt2ipv6(ipstr, &agent_cfg.agent_ip_addr.addr.ipv6) == VTSS_RC_OK) {
                agent_cfg.agent_ip_addr.type = MESA_IP_TYPE_IPV6;
            }
#endif
            else {
        ...
        if (!errors && memcmp(&rcvr_cur_cfg, &rcvr_new_cfg, sizeof(rcvr_cur_cfg)) != 0) {
            ...
            strcpy(rcvr_new_cfg.owner, VTSS_APPL_SFLOW_OWNER_LOCAL_MANAGEMENT_STRING);
        ...
        while (!errors && port_iter_getnext(&pit)) {
            ...
            if (fs.enabled) {
                ...
                if (!cyg_httpd_form_variable_u32_fmt(p, &fs.sampling_rate, "fs_sampling_rate_%u", pit.uport)) {
                    errors++;
                }
                if (!cyg_httpd_form_variable_u32_fmt(p, &fs.max_header_size, "fs_max_header_size_%u", pit.uport)) {
                    errors++;
                }
            }
            ...
            if (cp.enabled) {
                ...
                if (!cyg_httpd_form_variable_u32_fmt(p, &cp.interval, "cp_interval_%u", pit.uport)) {
                    errors++;
                }
            }
        ...
        char encoded_owner[3 * VTSS_APPL_SFLOW_OWNER_LEN];
        ...
        allow_changes = strcmp(rcvr_cur_cfg.owner, VTSS_APPL_SFLOW_OWNER_LOCAL_MANAGEMENT_STRING) == 0 || rcvr_cur_cfg.owner[0] == '\0';
...
/******************************************************************************/
static i32 SFLOW_web_handler_status(CYG_HTTPD_STATE *p)
{
    vtss_isid_t               isid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    ...
    cnt = snprintf(p->outbuffer, sizeof(p->outbuffer), "%s/%u/%s/" VPRI64u"/" VPRI64u"/" VPRI64u"/" VPRI64u,
                   encoded_owner,
                   rcvr_info.timeout_left, // Not the previously configured timeout, but what is left now.
                   encoded_hostname,
    ...
    if (clear_ports) {
        ...
        // switch_statistics, is initialized to 0x0 (done by CapArray)
    } else {
    ...
    (void)port_iter_init(&pit, NULL, isid, PORT_ITER_SORT_ORDER_IPORT, PORT_ITER_FLAGS_NORMAL);
    while (port_iter_getnext(&pit)) {
        cnt = snprintf(p->outbuffer, sizeof(p->outbuffer), "#%u/" VPRI64u"/" VPRI64u,
                       pit.uport,
...
#define SFLOW_WEB_BUF_LEN 512

static size_t sflow_lib_config_js(char **base_ptr, char **cur_ptr, size_t *length)
{
    char buff[SFLOW_WEB_BUF_LEN];
    (void) snprintf(buff, SFLOW_WEB_BUF_LEN,
                    "var sflowSamplingRateMax = %d;\n",
                    fast_cap(MESA_CAP_L2_SFLOW_SAMPLE_RATE_MAX));
...
/****************************************************************************/
web_lib_config_js_tab_entry(sflow_lib_config_js);
