...
// SFLOW_snmp_ip2bin()
/******************************************************************************/
static void SFLOW_snmp_ip2bin(mesa_ip_addr_t *ip_addr, char *buf, size_t *len)
{
...
// SFLOW_snmp_oid2str()
/******************************************************************************/
static const char *SFLOW_snmp_oid2str(uchar magic)
{
    switch (magic) {
    case SFLOWVERSION:
        return "AgentVersion";
...
/******************************************************************************/
static const char *SFLOW_snmp_action2str(int action)
{
    switch (action) {
    case RESERVE1:
        return "RESERVE1";
...
//
// Returns the ifIndex of the first (#first == TRUE) or
// the next (#first == FALSE) non-stacking port.
//
// If an error occurs or there are no more non-stacking ports, the function
// returns 0. Otherwise it returns a valid ifIndex.
/******************************************************************************/
static u32 SFLOW_snmp_ifindex_getnext(u32 ifindex_input, BOOL first)
{
    u32            if_types;
    iftable_info_t info;
    ...
    if (ifIndex_get_next_by_type(&info, if_types) == FALSE) {
        return 0;
    }
...
static int SFLOW_snmp_oid_table_parse(oid *name, size_t length, int exact, sflow_snmp_table_keys_t *keys)
{
    ...
    // Filter out the mismatched length when the SNMP request is GET operation(exact = 1)
    if (exact && (length != exact_len || (ifindex_len != ARRSZ(SFLOW_ifindex_oid) + 1))) {
        return -1; // Error
    }
    ...
    if (length > ifindex_oid_idx) {
        ...
        if (cmp_rc > 0) {
            if (exact_len - length == 1) {
                //when request OID is SFLOW_ifindex_oid + port id, ex: 1.3.6.1.4.1.14706.1.1.5.1.3.11.1.3.6.1.2.1.2.2.1.1.1000001
...
// SFLOW_snmp_rcvr_get()
/******************************************************************************/
static mesa_rc SFLOW_snmp_rcvr_get(sFlowRcvrTable_entry_t *table_entry)
{
    mesa_rc rc;
...
// SFLOW_snmp_rcvr_set()
/******************************************************************************/
static mesa_rc SFLOW_snmp_rcvr_set(sFlowRcvrTable_entry_t *table_entry)
{
    mesa_rc rc;
...
{
    if (getnext) {
        if (++table_entry->rcvr_idx > VTSS_APPL_SFLOW_RECEIVER_CNT) {
            return 1;
        }
    }
    return SFLOW_snmp_rcvr_get(table_entry) == VTSS_RC_OK ? 0 : 1;
...
static int write_sFlowRcvrTimeout(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid *name, size_t name_len)
{
    ...
    case RESERVE1:
        ...
        if (set_value > VTSS_APPL_SFLOW_RECEIVER_TIMEOUT_MAX) {
...
static int write_sFlowRcvrMaximumDatagramSize(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid *name, size_t name_len)
{
    ...
    case RESERVE1:
        ...
        if (set_value < VTSS_APPL_SFLOW_RECEIVER_DATAGRAM_SIZE_MIN || set_value > VTSS_APPL_SFLOW_RECEIVER_DATAGRAM_SIZE_MAX) {
...
static int write_sFlowRcvrAddressType(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid *name, size_t name_len)
{
    ...
    case RESERVE1:
        ...
        if (set_value != MESA_IP_TYPE_IPV4
#ifdef VTSS_SW_OPTION_IPV6
            && set_value != MESA_IP_TYPE_IPV6
#endif
           ) {
            (void)snmp_log(LOG_ERR, "Write to sFlowRcvrAddressType: Bad value\n");
            return SNMP_ERR_WRONGVALUE;
...
static int write_sFlowRcvrPort(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid *name, size_t name_len)
{
    ...
    case RESERVE1:
        ...
        if (set_value < 0 || set_value > VTSS_APPL_SFLOW_RECEIVER_UDP_PORT_MAX) {
...
static int write_sFlowRcvrDatagramVersion(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid *name, size_t name_len)
{
    ...
    case RESERVE1:
        ...
        if (set_value != VTSS_APPL_SFLOW_DATAGRAM_VERSION) {
            (void)snmp_log(LOG_ERR, "Write to sFlowRcvrAddressType: Bad value\n");
            return SNMP_ERR_WRONGVALUE;
        }
        break;
...
// SFLOW_snmp_fs_get()
/******************************************************************************/
static mesa_rc SFLOW_snmp_fs_get(sFlowFsTable_entry_t *table_entry)
{
    mesa_rc        rc;
    iftable_info_t info;
    ...
    if ((rc = sflow_mgmt_flow_sampler_cfg_get(info.isid, (mesa_port_no_t)info.if_id, table_entry->keys.instance, &table_entry->fs)) != VTSS_RC_OK) {
        T_W("rc = %s", error_txt(rc));
    }
    return rc;
}
...
// SFLOW_snmp_fs_set()
/******************************************************************************/
static mesa_rc SFLOW_snmp_fs_set(sFlowFsTable_entry_t *table_entry)
{
    mesa_rc        rc;
    iftable_info_t info;
    ...
    if ((rc = sflow_mgmt_flow_sampler_cfg_set(info.isid, (mesa_port_no_t)info.if_id, table_entry->keys.instance, &table_entry->fs)) != VTSS_RC_OK) {
        T_W("rc = %s", error_txt(rc));
    }
    return rc;
}
...
{
    if ((table_entry->keys.if_index = SFLOW_snmp_ifindex_getnext(0, TRUE)) == 0) {
...
{
    if (getnext) {
        if (++table_entry->keys.instance > VTSS_APPL_SFLOW_INSTANCE_CNT) {
            // Instances exhausted. Back to first instance on next port.
            table_entry->keys.instance = 1;

            // Get next non-stack-port ifIndex.
            if ((table_entry->keys.if_index = SFLOW_snmp_ifindex_getnext(table_entry->keys.if_index, FALSE)) == 0) {
                return 1;
            }
        }
    }
...
static int write_sFlowFsReceiver(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid *name, size_t name_len)
{
    ...
    case RESERVE1:
        ...
        if (set_value > VTSS_APPL_SFLOW_RECEIVER_CNT) {
...
static int write_sFlowFsMaximumHeaderSize(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid *name, size_t name_len)
{
    ...
    case RESERVE1:
        ...
        if (set_value < VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_MIN || set_value > VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_MAX) {
...
// SFLOW_snmp_cp_get()
/******************************************************************************/
static mesa_rc SFLOW_snmp_cp_get(sFlowCpTable_entry_t *table_entry)
{
    mesa_rc        rc;
    iftable_info_t info;
    ...
    if ((rc = sflow_mgmt_counter_poller_cfg_get(info.isid, (mesa_port_no_t)info.if_id, table_entry->keys.instance, &table_entry->cp)) != VTSS_RC_OK) {
        T_W("rc = %s", error_txt(rc));
    }
    return rc;
}
...
// SFLOW_snmp_cp_set()
/******************************************************************************/
static mesa_rc SFLOW_snmp_cp_set(sFlowCpTable_entry_t *table_entry)
{
    mesa_rc        rc;
    iftable_info_t info;
    ...
    if ((rc = sflow_mgmt_counter_poller_cfg_set(info.isid, (mesa_port_no_t)info.if_id, table_entry->keys.instance, &table_entry->cp)) != VTSS_RC_OK) {
        T_W("rc = %s", error_txt(rc));
    }
    return rc;
}
...
static int write_sFlowCpInterval(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid *name, size_t name_len)
{
    ...
    case RESERVE1:
        ...
        if (set_value < VTSS_APPL_SFLOW_POLLING_INTERVAL_MIN || set_value > VTSS_APPL_SFLOW_POLLING_INTERVAL_MAX) {
...
static struct variable4 sFlowAgent_variables[] = {
    ...
    {SFLOWRCVRADDRESS,             ASN_IPADDRESS, RWRITE, SFLOW_snmp_cb_rcvr_table, 3, {4, 1, 6}},
