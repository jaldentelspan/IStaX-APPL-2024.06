...
#include "port_api.h"            /* For port_count_max() et al.                       */
...
#include "vtss_network.h"        /* For htonl() and htonll()                          */
#endif
#include "ip_utils.hxx"          /* For vtss_ip_addr_is_zero()                        */

// Since we transfer (from a switch to the primary switch) all counter and flow samples
// only once per such sample, we pass an instance-mask telling the primary switch which
// instances were the reason for that sample. That instance-mask is only 32 bits wide.
#if VTSS_APPL_SFLOW_INSTANCE_CNT > 32
...
static vtss_trace_reg_t trace_reg = {
    VTSS_TRACE_MODULE_ID, "sFlow", "sFlow"
};
...
#define SFLOW_CRIT_ENTER()             critd_enter(        &SFLOW_crit,     __FILE__, __LINE__)
#define SFLOW_CRIT_EXIT()              critd_exit(         &SFLOW_crit,     __FILE__, __LINE__)
#define SFLOW_CRIT_ASSERT_LOCKED()     critd_assert_locked(&SFLOW_crit,     __FILE__, __LINE__)
#define SFLOW_BIP_CRIT_ENTER()         critd_enter(        &SFLOW_bip_crit, __FILE__, __LINE__)
#define SFLOW_BIP_CRIT_EXIT()          critd_exit(         &SFLOW_bip_crit, __FILE__, __LINE__)
#define SFLOW_BIP_CRIT_ASSERT_LOCKED() critd_assert_locked(&SFLOW_bip_crit, __FILE__, __LINE__)
...
typedef struct {
    BOOL                   cp_active;
    u32                    cp_seconds_left[VTSS_APPL_SFLOW_INSTANCE_CNT];
    mesa_port_counter_t    cp_prev_rx_octets[VTSS_APPL_SFLOW_INSTANCE_CNT];
    mesa_port_counter_t    cp_prev_tx_octets[VTSS_APPL_SFLOW_INSTANCE_CNT];
    u32                    cp_sequence_number[VTSS_APPL_SFLOW_INSTANCE_CNT];
    mesa_port_counter_t    fs_start_rx_packets[VTSS_APPL_SFLOW_INSTANCE_CNT];
    mesa_port_counter_t    fs_start_tx_packets[VTSS_APPL_SFLOW_INSTANCE_CNT];
    u32                    fs_sequence_number[VTSS_APPL_SFLOW_INSTANCE_CNT];
    u32                    fs_samples_left[VTSS_APPL_SFLOW_INSTANCE_CNT];
    mesa_sflow_port_conf_t fs_api_conf;
    u64                    port_speed_bps; // A mesa_port_speed_t converted to bits per second.
    BOOL                   port_link;
    BOOL                   port_fdx;
} sflow_port_state_t;

typedef struct {
    CapArray<sflow_port_state_t, MEBA_CAP_BOARD_PORT_MAP_COUNT> port;
...
typedef struct {
    ...
    u32 drops;           // Written by SFLOW_sample_rx() and read by SFLOW_thread(). Only reset with a primary switch change or boot.
} sflow_info_exchange_t;
 ...
 */
typedef struct {
    sflow_fs_t fs[VTSS_APPL_SFLOW_INSTANCE_CNT];
    sflow_cp_t cp[VTSS_APPL_SFLOW_INSTANCE_CNT];
} sflow_port_cfg_t;
 ...
 */
typedef struct {
    CapArray<sflow_port_cfg_t, MEBA_CAP_BOARD_PORT_MAP_COUNT> port;
} sflow_switch_cfg_t;
...
typedef struct {
    ...
    u32             dgram[(VTSS_APPL_SFLOW_RECEIVER_DATAGRAM_SIZE_MAX + 3) / 4];
    u32             sample_cnt;
    u32             sequence_number;
    u32             timeout_left;
} sflow_rcvr_state_t;
...
// In order to offload the primary switch, each secondary switch assembles flow
...
//   The SFLOW_MSG_SAMPLE_HEADER_IDX_xxx are used by the primary switch to index meta
...
//   The primary switch, who is the only one capable of translating a secondary switch's iport
...
#define SFLOW_MSG_SAMPLE_HEADER_IDX_PORT           2 /* Port on the secondary switch to which this sample pertains */
...
#define SFLOW_MSG_SAMPLE_HEADER_IDX_SEQ_NUMBERS    (SFLOW_MSG_SAMPLE_HEADER_IDX_SAMPLE_POOL + VTSS_APPL_SFLOW_INSTANCE_CNT) /* Array of sequence numbers (one per possible sFlow instance) to be replaced in the appropriate field of the UDP datagram. */
#define SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS         (SFLOW_MSG_SAMPLE_HEADER_IDX_SEQ_NUMBERS + VTSS_APPL_SFLOW_INSTANCE_CNT) /* DWORD length of internal header */
...
#define SFLOW_MSG_SAMPLE_COUNTER_IDX_SEQ_NUMBER    (SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS +  2) /* Index of sequence number to be replaced by primary switch prior to transmission to a particular receiver. The sequence number comes from the internal header */
#define SFLOW_MSG_SAMPLE_COUNTER_IDX_SOURCE_ID     (SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS +  3) /* Required by primary switch to substitute local switch port number with an USID-based. */
#define SFLOW_MSG_SAMPLE_COUNTER_IDX_IF_IDX        (SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS +  7) /* Required by primary switch to substitute local switch port number with an USID-based. */
...
#define SFLOW_MSG_SAMPLE_FLOW_IDX_SEQ_NUMBER       (SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS +  2) /* Index of sequence number to be replaced by primary switch prior to transmission to a particular receiver. The sequence number comes from the internal header */
#define SFLOW_MSG_SAMPLE_FLOW_IDX_SOURCE_ID        (SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS +  3) /* Required by primary switch to substitute local switch port number with an USID-based. */
#define SFLOW_MSG_SAMPLE_FLOW_IDX_SAMPLING_RATE    (SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS +  4) /* This is a per-instance property, so replaced by primary switch */
#define SFLOW_MSG_SAMPLE_FLOW_IDX_SAMPLE_POOL      (SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS +  5) /* This is a per-instance property, carried in the header and inserted by primary switch */
#define SFLOW_MSG_SAMPLE_FLOW_IDX_INPUT            (SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS +  7) /* Ditto. Input on secondary switch is in host order, and may be VTSS_PORT_NO_NONE */
#define SFLOW_MSG_SAMPLE_FLOW_IDX_OUTPUT           (SFLOW_MSG_SAMPLE_HEADER_LEN_DWORDS +  8) /* Ditto. Input on secondary switch is in host order, and may be VTSS_PORT_NO_NONE */
...
#define SFLOW_THREAD_FLAG_ICFG_LOADING_PRE 0x10

typedef enum {
    SFLOW_MSG_ID_CFG_TO_SECONDARY_SWITCH,  // Configuration set request (no reply)
    SFLOW_MSG_ID_SAMPLES_TO_PRIMARY_SWITCH // Counter and flow samples from secondary to primary switch in almost correct datagram format (to offload primary switch).
} sflow_msg_id_t;

// Message for configuration sent by primary switch to secondary switch.
typedef struct {
    sflow_msg_id_t     msg_id;     // Message ID
} sflow_msg_local_switch_cfg_t;

// The maximum number of 32-bit words to transmit from secondary switch to primary switch in one single message.
#define SFLOW_MSG_SIZE_DWORDS_MAX 2500 /* 10 kbytes */

// The maximum number of bytes in the primary switch BIP-buffer,
// which holds prepared counter and flow samples coming from secondary switches
// to be processed by primary switch.
#define SFLOW_PRIMARY_SWITCH_BIP_BUFFER_SIZE_BYTES 10000
...
/******************************************************************************/
// sflow_msg_samples_t
// Layout of message sent from secondary switch to primary switch containing counter and flow
// samples.
...
// samples sent from a secondary switch to the primary switch.
// This module will allocate two such buffers to swap amongst, so that
// one can be sent to the primary switch through the message module while the other
...
// Global data
/******************************************************************************/
static vtss_handle_t             SFLOW_thread_handle;
static vtss_thread_t             SFLOW_thread_block;
...
static vtss_flag_t               SFLOW_wakeup_thread_flag;                                // Wake-up-thread-flag (SIC!).
static sflow_switch_cfg_t        SFLOW_local_new_cfg;                                     // Local configuration to be applied ASAP.
static sflow_switch_cfg_t        SFLOW_stack_cfg[VTSS_ISID_CNT + 1];                      // Index 0 is used for local configuration by the local switch. Index [1; VTSS_ISID_CNT] is used by the primary switch.
static sflow_rcvr_t              SFLOW_rcvr_cfg[VTSS_APPL_SFLOW_RECEIVER_CNT + 1];        // Index 0 is unused. Index [1; VTSS_APPL_SFLOW_RECEIVER_CNT] contains receiver info.
static sflow_rcvr_state_t        SFLOW_rcvr_state[VTSS_APPL_SFLOW_RECEIVER_CNT + 1];      // Index 0 is unused. Index [1; VTSS_APPL_SFLOW_RECEIVER_CNT] contains socket handle and statistics.
...
static vtss_bip_buffer_t         SFLOW_primary_switch_bip;                                // Counter and flow samples from secondary switches are sent through a BIP-buffer on the primary switch from the Message Rx thread to the SFLOW thread. This is in order to avoid the sFlow module taking up CPU resources when under heavy load.
...
static CapArray<sflow_info_exchange_t, MEBA_CAP_BOARD_PORT_MAP_COUNT> SFLOW_local_info_exchange;      // Exchange of info between SFLOW_sample_rx() and SFLOW_thread(), protected by SFLOW_bip_crit.
static sflow_rcvr_statistics_t   SFLOW_rcvr_statistics[VTSS_APPL_SFLOW_RECEIVER_CNT + 1]; // Index 0 is unused.
...
static sflow_agent_t             SFLOW_agent_cfg = {                                      // Semi-read/only info about ourselves.
    /*.version            = */(char *)SFLOW_MIB_VERSION,                                  // Unchangeable
    /*.agent_ip_addr.type = */{
        VTSS_APPL_SFLOW_AGENT_IP_TYPE_DEFAULT, // Default is loopback, i.e. 127.0.0.1, but can be changed from CLI/Web management.
        /*.agent_ip_addr.addr = */{VTSS_APPL_SFLOW_AGENT_IP_ADDR_DEFAULT}
    }
};
...
/****************************************************************************/
// htonll()
// Linux doesn't have built-in support for this func.
/****************************************************************************/
...
#else
    return value;
#endif /* !defined(__BIG_ENDIAN__) && !defined(WORDS_BIGENDIAN) */
}

/****************************************************************************/
// SFLOW_msg_tx_switch_cfg()
// Sends the current configuration to a secondary switch.
...
// spent on the secondary switch if the receiver really doesn't want the datagrams.
...
static void SFLOW_msg_tx_switch_cfg(vtss_isid_t isid)
{
    ...
    VTSS_MALLOC_CAST(msg, sizeof(sflow_msg_local_switch_cfg_t));
    if (msg == NULL) {
        T_E("Allocation failed.\n");
        return;
    }

    msg->msg_id = SFLOW_MSG_ID_CFG_TO_SECONDARY_SWITCH;
...
// SFLOW_msg_rx()
/****************************************************************************/
static BOOL SFLOW_msg_rx(void *contxt, const void *const rx_msg, const size_t len, const vtss_module_id_t modid, const u32 isid)
{
    sflow_msg_id_t msg_id = *(sflow_msg_id_t *)rx_msg;

    switch (msg_id) {
    case SFLOW_MSG_ID_CFG_TO_SECONDARY_SWITCH: {
        mesa_port_no_t iport;
        int            inst;
        BOOL           rcvr_disabled[VTSS_APPL_SFLOW_RECEIVER_CNT + 1];
        u32            rcvr;
        ...
        SFLOW_CRIT_ENTER();
        SFLOW_local_new_cfg = SFLOW_stack_cfg[VTSS_ISID_START];

        // Speed up the tests below.
        rcvr_disabled[0] = TRUE;
        for (rcvr = 1; rcvr <= VTSS_APPL_SFLOW_RECEIVER_CNT; rcvr++) {
        ...
        for (iport = 0; iport < fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT); iport++) {
            sflow_port_cfg_t *cfg = &SFLOW_local_new_cfg.port[iport];
            for (inst = 0; inst < VTSS_APPL_SFLOW_INSTANCE_CNT; inst++) {
                ...
                if (fs->enabled == FALSE || rcvr_disabled[fs->receiver]) {
                    ...
                    // on sampling frames and sending them to the primary switch.
                ...
                if (cp->enabled == FALSE || rcvr_disabled[cp->receiver]) {
                    // Either this instance is disabled or a receiver is configured for it, but
                    // there's no valid socket to send the datagrams to, so don't spend resources
                    // on polling counters and sending them to the primary switch.
        ...
        SFLOW_CRIT_EXIT();
        vtss_flag_setbits(&SFLOW_wakeup_thread_flag, SFLOW_THREAD_FLAG_CFG_CHANGE);
        break;
    }

    case SFLOW_MSG_ID_SAMPLES_TO_PRIMARY_SWITCH: {
        sflow_msg_samples_t *msg = (sflow_msg_samples_t *)rx_msg;
        u32 *buf;

        // Received message samples from a secondary switch.
        ...
        if ((buf = (u32 *)vtss_bip_buffer_reserve(&SFLOW_primary_switch_bip, (msg->dword_len + 2) * sizeof(u32))) != NULL) {
            ...
            vtss_bip_buffer_commit(&SFLOW_primary_switch_bip);
        }
        SFLOW_BIP_CRIT_EXIT();
        vtss_flag_setbits(&SFLOW_wakeup_thread_flag, SFLOW_THREAD_FLAG_SAMPLES);
        break;
    }
...
/******************************************************************************/
BOOL SFLOW_sample_rx(void *contxt, const uchar *const frm, const mesa_packet_rx_info_t *const rx_info)
{
    mesa_port_no_t iport = rx_info->sflow_port_no;
    ...
    if (iport >= fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT)) {
        T_E("FIXME: SFLOW sampled on iport = %u, but the DUT only has %u ports", iport, fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT));
        return FALSE;
    }
    ...
    T_I("Ingress port = %u, sflow_port = %u, sflow_type = %s",
        ...
        rx_info->sflow_type == MESA_SFLOW_TYPE_NONE ? "None" :
        rx_info->sflow_type == MESA_SFLOW_TYPE_RX   ? "Rx" :
        rx_info->sflow_type == MESA_SFLOW_TYPE_TX   ? "Tx" :
        rx_info->sflow_type == MESA_SFLOW_TYPE_ALL  ? "All" : "?");
    ...
    stripped = FCS_SIZE_BYTES + rx_info->tag_type != MESA_TAG_TYPE_UNTAGGED ? 4 : 0;
    ...
    if ((buf = (u32 *)vtss_bip_buffer_reserve(&SFLOW_local_bip, buf_len_dwords * sizeof(u32))) != NULL) {
        ...
        vtss_flag_setbits(&SFLOW_wakeup_thread_flag, SFLOW_THREAD_FLAG_FLOW_SAMPLE);
    } else {
...
/******************************************************************************/
static mesa_rc SFLOW_check_isid_port(vtss_isid_t isid, mesa_port_no_t port, BOOL allow_local, BOOL check_port)
{
    ...
    if (isid != VTSS_ISID_LOCAL && !msg_switch_is_primary()) {
        return SFLOW_ERROR_NOT_PRIMARY_SWITCH;
    }
...
/******************************************************************************/
static SFLOW_INLINE int SFLOW_hostname_lookup(const char *hostname, u16 udp_port, struct addrinfo **addr)
{
...
static BOOL SFLOW_dgram_header_update(u32 rcvr_idx)
{
    ...
    is_ipv6 = SFLOW_agent_cfg.agent_ip_addr.type != MESA_IP_TYPE_IPV4;
    ...
    uptime_ms = VTSS_OS_TICK2MSEC(vtss_current_time());
...
static void SFLOW_rcvr_state_reset(u32 rcvr_idx, BOOL close_connection)
{
    ...
    if (rcvr_idx == 0) {
        rcvr_min = 1;
        rcvr_max = VTSS_APPL_SFLOW_RECEIVER_CNT;
    } else {
...
/******************************************************************************/
static void SFLOW_ip_addr_from_sockaddr(mesa_ip_addr_t *ip, struct sockaddr *sa)
{
    switch (sa->sa_family) {
    case AF_INET:
...
/******************************************************************************/
static SFLOW_INLINE mesa_rc SFLOW_rcvr_hostname_update(u32 rcvr_idx, sflow_rcvr_t *cur_cfg, sflow_rcvr_t *new_cfg, BOOL *send_cfg_to_secondary_switches)
{
    ...
    *send_cfg_to_secondary_switches = FALSE;
    ...
    if (strcmp((char *)cur_cfg->hostname, (char *)new_cfg->hostname) != 0 || cur_cfg->udp_port != new_cfg->udp_port) {
        ...
        if (!is_null) {
            ...
            if (new_addrinfo->ai_family == AF_INET6 && IN6_IS_ADDR_LINKLOCAL(&((struct sockaddr_in6 *)new_addrinfo->ai_addr)->sin6_addr)) {
                ((struct sockaddr_in6 *)new_addrinfo->ai_addr)->sin6_addr.s6_addr[1] |= htons(0x2);
            }

            // Create a new compatible socket.
            new_sockfd = vtss_socket(new_addrinfo->ai_family, new_addrinfo->ai_socktype, new_addrinfo->ai_protocol);
            if (new_sockfd == -1) {
        ...
        // Send new configuration to secondary switches either because the secondary switches need to
        // start or stop sampling, or because we need the secondary switches to reset their
        // sequence numbers.
        *send_cfg_to_secondary_switches = TRUE;
...
static void SFLOW_rcvr_cfg_init(u32 rcvr_idx)
{
    ...
    for (rcvr_idx = rcvr_min; rcvr_idx <= rcvr_max; rcvr_idx++) {
        ...
        rcvr->owner[0]          = '\0';
        rcvr->timeout           = 0;
        rcvr->max_datagram_size = VTSS_APPL_SFLOW_RECEIVER_DATAGRAM_SIZE_DEFAULT;
        strcpy((char *)rcvr->hostname, "0.0.0.0");
        rcvr->udp_port          = VTSS_APPL_SFLOW_RECEIVER_UDP_PORT_DEFAULT;
        rcvr->datagram_version  = VTSS_APPL_SFLOW_DATAGRAM_VERSION;
    }
}
...
// SFLOW_fs_cfg_init()
/******************************************************************************/
static void SFLOW_fs_cfg_init(vtss_isid_t isid, mesa_port_no_t port, u16 instance)
{
    vtss_isid_t    isid_min, isid_max, isid_iter;
    mesa_port_no_t port_min, port_max, port_iter;
    ...
    if (port == VTSS_PORT_NO_NONE) {
        port_min = 0;
        port_max = fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT) - 1;
    } else {
    ...
    if (instance == 0) {
        inst_min = 1;
        inst_max = VTSS_APPL_SFLOW_INSTANCE_CNT;
    } else {
    ...
    for (isid_iter = isid_min; isid_iter <= isid_max; isid_iter++) {
        for (port_iter = port_min; port_iter <= port_max; port_iter++) {
            for (inst_iter = inst_min; inst_iter <= inst_max; inst_iter++) {
                ...
                SFLOW_stack_cfg[isid_iter].port[port_iter].fs[inst_iter - 1].sampling_rate   = VTSS_APPL_SFLOW_FLOW_SAMPLING_RATE_DEFAULT;
                SFLOW_stack_cfg[isid_iter].port[port_iter].fs[inst_iter - 1].max_header_size = VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_DEFAULT;
                SFLOW_stack_cfg[isid_iter].port[port_iter].fs[inst_iter - 1].type            = MESA_SFLOW_TYPE_TX;
            }
        }
    }
}
...
static void SFLOW_release_receiver(u32 rcvr_idx)
{
    ...
    if (rcvr_idx == 0) {
        rcvr_idx_min = 1;
        rcvr_idx_max = VTSS_APPL_SFLOW_RECEIVER_CNT;
    } else {
    ...
    for (rcvr_idx = rcvr_idx_min; rcvr_idx <= rcvr_idx_max; rcvr_idx++) {
        ...
        for (isid = VTSS_ISID_START; isid < VTSS_ISID_END; isid++) {
            ...
            if (switch_cfg_changed && msg_switch_exists(isid)) {
                // Tell the secondary switch about the new configuration.
...
/****************************************************************************/
static SFLOW_INLINE u32 SFLOW_cp_find_sync_seconds_left(mesa_port_no_t match_port, int match_inst, u32 match_interval)
{
    port_iter_t pit;
    int         inst;
...
static SFLOW_INLINE void SFLOW_fs_state_update(void)
{
    ...
    while (port_iter_getnext(&pit)) {
        ...
        mesa_sflow_port_conf_t *old_api_conf = &port_state->fs_api_conf;
        mesa_sflow_port_conf_t new_api_conf;
        ...
        // It is already taken care of that if VTSS_APPL_SFLOW_INSTANCE_CNT > 1, all sampling rates are
        // powers of two.
        for (inst = 0; inst < VTSS_APPL_SFLOW_INSTANCE_CNT; inst++) {
        ...
        if (get_port_counters) {
            ...
            if (vtss_appl_port_statistics_get(ifindex, &port_counters) != VTSS_RC_OK) {
                T_E("vtss_appl_port_statistics_get(%s/%u) failed", ifindex, pit.uport);
            }
        }
        ...
        for (inst = 0; inst < VTSS_APPL_SFLOW_INSTANCE_CNT; inst++) {
            ...
            // Here, we don't care about the receiver. The primary switch has set sampling rate to 0
            ...
            if (cur_fs->receiver != new_fs->receiver || cur_fs->type != new_fs->type || cur_fs->sampling_rate != new_fs->sampling_rate) {
                ...
                SFLOW_ASSERT(new_fs->sampling_rate == 0 || new_fs->type != MESA_SFLOW_TYPE_NONE);
        ...
        if (new_api_conf.sampling_rate == 0) {
            // Default to MESA_SFLOW_TYPE_TX (to be able to specify both an input and an output interface in PDUs to receiver, so that it can compute the ingress and egress rates correctly).
            new_api_conf.type = MESA_SFLOW_TYPE_TX;
        }
        ...
        if (memcmp(&new_api_conf, old_api_conf, sizeof(new_api_conf)) != 0) {
            if (mesa_sflow_port_conf_set(NULL, pit.iport, &new_api_conf) != VTSS_RC_OK) {
                T_E("mesa_sflow_port_conf_set(uport=%u) failed", pit.uport);
            } else {
...
static SFLOW_INLINE BOOL SFLOW_cp_state_update(void)
{
    ...
    while (port_iter_getnext(&pit)) {
        ...
        for (inst = 0; inst < VTSS_APPL_SFLOW_INSTANCE_CNT; inst++) {
            ...
            // Here, we don't care about the receiver. The primary switch  has set interval to 0
            ...
            if (cur_cp->receiver != new_cp->receiver || cur_cp->interval != new_cp->interval) {
                *cur_cp = *new_cp; // Update cfg.
                ...
                // The idea behind synchronizing is to avoid sending too many small messages from the secondary switch
                // to the primary switch. The drawback is that synchronized counters updates will cause bursty CPU load.
...
// be sent to current primary switch.
/****************************************************************************/
...
// current primary switch. We free it.
/****************************************************************************/
...
static void SFLOW_msg_flush(void)
{
    ...
    if (meta->in_msg_tx_pipeline) {
        // The next to be sent to the primary switch is already being sent.
        return;
    }
    ...
    meta->msg.msg_id         = SFLOW_MSG_ID_SAMPLES_TO_PRIMARY_SWITCH;
    meta->msg.dword_len      = meta->samples_idx;
    meta->in_msg_tx_pipeline = TRUE;

    // Transmit the message to the current primary switch.
    ...
    // currently primary switch. The callback order will be msg_rx, msg_tx_done.
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wint-to-pointer-cast"
    msg_tx_adv((void *)SFLOW_local_fill_idx, SFLOW_msg_tx_done, (msg_tx_opt_t)(MSG_TX_OPT_DONT_FREE | MSG_TX_OPT_NO_ALLOC_ON_LOOPBACK), VTSS_MODULE_ID_SFLOW, 0, &meta->msg, offsetof(sflow_msg_samples_t, samples[0]) + 4 * meta->msg.dword_len);
...
static u32 *SFLOW_msg_sample_buffer_get(u32 len_dwords)
{
    ...
    for (i = 0; i < ARRSZ(SFLOW_local_samples); i++) {
        sflow_msg_meta_samples_t *meta = &SFLOW_local_samples[SFLOW_local_fill_idx];
        if (meta->in_msg_tx_pipeline) {
            // Both this and the alternate buffer are currently being sent to the primary switch.
...
// Collects flow sample in a message buffer supposed to be sent to the primary
// switch when it's full or triggered.
// Returns TRUE if successfully stored in buffer. FALSE if out of space.
/****************************************************************************/
static SFLOW_INLINE BOOL SFLOW_msg_fs_collect(u32 *frm, mesa_port_no_t sample_port, mesa_port_no_t ingr_port, u32 copy_bytes, u32 orig, u32 stripped, u32 drops, u32 instance_mask)
{
    ...
    mesa_port_counters_t port_counters;
    mesa_port_counter_t  rx_packets, tx_packets;
    vtss_ifindex_t       ifindex;
    int                  inst;
    ...
    (void)vtss_ifindex_from_port(VTSS_ISID_LOCAL, sample_port, &ifindex);
    if (vtss_appl_port_statistics_get(ifindex, &port_counters) != VTSS_RC_OK) {
        T_E("vtss_appl_port_statistics_get(%s/%u) failed.", ifindex, iport2uport(sample_port));
        memset(&port_counters, 0, sizeof(port_counters));
    }
    ...
    for (inst = 0; inst < VTSS_APPL_SFLOW_INSTANCE_CNT; inst++) {
        if (instance_mask & VTSS_BIT(inst)) {
            mesa_sflow_type_t type        = SFLOW_stack_cfg[VTSS_ISID_LOCAL].port[sample_port].fs[inst].type;
            u32               sample_pool = 0;

            if (type == MESA_SFLOW_TYPE_RX || type == MESA_SFLOW_TYPE_ALL) {
            ...
            if (type == MESA_SFLOW_TYPE_TX || type == MESA_SFLOW_TYPE_ALL) {
    ...
    // u32 sequence_number. To be filled in by the primary switch, since this sample may go to multiple receivers.
    ptr++;

    // u32 source_id. Local iport number inserted locally and replaced by primary switch switch who knows the USID of secondary switches.
    ptr++;

    // u32 sampling_rate. Also a per-instance property to be filled in by the primary switch. There may be race-conditions here, because
    // the secondary switch uses one sampling rate while the primary switch is re-configured with another, which is sent to secondary switches only after
    // the primary switch receives this sample, but we live with that.
    ptr++;
    ...
    // u32 input. In case of Rx flow sample: Same as #source_id. In case of Tx flow sample: VTSS_PORT_NO_NONE if received on an interconnect on JR-48, otherwise local source port. To be replaced by primary switch.
    ...
    *(ptr++) = sample_port == ingr_port ? VTSS_PORT_NO_NONE : sample_port; // Host order
    ...
    // u32 header[(VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_MAX + 3) / 4]. Must pad to 32-bit boundary (XDR variable-length opaque type).
...
// SFLOW_fs_construct()
/****************************************************************************/
static SFLOW_INLINE BOOL SFLOW_fs_construct(u32 *frm, mesa_port_no_t sample_port, mesa_port_no_t ingr_port, u32 copy_bytes, u32 orig, u32 stripped, u32 drops)
{
    ...
    for (inst = 0; inst < VTSS_APPL_SFLOW_INSTANCE_CNT; inst++) {
        if (port_state->fs_samples_left[inst]) {
            ...
            if (type == MESA_SFLOW_TYPE_ALL || (is_tx && type == MESA_SFLOW_TYPE_TX) || (is_tx == FALSE && type == MESA_SFLOW_TYPE_RX)) {
    ...
    if (instance_mask != 0) {
        // Collect the flow sample in a message, which will be sent to the current primary switch whenever we're done here, or if the message overflows.
...
// Collects counters in a message buffer supposed to be sent to the primary
// switch when it's full or triggered.
// Returns TRUE if successfully stored in buffer. FALSE if out of space.
/****************************************************************************/
static SFLOW_INLINE BOOL SFLOW_msg_cp_collect(mesa_port_no_t port, u32 instance_mask, mesa_port_if_group_counters_t *counters)
{
    ...
    // u32 sequence_number. This must be updated per instance by the primary switch.
    // The real sequence number for a given instance is included in the internal header
    // of this sample.
    ptr++;

    // u32 source_id. iport inserted by primary switch who knows the USID of secondary switches.
    ptr++;
    ...
    // u32 if_index. Port number inserted locally and replaced by primary switch who knows the USID of secondary switches.
    ptr++;
    ...
    (void)vtss_ifindex_from_port(VTSS_ISID_LOCAL, port, &ifindex);
    if ((rc = vtss_appl_port_conf_get(ifindex, &port_conf)) != VTSS_RC_OK) {
        T_E("vtss_appl_port_conf_get(%s) failed: %s", ifindex, error_txt(rc));
        port_conf.admin.enable = true;
    }

    // Bit 0: ifAdminStatus: 0 = down, 1 = up (administratively disabled/enabled)
    // Bit 1: ifOperStatus : 0 = down, 1 = up (link down/up)
    SFLOW_ENCODE_32(ptr, port_conf.admin.enable ? (port_state->port_link ? 3 : 1) : 0); // Better make sure not to report admin status down while link is up.
...
static SFLOW_INLINE void SFLOW_cp_poll(void)
{
    ...
    while (port_iter_getnext(&pit)) {
        ...
        if (port_state->cp_active) {
            ...
            if (instance_mask != 0) {
                mesa_port_counters_t port_counters;
                mesa_port_counter_t  rx_octets, tx_octets;
                ...
                // Collect the counters in a message, which will be sent to the current primary switch whenever we're done here,
...
// SFLOW_timeout()
/****************************************************************************/
static void SFLOW_timeout(struct vtss::Timer *timer)
{
    vtss_flag_setbits(&SFLOW_wakeup_thread_flag, SFLOW_THREAD_FLAG_TIMEOUT);
}
...
static SFLOW_INLINE void SFLOW_dgram_update(vtss_isid_t isid, u32 *buf, i32 len_dwords)
{
    ...
    while (len_dwords > 0) {
        sflow_meta_sample_type_t type              = (sflow_meta_sample_type_t)buf[SFLOW_MSG_SAMPLE_HEADER_IDX_TYPE];
        u32                      sample_len_dwords = buf[SFLOW_MSG_SAMPLE_HEADER_IDX_LEN_DWORDS];
        mesa_port_no_t           sample_port       = buf[SFLOW_MSG_SAMPLE_HEADER_IDX_PORT];
        ...
        SFLOW_ASSERT(sample_port < fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT) && VTSS_ISID_LEGAL(isid), return;);
        ...
        for (inst = 0; inst < VTSS_APPL_SFLOW_INSTANCE_CNT; inst++) {
            ...
            SFLOW_ASSERT(rcvr_idx <= VTSS_APPL_SFLOW_RECEIVER_CNT, return;);
            ...
            if (rcvr_state->dgram_len_dwords + sample_len_dwords <= max_dgram_len_dwords) {
                ...
                // on the secondary switch due to either lack of information or duplicate information (due to
                ...
                case SFLOW_META_SAMPLE_TYPE_FLOW: {
                    mesa_port_no_t ingr_port;
                    mesa_port_no_t egr_port;
...
// SFLOW_thread()
/****************************************************************************/
static vtss::Timer sflow_sm_timer;

static void SFLOW_thread(vtss_addrword_t data)
{
    // Timer which fires and wakes up ourselves every one second.
    // Upon expiration, it's the intention that we sample interesting
    // counters and send them to the primary switch. Also, we need to send
    // possible flow samples gathered within the last second to the primary switch.
    // Finally, the primary switch needs to keep track of receiver timeouts.
    ...
    // Initialize Periodical Wakeup Timer instead of timed wait.
    // Timed wait doesn't work well with flow sampling event, when
    // both cp and fs are enabled at the same time.

    sflow_sm_timer.set_repeat(true);
    sflow_sm_timer.set_period(vtss::milliseconds(1000));
    sflow_sm_timer.callback = SFLOW_timeout;
    sflow_sm_timer.modid = VTSS_MODULE_ID_SFLOW;
    if (vtss_timer_start(&sflow_sm_timer) != VTSS_RC_OK) {
        T_D("vtss_timer_start failed");
    }
    vtss_clear(SFLOW_local_state);
    ...
    while (1) {
        flag = vtss_flag_wait(&SFLOW_wakeup_thread_flag, 0xFFFFFFFF, VTSS_FLAG_WAITMODE_OR_CLR);
        ...
        if (flag & SFLOW_THREAD_FLAG_ICFG_LOADING_PRE) {
            SFLOW_BIP_CRIT_ENTER();
            vtss_bip_buffer_clear(&SFLOW_primary_switch_bip);
            SFLOW_BIP_CRIT_EXIT();
        }
        ...
        if (flag & SFLOW_THREAD_FLAG_FLOW_SAMPLE) {
            ...
            do {
                int            size;
                u32            *bip_buf;
                u32            buf[VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_MAX]; // Provided there's room on the stack.
                u32            dword_len = 0;
                u32            copy_bytes = 0, orig = 0, stripped = 0, aligned_byte_len;
                u32            drops = 0;
                BOOL           success;
                mesa_port_no_t sample_port = VTSS_PORT_NO_START, ingr_port = VTSS_PORT_NO_START;
                ...
                if ((bip_buf = (u32 *)vtss_bip_buffer_get_contiguous_block(&SFLOW_local_bip, &size)) != NULL) {
                    ...
                    SFLOW_ASSERT(dword_len > 6 && dword_len * sizeof(u32) <= (u32)size && copy_bytes != 0 && aligned_byte_len <= VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_MAX);
                ...
                if (!done) {
                    // Process the flow sample, i.e. pack it into datagram format and send it to the primary switch.
                    SFLOW_CRIT_ENTER();
        ...
        if (flag & SFLOW_THREAD_FLAG_TIMEOUT) {
            ...
            // to the primary switch, who will have to put them into the relevant
            // UDP datagrams and ship them off, but that's not a big deal.
            SFLOW_msg_flush();

            // If we're primary switch, we need to check all receivers for timeout.
            if (msg_switch_is_primary()) {
                int rcvr_idx;
                for (rcvr_idx = 1; rcvr_idx <= VTSS_APPL_SFLOW_RECEIVER_CNT; rcvr_idx++) {
        ...
        if (flag & SFLOW_THREAD_FLAG_SAMPLES) {
            // Secondary switch has sent us some samples to be transmitted to receivers.
            int orig_size;
            u32 *bip_buf;

            // We've received new samples in the BIP-buffer from a secondary switch. Get'em.
            // The BIP-buffer is protected by another mutex than our normal structures
            // because we don't want the message Rx thread to wait for us if we're busy.
            SFLOW_BIP_CRIT_ENTER();
            bip_buf = (u32 *)vtss_bip_buffer_get_contiguous_block(&SFLOW_primary_switch_bip, &orig_size);
            SFLOW_BIP_CRIT_EXIT();

            // Process the samples from the secondary switch.
            if (bip_buf != NULL) {
                int size = orig_size;
                ...
                SFLOW_BIP_CRIT_ENTER();
                vtss_bip_buffer_decommit_block(&SFLOW_primary_switch_bip, orig_size);
                SFLOW_BIP_CRIT_EXIT();
...
/****************************************************************************/
static SFLOW_INLINE u64 SFLOW_port_speed_to_bps(mesa_port_speed_t speed)
{
...
/****************************************************************************/
static void SFLOW_port_change_callback(mesa_port_no_t port, const vtss_appl_port_status_t *status)
{
    SFLOW_CRIT_ENTER();
    SFLOW_local_state.port[port].port_speed_bps = SFLOW_port_speed_to_bps(status->speed);
    SFLOW_local_state.port[port].port_link      = status->link;
    SFLOW_local_state.port[port].port_fdx       = status->fdx;
    SFLOW_CRIT_EXIT();
}
...
static void SFLOW_sampling_rate_api_test(void)
{
    ...
    in = 1;
    do {
        (void)mesa_sflow_sampling_rate_convert(NULL, FALSE, in, &out);
        if (out == out_max) {
            break;
        }
        out_max = out;
    ...
    if (out_max == 0) {
        ...
        T_D("Highest sampling rate = %u", out_max);
    }
    ...
    for (in = 0; in <= out_max; in++) {
        ...
        if (in != out) {
            u32 new_out;
            (void)mesa_sflow_sampling_rate_convert(NULL, FALSE, out, &new_out);
            if (out != new_out) {
                T_E("Hysteresis in sampler if 'in' gives 'out', then 'out' must give 'out', but it gives 'in'=%u, 'out'=%u, 'new_out'=%u", in, out, new_out);
                error_cnt++;
            }
        }
    }
    ...
    while (in <= out_max) {
        (void)mesa_sflow_sampling_rate_convert(NULL, FALSE, in, &out);
        if (in != out) {
            T_E("Power-of-two test failed: Input sampling rate of '%u' gives realized sampling rate of '%u", in, out);
            error_cnt++;
        }
        in <<= 1;
    }
    ...
    for (in = 1; in <= out_max; in++) {
        (void)mesa_sflow_sampling_rate_convert(NULL, TRUE, in, &out);
        if (out == 0) {
            T_E("Power-of-two return test failed: in = %u produces out = 0", in);
            error_cnt++;
            continue;
        }
        if ((out & (out - 1)) != 0) {
            // Returned sampling rate is not a power of two.
            T_E("Power-of-two return test failed: Input sampling rate of '%u = 0x%x' gives realized sampling rate of '%u = 0x%x'", in, in, out, out);
            error_cnt++;
            continue;
        }
        ...
        if (out > in) {
            ...
            if (out > out_max) {
                T_E("Power-of-two return test failed: Returned a sampling rate (%u) higher than it's maximum supported (%u)", out, out_max);
                error_cnt++;
...
do_exit:
    T_E("Sampling rate test completed with %u errors", error_cnt);
}

/****************************************************************************/
// SFLOW_default_all()
/****************************************************************************/
static void SFLOW_default_all(void)
{
    SFLOW_release_receiver(0); // 0 => release all.
    SFLOW_agent_cfg.agent_ip_addr.type      = VTSS_APPL_SFLOW_AGENT_IP_TYPE_DEFAULT;
    SFLOW_agent_cfg.agent_ip_addr.addr.ipv4 = VTSS_APPL_SFLOW_AGENT_IP_ADDR_DEFAULT;
}
...
const char *sflow_error_txt(mesa_rc rc)
{
    ...
    case SFLOW_ERROR_NOT_PRIMARY_SWITCH:
    ...
    case SFLOW_ERROR_INSTANCE:
        ...
        return "Invalid port instance index. Valid values: [1; " vtss_xstr(VTSS_APPL_SFLOW_INSTANCE_CNT) "].";

    case SFLOW_ERROR_RECEIVER:
        // When configuring counter and flow samplers, 0 is valid. 0 = No receiver. 1-based for valid receivers.
        return "Invalid receiver index. Valid values: [0; " vtss_xstr(VTSS_APPL_SFLOW_RECEIVER_CNT) "].";

    case SFLOW_ERROR_RECEIVER_IDX:
        // When configuring a given receiver, 0 is invalid.
        return "Invalid receiver index. Valid values: [1; " vtss_xstr(VTSS_APPL_SFLOW_RECEIVER_CNT) "].";
    ...
    case SFLOW_ERROR_RECEIVER_TIMEOUT:
        return "Invalid receiver timeout. Valid values are [0; " vtss_xstr(VTSS_APPL_SFLOW_RECEIVER_TIMEOUT_MAX) "].";

    case SFLOW_ERROR_RECEIVER_DATAGRAM_SIZE:
        return "Invalid receiver maximum datagram size. Valid values are [" vtss_xstr(VTSS_APPL_SFLOW_RECEIVER_DATAGRAM_SIZE_MIN) "; " vtss_xstr(VTSS_APPL_SFLOW_RECEIVER_DATAGRAM_SIZE_MAX) "].";
    ...
    case SFLOW_ERROR_DATAGRAM_VERSION:
        return "Unsupported datagram version. This agent only supports v. " vtss_xstr(VTSS_APPL_SFLOW_DATAGRAM_VERSION) ".";
...
mesa_rc vtss_appl_sflow_agent_cfg_get(vtss_appl_sflow_agent_t *const cfg)
{
    mesa_rc         rc = VTSS_RC_OK;
    sflow_agent_t   tmp_cfg;
    if ((rc = sflow_mgmt_agent_cfg_get(&tmp_cfg)) == VTSS_RC_OK) {
        cfg->agent_ip_addr = tmp_cfg.agent_ip_addr;
    } else {
        T_E("%s", error_txt(rc));
    }
    return rc;
...
mesa_rc vtss_appl_sflow_agent_cfg_set(const vtss_appl_sflow_agent_t *const cfg)
{
    mesa_rc         rc = VTSS_RC_OK;
    sflow_agent_t   tmp_cfg;

    memset(&tmp_cfg, 0, sizeof(sflow_agent_t));
    tmp_cfg.agent_ip_addr = cfg->agent_ip_addr;

    if ((rc = sflow_mgmt_agent_cfg_set(&tmp_cfg)) != VTSS_RC_OK) {
        T_E("%s", error_txt(rc));
    }
    return rc;
}
...
/******************************************************************************/
mesa_rc sflow_mgmt_agent_cfg_get(sflow_agent_t *cfg)
{
    // Configuration can only be fetched on primary switch
    if (!msg_switch_is_primary()) {
        return SFLOW_ERROR_NOT_PRIMARY_SWITCH;
    }
...
/******************************************************************************/
mesa_rc sflow_mgmt_agent_cfg_set(sflow_agent_t *cfg)
{
    // Configuration can only be changed on primary switch
    if (!msg_switch_is_primary()) {
        return SFLOW_ERROR_NOT_PRIMARY_SWITCH;
    }
    ...
    if (cfg->agent_ip_addr.type != MESA_IP_TYPE_IPV4 && cfg->agent_ip_addr.type != MESA_IP_TYPE_IPV6) {
        return SFLOW_ERROR_AGENT_IP;
    }
#else
    if (cfg->agent_ip_addr.type != MESA_IP_TYPE_IPV4) {
        return SFLOW_ERROR_AGENT_IP;
    }
#endif
...
mesa_rc vtss_appl_sflow_rcvr_status_get(u32 rcvr_idx, vtss_appl_sflow_rcvr_info_t *const rcvr_info)
{
    mesa_rc rc = VTSS_RC_ERROR;
    sflow_rcvr_t tmp_cfg;
    sflow_rcvr_info_t tmp_info;

    if ((rc = sflow_mgmt_rcvr_cfg_get(rcvr_idx, &tmp_cfg, &tmp_info)) == VTSS_RC_OK) {
        rcvr_info->timeout_left = tmp_info.timeout_left;
        rcvr_info->ip_addr = tmp_info.ip_addr;
    }
    return rc;
}

mesa_rc vtss_appl_sflow_rcvr_cfg_get(u32 rcvr_idx, vtss_appl_sflow_rcvr_t *const rcvr_cfg)
{
    mesa_rc rc;
    sflow_rcvr_info_t info;
    sflow_rcvr_t      tmp_cfg;

    if ((rc = sflow_mgmt_rcvr_cfg_get(rcvr_idx, &tmp_cfg, &info)) == VTSS_RC_OK) {
        memcpy(rcvr_cfg->owner, tmp_cfg.owner, VTSS_APPL_SFLOW_OWNER_LEN);
        rcvr_cfg->udp_port = tmp_cfg.udp_port;
        rcvr_cfg->timeout = tmp_cfg.timeout;
        rcvr_cfg->datagram_version = tmp_cfg.datagram_version;
        rcvr_cfg->max_datagram_size = tmp_cfg.max_datagram_size;

        rcvr_cfg->hostname.type = VTSS_INET_ADDRESS_TYPE_DNS;
        memcpy(rcvr_cfg->hostname.address.domain_name.name, tmp_cfg.hostname, SFLOW_HOSTNAME_LEN);
    }
    return rc;
}
...
/******************************************************************************/
mesa_rc sflow_mgmt_rcvr_cfg_get(u32 rcvr_idx /* 1..N */, sflow_rcvr_t *cfg, sflow_rcvr_info_t *info)
{
    mesa_rc result;

    // Configuration changes only allowed on primary switch
    if ((result = SFLOW_check_isid_port(VTSS_ISID_START, VTSS_PORT_NO_START, FALSE, FALSE)) != VTSS_RC_OK) {
        return result;
    }

    if (rcvr_idx < 1 || rcvr_idx > VTSS_APPL_SFLOW_RECEIVER_CNT) {
    ...
    *cfg = SFLOW_rcvr_cfg[rcvr_idx];
    if (info) {
        if (SFLOW_rcvr_state[rcvr_idx].addrinfo == NULL) {
            info->ip_addr.type = MESA_IP_TYPE_IPV4;
            info->ip_addr.addr.ipv4 = 0;
        } else {
            struct sockaddr *sa = SFLOW_rcvr_state[rcvr_idx].addrinfo->ai_addr;
            if ((info->ip_addr.type = sa->sa_family == AF_INET6 ? MESA_IP_TYPE_IPV6 : MESA_IP_TYPE_IPV4) == MESA_IP_TYPE_IPV4) {
                info->ip_addr.addr.ipv4 = ntohl(((struct sockaddr_in *)sa)->sin_addr.s_addr);
            } else {
                memcpy(info->ip_addr.addr.ipv6.addr, ((struct sockaddr_in6 *)sa)->sin6_addr.s6_addr, sizeof(info->ip_addr.addr.ipv6.addr));
            }
        }
...
static mesa_rc vtss_sockaddr_to_hostname(const vtss_inet_address_t *const addr, char *host)
{
    int length = 0;
    mesa_ipv4_t        ipv4;
    ...
    if (addr->type == VTSS_INET_ADDRESS_TYPE_IPV4) {
        sin.sin_family = AF_INET;
        ipv4           = htonl(addr->address.ipv4);
        memcpy((void *)&sin.sin_addr, &ipv4, sizeof(sin.sin_addr));
        sa = (struct sockaddr *)&sin;
        length = sizeof(sin);
    }

    if (addr->type == VTSS_INET_ADDRESS_TYPE_IPV6) {
        sin6.sin6_family = AF_INET6;
        memcpy(&sin6.sin6_addr, &addr->address.ipv6, sizeof(sin6.sin6_addr));
        sa = (struct sockaddr *)&sin6;
        length = sizeof(sin6);
    }
    if (addr->type == VTSS_INET_ADDRESS_TYPE_IPV4 || addr->type == VTSS_INET_ADDRESS_TYPE_IPV6) {
        int ret = getnameinfo(sa, length, host, SFLOW_HOSTNAME_LEN, NULL, 0, 0);
        T_D("ret: %d, host:%s\n", ret, host);
    } else if (addr->type == VTSS_INET_ADDRESS_TYPE_DNS) {
        memcpy(host, addr->address.domain_name.name, SFLOW_HOSTNAME_LEN);
    } else {
        return VTSS_RC_ERROR;
    }
    return VTSS_RC_OK;
...
mesa_rc vtss_appl_sflow_rcvr_cfg_del(u32 rcvr_idx)
{
    sflow_rcvr_t def_cfg;

    memset(&def_cfg, 0, sizeof(sflow_rcvr_t));

    return sflow_mgmt_rcvr_cfg_set(rcvr_idx, &def_cfg);
}

mesa_rc vtss_appl_sflow_rcvr_cfg_set(u32 rcvr_idx, const vtss_appl_sflow_rcvr_t *const rcvr_cfg)
{
    sflow_rcvr_t  tmp_cfg;
    mesa_rc                 rc = VTSS_RC_OK;
    ...
    memset(&tmp_cfg, 0, sizeof(sflow_rcvr_t));

    memcpy(tmp_cfg.owner, rcvr_cfg->owner, VTSS_APPL_SFLOW_OWNER_LEN);
    tmp_cfg.udp_port    = rcvr_cfg->udp_port;
    tmp_cfg.timeout     = rcvr_cfg->timeout;
    tmp_cfg.datagram_version    = rcvr_cfg->datagram_version;
    tmp_cfg.max_datagram_size   = rcvr_cfg->max_datagram_size;

    if (vtss_sockaddr_to_hostname(&rcvr_cfg->hostname, hostname) != VTSS_RC_OK) {
        T_E("vtss_sockaddr_to_hostname() returned error");
        return VTSS_RC_ERROR;
    }
    memcpy(tmp_cfg.hostname, hostname, SFLOW_HOSTNAME_LEN);

    T_D("own: %s, hn: %s, udp port: %u, dgram ver: %u, max dgram size: %u, to: %u\n", rcvr_cfg->owner,
        hostname, rcvr_cfg->udp_port, rcvr_cfg->datagram_version, rcvr_cfg->max_datagram_size, rcvr_cfg->timeout);

    if ((rc = sflow_mgmt_rcvr_cfg_set(rcvr_idx, &tmp_cfg)) != VTSS_RC_OK) {
        T_W("%s", error_txt(rc));
    }
    return rc;
}
...
/******************************************************************************/
mesa_rc sflow_mgmt_rcvr_cfg_set(u32 rcvr_idx /* 1..N */, sflow_rcvr_t *new_cfg)
{
    mesa_rc      result;
    ...
    if (new_cfg->owner[0] == '\0') {
        ...
        BOOL send_cfg_to_secondary_switches = FALSE;
        ...
        if (new_cfg->datagram_version < VTSS_APPL_SFLOW_DATAGRAM_VERSION) {
        ...
        if (new_cfg->timeout > VTSS_APPL_SFLOW_RECEIVER_TIMEOUT_MAX) {
            result = SFLOW_ERROR_RECEIVER_TIMEOUT;
            goto do_exit;
        }

        if (new_cfg->max_datagram_size < VTSS_APPL_SFLOW_RECEIVER_DATAGRAM_SIZE_MIN || new_cfg->max_datagram_size > VTSS_APPL_SFLOW_RECEIVER_DATAGRAM_SIZE_MAX) {
            result = SFLOW_ERROR_RECEIVER_DATAGRAM_SIZE;
            goto do_exit;
        }

        if (new_cfg->udp_port == 0) {
            new_cfg->udp_port = VTSS_APPL_SFLOW_RECEIVER_UDP_PORT_DEFAULT;
        }
        ...
        // possibly send configuration to the secondary switches.
        // If result != VTSS_RC_OK, something went wrong, and the existing socket
        // (if any) is kept.
        result = SFLOW_rcvr_hostname_update(rcvr_idx, cur_cfg, new_cfg, &send_cfg_to_secondary_switches);
        ...
        *cur_cfg = *new_cfg;
        ...
        T_D("Send to secondary switches: %d", send_cfg_to_secondary_switches);

        if (send_cfg_to_secondary_switches) {
            switch_iter_t sit;
...
mesa_rc vtss_appl_sflow_flow_sampler_cfg_get(vtss_ifindex_t ifindex, u16 instance, vtss_appl_sflow_fs_t *const cfg)
{
    vtss_ifindex_elm_t  ife;
    mesa_rc             rc;
    if ((rc = vtss_ifindex_decompose(ifindex, &ife)) == VTSS_RC_OK) {
        rc = sflow_mgmt_flow_sampler_cfg_get(ife.isid, ife.ordinal, instance, cfg);
    }
    return rc;
}
...
mesa_rc sflow_mgmt_flow_sampler_cfg_get(vtss_isid_t isid, mesa_port_no_t port, u16 instance, sflow_fs_t *cfg)
{
    ...
    // Configuration get only allowed by primary switch
    if ((result = SFLOW_check_isid_port(isid, port, FALSE, TRUE)) != VTSS_RC_OK) {
        return result;
    }

    if (instance < 1 || instance > VTSS_APPL_SFLOW_INSTANCE_CNT) {
...
mesa_rc vtss_appl_sflow_flow_sampler_cfg_del(vtss_ifindex_t ifindex, u16 instance)
{
    vtss_appl_sflow_fs_t def_cfg;

    memset(&def_cfg, 0, sizeof(vtss_appl_sflow_fs_t));

    def_cfg.enabled         = false;
    def_cfg.receiver        = 0;
    def_cfg.sampling_rate   = VTSS_APPL_SFLOW_FLOW_SAMPLING_RATE_DEFAULT;
    def_cfg.max_header_size = VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_DEFAULT;
    def_cfg.type            = MESA_SFLOW_TYPE_TX;

    return vtss_appl_sflow_flow_sampler_cfg_set(ifindex, instance, &def_cfg);
}

mesa_rc vtss_appl_sflow_flow_sampler_cfg_set(vtss_ifindex_t ifindex, u16 instance, const vtss_appl_sflow_fs_t *const cfg)
{
    vtss_ifindex_elm_t  ife;
    mesa_rc             rc;
    vtss_appl_sflow_fs_t tmp_cfg;

    memset(&tmp_cfg, 0, sizeof(vtss_appl_sflow_fs_t));
    tmp_cfg = *cfg;

    if ((rc = vtss_ifindex_decompose(ifindex, &ife)) == VTSS_RC_OK) {
        rc = sflow_mgmt_flow_sampler_cfg_set(ife.isid, ife.ordinal, instance, &tmp_cfg);
    }
    return rc;
}
/******************************************************************************/
...
mesa_rc sflow_mgmt_flow_sampler_cfg_set(vtss_isid_t isid, mesa_port_no_t port, u16 instance, sflow_fs_t *cfg)
{
    ...
    if (cfg->receiver > VTSS_APPL_SFLOW_RECEIVER_CNT) {
    ...
    if (cfg->max_header_size < VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_MIN) {
        // Auto-adjust.
        cfg->max_header_size = VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_MIN;
    }

    if (cfg->max_header_size > VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_MAX) {
        // Auto-adjust.
        cfg->max_header_size = VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_MAX;
    }

    if (cfg->receiver != 0 && cfg->type == MESA_SFLOW_TYPE_NONE) {
    ...
    if (cfg->receiver != 0) {
        for (inst_iter = 0; inst_iter < VTSS_APPL_SFLOW_INSTANCE_CNT; inst_iter++) {
...
// one, because we do software-based sub-sampling in that case.
    ...
    power2 = FALSE;
#endif
    (void)mesa_sflow_sampling_rate_convert(NULL, power2, cfg->sampling_rate, &realizable_sampling_rate);
...
mesa_rc vtss_appl_sflow_counter_poller_cfg_get(vtss_ifindex_t ifindex, u16 instance, vtss_appl_sflow_cp_t *const cfg)
{
    vtss_ifindex_elm_t      ife;
    mesa_rc                 rc;
    ...
    if ((rc = vtss_ifindex_decompose(ifindex, &ife)) == VTSS_RC_OK) {
        rc = sflow_mgmt_counter_poller_cfg_get(ife.isid, ife.ordinal, instance, cfg);
    }
    return rc;
}
/******************************************************************************/
...
mesa_rc vtss_appl_sflow_counter_poller_cfg_del(vtss_ifindex_t ifindex, u16 instance)
{
    vtss_appl_sflow_cp_t def_cfg;

    memset(&def_cfg, 0, sizeof(vtss_appl_sflow_cp_t));

    return vtss_appl_sflow_counter_poller_cfg_set(ifindex, instance, &def_cfg);
}

mesa_rc vtss_appl_sflow_counter_poller_cfg_set(vtss_ifindex_t ifindex, u16 instance, const vtss_appl_sflow_cp_t *const cfg)
{
    vtss_ifindex_elm_t      ife;
    mesa_rc                 rc;
    vtss_appl_sflow_cp_t    tmp_cfg;

    memset(&tmp_cfg, 0, sizeof(vtss_appl_sflow_cp_t));
    tmp_cfg = *cfg;

    if ((rc = vtss_ifindex_decompose(ifindex, &ife)) == VTSS_RC_OK) {
        rc = sflow_mgmt_counter_poller_cfg_set(ife.isid, ife.ordinal, instance, &tmp_cfg);
    }
    return rc;
}
/******************************************************************************/
...
mesa_rc sflow_mgmt_counter_poller_cfg_set(vtss_isid_t isid, mesa_port_no_t port, u16 instance, sflow_cp_t *cfg)
{
    ...
    if (cfg->interval > VTSS_APPL_SFLOW_POLLING_INTERVAL_MAX) {
        // Auto-adjust (according to standard).
        cfg->interval = VTSS_APPL_SFLOW_POLLING_INTERVAL_MAX;
    }
...
mesa_rc vtss_appl_sflow_rcvr_statistics_clear(u32 rcvr_idx)
{
    vtss_appl_sflow_rcvr_statistics_t tmp_cfg;

    return sflow_mgmt_rcvr_statistics_get(rcvr_idx, &tmp_cfg, true);
}
mesa_rc vtss_appl_sflow_rcvr_statistics_get(u32 rcvr_idx, vtss_appl_sflow_rcvr_statistics_t *const statistics)
{
    return sflow_mgmt_rcvr_statistics_get(rcvr_idx, statistics, false);
}
/******************************************************************************/
...
mesa_rc sflow_mgmt_rcvr_statistics_get(u32 rcvr_idx, sflow_rcvr_statistics_t *statistics, BOOL clear)
{
    ...
    // Statistics reading only allowed by primary switch
    if ((result = SFLOW_check_isid_port(VTSS_ISID_START, VTSS_PORT_NO_START, FALSE, FALSE)) != VTSS_RC_OK) {
        return result;
    }
...
)
{
    mesa_rc                     rc = VTSS_RC_OK;
    vtss_ifindex_elm_t          ife;
    sflow_switch_statistics_t   tmp_cfg;
    u16                         inst_idx = instance - 1; //0-based index, adjustment
    ...
    if ((rc = vtss_ifindex_decompose(ifindex, &ife)) == VTSS_RC_OK) {
        if ((rc = sflow_mgmt_switch_statistics_get(ife.isid, &tmp_cfg)) == VTSS_RC_OK) {
            statistics->fs_rx = tmp_cfg.port[ife.ordinal].fs_rx[inst_idx];
            statistics->fs_tx = tmp_cfg.port[ife.ordinal].fs_tx[inst_idx];
            statistics->cp    = tmp_cfg.port[ife.ordinal].cp[inst_idx];
        }
    }
    return rc;
}
...
mesa_rc vtss_appl_sflow_instance_statistics_clear(vtss_ifindex_t ifindex, u16 instance)
{
    mesa_rc                     rc = VTSS_RC_OK;
    vtss_ifindex_elm_t          ife;

    if ((rc = vtss_ifindex_decompose(ifindex, &ife)) == VTSS_RC_OK) {
        rc = sflow_mgmt_instance_statistics_clear(ife.isid, ife.ordinal, instance);
    }
    return rc;
}
/******************************************************************************/
...
mesa_rc sflow_mgmt_instance_statistics_clear(vtss_isid_t isid, mesa_port_no_t port, u16 instance)
{
    ...
    // Statistics clearing only allowed by primary switch
    if ((result = SFLOW_check_isid_port(isid, port, FALSE, TRUE)) != VTSS_RC_OK) {
        return result;
    }
...
VTSS_PRE_DECLS void vtss_appl_sflow_json_init();
#endif
extern "C" int sflow_icli_cmd_register();
...
mesa_rc sflow_init(vtss_init_data_t *data)
{
    ...
    switch (data->cmd) {
    case INIT_CMD_INIT:
        if (!vtss_bip_buffer_init(&SFLOW_primary_switch_bip, SFLOW_PRIMARY_SWITCH_BIP_BUFFER_SIZE_BYTES)) {
            T_E("Unable to create primary switch BIP buffer");
        }
        ...
        critd_init_legacy(&SFLOW_crit, "sFlow", VTSS_MODULE_ID_SFLOW, CRITD_TYPE_MUTEX);

        // The sFlow thread has low priority and may therefore be starved
        // by other threads, which is fine, but we need to disable
        // critd's check for "deadlock" on SFLOW_crit then, by allowing
        // a very high lock time.
        SFLOW_crit.max_lock_time = 3600; // 1 hour

        critd_init_legacy(&SFLOW_bip_crit, "sFlow BIP buffers", VTSS_MODULE_ID_SFLOW, CRITD_TYPE_MUTEX);

        vtss_flag_init(&SFLOW_wakeup_thread_flag);
        vtss_clear(SFLOW_stack_cfg[VTSS_ISID_LOCAL]);
        ...
        // before sending them to the primary switch.
        for (i = 0; i < ARRSZ(SFLOW_local_samples); i++) {
            SFLOW_msg_reset_sample_msg(&SFLOW_local_samples[i]);
        }
#ifdef VTSS_SW_OPTION_JSON_RPC
        vtss_appl_sflow_json_init();
#endif
    ...
    case INIT_CMD_START:
        ...
        if (!fast_cap(MESA_CAP_L2_SFLOW_TESTED)) {
    ...
    case INIT_CMD_ICFG_LOADING_PRE:
        SFLOW_CRIT_ENTER();
        // We don't persist any configuration (mainly due to a timeout defined
        // by sFlow receivers, so let's reset our whole stack configuration upon
        // the current event. SFLOW_stack_cfg[0] is managed by the local
        // switch-part of this code, so we don't reset that.
        vtss_clear(SFLOW_stack_cfg[VTSS_ISID_LOCAL + 1]);
        SFLOW_fs_cfg_init(VTSS_ISID_GLOBAL, VTSS_PORT_NO_NONE, 0); // Reset the whole stack's flow sampler configuration.
        vtss_flag_setbits(&SFLOW_wakeup_thread_flag, SFLOW_THREAD_FLAG_ICFG_LOADING_PRE);
        SFLOW_CRIT_EXIT();

        SFLOW_BIP_CRIT_ENTER();
        // The drop counter is reset only when the agent resets, which is upon boot and upon primary switch change.
        vtss_clear(SFLOW_local_info_exchange);
        SFLOW_BIP_CRIT_EXIT();
        break;
