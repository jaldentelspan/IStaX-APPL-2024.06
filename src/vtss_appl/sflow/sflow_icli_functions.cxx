...
static mesa_rc sflow_icli_statistics_receiver(i32 session_id, u32 rcvr_idx, BOOL has_clear, BOOL print_header)
{
    ...
    if (!has_clear) {
...
#endif
            // Add rest of header
            sprintf(str_buf + strlen(str_buf), "%-15s  %-15s  %-15s  %-15s", "Tx Successes", "Tx Errors   ", "Flow Samples", "Counter Samples");
        ...
        ICLI_PRINTF(VPRI64Fu("15") "  " VPRI64Fu("15") "  " VPRI64Fu("15") "  " VPRI64Fu("15") "\n", rcvr.dgrams_ok, rcvr.dgrams_err, rcvr.fs, rcvr.cp);
    }
    return VTSS_RC_OK;
}
...
static mesa_rc sflow_icli_statistics_receiver_loop(i32 session_id, icli_range_t *rcvr_idx_list, BOOL has_clear)
{
    u8 rcvr_idx;
...
static mesa_rc sflow_icli_statistics_samplers(i32 session_id, u32 inst, BOOL has_clear, BOOL print_header, const switch_iter_t *sit, const port_iter_t *pit)
{
    char str_buf[100];
    if (has_clear) {
        ...
        if (print_header) {
#if VTSS_APPL_SFLOW_INSTANCE_CNT > 1
            ICLI_PRINTF("Interface              Inst Flow Samples     Counter Samples\n");
            ICLI_PRINTF("---------------------- ---- ---------------- ----------------\n");
#else
            ICLI_PRINTF("Interface              Flow Samples     Counter Samples\n");
            ICLI_PRINTF("---------------------- ---------------- ----------------\n");
#endif
        }

#if VTSS_APPL_SFLOW_INSTANCE_CNT > 1
        ICLI_PRINTF("%-22s %4u %16llu %16llu\n", icli_port_info_txt(sit->usid, pit->uport, str_buf), inst, fs_tx, cp);
#else
        ICLI_PRINTF("%-22s " VPRI64Fu("16") " " VPRI64Fu("16") "\n", icli_port_info_txt(sit->usid, pit->uport, str_buf), fs_tx, cp);
#endif
    }
    return VTSS_RC_OK;
}
...
// In  - Session_Id - For ICLI print
static mesa_rc sflow_print_agent_conf(i32 session_id)
{
    sflow_agent_t agent;
    char          buf[50];
...
static mesa_rc sflow_print_rcvr_conf(i32 session_id, BOOL *no_rcvr_enabled)
{
    sflow_rcvr_t      rcvr;
...
static mesa_rc sflow_print_samplers_conf(i32 session_id, const switch_iter_t *sit)
{
    char str_buf[100];
    port_iter_t pit;
    int         display_cnt = 0;
    ...
    while (icli_port_iter_getnext(&pit, NULL)) {
        ...
        for (inst = inst_min; inst <= inst_max; inst++) {
            ...
            if (display_cnt++ == 0) {
#if VTSS_APPL_SFLOW_RECEIVER_CNT > 1
                ICLI_PRINTF("Interface              Inst Sampling Rate Max Hdr\n");
                ICLI_PRINTF("---------------------- ---- ------------- -------\n");
#else
                ICLI_PRINTF("Interface              Sampling Rate Max Hdr\n");
                ICLI_PRINTF("---------------------- ------------- -------\n");
#endif
            }

#if VTSS_APPL_SFLOW_RECEIVER_CNT > 1
            ICLI_PRINTF("%-22s %4hu %13u %6u\n", icli_port_info_txt(sit->usid, pit.uport, str_buf), inst, fs.sampling_rate, fs.max_header_size);
#else
            ICLI_PRINTF("%-22s %13u %6u\n", icli_port_info_txt(sit->usid, pit.uport, str_buf), fs.sampling_rate, fs.max_header_size);
#endif
        }
    }
    if (display_cnt == 0) {
...
// In  - Session_Id - For ICLI print
//     - sit - Containing switch information
static mesa_rc sflow_print_counter_poll_conf(i32 session_id, const switch_iter_t *sit)
{
    int         display_cnt = 0;
    port_iter_t pit;
    char str_buf[100];
    ...
    while (icli_port_iter_getnext(&pit, NULL)) {
        ...
        for (inst = inst_min; inst <= inst_max; inst++) {
            ...
            if (display_cnt++ == 0) {
#if VTSS_APPL_SFLOW_RECEIVER_CNT > 1
                ICLI_PRINTF("Interface              Inst Interval\n");
                ICLI_PRINTF("---------------------- ---- --------\n");
#else
                ICLI_PRINTF("Interface              Interval\n");
                ICLI_PRINTF("---------------------- --------\n");
#endif
            }

#if VTSS_APPL_SFLOW_RECEIVER_CNT > 1
            ICLI_PRINTF("%-22s %4hu %8u\n", icli_port_info_txt(sit->usid, pit.uport, str_buf), inst, cp.interval);
#else
            ICLI_PRINTF("%-22s %8u\n", icli_port_info_txt(sit->usid, pit.uport, str_buf), cp.interval);
#endif
        }
    }
...
// In - Session_Id - For ICLI print
static mesa_rc sflow_print_conf(i32 session_id)
{
...
static mesa_rc sflow_icli_receiver_conf(u32 rcvr_idx, sflow_rcvr_conf_type_t sflow_rcvr_conf_type, const sflow_rcvr_conf_value_t *sflow_conf_value)
{
    ...
    strcpy(new_rcvr.owner, VTSS_APPL_SFLOW_OWNER_LOCAL_MANAGEMENT_STRING);
...
//    - sflow_conf_value - Containing the new configuration.
static mesa_rc sflow_icli_samplers_conf (i32 session_id, u32 inst, sflow_samplers_conf_type_t sflow_samplers_conf_type, const sflow_conf_value_t *sflow_conf_value,   const switch_iter_t *sit, const port_iter_t *pit, BOOL *note_shown)
{
    sflow_fs_t fs;
...
static mesa_rc sflow_icli_counter_poll_interval_conf(u32 inst, const sflow_conf_value_t *sflow_conf_value, const switch_iter_t *sit, const port_iter_t *pit)
{
    sflow_cp_t cp;
...
static mesa_rc sflow_icli_command(i32 session_id, icli_stack_port_range_t *plist, icli_range_t *samplers_list, sflow_command_type_t command_type, const sflow_samplers_conf_type_t sflow_samplers_conf_type, const sflow_conf_value_t *sflow_conf_value)
{
    switch_iter_t   sit;
    ...
    while (icli_switch_iter_getnext(&sit, plist)) {
        ...
        while (icli_port_iter_getnext(&pit, plist)) {
            ...
            if (samplers_list == NULL) {
                for (inst = 1; inst <= VTSS_APPL_SFLOW_INSTANCE_CNT; inst++) {
                    switch (command_type) {
                    case SAMPLER:
...
static mesa_rc sflow_icli_receiver_set(icli_range_t *rcvr_idx_list, sflow_rcvr_conf_type_t sflow_rcvr_conf_type, const sflow_rcvr_conf_value_t *sflow_conf_value)
{
    u8 rcvr_idx;
    if (rcvr_idx_list == NULL) {
        for (rcvr_idx = 1; rcvr_idx <= VTSS_APPL_SFLOW_RECEIVER_CNT; rcvr_idx++) {
...
#if VTSS_APPL_SFLOW_INSTANCE_CNT > 1
        icli_sprintf(runtime->byword, "<instance index : %u-%u>", 1, VTSS_APPL_SFLOW_INSTANCE_CNT);
#endif
        return TRUE;
    case ICLI_ASK_RANGE:
        runtime->range.type = ICLI_RANGE_TYPE_UNSIGNED;
        runtime->range.u.sr.cnt = VTSS_APPL_SFLOW_RECEIVER_CNT;
        runtime->range.u.sr.range[0].min = 1;
        runtime->range.u.sr.range[0].max = VTSS_APPL_SFLOW_RECEIVER_CNT;
        return TRUE;
    case ICLI_ASK_HELP:
#if VTSS_APPL_SFLOW_INSTANCE_CNT > 1
...
BOOL sflow_icli_run_time_receiver_instances(u32 session_id, icli_runtime_ask_t ask, icli_runtime_t *runtime)
{
    ...
    case ICLI_ASK_BYWORD:
#if VTSS_APPL_SFLOW_RECEIVER_CNT > 1
        icli_sprintf(runtime->byword, "<receiver index : %u-%u>", 1, VTSS_APPL_SFLOW_RECEIVER_CNT);
#endif
        return TRUE;
    case ICLI_ASK_RANGE:
        runtime->range.type = ICLI_RANGE_TYPE_UNSIGNED;
...
mesa_rc sflow_icli_timeout(i32 session_id, u32 timeout, icli_range_t *rcvr_idx_list)
{
    ...
    ICLI_RC_CHECK_PRINT_RC(sflow_icli_receiver_set(rcvr_idx_list, sflow_rcvr_conf_type, &sflow_conf_value));
    return VTSS_RC_OK;
}
...
mesa_rc sflow_icli_collector_port(i32 session_id, u32 collector_port, icli_range_t *rcvr_idx_list, BOOL no)
{
    ...
    sflow_rcvr_conf_value_t sflow_conf_value;
    if (no) {
        sflow_conf_value.collector_port = VTSS_APPL_SFLOW_RECEIVER_UDP_PORT_DEFAULT;
    } else {
...
mesa_rc sflow_icli_max_datagram_size(i32 session_id, u32 max_datagram_size, icli_range_t *rcvr_idx_list, BOOL no)
{
    ...
    sflow_rcvr_conf_value_t sflow_conf_value;
    if (no) {
        sflow_conf_value.max_datagram_size = VTSS_APPL_SFLOW_RECEIVER_DATAGRAM_SIZE_DEFAULT;
    } else {
...
// See sflow_icli_functions.h
mesa_rc sflow_icli_statistics(i32 session_id, BOOL has_receiver, BOOL has_samplers, icli_range_t *rcvr_idx_list, icli_range_t *sampler_idx_list, icli_stack_port_range_t *plist, BOOL has_clear)
{
    ...
    if (has_receiver) {
        ICLI_RC_CHECK_PRINT_RC(sflow_icli_statistics_receiver_loop(session_id, rcvr_idx_list, has_clear));
    } else if (has_samplers) {
        ICLI_RC_CHECK_PRINT_RC(sflow_icli_command(session_id, plist, sampler_idx_list, command_type, sflow_conf_type, &sflow_conf_value));
    } else {
...
{
    ICLI_RC_CHECK_PRINT_RC(sflow_print_conf(session_id));
    return VTSS_RC_OK;
}
...
mesa_rc sflow_icli_sampling_rate(i32 session_id, icli_stack_port_range_t *plist, u32 sampling_rate, icli_range_t *sampler_idx_list, BOOL no)
{
    ...
    if (no) {
        sflow_conf_value.sampling_rate = VTSS_APPL_SFLOW_FLOW_SAMPLING_RATE_DEFAULT;
    } else {
...
mesa_rc sflow_icli_max_hdr_size(i32 session_id, icli_stack_port_range_t *plist, u32 max_hdr_size, icli_range_t *sampler_idx_list, BOOL no)
{
    ...
    if (no) {
        sflow_conf_value.max_hdr_size = VTSS_APPL_SFLOW_FLOW_HEADER_SIZE_DEFAULT;
    } else {
...
mesa_rc sflow_icli_counter_poll_interval(i32 session_id, icli_stack_port_range_t *plist, u32 poll_interval, icli_range_t *sampler_idx_list, BOOL no)
{
    ...
    if (no) {
        sflow_conf_value.poll_interval = VTSS_APPL_SFLOW_POLLING_INTERVAL_DEFAULT;
    } else {
...
mesa_rc sflow_icli_agent_ip(i32 session_id, BOOL has_ipv4, mesa_ip_t *v_ipv4_addr, BOOL has_ipv6, mesa_ipv6_t *v_ipv6_addr, BOOL no)
{
    ...
    if (no) {
        agent.agent_ip_addr.type      = VTSS_APPL_SFLOW_AGENT_IP_TYPE_DEFAULT;
        agent.agent_ip_addr.addr.ipv4 = VTSS_APPL_SFLOW_AGENT_IP_ADDR_DEFAULT;
    } else if (has_ipv4 && v_ipv4_addr != NULL) {
        agent.agent_ip_addr.type      = MESA_IP_TYPE_IPV4;
        agent.agent_ip_addr.addr.ipv4 = *v_ipv4_addr;
    } else if (has_ipv6 && v_ipv6_addr != NULL) {
        agent.agent_ip_addr.type      = MESA_IP_TYPE_IPV6;
    ...
    ICLI_RC_CHECK_PRINT_RC(sflow_mgmt_agent_cfg_set(&agent));
    return VTSS_RC_OK;
}
