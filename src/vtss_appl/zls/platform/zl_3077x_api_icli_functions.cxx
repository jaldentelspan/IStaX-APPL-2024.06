...
#include "zl_3077x_api_icli_functions.h"
...
icli_rc_t zl_3077x_api_icli_debug_read(i32 session_id, u32 mem_page, u32 mem_addr, u32 mem_size)
{
    ...
    {
        if (zl_3077x_register_get(mem_page,  mem_addr+i,  &value) != VTSS_RC_OK) {
            rc = ICLI_RC_ERROR;
        } else {
            n = (i & 0x7);
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3077x_register_set(mem_page,  mem_addr,  value) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3077x_debug_pll_status() != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3077x_debug_hw_ref_status(refid) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
icli_rc_t zl_3077x_api_icli_debug_ref_cfg(i32 session_id, u32 refid)
{
    ...
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3077x_debug_hw_ref_cfg(refid) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3077x_debug_dpll_status(pllid) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3077x_debug_dpll_cfg(pllid) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
{
    return zl_3077x_trace_level_module_set(module, level) ? ICLI_RC_OK : ICLI_RC_ERROR;
}
//icli_rc_t zl_3077x_api_icli_debug_dco_freq_read(i32 session_id)
//{
//    i32 freqOffsetUppm;
//    icli_rc_t rc =  ICLI_RC_OK;
//    if (zl_3077x_debug_dco_freq_read(&freqOffsetUppm) != VTSS_RC_OK) {
//        rc = ICLI_RC_ERROR;
//    } else {
...
icli_rc_t zl_303xx_icli_spi_read(i32 session_id, u8 page, u16 addr, u8 cnt)
{
    ...
    zl_3077x_spi_write(0x7f, &data[0], 1);
    ...
    for (int i = 0; i < cnt;i+=16) {
        ...
        zl_3077x_spi_read(addr + i, data, nread);
...
{
    /* Set page number first */
    zl_3077x_spi_write(0x7f, &page, 1);
    /* Write data */
    zl_3077x_spi_write(addr, &data, 1);
...
icli_rc_t zl_3077x_api_icli_print_mailbox_cfg(i32 session_id, bool is_ref, u8 id)
{
    if (is_ref) {
        ...
        zl_3077x_debug_print_mailbox_cfg(true,id);
    } else {
        ICLI_PRINTF("DPLL id %u\n",id);
        zl_3077x_debug_print_mailbox_cfg(false,id);
    }
    return ICLI_RC_OK;
}

// Dump the zl30772 registers using ptp dpll
icli_rc_t zl_3077x_icli_regdump(i32 session_id)
{
    ICLI_PRINTF("\n  Dump registers using PTP dpll\n");
    zl_3077x_reg_dump_all();
...
{
    zl_3077x_1pps_ref_conf(ref, enable);
}
...
icli_rc_t zl_3077x_icli_ref_freq_update(i32 session_id, uint8_t ref, uint32_t freq)
{
    ...
    if (zl_3077x_ref_input_freq_set(refId, freq) != MESA_RC_OK) {
...
{
    (void)zl_3077x_dpll_force_lock_ref(dpll, ref, enable);
}
...
{
    (void)zl_30380_apr_dbg_cmds(cguId, servId, cmd, val);
    return ICLI_RC_OK;
}
#endif
