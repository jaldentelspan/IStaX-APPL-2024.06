...
icli_rc_t zl_3034x_api_icli_debug_read(i32 session_id, u32 mem_page, u32 mem_addr, u32 mem_size)
{
    ...
    {
        if (zl_3034x_register_get(mem_page,  mem_addr+i,  &value) != VTSS_RC_OK) {
            rc = ICLI_RC_ERROR;
        } else {
            n = (i & 0x7);
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3034x_register_set(mem_page,  mem_addr,  value) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3034x_debug_hw_ref_status(refid) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3034x_debug_hw_ref_cfg(refid) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3034x_debug_dpll_status(pllid) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
{
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3034x_debug_dpll_cfg(pllid) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    }
    return rc;
}
...
icli_rc_t zl_3034x_api_icli_debug_dco_freq_read(i32 session_id)
{
    i32 freqOffsetUppm;
    icli_rc_t rc =  ICLI_RC_OK;
    if (zl_3034x_debug_dco_freq_read(&freqOffsetUppm) != VTSS_RC_OK) {
        rc = ICLI_RC_ERROR;
    } else {
...
{
    return zl_3034x_trace_level_module_set(v_uint_1, v_uint_2) ? ICLI_RC_OK : ICLI_RC_ERROR;
}

icli_rc_t zl_3034x_api_icli_debug_trace_level_all_set(i32 session_id, u32 v_uint_1)
{
    return zl_3034x_trace_level_all_set(v_uint_1) ? ICLI_RC_OK : ICLI_RC_ERROR;
}
