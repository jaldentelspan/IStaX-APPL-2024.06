...
#include "dot1x_acct.h"
#include <vtss/appl/port.h>        /* For vtss_appl_port_statistics_get()  */
...
/****************************************************************************/
static void DOT1X_ACCT_radius_rx_callback(u8 handle, ctx_t ctx, vtss_radius_access_codes_e code, vtss_radius_rx_callback_result_e res, u32 server_id)
{
    nas_sm_t       *sm = (nas_sm_t *)ctx.ptr;
...
#define DOT1X_ADD_OPTIONAL_RADIUS_ATTRIB(t, l, v)                                                 \
  ...
  if (res != VTSS_RC_OK && res != VTSS_RADIUS_ERROR_NOT_ROOM_FOR_TLV) {                           \
    T_EG(TRACE_GRP_ACCT, "Got \"%s\" from vtss_tlv_set() on optional attribute", error_txt(res)); \
    return;                                                                                       \
  }
    ...
    if ((status_type == DOT1X_ACCT_STATUS_TYPE_STOP) || (status_type == DOT1X_ACCT_STATUS_TYPE_INTERIM_UPDATE)) {
        // Acct-Session-Time
        u32 session_time = HOST2NETL((VTSS_OS_TICK2MSEC(vtss_current_time()) / 1000) - acct->session_time);
        ...
        if (acct->counters) {
            ...
            if (VTSS_ISID_LEGAL(isid) && msg_switch_exists(isid)) {
                if ((rc = vtss_appl_port_statistics_get(ifindex, &counters)) == VTSS_RC_OK) {
                    u32 cnt;
                    // Acct-Input-Packets
    ...
    if (status_type == DOT1X_ACCT_STATUS_TYPE_STOP) {
        ...
        if (eap_info->stop_reason == NAS_STOP_REASON_SWITCH_REBOOT) {
        ...
        } else if ((vtss_appl_port_conf_get(ifindex, &conf) == VTSS_RC_OK) && (conf.admin.enable == FALSE)) {
            ...
            temp_str[3] = VTSS_RADIUS_ACCT_TERMINATE_CAUSE_USER_ERROR;
                   ...
                   (eap_info->stop_reason == NAS_STOP_REASON_PORT_STP_MSTI_DISCARDING)) {
...
#undef DOT1X_ADD_OPTIONAL_RADIUS_ATTRIB
    ...
    ctx_t ctx;
    ctx.ptr = (status_type != DOT1X_ACCT_STATUS_TYPE_STOP) ? sm : NULL;
    if ((res = vtss_radius_tx(handle, ctx, DOT1X_ACCT_radius_rx_callback, 0)) != VTSS_RC_OK) {
        T_EG(TRACE_GRP_ACCT, "vtss_radius_tx() returned \"%s\"", error_txt(res));
        return;
    }
    if (status_type != DOT1X_ACCT_STATUS_TYPE_STOP) {
        acct->handle = handle;
        ...
        case DOT1X_ACCT_EVENT_UP:
            if (vtss_radius_acct_ready()) {
                if (NAS_PORT_CONTROL_IS_SINGLE_CLIENT(port_info->port_control)) { // Save counters
                    ...
                    mesa_port_no_t       api_port = DOT1X_NAS_PORT_2_SWITCH_API_PORT(port_info->port_no);
                    mesa_rc              rc;
                    mesa_port_counters_t counters;
                    acct->counters = FALSE;
                    if (VTSS_ISID_LEGAL(isid) && msg_switch_exists(isid)) {
                        vtss_ifindex_t ifindex;
                        if (vtss_ifindex_from_port(isid, api_port, &ifindex) != VTSS_RC_OK) {
                            T_E("Could not get ifindex");
                        };
                ...
                acct->session_time = VTSS_OS_TICK2MSEC(vtss_current_time()) / 1000; /* Save uptime in seconds */
...
/******************************************************************************/
static void DOT1X_ACCT_reset_callback(mesa_restart_t restart)
{
...
BOOL dot1x_acct_append_radius_tlv(u8 radius_handle, nas_eap_info_t *eap_info)
{
    ...
    if (rc != VTSS_RC_OK && rc != VTSS_RADIUS_ERROR_NOT_ROOM_FOR_TLV) {
        T_E("Got \"%s\" from vtss_tlv_set() on optional attribute", error_txt(rc));
        return FALSE;
    }
    return TRUE;
}
...
void dot1x_acct_init(void)
{
    ...
    control_system_reset_register(DOT1X_ACCT_reset_callback, VTSS_MODULE_ID_DOT1X);
}
