...
#include "vlan_api.h"    /* For VTSS_APPL_VLAN_ID_MIN/MAX */
...
/******************************************************************************/
static i32 DOT1X_web_handler_nas_reset(CYG_HTTPD_STATE *p)
{
    ...
    if (p->method == CYG_HTTPD_METHOD_GET) {
        ...
        if (iport >= VTSS_PORT_NO_START + fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT)) {
            errors++;
        }
        ...
        (void) vtss_ifindex_from_port(isid, iport, &ifindex);

        if (!errors && vtss_nas_reauth(ifindex, now) != VTSS_RC_OK) {
            errors++;
        }
...
/*lint -e{644} */
static i32 DOT1X_web_handler_nas_config(CYG_HTTPD_STATE *p)
{
    vtss_isid_t           isid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    ...
    if ((rc = vtss_appl_nas_glbl_cfg_get(&glbl_cfg))     != VTSS_RC_OK ||
        (rc = vtss_nas_switch_cfg_get(usid, &switch_cfg)) != VTSS_RC_OK) {
        errors++;
    }
    ...
    if (p->method == CYG_HTTPD_METHOD_POST) {
        if (!errors) {
            ...
            if (!errors) {
                if ((rc = vtss_appl_nas_glbl_cfg_set(&glbl_cfg)) != VTSS_RC_OK) {
                    T_D("vtss_appl_nas_glbl_cfg_set() failed");
                    errors++;
                }
            }
            ...
            while (!errors && port_iter_getnext(&pit)) {
                ...
                if (cyg_httpd_form_varable_int(p, var_name, &an_integer)) {
                    switch_cfg.port_cfg[pit.iport - VTSS_PORT_NO_START].admin_state = (vtss_appl_nas_port_control_t)an_integer;
                } else {
                    errors++;
                }
            ...
            if (!errors) {
                if ((rc = vtss_nas_switch_cfg_set(usid, &switch_cfg)) != VTSS_RC_OK) {
                    T_D("vtss_nas_switch_cfg_set() failed");
                    errors++;
                }
            }
        ...
        if (vtss_nas_switch_status_get(usid, &switch_status) != VTSS_RC_OK) {
            errors++;
        }
        ...
        if (!errors) {
            ...
            // [PortConfig]  = PortNumber/AdminState/PortState/BackendQoS/BackendVLAN/GuestVLAN/AuthCnt/UnauthCnt
            ...
            while (port_iter_getnext(&pit)) {
                ...
                // [PortConfigs] = [PortConfig1]#[PortConfig2]#...#[PortConfigN]
                // [PortConfig]  = PortNumber/AdminState/PortState/BackendQoS/BackendVLAN/GuestVLAN/AuthCnt/UnauthCnt
                cnt = snprintf(p->outbuffer, sizeof(p->outbuffer), "%d/%d/%d/%d/%d/%d/%u/%u",
                               pit.uport,
                               ...
                               0,
#endif
                               switch_status.auth_cnt[pit.iport - VTSS_PORT_NO_START],
                               switch_status.unauth_cnt[pit.iport - VTSS_PORT_NO_START]);
                (void)cyg_httpd_write_chunked(p->outbuffer, cnt);
                if (!pit.last) {
...
/******************************************************************************/
static i32 DOT1X_web_handler_nas_switch_status(CYG_HTTPD_STATE *p)
{
    vtss_isid_t           isid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    vtss_nas_switch_status_t switch_status;
    vtss_appl_nas_client_info_t last_supplicant_info;
    char                  encoded_supplicant_identity[3 * VTSS_APPL_NAS_SUPPLICANT_ID_MAX_LENGTH];
    int                   cnt;
    port_iter_t           pit;
    ...
    if (vtss_nas_switch_status_get(usid, &switch_status) == VTSS_RC_OK) {
        ...
        // [PortState]  = PortNumber/AdminState/PortState/LastSource/LastIdentity/QosClass/VLAN/auth_cnt/unauth_cnt
        ...
        while (port_iter_getnext(&pit)) {
            vtss_ifindex_t ifindex;
            (void) vtss_ifindex_from_port(isid, pit.iport, &ifindex);

            if (vtss_appl_nas_last_supplicant_info_get(ifindex, &last_supplicant_info) == VTSS_RC_OK) {
                ...
                nas_last_supplicant_info_mac_addr_as_str_get(ifindex, mac_addr_str);

                // Format: PortNumber/AdminState_1/PortState_1/LastSource_1/LastIdentity_1/QosClass_1/VLAN/auth_cnt/unauth_cnt
                cnt = snprintf(p->outbuffer, sizeof(p->outbuffer), "%d/%d/%d/%s/%s/%s/%s/%u/%u",
                               pit.uport,
...
// Lint can't see it, but it really *is* initialized.
/*lint -e{644} */
static i32 DOT1X_web_handler_nas_port_status(CYG_HTTPD_STATE *p)
{
    vtss_isid_t        isid = web_retrieve_request_sid(p); /* Includes USID = ISID */
    int                uport, clear;
    mesa_port_no_t     iport;
    vtss_nas_statistics_t statistics;
    char               encoded_identity[3 * VTSS_APPL_NAS_SUPPLICANT_ID_MAX_LENGTH];
    int                cnt, errors = 0, i;
    ...
    // vid=%s
    if (cyg_httpd_form_varable_int(p, "vid", &an_integer) && an_integer >= VTSS_APPL_VLAN_ID_MIN && an_integer <= VTSS_APPL_VLAN_ID_MAX) {
    ...
    if (!errors && cyg_httpd_form_varable_int(p, "clear", &clear) && clear == 1) {
        ...
        if (vtss_nas_statistics_clear(ifindex, &vid_mac) != VTSS_RC_OK) {
            if (vid_mac.vid == 0) {
    ...
    // Where
    //   [port_cfg]         = port_number/protocols/options/admin_state/qos_class/vlan_type/vid/auth_cnt/unauth_cnt
    ...
    if (!errors) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (!skip) {
                ...
                if (!errors && vtss_nas_statistics_get(ifindex, used_vid_mac, &statistics) != VTSS_RC_OK) {
                    if (i == 0) {
...
#endif
#ifdef VTSS_SW_OPTION_NAS_BACKEND_ASSIGNED_VLAN
                if (statistics.vlan_type == VTSS_APPL_NAS_VLAN_TYPE_BACKEND_ASSIGNED) {
                    sprintf(vlan_str, "%d (RADIUS-assigned)", statistics.vid);
                }
#endif
#ifdef VTSS_SW_OPTION_NAS_GUEST_VLAN
                if (statistics.vlan_type == VTSS_APPL_NAS_VLAN_TYPE_GUEST_VLAN) {
                    sprintf(vlan_str, "%d (Guest)", statistics.vid);
                }
#endif
                cnt = snprintf(p->outbuffer, sizeof(p->outbuffer), "%u/%u/%u/%d/%s/%d/%s/%u/%u#",
                               iport2uport(iport),
                               DOT1X_web_compose_supported_protocols(),
                               DOT1X_web_compose_supported_options(),
                               skip ? VTSS_APPL_NAS_PORT_CONTROL_FORCE_UNAUTHORIZED : statistics.admin_state, // If an error occurred, provide the web-page with some more or less useful info.
                               qos_str,
#ifdef NAS_USES_VLAN
        ...
        vtss_nas_multi_client_status_t client_status;
        ...
        do {
            ...
            if (vtss_nas_multi_client_status_get(ifindex, &client_status, &client_found, client_first) == VTSS_RC_OK && client_found) {
