...
#define setPortAuthorized()                                            \
  do {                                                                 \
    BOOL chgd = AUTH_SM(sm)->authStatus != VTSS_APPL_NAS_PORT_STATUS_AUTHORIZED; \
    AUTH_SM(sm)->authStatus = VTSS_APPL_NAS_PORT_STATUS_AUTHORIZED;              \
...
#define setPortUnauthorized()                                            \
  do {                                                                   \
    BOOL chgd = AUTH_SM(sm)->authStatus != VTSS_APPL_NAS_PORT_STATUS_UNAUTHORIZED; \
...
SM_STEP(AUTH_PAE)
{
    ...
    } else if (sm->port_info->port_control == VTSS_APPL_NAS_PORT_CONTROL_FORCE_AUTHORIZED && AUTH_SM(sm)->auth_pae.portMode != sm->port_info->port_control && !(BVTST(initialize) || BVTSTNOT(portEnabled))) {
        SM_ENTER(AUTH_PAE, FORCE_AUTH);
    } else if (sm->port_info->port_control == VTSS_APPL_NAS_PORT_CONTROL_FORCE_UNAUTHORIZED && AUTH_SM(sm)->auth_pae.portMode != sm->port_info->port_control && !(BVTST(initialize) || BVTSTNOT(portEnabled))) {
