...
static void AUTH_send(nas_sm_t *sm)
{
    ...
    if (BVTSTNOT(skipFirstEapolTx)) {
        ...
        if (sm->port_info->port_control == VTSS_APPL_NAS_PORT_CONTROL_DOT1X_MULTI && BVTSTNOT(isReqIdentitySM)) {
...
static void AUTH_handle_eap_response(nas_sm_t *sm, u8 *eap_hdr, u16 len)
{
    ...
    if (*dat == (u8)EAP_TYPE_IDENTITY) {
        ...
        if (id_length >= VTSS_APPL_NAS_SUPPLICANT_ID_MAX_LENGTH) {
...
/******************************************************************************/
static void IEEE8021X_do_update_client_info(nas_sm_t *any_sm, mesa_vid_mac_t *vid_mac)
{
...
/******************************************************************************/
static void IEEE8021X_update_client_info(nas_sm_t *sub_sm, mesa_vid_mac_t *vid_mac, BOOL called_from_init)
{
...
/******************************************************************************/
static void IEEE8021X_init_sm(nas_sm_t *sm, mesa_vid_mac_t *vid_mac, BOOL is_req_identity_sm)
{
    ...
    AUTH_SM(sm)->authStatus                = VTSS_APPL_NAS_PORT_STATUS_UNAUTHORIZED;
    AUTH_SM(sm)->eap_info.radius_handle    = -1;   // Not in use
    AUTH_SM(sm)->eap_info.radius_server_id = 0; // Don't care which RADIUS server to use.
...
static void IEEE8021X_reinitialize(nas_sm_t *top_sm)
{
    ...
    for (sm = top_sm->next; sm; sm = sm->next) {
        AUTH_SM(sm)->auth_pae.portMode = VTSS_APPL_NAS_PORT_CONTROL_DISABLED;
        BVSET(initialize);
        auth_sm_step(sm);
        BVCLR(initialize);
        auth_sm_step(sm);
...
static void IEEE8021X_new_auth_session(nas_sm_t *sm)
{
    ...
    eap_info->radius_eap_attribute_len = 0;
}

/******************************************************************************/
// IEEE8021X_get_port_status()
/******************************************************************************/
static vtss_appl_nas_port_status_t IEEE8021X_get_port_status(nas_sm_t *top_sm, u32 *auth_cnt, u32 *unauth_cnt)
{
    *auth_cnt   = 0;
    *unauth_cnt = 0;
    ...
    if (NAS_PORT_CONTROL_IS_MULTI_CLIENT(top_sm->port_info->port_control)) {
        ...
        for (sm = top_sm->next; sm; sm = sm->next) {
            if (AUTH_SM(sm)->authStatus == VTSS_APPL_NAS_PORT_STATUS_AUTHORIZED) {
                (*auth_cnt)++;
            } else {
                (*unauth_cnt)++;
            }
        ...
        return VTSS_APPL_NAS_PORT_STATUS_MULTI;
    } else
#endif
    {
        if (top_sm->next == NULL) {
...
/******************************************************************************/
static vtss_appl_nas_port_status_t IEEE8021X_get_status(nas_sm_t *sub_sm)
{
...
/******************************************************************************/
static vtss_nas_eapol_counters_t *IEEE8021X_get_eapol_counters(nas_sm_t *top_or_sub_sm)
{
    if (top_or_sub_sm->sm_type == NAS_SM_TYPE_TOP) {
...
// that SM.
/******************************************************************************/
static vtss_appl_nas_backend_counters_t *IEEE8021X_get_backend_counters(nas_sm_t *top_or_sub_sm)
{
    if (top_or_sub_sm->sm_type == NAS_SM_TYPE_TOP) {
...
/******************************************************************************/
static void IEEE8021X_eapol_frame_received(nas_sm_t *top_or_sub_sm, u8 *frame, mesa_vid_t vid, size_t len)
{
    nas_sm_t                     *sm;
...
{
    nas_eap_info_t    *eap_info    = &AUTH_SM(sm)->eap_info;
    vtss_nas_client_info_t *client_info = &sm->client_info;
