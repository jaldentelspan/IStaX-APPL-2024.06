 ...
 */
#define NAS_MAX_FRAME_SIZE 1622

#define NAS_RADIUS_STATE_MAX_LENGTH         ( 256)
#define NAS_RADIUS_EAP_MAX_LENGTH           ( 512)
#define NAS_RADIUS_EXTRA_SIZE               (  72) /* Extra bytes to encapsulate in RADIUS protocol */
#define NAS_IEEE8021X_MAX_EAPOL_PACKET_SIZE (NAS_MAX_FRAME_SIZE - NAS_RADIUS_EXTRA_SIZE)
#define NAS_PORT_CNT                        fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT)

#if 0 // OBSOLETE, use VTSS_APPL_CAP_NAS_STATE_MACHINE_CNT
#ifdef NAS_USES_PSEC
...
typedef enum {
    ...
    NAS_STOP_REASON_PORT_STP_MSTI_DISCARDING,
...
typedef struct acct_sm {
    ...
    mesa_port_counter_t input_octets;       // Contains the number of the input octets when the session started
    mesa_port_counter_t output_octets;      // Contains the number of the output octets when the session started
    mesa_port_counter_t input_packets;      // Contains the number of the input packets when the session started
    mesa_port_counter_t output_packets;     // Contains the number of the output packets when the session started
...
typedef struct {
    ...
    u8                     radius_eap_attribute[NAS_RADIUS_EAP_MAX_LENGTH];
    u16                    radius_eap_attribute_len;

    int                    radius_handle;    // -1 = not in use.
    u32                    radius_server_id; // In order to use the same RADIUS server throughout a conversation
...
typedef struct {
    nas_port_t             port_no;
    vtss_appl_nas_port_control_t     port_control;
    u32                    cur_client_cnt;    // Counts number of SMs attached to this port.
    nas_sm_t               *top_sm;           // For fast reference to the top-level SM
    vtss_appl_nas_backend_counters_t *backend_counters; // Per-port backend counters (the summed if multi-client).
    vtss_nas_eapol_counters_t   *eapol_counters;   // Per-port EAPOL counters (the summed if multi-client).
    ...
    mesa_vid_t             backend_assigned_vid; // Only non-zero sporadically.
#endif
...
#ifdef NAS_USES_VLAN
    mesa_vid_t                current_vid;          // 0 if not overridden.
    vtss_appl_nas_vlan_type_t vlan_type;            // To be able to see why the current vid is non-zero.
#endif
} nas_port_info_t;
...
vtss_appl_nas_port_status_t nas_get_port_status(nas_port_t port_no, u32 *auth_cnt, u32 *unauth_cnt); // auth_cnt and unauth_cnt may only be none-zero if it's a multi-client.
vtss_appl_nas_port_status_t nas_get_status(nas_sm_t *sm); // Never a Top-SM
void                    nas_set_status(nas_sm_t *sm, vtss_appl_nas_port_status_t status); // Never a Top-SM. Use with care. Only implemented for MAC-limit
...
vtss_nas_client_info_t *nas_get_client_info(nas_sm_t *top_or_sub_sm); // SM = Top => last client. SM = Sub => the one's
...
nas_sm_t *nas_get_sm_from_mac(mesa_vid_mac_t *mac);
...
BOOL          nas_os_backend_server_tx_request(nas_sm_t *sm, int *handle, u8 *eap, u16 eap_len, u8 *state, u8 state_len, char *user, nas_port_t nas_port, u8 *mac_addr);
