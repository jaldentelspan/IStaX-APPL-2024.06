...
extern "C" nas_funcs_t const *mac_based_get_funcs(void);
#else
...
// user name is wider than VTSS_APPL_NAS_SUPPLICANT_ID_MAX_LENGTH, this code
// falls back to using the MAC address as user name/supplicant ID.
#if VTSS_APPL_NAS_SUPPLICANT_ID_MAX_LENGTH < 18
...
static CapArray<nas_sm_t, MEBA_CAP_BOARD_PORT_MAP_COUNT> NAS_top_level_state_machines;
static CapArray<nas_port_info_t, MEBA_CAP_BOARD_PORT_MAP_COUNT> NAS_top_level_port_info;

// Pool of SMs that can be used by any agent. These SMs need to be attached to
// the individual top-level SMs once allocated.
static CapArray<nas_sm_t, VTSS_APPL_CAP_NAS_STATE_MACHINE_CNT> NAS_state_machines;
// This is a union of all agents, and each one of them will get attached to
// a SM from the NAS_state_machines[] array.
static CapArray<nas_agent_sm_t, VTSS_APPL_CAP_NAS_STATE_MACHINE_CNT> NAS_agent_state_machines;
...
static CapArray<vtss_appl_nas_backend_counters_t, MEBA_CAP_BOARD_PORT_MAP_COUNT> NAS_port_backend_counters;

// Per-port EAPOL counters.
static CapArray<vtss_nas_eapol_counters_t, MEBA_CAP_BOARD_PORT_MAP_COUNT> NAS_port_eapol_counters;
...
static CapArray<nas_sm_t *, MEBA_CAP_BOARD_PORT_MAP_COUNT> NAS_request_identity_state_machines;
#endif
...
static void NAS_get_nas_funcs(void)
{
    ...
    for (i = 0; i < sizeof(NAS_top_level_funcs) / sizeof(void *); i++) {
        //lint -e{611} Avoid "suspicious cast". The following is required by C++ compiler
        *(p++) = (void *)NAS_error_func;
    }
...
/******************************************************************************/
static nas_sm_type_t NAS_get_type_from_port_mode(nas_sm_t *sm)
{
    switch (sm->port_info->port_control) {
    ...
    default:
        T_EG(TRACE_GRP_BASE, "Port %u: Invalid mode (%d)", sm->port_info->port_no, sm->port_info->port_control);
...
/******************************************************************************/
static nas_sm_type_t NAS_get_type_from_port_mode_allow_top(vtss_appl_nas_port_control_t mode)
{
    switch (mode) {
    case VTSS_APPL_NAS_PORT_CONTROL_AUTO:
    case VTSS_APPL_NAS_PORT_CONTROL_FORCE_AUTHORIZED:
...
void nas_init(void)
{
    nas_sm_type_t agent;
    nas_sm_t      *sm;
    u32           i, nas_machine_cnt = fast_cap(VTSS_APPL_CAP_NAS_STATE_MACHINE_CNT);
    ...
    vtss_clear(NAS_top_level_state_machines);
    vtss_clear(NAS_top_level_port_info);
    vtss_clear(NAS_port_backend_counters);
    vtss_clear(NAS_port_eapol_counters);
...
#ifdef NAS_DOT1X_SINGLE_OR_MULTI
    vtss_clear(NAS_request_identity_state_machines);
#endif

    // Also initialize the normal state machines.
    vtss_clear(NAS_state_machines);
    vtss_clear(NAS_agent_state_machines);

    // Create a list of free state machines
    NAS_state_machines_left = 0;
    for (i = 0; i < nas_machine_cnt; i++) {
        sm            = &NAS_state_machines[i];
        sm->u.real_sm = &NAS_agent_state_machines[NAS_state_machines_left++];
        sm->next      = (i == (nas_machine_cnt - 1) ? NULL : &NAS_state_machines[i + 1]);
    }
...
void nas_1sec_timer_tick(void)
{
    ...
    for (port = 1; port <= NAS_PORT_CNT; port++) {
        ...
        if (top_sm->port_info->port_control == VTSS_APPL_NAS_PORT_CONTROL_DISABLED) {
            continue;
        }

        agent = NAS_get_type_from_port_mode(top_sm);
...
/******************************************************************************/
nas_sm_t *nas_alloc_sm(nas_port_t port_no, mesa_vid_mac_t *vid_mac)
{
    ...
    if (port_info->port_control == VTSS_APPL_NAS_PORT_CONTROL_DISABLED) {
    ...
    if (!sm) {
        ...
        if (sm) {
            ...
            T_IG(TRACE_GRP_BASE, "Out of state machines uport %u, vid %u, mac %02x:%02x:%02x:%02x:%02x:%02x",
                 port_no,
                 vid_mac->vid,
    ...
    // Get the SM's type
    sm->sm_type = NAS_get_type_from_port_mode(top_sm);
...
/******************************************************************************/
vtss_appl_nas_port_control_t nas_get_port_control(nas_port_t port_no)
{
    nas_sm_t *top_sm = nas_get_top_sm(port_no);
    ...
    return VTSS_APPL_NAS_PORT_CONTROL_DISABLED;
}

/******************************************************************************/
// nas_set_port_control()
/******************************************************************************/
void nas_set_port_control(nas_port_t port_no, vtss_appl_nas_port_control_t new_mode)
{
    vtss_appl_nas_port_control_t old_mode;
    nas_sm_t           *top_sm = nas_get_top_sm(port_no);
#ifdef NAS_DOT1X_SINGLE_OR_MULTI
    nas_sm_t           *sm;
#endif
    ...
    if (new_mode != VTSS_APPL_NAS_PORT_CONTROL_DISABLED) {
...
void nas_set_fake_force_authorized(nas_sm_t *top_sm, BOOL enable)
{
    ...
    } else if (top_sm->port_info->port_control == VTSS_APPL_NAS_PORT_CONTROL_AUTO) {
...
/******************************************************************************/
nas_sm_t *nas_get_sm_from_mac(mesa_vid_mac_t *vid_mac)
{
    nas_sm_t      *top_sm, *sm;
    nas_port_t    port_no;
...
/******************************************************************************/
nas_sm_t *nas_get_sm_from_vid_mac_port(const mesa_vid_mac_t *vid_mac, nas_port_t port_no)
{
    nas_sm_t *top_sm = nas_get_top_sm(port_no), *sm;
...
// nas_get_port_status()
/******************************************************************************/
vtss_appl_nas_port_status_t nas_get_port_status(nas_port_t port_no, u32 *auth_cnt, u32 *unauth_cnt)
{
    nas_sm_t      *top_sm = nas_get_top_sm(port_no);
    nas_sm_type_t agent;

    if (!top_sm) {
        return  VTSS_APPL_NAS_PORT_STATUS_UNAUTHORIZED; // Error already printed.
    }
    ...
    return NAS_funcs[agent]->get_port_status(top_sm, auth_cnt, unauth_cnt);
}
...
/******************************************************************************/
vtss_appl_nas_port_status_t nas_get_status(nas_sm_t *sub_sm)
{
    if (!sub_sm) {
        T_EG(TRACE_GRP_BASE, "Invalid parameter");
        return VTSS_APPL_NAS_PORT_STATUS_UNAUTHORIZED;
    }
...
// sm is never a top_sm
/******************************************************************************/
void nas_set_status(nas_sm_t *sub_sm, vtss_appl_nas_port_status_t status)
{
    if (!sub_sm) {
        T_EG(TRACE_GRP_BASE, "Invalid parameter");
        return;
...
/******************************************************************************/
vtss_nas_eapol_counters_t *nas_get_eapol_counters(nas_sm_t *top_or_sub_sm)
{
    nas_sm_type_t agent;
...
/******************************************************************************/
vtss_appl_nas_backend_counters_t *nas_get_backend_counters(nas_sm_t *top_or_sub_sm)
{
    nas_sm_type_t agent;
...
/******************************************************************************/
void nas_clear_statistics(nas_port_t port_no, mesa_vid_mac_t *vid_mac)
{
...
/******************************************************************************/
vtss_nas_client_info_t *nas_get_client_info(nas_sm_t *sm)
{
    if (!sm) {
        T_EG(TRACE_GRP_BASE, "Invalid parameter");
        return NULL;
...
/******************************************************************************/
void nas_ieee8021x_eapol_frame_received(nas_port_t port_no, u8 *frame, mesa_vid_t vid, size_t len)
{
    nas_sm_t      *top_sm = nas_get_top_sm(port_no);
    nas_sm_type_t agent;
...
/******************************************************************************/
void nas_ieee8021x_eapol_frame_received_sm(nas_sm_t *sub_sm, u8 *frame, mesa_vid_t vid, size_t len)
{
    nas_sm_type_t agent;
    ...
    agent = NAS_get_type_from_port_mode(sub_sm);
...
// nas_get_sm_type()
/******************************************************************************/
bool nas_is_sm_mac_based(nas_sm_t *sm)
{
    VTSS_ASSERT(sm != NULL);
    return sm->sm_type == NAS_SM_TYPE_MAC_BASED;
}
