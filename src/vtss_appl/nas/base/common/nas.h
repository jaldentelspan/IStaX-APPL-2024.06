...
typedef struct {
    // All SMs:
    void                    (*init)(void);
    void                    (*init_sm)(nas_sm_t *sm, mesa_vid_mac_t *vid_mac, BOOL is_req_identity_sm); // This SM has just been allocated. The vid_mac arg may not be used for single-client agents.
    ...
    vtss_appl_nas_port_status_t (*get_port_status)(nas_sm_t *top_sm, u32 *auth_cnt, u32 *unauth_cnt); // auth_cnt and unauth_cnt are only used for multi-client modes.
    vtss_appl_nas_port_status_t (*get_status)(nas_sm_t *sub_sm); // Never a top_sm
    void                    (*set_status)(nas_sm_t *sub_sm, vtss_appl_nas_port_status_t status); // Never a top_sm. Optional

    // IEEE8021X SMs:
    vtss_nas_eapol_counters_t *(*get_eapol_counters)(nas_sm_t *top_or_sub_sm);
    void                    (*ieee8021x_eapol_frame_received)(nas_sm_t *top_or_sub_sm, u8 *frame, mesa_vid_t vid, size_t len);
    ...
    vtss_appl_nas_backend_counters_t *(*get_backend_counters)(nas_sm_t *top_or_sub_sm);  // If SM is top_sm, get overall counters for that port, otherwise get for that SM
