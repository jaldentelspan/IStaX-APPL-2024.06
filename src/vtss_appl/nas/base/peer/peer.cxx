...
/******************************************************************************/
static void MB_init_sm(nas_sm_t *sm, mesa_vid_mac_t *vid_mac, BOOL is_req_identity_sm)
{
...
#define MB_FAKE_REQUEST_IDENTITY_CNT 2
    ...
    PEER_SM(sm)->eap_info.radius_server_id        = 0;
    PEER_SM(sm)->eap_info.stop_reason             = NAS_STOP_REASON_UNKNOWN;
    PEER_SM(sm)->authStatus                       = VTSS_APPL_NAS_PORT_STATUS_UNAUTHORIZED;
...
static void MB_reauthenticate(nas_sm_t *top_sm)
{
    ...
    for (sm = top_sm->next; sm; sm = sm->next) {
        ...
        if (PEER_SM(sm)->authStatus == VTSS_APPL_NAS_PORT_STATUS_AUTHORIZED) {
...
// MB_get_port_status()
/******************************************************************************/
static vtss_appl_nas_port_status_t MB_get_port_status(nas_sm_t *top_sm, u32 *auth_cnt, u32 *unauth_cnt)
{
    nas_sm_t *sm;
...
// MB_get_status()
/******************************************************************************/
static vtss_appl_nas_port_status_t MB_get_status(nas_sm_t *sub_sm)
{
...
// that SM.
/******************************************************************************/
static vtss_appl_nas_backend_counters_t *MB_get_backend_counters(nas_sm_t *top_or_sub_sm)
{
    if (top_or_sub_sm->sm_type == NAS_SM_TYPE_TOP) {
