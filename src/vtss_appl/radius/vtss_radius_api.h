...
/**
 * \brief Maximum RADIUS frame length.
 *
 * It is expected that the IP stack fragments the frames if larger than IP stack
 * MTU.
 */
#define RADIUS_MAX_FRAME_SIZE_BYTES 2048
 ...
 */
typedef struct {
    char                       host[VTSS_RADIUS_HOST_LEN]; /**< IPv4, IPv6 or hostname of this server */
...
/**
 * \brief Value to put in NAS-Service-Type attribute for BPDU-based Auth.
 */
#define VTSS_RADIUS_NAS_SERVICE_TYPE_FRAMED_USER 2

/**
 * \brief Value to put in NAS-Service-Type attribute for MAC-based Auth.
 */
#define VTSS_RADIUS_NAS_SERVICE_TYPE_CALL_CHECK 10
...
enum {
    ...
    VTSS_RADIUS_ERROR_MUST_BE_PRIMARY_SWITCH,                            /**< The API functions don't work when we aren't the primary switch. */
 ...
 * \param code      [IN] The RADIUS access code from the received frame. Only valid if rc == VTSS_RC_OK.
 * \param res       [IN] Return code used to indicate why the callback is called.
 * \param server_id [IN] Can be used to make sure to hit the same RADIUS server in subsequent calls to vtss_radius_tx(). Only valid (non-zero) if @res == VTSS_RADIUS_RX_CALLBACK_OK.
 */
typedef void (vtss_radius_rx_callback_f)(u8 handle, ctx_t ctx, vtss_radius_access_codes_e code, vtss_radius_rx_callback_result_e res, u32 server_id);
 ...
 * \param res    [IN] Return code used to indicate why the callback is called.
 */
typedef void (vtss_radius_init_chg_callback_f)(ctx_t ctx, vtss_radius_init_chg_callback_result_e res);
 ...
 * \param res    [IN] Return code used to indicate why the callback is called.
 */
typedef void (vtss_radius_id_state_chg_callback_f)(ctx_t ctx, vtss_radius_id_state_chg_callback_result_e res);
mesa_rc vtss_radius_id_state_chg_callback_register(void **register_id,  ctx_t ctx, vtss_radius_id_state_chg_callback_f *cb);
mesa_rc vtss_radius_id_state_chg_callback_unregister(void *register_id);
 ...
 * In case of a switch in primary, all callbacks are invoked with an error code, and
 ...
 *
 * \param handle             [OUT] Receives a unique value that must be used in subsequent calls to this module.
 * \param code               [IN]  Indicates what type of RADIUS frame this is. Valid values are VTSS_RADIUS_CODE_ACCESS_REQUEST and VTSS_RADIUS_CODE_ACCOUNTING_REQUEST.
 *
 ...
 *         VTSS_RC_OK: Success. @handle is now valid.
 */
 ...
 *
 * \return VTSS_RC_OK if the params are OK and the next TLV is OK,\
 ...
 *         VTSS_RC_OK if the params are OK and the next TLV is OK,\
 */
 ...
 *
 * \return VTSS_RC_OK: if @len == 0 (TLV not added) or TLV successfully added.
 ...
 *
 *   If this function returns VTSS_RC_OK, the frame will be transmitted and the callback function
 *   will be called from the RADIUS thread.
 *
 ...
 *   your code.
 *
 *   If this function returns VTSS_RADIUS_ERROR_MUST_BE_PRIMARY_SWITCH or VTSS_RADIUS_ERROR_NOT_INITIALIZED.
 *
 ...
 * \param use_this_server_id [IN] When 0, pick best server. Otherwise find server by this ID and transmit to that. Used in ongoing conversations.
 *
 * \return VTSS_RC_OK if the RADIUS frame was successfully transmitted. In this case the @cb will be called upon
 *         a RADIUS reply or timeout. If this function returns anything else but VTSS_RC_OK, the callback
 ...
 * \param handle [IN] Handle previously allocated with vtss_radius_alloc().
 *
 * \return VTSS_RC_OK if the handle was successfully released.
 */
 ...
 *
 * \return VTSS_RC_OK on success, or any of the vtss_radius_error_e codes on failure.
 */
...
typedef struct {
    ...
    mesa_ipv4_t               nas_ip_address;                              /**< NAS-IP-Address. NOTE: In network order!. */
    BOOL                      nas_ipv6_address_enable;                     /**< Use NAS-IPv6-Address if TRUE. */
    mesa_ipv6_t               nas_ipv6_address;                            /**< NAS-IPv6-Address. */
 ...
 *
 * \return VTSS_RC_OK if configuration successfully applied, or any of the\
