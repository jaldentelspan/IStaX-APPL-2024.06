...
struct ptp_servo {
    ...
    // needed for two-step clock + virtual port combination.
    mesa_timestamp_t alt_rcv_ts;
    ...
    virtual mesa_rc force_holdover_set(BOOL enable) = 0;
    virtual mesa_rc force_holdover_get(BOOL *enable) = 0;
    ...
    virtual int offset_calc(CapArray<ptp_clock_t *, VTSS_APPL_CAP_PTP_CLOCK_CNT> &ptpClock, int clock_inst, const mesa_timestamp_t& send_time, const mesa_timestamp_t& recv_time, mesa_timeinterval_t correction, i8 logMsgIntv, u16 sequenceId, bool peer_delay, bool virtual_port) = 0;
    virtual void clock_servo_reset(vtss_ptp_set_vcxo_freq setVCXOfreq) = 0;
    virtual void clock_servo_status(uint32_t domain, vtss_ptp_servo_status_t *status) = 0;
    virtual bool display_stats() = 0;
    virtual int display_parm(char* buf) { return 0; };
    virtual mesa_rc activate(uint32_t domain, bool hybrid_init) = 0;
    virtual void deactivate(uint32_t domain) = 0;
    virtual mesa_rc switch_1pps_virtual_ref(ptp_clock_t *ptp_clk, uint16_t instance, uint32_t domain, bool enable, bool warm_start) = 0;
    virtual mesa_rc set_device_1pps_virtual_ref(ptp_clock_t *ptp_clk, uint16_t instance, uint32_t domain, bool enable) = 0;
    virtual void process_alternate_timestamp(ptp_clock_t *ptp_clk, uint32_t domain, u16 instance, const mesa_timestamp_t *send_time, const mesa_timestamp_t *recv_time, mesa_timeinterval_t correction, i8 logMsgIntv, bool virtual_port) = 0;
