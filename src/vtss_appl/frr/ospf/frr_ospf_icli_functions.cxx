                                           ...
                                           vtss_appl_ospf_area_id_t area_id)
{
    ...
    while (vtss_appl_ospf_area_range_conf_itr(&cur_id, &next_id, &cur_area_id,
        ...
        if (area_range_conf.is_advertised && backbone_area_status.attached_intf_active_count && !got_ipv4_routes) {
            if (vtss_appl_ip_route_status_get_all(ipv4_routes, VTSS_APPL_IP_ROUTE_TYPE_IPV4_UC, VTSS_APPL_IP_ROUTE_PROTOCOL_OSPF) == VTSS_RC_OK) {
        ...
        if (area_range_conf.is_advertised &&
            ...
            route_ipv4_key.route.route.ipv4_uc.network = cur_network;
            ...
            for (auto itr = ipv4_routes.greater_than_or_equal(route_ipv4_key); itr != ipv4_routes.end(); ++itr) {
                is_active = vtss_ipv4_net_include(cur_network_p, &itr->first.route.route.ipv4_uc.network.address);
                                            ...
                                            vtss_appl_ospf_area_id_t area_id)
{
    ...
    if (vtss_appl_ospf_area_status_get(id, area_id, &status) == VTSS_RC_OK) {
        ...
        } else if (status.area_type == VTSS_APPL_OSPF_AREA_NSSA) {
            ICLI_PRINTF("        It is a NSSA area\n");
            if (status.nssa_trans_state ==
                VTSS_APPL_OSPF_NSSA_TRANSLATOR_STATE_ELECTED) {
                ICLI_PRINTF("        Perform type-7/type-5 LSA translation\n");
                ICLI_PRINTF("        NSSA translator state is elected\n");
            } else if (status.nssa_trans_state ==
                       VTSS_APPL_OSPF_NSSA_TRANSLATOR_STATE_ENABLED) {
                ICLI_PRINTF("        Perform type-7/type-5 LSA translation\n");
                ICLI_PRINTF("        NSSA translator state is enabled\n");
            } else {
                ICLI_PRINTF("        NSSA translator state is disabled\n");
            }
        }
        ...
        ICLI_PRINTF("        Number of NSSA LSA %d. Checksum Sum 0x%08x\n",
                    status.nssa_lsa_count, status.nssa_lsa_checksum);
...
mesa_rc FRR_ICLI_ospf_router_id_set(const FrrOspfCliReq &req)
{
    ...
    if (vtss_appl_ospf_router_conf_get(req.inst_id, &router_conf) != VTSS_RC_OK) {
...
mesa_rc FRR_ICLI_ospf_network_area_set(const FrrOspfCliReq &req)
{
    ...
    if (rc != VTSS_RC_OK) {
        ...
        } else if (rc == FRR_RC_ADDR_RANGE_OVERLAP) {
            ICLI_PRINTF("%% %s.\n", error_txt(rc));
...
mesa_rc FRR_ICLI_ospf_instance_process_del(const FrrOspfCliReq &req)
{
    ...
    if (vtss_appl_ospf_get(req.inst_id) != VTSS_RC_OK) {
        // Quit silently when the OSPF process is already disabled.
        return ICLI_RC_OK;
    }

    vtss_appl_ospf_router_conf_t rconf;
    mesa_rc rc = vtss_appl_ospf_router_conf_get(req.inst_id, &rconf);
    if (rc != VTSS_RC_OK) {
        ICLI_PRINTF(
            "%% Disable OSPF routing process failed. Cannot get router "
            "config\n");
        return ICLI_RC_ERROR;
    }

    rc = vtss_appl_ospf_del(req.inst_id);
    if (rc != VTSS_RC_OK) {
        ICLI_PRINTF("%% Disable OSPF routing process failed.\n");
        return ICLI_RC_ERROR;
    }

    if (rconf.stub_router.is_on_shutdown) {
        ICLI_PRINTF(
            "%% Router will be terminated after stub router advertisement "
            "for %d(s)\n",
            rconf.stub_router.on_shutdown_interval);
    }
...
mesa_rc FRR_ICLI_ospf_admin_distance_set(const FrrOspfCliReq &req)
{
    ...
    if (rc != VTSS_RC_OK) {
        ICLI_PRINTF("%% Set OSPF administrative distance failed.\n");
        return rc;
    }
...
mesa_rc FRR_ICLI_ospf_redist_set(const FrrOspfCliReq &req)
{
    ...
    router_conf.redist_conf[req.redist_protocol].is_specific_metric =
        req.has_metric;
...
mesa_rc FRR_ICLI_ospf_def_route_set(const FrrOspfCliReq &req)
{
    ...
    /* Update the new setting */
    router_conf.def_route_conf.is_always = req.has_always;
    router_conf.def_route_conf.is_specific_metric = req.has_metric;
    router_conf.def_route_conf.metric = req.metric;
    router_conf.def_route_conf.type = req.metric_type;
...
mesa_rc FRR_ICLI_ospf_intf_auth_set(const FrrOspfCliReq &req)
{
    ...
    for (u32 idx = 0; idx < req.vlan_list->cnt; ++idx) {
        for (mesa_vid_t vid = req.vlan_list->range[idx].min;
             vid <= req.vlan_list->range[idx].max; ++vid) {
            if (vtss_ifindex_from_vlan(vid, &ifindex) != VTSS_RC_OK ||
                vtss_appl_ospf_intf_conf_get(ifindex, &conf) != VTSS_RC_OK) {
                continue;
            }
...
mesa_rc FRR_ICLI_ospf_vlink_md_key_set(const FrrOspfCliReq &req)
{
    ...
    if (rc == VTSS_RC_OK) {  // already existing
        rc = FRR_RC_ENTRY_ALREADY_EXISTS;
        ICLI_PRINTF("%% %s.\n", error_txt(rc));
        return rc;
    }
...
mesa_rc FRR_ICLI_ospf_area_stub_set(const FrrOspfCliReq &req)
{
    ...
    if (rc == VTSS_RC_OK) {  // already existing
        new_entry = false;
        is_stub_type_chg = (conf.is_nssa != req.is_nssa);
    } else if (rc != FRR_RC_ENTRY_NOT_FOUND) {
        ICLI_PRINTF("%% %s.\n", error_txt(rc));
        return rc;
    } else {  // New entry
    ...
    /* Update the new values */
    conf.is_nssa = req.is_nssa;
    // no-summary
    if (is_stub_type_chg) {
        conf.no_summary = req.has_no_summary;
    } else if (req.has_no_summary) {
        conf.no_summary = req.has_no_form ? false : true;
    }
    // translator role
    if (req.has_nssa_translator_role) {
        conf.nssa_translator_role = req.nssa_translator_role;
    }
...
mesa_rc FRR_ICLI_ospf_area_stub_no(const FrrOspfCliReq &req)
{
    ...
    } else if (req.has_nssa_translator_role) {
        if (rc == VTSS_RC_OK) {
            conf.is_nssa = req.is_nssa;
            conf.nssa_translator_role = req.nssa_translator_role;
            rc = vtss_appl_ospf_stub_area_conf_set(req.inst_id, req.area_id,
...
mesa_rc FRR_ICLI_ospf_vlan_interface_set(const FrrOspfCliReq &req)
{
    ...
    for (u32 idx = 0; idx < req.vlan_list->cnt; ++idx) {
        for (mesa_vid_t vid = req.vlan_list->range[idx].min;
            ...
            /* Apply the new configuration */
            if ((rc = vtss_appl_ospf_intf_conf_set(ifindex, &conf)) != VTSS_RC_OK) {
...
mesa_rc FRR_ICLI_ospf_vlan_interface_set_default(const FrrOspfCliReq &req)
{
    ...
    for (u32 idx = 0; idx < req.vlan_list->cnt; ++idx) {
        for (mesa_vid_t vid = req.vlan_list->range[idx].min;
            ...
            if (vtss_ifindex_from_vlan(vid, &ifindex) != VTSS_RC_OK ||
                vtss_appl_ospf_intf_conf_get(ifindex, &conf) != VTSS_RC_OK ||
                frr_ospf_intf_conf_def(&ifindex, &def_conf) != VTSS_RC_OK) {
                continue;
            }
                                             ...
                                             bool *add_new_line)
{
    ...
    ICLI_PRINTF("  Transmit Delay is %u sec", entry.transmit_delay);
    ICLI_PRINTF(", State ");
    if (entry.status) {
        switch (entry.state) {
...
mesa_rc FRR_ICLI_ospf_show_interface(const FrrOspfCliReq &req)
{
    ...
    if (!req.vlan_list && !req.vlink_list) {
    ...
    for (u32 idx = 0; req.vlink_list && idx < req.vlink_list->cnt; ++idx) {
        for (u32 vlink_id = req.vlink_list->range[idx].min;
            ...
            rc = FRR_ICLI_ospf_interface_print(req.session_id, ifindex,
                                               &first_valid_entry);
            if (rc != VTSS_RC_OK) {
                // Unlike the OSPF VLAN interface, there are no such case:
                // 'OSPF not enabled on VLINK interface'.
...
mesa_rc FRR_ICLI_ospf_stub_router_set(const FrrOspfCliReq &req)
{
    ...
    // router_conf.stub_router.is_on_startup = req.has_on_startup;
    if (req.has_on_startup) {
        router_conf.stub_router.is_on_startup = true;
        router_conf.stub_router.on_startup_interval = req.on_startup_interval;
    }

    // router_conf.stub_router.is_on_shutdown = req.has_on_shutdown;
    if (req.has_on_shutdown) {
        router_conf.stub_router.is_on_shutdown = true;
        router_conf.stub_router.on_shutdown_interval = req.on_shutdown_interval;
    }

    if (req.has_administrative) {
        router_conf.stub_router.is_administrative = true;
    }
    ...
    if (rc == VTSS_APPL_FRR_OSPF_ERROR_INVALID_ROUTER_ID) {
        ICLI_PRINTF("%% The OSPF router ID is invalid.\n");
               ...
               rc != VTSS_APPL_FRR_OSPF_ERROR_ROUTER_ID_CHANGE_NOT_TAKE_EFFECT) {
        ICLI_PRINTF("%% Set OSPF stub router failed.\n");
        return ICLI_RC_ERROR;
    }
...
mesa_rc FRR_ICLI_ospf_stub_router_disable(const FrrOspfCliReq &req)
{
    ...
    // Update new configuration
    if (req.has_on_startup) {
        router_conf.stub_router.is_on_startup = false;
    }

    if (req.has_on_shutdown) {
        router_conf.stub_router.is_on_shutdown = false;
    }

    if (req.has_administrative) {
        router_conf.stub_router.is_administrative = false;
    }
...
static void FRR_ICLI_ospf_route_ipv4_table_header_print(
    const u32 session_id, const vtss_appl_ospf_route_type_t type)
{
    switch (type) {
...
*/
static void FRR_ICLI_ospf_route_ipv4_entry_print(
    const u32 session_id, const vtss_appl_ospf_route_ipv4_data_t *const entry,
    bool is_multi_nexthop)
{
    StringStream dest_str;
    StringStream nexthop_str;  // The common output for all route type.
    mesa_ipv4_t nexthop = entry->nexthop, area = entry->area;

    // format: via 1.3.12.1, VLAN 300
    nexthop_str << "via " << AsIpv4(nexthop) << ", " << entry->status.ifindex;
    switch (entry->rt_type) {
    case VTSS_APPL_OSPF_ROUTE_TYPE_INTRA_AREA:
    case VTSS_APPL_OSPF_ROUTE_TYPE_INTER_AREA:
        // format: 1.0.1.0/24, Intra, cost 1, area 0.0.0.0, Connected
        dest_str << entry->dest << ", "
                 << (entry->rt_type == VTSS_APPL_OSPF_ROUTE_TYPE_INTRA_AREA
                     ? "Intra"
                     : "Inter")
    ...
    case VTSS_APPL_OSPF_ROUTE_TYPE_EXTERNAL_TYPE_2:
        ...
        dest_str << entry->dest << ", "
                 << (entry->rt_type == VTSS_APPL_OSPF_ROUTE_TYPE_EXTERNAL_TYPE_1
                     ? "Ext1"
                     : "Ext2")
                 << ", cost " << entry->status.cost;
        if (entry->rt_type == VTSS_APPL_OSPF_ROUTE_TYPE_EXTERNAL_TYPE_2) {
    ...
    case VTSS_APPL_OSPF_ROUTE_TYPE_BORDER_ROUTER:
        ...
        {
            mesa_ipv4_t router_id = entry->dest.address;
            dest_str << AsIpv4(router_id) << " [" << entry->status.cost << "] ";
...
{
    vtss_appl_ospf_route_status_t entry;
    StringStream str_buf;
    u32 session_id = req.session_id; /* used for ICLI_PRINTF */
    vtss::Vector<vtss_appl_ospf_route_ipv4_data_t> routes;
    vtss_appl_ospf_route_ipv4_data_t prev_entry = {};
    bool is_multi_nb = false;

    /* Initail 'prev_entry' with an impossible entry */
    prev_entry.rt_type = VTSS_APPL_OSPF_ROUTE_TYPE_UNKNOWN;
    prev_entry.dest.address = 0xffffffff;
    prev_entry.dest.prefix_size = 32;
    prev_entry.id = 0;

    /* Return silently if error or no entries. */
    if (vtss_appl_ospf_route_ipv4_status_get_all(routes) != VTSS_RC_OK ||
        !routes.size()) {
        return VTSS_RC_OK;
    }
    ...
    for (const auto &itr_status : routes) {
        ...
        if (prev_entry.id != itr_status.id) {
            vtss_appl_ospf_router_status_t status;
            VTSS_RC(vtss_appl_ospf_router_status_get(itr_status.id, &status));
            str_buf.clear();
            str_buf << AsIpv4(status.ospf_router_id);
        ...
        if (prev_entry.rt_type != itr_status.rt_type &&
            ...
            prev_entry.rt_type != VTSS_APPL_OSPF_ROUTE_TYPE_EXTERNAL_TYPE_1 &&
            prev_entry.rt_type != VTSS_APPL_OSPF_ROUTE_TYPE_EXTERNAL_TYPE_2) {
            FRR_ICLI_ospf_route_ipv4_table_header_print(session_id,
                                                        itr_status.rt_type);
        }
        ...
        */
        if (itr_status.rt_type != VTSS_APPL_OSPF_ROUTE_TYPE_BORDER_ROUTER &&
            prev_entry.dest == itr_status.dest &&
            prev_entry.status.cost == itr_status.status.cost) {
            is_multi_nb = true;
        }

        FRR_ICLI_ospf_route_ipv4_entry_print(session_id, &itr_status, is_multi_nb);
...
static const char *frr_ospf_lsdb_to_type_mapping(
...
static const char *frr_ospf_link_connected_type_mapping(const int type)
{
    switch (type) {
    case 1:
...
mesa_rc FRR_ICLI_ospf_show_db_general_info(const FrrOspfCliReq &req)
{
    ...
    StringStream str_buf;
    char ip_str_buf[128];
    vtss_appl_ospf_router_status_t ospf_status;
    ...
    vtss_appl_ospf_id_t prev_inst_id = 0;
    vtss_appl_ospf_area_id_t prev_area_id = 0;
    vtss_appl_ospf_lsdb_type_t prev_lsdb_type = VTSS_APPL_OSPF_LSDB_TYPE_NONE;

    /* Get OSPF database information. */
    if (vtss_appl_ospf_router_status_get(req.inst_id, &ospf_status) != VTSS_RC_OK ||
        vtss_appl_ospf_db_get_all(db_entries) != VTSS_RC_OK || !db_entries.size()) {
        // Quit silently when the get operation failed(OSPF is diabled)
    ...
    for (const auto &itr : db_entries) {
        ...
        if (prev_inst_id != itr.inst_id) {
            ...
            ICLI_PRINTF("\n            ");
            ICLI_PRINTF("OSPF Router with ID (%s)\n",
                        misc_ipv4_txt(ospf_status.ospf_router_id, ip_str_buf));
        }
        ...
        if (req.has_self_originate && itr.adv_router_id != ospf_status.ospf_router_id) {
            // Not self originated
            continue;
        }
        ...
        if (prev_area_id != itr.area_id || prev_lsdb_type != itr.lsdb_type) {
            ...
            case VTSS_APPL_OSPF_LSDB_TYPE_ASBR_SUMMARY:
                ...
                break;
            case VTSS_APPL_OSPF_LSDB_TYPE_EXTERNAL:
                str_buf << "Type 5 AS External";
                break;
            case VTSS_APPL_OSPF_LSDB_TYPE_NSSA_EXTERNAL:
                str_buf << "Type 7 NSSA External";
                break;
            case VTSS_APPL_OSPF_LSDB_TYPE_NONE:
            default:
            ...
            if (itr.lsdb_type == VTSS_APPL_OSPF_LSDB_TYPE_EXTERNAL ||
                itr.lsdb_type == VTSS_APPL_OSPF_LSDB_TYPE_NSSA_EXTERNAL) {
                ICLI_PRINTF("\n%s Link States\n\n", str_buf.cstring());
            } else {
            ...
            ICLI_PRINTF("Link ID         ADV Router      Age         Seq#       Checksum");
            if (itr.lsdb_type == VTSS_APPL_OSPF_LSDB_TYPE_ROUTER) {
                ICLI_PRINTF("   Link count");
            }
        ...
        ICLI_PRINTF("0x%-9x", db.checksum);
        if (itr.lsdb_type == VTSS_APPL_OSPF_LSDB_TYPE_ROUTER) {
            ICLI_PRINTF("%u", db.router_link_count);
        }
...
//----------------------------------------------------------------------------
//** OSPF database detail information
//----------------------------------------------------------------------------
/* Show OSPF database detail header */
template <typename TYPE>
mesa_rc FRR_ICLI_ospf_show_db_detail_common_part_header(const FrrOspfCliReq &req,
                                                        TYPE &entry)
{
    StringStream str_buf;
    char ip_str_buf[128];
    ...
    if (entry.lsdb_type == VTSS_APPL_OSPF_LSDB_TYPE_EXTERNAL ||
        entry.lsdb_type == VTSS_APPL_OSPF_LSDB_TYPE_NSSA_EXTERNAL) {
        ICLI_PRINTF("\n%s Link States\n", str_buf.cstring());
        ICLI_PRINTF("\n");
    } else {
        ICLI_PRINTF("\n%s Link States (Area %s)\n", str_buf.cstring(),
...
/* Show OSPF database detail common information */
template <typename TYPE>
mesa_rc FRR_ICLI_ospf_show_db_detail_common_part_info(const FrrOspfCliReq &req,
                                                      TYPE &entry)
{
    u32 session_id = req.session_id; /* used for ICLI_PRINTF */
    char ip_str_buf[128];
    ...
    ICLI_PRINTF(
        "  Options: %d %s|%s|%s|%s|%s|%s|%s|%s\n", entry.data.options,
        /* bit 7(DN) is unsed. Use * to represent it */
        "*", (entry.data.options & VTSS_APPL_OSPF_OPTION_FIELD_O) ? "O" : "-",
        (entry.data.options & VTSS_APPL_OSPF_OPTION_FIELD_DC) ? "DC" : "-",
        (entry.data.options & VTSS_APPL_OSPF_OPTION_FIELD_EA) ? "EA" : "-",
        (entry.data.options & VTSS_APPL_OSPF_OPTION_FIELD_NP) ? "NP" : "-",
        (entry.data.options & VTSS_APPL_OSPF_OPTION_FIELD_MC) ? "MC" : "-",
        (entry.data.options & VTSS_APPL_OSPF_OPTION_FIELD_E) ? "E" : "-",
        (entry.data.options & VTSS_APPL_OSPF_OPTION_FIELD_MT) ? "MT" : "-");

    ICLI_PRINTF("  LS Type: %s\n",
                frr_ospf_lsdb_to_type_mapping(entry.lsdb_type));
...
template <typename TYPE>
void FRR_ICLI_ospf_show_db_detail_specific_info(u32 session_id, TYPE const &,
                                                char *);

template <>
void FRR_ICLI_ospf_show_db_detail_specific_info(
    u32 session_id, vtss_appl_ospf_db_detail_router_entry_t const &entry,
    char *ip_str_buf)
{
    int i;
    vtss_appl_ospf_db_router_link_info_t data;
    ...
    for (i = 0; i < entry.data.router_link_count; i++) {
        if (vtss_appl_ospf_db_detail_router_entry_get_by_index(
        ...
        ICLI_PRINTF("    Link connected to: %s\n",
                    frr_ospf_link_connected_type_mapping(data.link_connected_to));
        ICLI_PRINTF("     (Link ID) Net: %s\n",
                    misc_ipv4_txt(data.link_id, ip_str_buf));
        ICLI_PRINTF("     (Link Data) Network Mask: %s\n",
...
template <>
void FRR_ICLI_ospf_show_db_detail_specific_info(
    u32 session_id, vtss_appl_ospf_db_detail_network_entry_t const &entry,
    char *)
{
    // display network part information
    ICLI_PRINTF("  Network Mask: /%d\n", entry.data.network_mask);
    ICLI_PRINTF("\n");
}

template <>
void FRR_ICLI_ospf_show_db_detail_specific_info(
    u32 session_id, vtss_appl_ospf_db_detail_summary_entry_t const &entry,
    char *)
{
...
template <>
void FRR_ICLI_ospf_show_db_detail_specific_info(
    u32 session_id, vtss_appl_ospf_db_detail_external_entry_t const &entry,
    char *ip_str_buf)
{
    // display external part information
    ICLI_PRINTF("  Network Mask: /%d\n", entry.data.network_mask);
    ICLI_PRINTF("        Metric Type: %d\n", entry.data.metric_type);
    ICLI_PRINTF("        Metric: %d\n", entry.data.metric);
    ICLI_PRINTF("        Forward Address: %s\n",
                misc_ipv4_txt(entry.data.forward_address, ip_str_buf));
    ICLI_PRINTF("\n");
}
...
/* Get OSPF database details */
template <typename TYPE>
mesa_rc FRR_ICLI_ospf_show_db_detail_common_info(
    const FrrOspfCliReq &req, GetAllFunction<TYPE> ospf_db_detail_get_all)
{
    u32 session_id = req.session_id; /* used for ICLI_PRINTF */
    char ip_str_buf[128];
    ...
    if (ospf_db_detail_get_all(db_entries) != VTSS_RC_OK || !db_entries.size()) {
    ...
    for (const auto &itr : db_entries) {
        ...
        if (req.has_self_originate &&
            itr.adv_router_id != ospf_status.ospf_router_id) {
            // Not self originated
            continue;
        }
        ...
        if (prev_area_id != itr.area_id || prev_lsdb_type != itr.lsdb_type) {
            ...
            FRR_ICLI_ospf_show_db_detail_common_part_header(req, itr);
        }

        // display common part information
        FRR_ICLI_ospf_show_db_detail_common_part_info(req, itr);

        // display specific part information
        FRR_ICLI_ospf_show_db_detail_specific_info(session_id, itr, ip_str_buf);
    }
...
/* Show OSPF database detail router information */
mesa_rc FRR_ICLI_ospf_show_db_detail_router_info(const FrrOspfCliReq &req)
{
    return FRR_ICLI_ospf_show_db_detail_common_info<vtss_appl_ospf_db_detail_router_entry_t>(
               req, vtss_appl_ospf_db_detail_router_get_all);
}

/* Show OSPF database detail network information */
mesa_rc FRR_ICLI_ospf_show_db_detail_network_info(const FrrOspfCliReq &req)
{
    return FRR_ICLI_ospf_show_db_detail_common_info <
           vtss_appl_ospf_db_detail_network_entry_t > (
               req, vtss_appl_ospf_db_detail_network_get_all);
}

/* Show OSPF database detail summary information */
mesa_rc FRR_ICLI_ospf_show_db_detail_summary_info(const FrrOspfCliReq &req)
{
    return FRR_ICLI_ospf_show_db_detail_common_info <
           vtss_appl_ospf_db_detail_summary_entry_t > (
               req, vtss_appl_ospf_db_detail_summary_get_all);
}

/* Show OSPF database detail asbr summary information */
mesa_rc FRR_ICLI_ospf_show_db_detail_asbr_summary_info(const FrrOspfCliReq &req)
{
    return FRR_ICLI_ospf_show_db_detail_common_info <
           vtss_appl_ospf_db_detail_summary_entry_t > (
               req, vtss_appl_ospf_db_detail_asbr_summary_get_all);
}

/* Show OSPF database detail external information */
mesa_rc FRR_ICLI_ospf_show_db_detail_external_info(const FrrOspfCliReq &req)
{
    return FRR_ICLI_ospf_show_db_detail_common_info <
           vtss_appl_ospf_db_detail_external_entry_t > (
               req, vtss_appl_ospf_db_detail_external_get_all);
}

/* Show OSPF database detail nssa external information */
mesa_rc FRR_ICLI_ospf_show_db_detail_nssa_external_info(const FrrOspfCliReq &req)
{
    return FRR_ICLI_ospf_show_db_detail_common_info <
           vtss_appl_ospf_db_detail_external_entry_t > (
               req, vtss_appl_ospf_db_detail_nssa_external_get_all);
}

/* Show OSPF database detail information */
mesa_rc FRR_ICLI_ospf_show_db_detail_info(const FrrOspfCliReq &req)
{
    // Get all entries from OSPF detail database
    switch (req.lsdb_type) {
    case VTSS_APPL_OSPF_LSDB_TYPE_ROUTER:
        FRR_ICLI_ospf_show_db_detail_router_info(req);
        return ICLI_RC_OK;
    case VTSS_APPL_OSPF_LSDB_TYPE_NETWORK:
        FRR_ICLI_ospf_show_db_detail_network_info(req);
        return ICLI_RC_OK;
    case VTSS_APPL_OSPF_LSDB_TYPE_SUMMARY:
        FRR_ICLI_ospf_show_db_detail_summary_info(req);
        return ICLI_RC_OK;
    case VTSS_APPL_OSPF_LSDB_TYPE_ASBR_SUMMARY:
        FRR_ICLI_ospf_show_db_detail_asbr_summary_info(req);
        return ICLI_RC_OK;
    case VTSS_APPL_OSPF_LSDB_TYPE_EXTERNAL:
        FRR_ICLI_ospf_show_db_detail_external_info(req);
        return ICLI_RC_OK;
    case VTSS_APPL_OSPF_LSDB_TYPE_NSSA_EXTERNAL:
        FRR_ICLI_ospf_show_db_detail_nssa_external_info(req);
        return ICLI_RC_OK;
    case VTSS_APPL_OSPF_LSDB_TYPE_NONE:
    default:
