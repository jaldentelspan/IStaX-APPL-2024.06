...
#define CLI_CRIT_ENTER() critd_enter(&cli.crit, __FILE__, __LINE__)
#define CLI_CRIT_EXIT()  critd_exit( &cli.crit, __FILE__, __LINE__)
...
static void cli_banner_motd(cli_iolayer_t *pIO)
{
    ...
    if (icli_session_data_get(&data) == VTSS_RC_OK) {
        if (data.b_motd_banner) {
            char banner[ICLI_BANNER_MAX_LEN + 1];
            if (icli_banner_motd_get(banner) == VTSS_RC_OK) {
                if (banner[0]) {
                    cli_putchar('\n');
                    cli_puts(banner);
                    cli_putchar('\n');
...
static void cli_pre_login(cli_iolayer_t *pIO)
{
    ...
    while (pIO->cli_getch(pIO, CLI_NO_CHAR_TIMEOUT, &ch) == VTSS_RC_OK) {
        if (ch == CR) {
            cli_putchar('\n');
            break;
        }
...
static void cli_banner_login(cli_iolayer_t *pIO, vtss_appl_auth_agent_t agent)
{
    ...
    if (icli_banner_login_get(banner) == VTSS_RC_OK) {
        if (banner[0]) {
            cli_putchar('\n');
            cli_puts(banner);
            cli_putchar('\n');
...
void cli_thread(vtss_addrword_t data)
{
    ...
    if (pIO->session_way == CLI_WAY_CONSOLE) {
        ...
        // ICFG_LOADED_POST event, where all config is supposed to be applied to
        // H/W.
        msg_wait(MSG_WAIT_UNTIL_ICFG_LOADED_POST, VTSS_MODULE_ID_CLI);
    }
...
{
    char c;
    while (pIO->cli_getch(pIO, 0, &c) == VTSS_RC_OK) {
        if ((ch == 0) || (ch == c)) {
            return c;
        }
    }
...
BOOL cli_io_login(cli_iolayer_t *pIO, vtss_appl_auth_agent_t agent, int timeout)
{
    ...
    while (auth_cnt--) {
        ...
        while (!pIO->bIOerr && ct < VTSS_SYS_USERNAME_LEN) {
            if (pIO->cli_getch(pIO, timeout, &ch) != VTSS_RC_OK) {
                return FALSE;
            }
        ...
        if (vtss_auth_login(agent, misc_ip_txt(&pIO->client_ip, hostname), username, passwd, &priv_lvl, &agent_id) == VTSS_RC_OK) {
            CLI_CRIT_ENTER();
            pIO->authenticated = TRUE;
...
mesa_rc cli_io_init(vtss_init_data_t *data)
{
    ...
    case INIT_CMD_INIT:
        ...
        critd_init(&cli.crit, "cli_io", VTSS_MODULE_ID_CLI, CRITD_TYPE_MUTEX);
