...
# define PEXIT(m)                         \
    do {                                  \
        ...
        T_EG(VTSS_TRACE_GRP_TELNET, "%s : %s", m, strerror(errno));        \
        return;                           \
    } while(0)
#else
# define PEXIT(m)                         \
...
static void cli_io_telnet_init(cli_iolayer_t *pIO)
{
    ...
    if (!pTIO->trace_registered) {
        if (vtss_trace_io_register(pTIO->trace_layer, VTSS_MODULE_ID_CLI, &pTIO->trace_reg) == VTSS_RC_OK) {
...
static void cli_telnet(vtss_addrword_t data)
{
    ...
    msg_wait(MSG_WAIT_UNTIL_ICFG_LOADING_POST, VTSS_MODULE_ID_CLI);
        ...
        while (is_running) {
            ...
            if ((rc > 0) && (FD_ISSET(s, &rfds))) {
                ...
                if (telnet_security_mode) {
                    const char *buf = "This device's SSH mode is enabled, please use the SSH instead of Telnet.\r\n";
...
void cli_telnet_start(void)
{
    ...
    control_system_reset_register(cli_telnet_do_close, VTSS_MODULE_ID_CLI);
