...
#define __UL_E(_line_, ...) \
     fprintf(stderr, "(%s, %d) %s.icli:%d: error: ", __FUNCTION__, __LINE__, g_script_file, _line_); \
     fprintf(stderr, __VA_ARGS__);

#define __U_E(...) \
     fprintf(stderr, "(%s, %d) %s.icli:%d: error: ", __FUNCTION__, __LINE__, g_script_file, g_current_cmd ? g_current_cmd->command->line : 0); \
     fprintf(stderr, __VA_ARGS__);
...
*/
#define _COMPLETE_HTML              1

#define _TAG_MAX_LEN                32
#define _VALUE_MAX_LEN              ICLI_STR_MAX_LEN
#define _CMD_MAX_CNT                (ICLI_CMD_MODE_MAX * ICLI_CMD_MODE_MAX)  // for each ICLI script, not for total. The reason for this is that .../icli/platform/script/icli_config.icli generates the same commands for each and every submode :-(
...
#if ICLI_LOOP_SYNTAX
static char                 *g_mode_loop_number[_CMD_MAX_CNT][_WORD_MAX_CNT];
#endif
...
#if ICLI_LOOP_SYNTAX
static char                 *g_variable_loop_number[_CMD_MAX_CNT][_WORD_MAX_CNT];
#endif
...
#if ICLI_LOOP_SYNTAX
#define _LOOP_END_CHECK_(c) \
    (ICLI_NOT_(SPACE,*(c)) && ICLI_NOT_(EOS,*(c)) && ICLI_NOT_(MANDATORY_END,*(c)) && ICLI_NOT_(OPTIONAL_END,*(c)))

#define _FOR_LOOP_CONSTANT_(c) \
    for (++(c); _LOOP_END_CHECK_(c); ++(c))
#endif
...
)
{
    ...
    for ( c = cmd, not_over = TRUE; not_over; ++c ) {
        ...
        if ( ICLI_IS_(EOS, *c) ) {
            __U_E("Current char = EOS\n");
        } else {
            __U_E("Current %u-th char = %c(%u)\n", (u32)(c - cmd + 1), *c, (u32)*c);
        }
        __U_E("invalid syntax command = %s\n", cmd);
        return ICLI_RC_ERROR;
    }
    ...
    while ( g_current_cmd != NULL ) {
        ...
        for ( t = g_current_cmd->cmd_mode->next; t != NULL; ___NEXT(t), ++n ) {
            ...
            *new_command = *cmd;
            ...
            new_command->mode_info = icli_platform_cmd_mode_info_get_by_str( new_command->cmd_mode->value_str );
...
static i32 _sub_mode_entry_add( void )
{
    ...
    for ( sub_mode = ICLI_CMD_MODE_CONFIG_VLAN; sub_mode < ICLI_CMD_MODE_MAX; ++sub_mode ) {
        ...
        *new_command = *g_current_cmd;
        ...
        mode_info = icli_platform_cmd_mode_info_get_by_mode( sub_mode );
        if ( mode_info == NULL ) {
...
#if ICLI_LOOP_SYNTAX
// check loop syntax
// must contain loop number
// infinite loop is allowed for <port_type_list> only
static BOOL _loop_syntax_check(
    IN  char    *cmd
)
{
    char    *c;
    ...
    for (c = cmd; ICLI_NOT_(EOS, *c); ++c) {
        if ( ICLI_IS_(LOOP_END, *c) ) {
            ++c;
            if ( ICLI_NOT_(LOOP_DELIMITER, *c) ) {
                return FALSE;
            }

            ++c;
            if ( ! _LOOP_END_CHECK_(c) ) {
                return FALSE;
            }
        }
    }
            ...
            case _SEGMENT_BEGIN:
                ...
                case _TAG_CMD_END:
                    ...
                    if ___CMD_NO_VALUE_STR( func_name ) {
                        ...
                        // make sure the function name can not include '-' when minus value
                        u = (icli_os_current_time_get() + rand()) & 0x7FFFffff;
                    ...
                    // must contain loop number
                    // infinite loop is allowed for <port_type_list> only
                    if (_loop_syntax_check(___VALUE_STR(command)) == FALSE) {
                        T_E("loop syntax\n");
                        return ICLI_RC_ERROR;
                    }
#endif
                    ...
                    /* get command mode information */
                    g_current_cmd->mode_info = icli_platform_cmd_mode_info_get_by_str( g_current_cmd->cmd_mode->value_str );
...
#if ICLI_RANDOM_MUST_NUMBER
    "        NULL,  // random_must_head\n"
    "        0,     // random_must_number\n"
    "        0,     // random_must_level\n"
    "        FALSE, // random_must_begin\n"
    "        FALSE, // random_must_middle\n"
    "        FALSE, // random_must_end\n"
#endif
#endif
    "        0,     // or_head\n"
    "    }%s";
...
static char loop_cmd_var_begin_string[] =
    "    // COMMAND\n"
    "    // %s\n"
    "    loop_head = NULL;\n"
    "    while ( cmd_var ) {\n"
    "        if ( cmd_var->match_node->loop_begin ) {\n"
    "            if ( loop_head && icli_is_sibling(loop_head, cmd_var->match_node) ) {\n"
    "                ++loop_index;\n"
    "            } else {\n"
    "                loop_head = cmd_var->match_node;\n"
    "                loop_index = 0;\n"
    "            }\n"
    "        }\n"
    "        switch ( cmd_var->word_id ) {\n";
...
#define ___CMD_ID_DB_GEN()\
    fprintf(g_fd, "/* number of commands */\n");\
...
#if ICLI_LOOP_SYNTAX
                if ( ICLI_IS_(LOOP_DELIMITER, *(prev_c+1)) ) {
                    loop_number[j] = (char *)icli_malloc(256);
                    if ( loop_number[j] == NULL ) {
                        T_E("fail to allocate memory for loop_number[%d]\n", j);
                        return ICLI_RC_ERR_MEMORY;
                    }
                    memset(loop_number[j], 0, 256);

                    for (prev_c += 2, k = 0; _LOOP_END_CHECK_(prev_c); ++prev_c, ++k) {
                        loop_number[j][k] = *prev_c;
                    }
                    c = prev_c - 1;

                    for (k = loop_begin; k < j; ++k) {
                        loop_number[k] = loop_number[j];
                    }
                }
#endif
                __ASSIGN_WORD_TYPE();
...
#define ___GET_VARIABLE(x,y) \
    case x:\
        if ( g_variable_loop_number[k][i] ) {\
            fprintf(g_fd, "%s[loop_index] = cmd_var->value.u."#y";\n", t->value_str);\
        } else {\
...
#define ___GET_ALL(_what_) \
    ...
    ___GET_##_what_( ICLI_VARIABLE_HOST_NAME,              u_host_name         ); \
    ___GET_##_what_( ICLI_VARIABLE_RANGE_HOST_NAME,        u_range_host_name   ); \
    ...
    ___GET_##_what_##_PTR( ICLI_VARIABLE_VLINK_LIST,       u_vlink_list        ); \
    ...
    ___GET_##_what_( ICLI_VARIABLE_DOMAIN_NAME,            u_domain_name       ); \
    ___GET_##_what_( ICLI_VARIABLE_RANGE_DOMAIN_NAME,      u_range_domain_name ); \
    ___GET_##_what_( ICLI_VARIABLE_URL,                    u_url               ); \
    ___GET_##_what_( ICLI_VARIABLE_RANGE_URL,              u_range_url         ); \
    ___GET_##_what_( ICLI_VARIABLE_FILE,                   u_file              ); \
    ___GET_##_what_( ICLI_VARIABLE_RANGE_FILE,             u_range_file        ); \
    ___GET_##_what_( ICLI_VARIABLE_URL_FILE,               u_url_file          ); \
    ___GET_##_what_( ICLI_VARIABLE_RANGE_URL_FILE,         u_range_url_file    ); \
    ___GET_##_what_##_PTR( ICLI_VARIABLE_NET_ADDR,         u_net_addr          ); \
    ___GET_##_what_( ICLI_VARIABLE_AREA_ID,                u_area_id           );
...
)
{
    ...
    if ( vtss_icli_str_to_int(s, &val) == ICLI_RC_OK ) {
        ...
        // constant should be enclosed by ' '
        if ( ICLI_NOT_(SQUOTE, *s) || ICLI_NOT_(SQUOTE, *(s + len - 1))) {
            __U_E("The constant %s is not enclosed by single quote\n", s);
            return FALSE;
        }
        // remove ' '
        *(s + len - 1) = 0;
...
static i32 _generate_C_and_H( void )
{
    ...
    if ( g_generate_path ) {
        ...
        icli_sprintf(g_icli_c_file, "%s/%s_icli.cxx", g_generate_path, g_script_file);
        ...
        icli_sprintf(g_icli_c_file, "%s_icli.cxx", g_script_file);
    ...
    */
    ...
    // Disabling warnings for complete blocks of generated code
    fprintf(g_fd, "#pragma GCC diagnostic push\n");
    fprintf(g_fd, "#pragma GCC diagnostic ignored \"-Wwrite-strings\"\n");
    ...
    for ( g_current_cmd = g_command, k = 0; g_current_cmd != NULL; ___NEXT(g_current_cmd), ++k ) {
        ...
        priv = icli_platform_privilege_get_by_str( ___VALUE_STR(privilege));
        ...
        for ( i = 0; i < n; ++i ) {
            ...
            if ( vtss_icli_variable_data_has_range_get(g_variable_type[k][i]) ) {
                b_range = TRUE;
                sprintf(range_str, "&%s_range_%d", func_name, i);
            }
            fprintf(g_fd, cmd_node_string,
                    i,
                    vtss_icli_variable_data_name_get(g_variable_type[k][i]),
                    b_range ? range_str : "NULL",
                    i,
                    func_name,
#if ICLI_LOOP_SYNTAX
                    g_variable_loop_number[k][i] ? g_variable_loop_number[k][i] : "0",
#endif
                    i == (n - 1) ? "\n" : ",\n");
        }
    ...
    // END - Disabling warnings for complete blocks of generated code
    fprintf(g_fd, "#pragma GCC diagnostic pop\n");
    ...
    */
    for ( g_current_cmd = g_command, k = 0; g_current_cmd != NULL; ___NEXT(g_current_cmd), ++k ) {
        ...
        // header
        if (strcmp(___VALUE_STR(func_name) + strlen(___VALUE_STR(func_name)) - 3, _NO_FORM_POSTFIX) == 0) {
            is_no_form = TRUE;

            // The no-form of the commands need the following in order to avoid
            // "-Wunused-but-set-variable, because often the no-form doesn't use
            // all the CMD_VAR variables, and they are indeed initialized by the
            // auto-generator.
            fprintf(g_fd, "#pragma GCC diagnostic push\n");
            fprintf(g_fd, "#pragma GCC diagnostic ignored \"-Wunused-but-set-variable\"\n");
        } else {
            is_no_form = FALSE;
        }
        ...
        // get count of command words
        cmd_word_cnt = g_current_cmd->command_word_cnt;

#if ICLI_LOOP_SYNTAX
        b_loop = FALSE;
        for ( i = 0; i < cmd_word_cnt; ++i ) {
            if ( g_variable_loop_number[k][i] ) {
                b_loop = TRUE;
                break;
            }
...
#if ICLI_LOOP_SYNTAX
        if ( b_loop ) {
            fprintf(g_fd, "    // for loop\n");
            fprintf(g_fd, "    i32                     loop_index = 0;\n");
            fprintf(g_fd, "    icli_parsing_node_t     *loop_head = NULL;\n");
            fprintf(g_fd, "\n");
        }
#endif
...
#if ICLI_LOOP_SYNTAX
        if ( b_loop ) {
            fprintf(g_fd, "    if ( loop_index ){}\n");
            fprintf(g_fd, "    if ( loop_head ){}\n");
        }
#endif
        fprintf(g_fd, "\n");
        ...
        if ( b_loop ) {
            for ( i = 0, t = g_current_cmd->cmd_var; i < cmd_word_cnt && t != NULL; ++i, ___NEXT(t) ) {
                if ( vtss_icli_str_len(t->value_str) > 0 && g_variable_loop_number[k][i] ) {
                    fprintf(g_fd, "    memset(%s, 0, sizeof(%s));\n", t->value_str, t->value_str);
                }
            }
            fprintf(g_fd, "\n");
        }
...
#if ICLI_LOOP_SYNTAX
            if ( b_loop ) {
                fprintf(g_fd, loop_cmd_var_begin_string, ___VALUE_STR(command));
            } else {
...
#if ICLI_LOOP_SYNTAX
                            if ( g_variable_loop_number[k][i] ) {
                                fprintf(g_fd, "%s[loop_index] = TRUE;\n", t->value_str);
                            } else {
                                fprintf(g_fd, "%s = TRUE;\n", t->value_str);
                            }
#else
        ...
        if (is_no_form) {
            // Pop the GCC options again, now that we're done with the no-form
            fprintf(g_fd, "#pragma GCC diagnostic pop\n");
        }
    ...
    */
    ...
    fprintf(g_fd, "#ifdef __cplusplus\n");
    fprintf(g_fd, "extern \"C\" {\n");
    fprintf(g_fd, "#endif\n");
    fprintf(g_fd, "\n");
...
/*
    ...
    if the item does not exist, then that line will be empty.
...
)
{
    ...
    for ( s = cmd, d = g_line_buf; ICLI_NOT_(EOS, *s); ++s, ++d ) {
        ...
        case ICLI_LOOP_END:
            d--;
#if ICLI_LOOP_SYNTAX
            /* TODO: actually, html not generated well now */
#endif
            break;
#endif
...
)
{
    ...
    priv = icli_platform_privilege_get_by_str( cmd->privilege->value_str );
    switch ( priv ) {
...
static BOOL _html_cmd_ref_add( void )
{
    ...
    for (g_current_cmd = g_command; g_current_cmd != NULL; ___NEXT(g_current_cmd)) {
        ...
        if ( g_current_cmd->privilege ) {
            priv = icli_platform_privilege_get_by_str( g_current_cmd->privilege->value_str );
#if 0 /* Bugzilla#14129 - remove debug level */
...
)
{
    ...
    srand( icli_os_current_time_get() + u + vtss_icli_str_len(script_file) );
...
#if ICLI_LOOP_SYNTAX
    memset( g_variable_loop_number,      0, sizeof(g_variable_loop_number) );
#endif

    // generate .c and .h file
    r = _generate_C_and_H();
    if (r != ICLI_RC_OK) {
        __U_E("generate %s_icli.cxx and %s_icli.h file\n", g_script_file, g_script_file);
        return ICLI_RC_ERROR;
    }
