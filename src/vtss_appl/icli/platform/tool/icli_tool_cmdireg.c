...
static BOOL _cmd_reg_add(void)
{
    ...
    */
    ...
    for ( i = 0; i < line; ++i ) {
        if ( include_line == -1 ) {
            if ( b_include ) {
                if ( g_line_buf[i][0] != '#' ) {
                    include_line = i;
                } else if ( vtss_icli_str_str(g_reg_include, g_line_buf[i]) != NULL ) {
                    reg_include_line = i;
                }
            } else {
                if ( g_line_buf[i][0] == '#' ) {
                    b_include = TRUE;
                    if ( vtss_icli_str_str(g_reg_include, g_line_buf[i]) != NULL ) {
                        reg_include_line = i;
                    }
                }
            }
        ...
        if ( b_start ) {
            ...
            if ( vtss_icli_str_sub("NULL", g_line_buf[i], TRUE, NULL) >= 0 ) {
                continue;
            }

            /* check end */
            if ( vtss_icli_str_cmp(g_line_buf[i], _CMD_REG_END) == 0 ) {
            ...
            if ( g_line_buf[i][j] == '&' ) {
                ++j;
                if ( vtss_icli_str_sub(g_reg_api, &(g_line_buf[i][j]), TRUE, NULL) >= 0 ) {
            ...
            if ( vtss_icli_str_cmp(g_line_buf[i], _CMD_REG_BEGIN) == 0 ) {
                b_start = TRUE;
            }
        }
    }
    ...
    if ( b_add ) {
        ...
        b_if = FALSE;
        if ( vtss_icli_str_sub("#if ", g_line_buf[reg_include_line - 1], TRUE, NULL) >= 0 ) {
            b_if = TRUE;
        }
        for ( i = 0; i < line; ++i ) {
            if ( b_if ) {
