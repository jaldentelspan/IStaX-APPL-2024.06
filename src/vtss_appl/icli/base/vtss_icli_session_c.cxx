...
)
{
    ...
    // go to the begin
    icli_platform_cursor_offset(handle, 0 - cursor_pos, 0);
...
)
{
    ...
    icli_platform_cursor_offset(handle, x - nx, y - ny);
...
)
{
    ...
    if ( _CURSOR_AT_LINE_END ) {
        ...
        // go forward
        icli_platform_cursor_forward( handle );
...
)
{
    ...
    if ( handle->runtime_data.left_scroll ) {
        ...
        /* go to the end */
        icli_platform_cursor_offset(handle, 0 - handle->runtime_data.cursor_pos, 0);
...
)
{
    ...
    if ( handle->runtime_data.right_scroll ) {
        ...
        /* go to the end */
        icli_platform_cursor_offset(handle, offset, 0);
...
)
{
    ...
    if ( vtss_icli_sutil_usr_char_get(handle, _BUF_WAIT_TIME, &c) ) {
        ...
        T_I("b_end:%d, b_buf:%d", b_end, b_buf);
        ...
        if ( b_end ) {
            ...
            T_D("handle->runtime_data.width:%d, handle->runtime_data.prompt_len:%d, _CMD_LINE_MAX_LEN:%d",
                handle->runtime_data.width, handle->runtime_data.prompt_len, _CMD_LINE_MAX_LEN);
...
/* Coverity reports the function to include non-reachable code (last
   statement after infinite loop. Removing this causes compiler to
   warn "reached end of scope without return statement of non-void func".
   Solution is to remove the statement (coverity) and suppress this single
   warning by the following pragma
   ThomasG 2023-02-17 */
...
{
    i32  c = 0, loop;
    BOOL in_multiline_mode, is_escaped;
    ...
    /* start loop */
    loop = 1;
    in_multiline_mode = handle->runtime_data.mode_para[handle->runtime_data.mode_level].mode == ICLI_CMD_MODE_MULTILINE;
    while ( loop ) {
        if ( loop == 1 ) {
            ++loop;
        } else {
        ...
        */
        ...
        // Make sure the user can escape a question mark
        is_escaped = handle->runtime_data.prev_char == ICLI_BACK_SLASH;
        ...
        case ICLI_KEY_DEL:
            // Default settings:
            //           |     serial       |       ip
            //           | putty | Teraterm | telnet | ssh
            //-----------+-------------------------------------------------------------------
            // Backspace |  ^?   |   ^H     |   ^?   |  ^?
            // Del       |  ^?   |   ^?     | ESC... | ESC...

            // In putty, it is possible to change configuration to make backspace send ^H, so
            // best compromise seems to be that:
            //   - On serial line, use ^H as backspace and ^? as delete.
            //   - On ip connections, use both ^H and ^? as backspace, and ESC sequence as delete

            if (handle->session_id == 0) {
                // This is the console. Console programs seems to use ASCII 8 for backspace and either
                // ascii 127 or escape sequence ICLI_HYPER_TERMINAL ICLI_HT_BEGIN ICLI_VKEY_DEL ICLI_HT_END
                // for delete key
                _fkey_del_char( handle );
                continue;
            } else {
                // This is a virtual terminal, i.e. either telnet or ssh. Both telnet and ssh use
                // ascii 8 and ascii 127 for backspace key. Delete key always seems to be the
                // escape sequence
    ...
    // for lint
    // remove due to coverity (see above)
    // return ICLI_RC_OK;
