...
#if 1 /* CP, Bugzilla#14843 - ambiguous between <uint> and <port_type> */
    IN char                     *w,
    IN icli_variable_value_t    *value,
#endif
    IN icli_rc_t                rc
)
{
    ...
    case ICLI_RC_OK:
        if ( vtss_icli_variable_data_string_type_get(type) ) {
            ...
            case ICLI_VARIABLE_PORT_TYPE:
                if ( vtss_icli_str_sub(w, vtss_icli_variable_port_type_get_name(value->u.u_port_type), FALSE, NULL) != 0 ) {
                    return ICLI_MATCH_TYPE_PARTIAL;
                }
...
static BOOL _node_prop_present(
    IN  icli_session_handle_t   *handle,
    IN  icli_node_property_t    *node_prop
)
{
...
{
    icli_node_property_t    *node_prop;

#if 1 /* Bugzilla#14129 - remove debug level */
    /*
        setting command privilege, not need to check enable
    */
    if ( handle->runtime_data.b_priv_cmd ) {
        return TRUE;
    }
#endif
...
/*
    check if the node is in the current traversed tree
    ...
    COMMENT
        n/a
*/
static BOOL _cmd_id_chk(
    IN  icli_session_handle_t   *handle,
    IN  icli_parsing_node_t     *node
)
{
    icli_parameter_t        *p;
    icli_node_property_t    *p_prop;
    icli_node_property_t    *node_prop;
    ...
    for ( ; p->next; ___NEXT(p) );

    // check if same cmd_id
    for ( p_prop = &(p->match_node->node_property); p_prop; ___NEXT(p_prop) ) {
        for ( node_prop = &(node->node_property); node_prop; ___NEXT(node_prop) ) {
            if ( p_prop->cmd_property->cmd_id == node_prop->cmd_property->cmd_id ) {
                return TRUE;
            }
        }
    }
...
)
{
    ...
    icli_node_property_t    *first_parent_prop;
...
)
{
    ...
    */
    ...
    /*
        if debug command is denied,
        then disable the first node that is 'debug'
    */
    if ( vtss_icli_session_debug_cmd_allow_get(handle) == FALSE ) {
        if ( node->parent_cnt == 0 && node->type == ICLI_VARIABLE_KEYWORD ) {
            if ( vtss_icli_str_sub(node->word, "debug", 0, NULL) == 0 ) {
                return FALSE;
            }
        }
    }
...
static const char *_port_list_str_get(
...
)
{
    ...
    // if protocol is incomplete, then do not check
    ...
    p = vtss_icli_variable_url_protocol_get(w);
    if ( p == ICLI_URL_PROTOCOL_MAX ) {
        return FALSE;
    }

    return ( ! runtime->url_not_support[p] );
}

#if 1 /* CP, 2012/10/24 14:08, Bugzilla#10092 - runtime variable word length */
#define CASE_VARIABLE_LENGTH_RANGE(x) \
    case ICLI_VARIABLE_##x: \
        if ( range->type == ICLI_RANGE_TYPE_UNSIGNED ) { \
            type = ICLI_VARIABLE_RANGE_##x; \
        } else { \
...
)
{
    ...
    /* check if in the same tree */
    if ( _cmd_id_chk(handle, node) == FALSE ) {
        return ICLI_MATCH_TYPE_ERR;
    }
    ...
    default :
        ...
        if ( vtss_icli_str_len(w) == 0 ) {
            ...
            case ICLI_VARIABLE_NET_ADDR:
                if (runtime_cb) {
                    b = vtss_icli_exec_runtime(handle, runtime_cb, ICLI_ASK_NET_ADDR, &(handle->runtime_data.runtime));
                    if (b) {
                        n = 0;
                        range = &(handle->runtime_data.runtime.range);
                        for (i = 0; i < ICLI_NET_ADDR_TYPE_MAX; ++i) {
                            if (handle->runtime_data.runtime.net_addr_not_support[i] == FALSE) {
                                range->u.ur.range[n].min = i;
                                range->u.ur.range[n].max = i;
                                ++n;
                            }
                        }
                        range->type = ICLI_RANGE_TYPE_UNSIGNED;
                    ...
                    range->type = ICLI_RANGE_TYPE_UNSIGNED;
                    range->u.ur.cnt = 1;
                    range->u.ur.range[0].min = ICLI_NET_ADDR_TYPE_IPV4;
                    range->u.ur.range[0].max = ICLI_NET_ADDR_TYPE_DOMAIN_NAME;
                }
                break;
...
#if 1 /* CP, 2012/10/24 14:08, Bugzilla#10092 - runtime variable word length */
                        CASE_VARIABLE_LENGTH_RANGE( WORD ); // ICLI_VARIABLE_RANGE_WORD
                        CASE_VARIABLE_LENGTH_RANGE( KWORD ); // ICLI_VARIABLE_RANGE_KWORD
                        CASE_VARIABLE_LENGTH_RANGE( DWORD ); // ICLI_VARIABLE_RANGE_DWORD
                        CASE_VARIABLE_LENGTH_RANGE( FWORD ); // ICLI_VARIABLE_RANGE_FWORD
                        CASE_VARIABLE_LENGTH_RANGE( STRING ); // ICLI_VARIABLE_RANGE_STRING
                        CASE_VARIABLE_LENGTH_RANGE( LINE ); // ICLI_VARIABLE_RANGE_LINE
                        CASE_VARIABLE_LENGTH_RANGE( HEXVAL ); // ICLI_VARIABLE_RANGE_HEXVAL
                        CASE_VARIABLE_LENGTH_RANGE( VWORD ); // ICLI_VARIABLE_RANGE_VWORD
                        CASE_VARIABLE_LENGTH_RANGE( HOST_NAME ); // ICLI_VARIABLE_RANGE_HOST_NAME
                        CASE_VARIABLE_LENGTH_RANGE( DOMAIN_NAME ); // ICLI_VARIABLE_RANGE_DOMAIN_NAME
                        CASE_VARIABLE_LENGTH_RANGE( URL ); // ICLI_VARIABLE_RANGE_URL
                        CASE_VARIABLE_LENGTH_RANGE( FILE ); // ICLI_VARIABLE_RANGE_FILE
                        CASE_VARIABLE_LENGTH_RANGE( URL_FILE ); // ICLI_VARIABLE_RANGE_URL_FILE
#endif
            ...
            case ICLI_VARIABLE_VLINK_LIST:
                _unsigned_range_sort( &(p->value.u.u_vlink_list) );
                break;
            ...
            if ( handle->runtime_data.last_port_type != ICLI_PORT_TYPE_NONE ) {
                port_type = (icli_port_type_t)(handle->runtime_data.last_port_type);
                ...
                case ICLI_VARIABLE_PORT_TYPE:
                    ...
                    case ICLI_VARIABLE_PORT_TYPE_ID:
                        ...
                        if ( vtss_icli_port_get(&spr) ) {
                            ...
                            (void)vtss_icli_session_error_printf( handle, "%% No such interface: %s %u/%u\n",
            ...
            case ICLI_RC_ERR_INCOMPLETE:
                ...
                case ICLI_VARIABLE_URL:
                case ICLI_VARIABLE_RANGE_URL:
                case ICLI_VARIABLE_URL_FILE:
                case ICLI_VARIABLE_RANGE_URL_FILE:
                    /* get runtime url */
                    b = FALSE;
                    if (runtime_cb) {
                        b = vtss_icli_exec_runtime(handle, runtime_cb, ICLI_ASK_URL, &(handle->runtime_data.runtime));
                        if (b && _url_runtime_chk(&(handle->runtime_data.runtime), w) == FALSE) {
                            rc = ICLI_RC_ERR_MATCH;
                            (void)vtss_icli_session_error_printf(handle, "%% Protocol not supported.\n");
                        }
                    }
                    break;
                ...
                case ICLI_VARIABLE_NET_ADDR:
                    ...
                    if (runtime_cb) {
                        b = vtss_icli_exec_runtime(handle, runtime_cb, ICLI_ASK_NET_ADDR, &(handle->runtime_data.runtime));
                        if (b && handle->runtime_data.runtime.net_addr_not_support[p->value.u.u_net_addr.type]) {
                            rc = ICLI_RC_ERR_MATCH;
                            switch (p->value.u.u_net_addr.type) {
                            case ICLI_NET_ADDR_TYPE_IPV4:
                                (void)vtss_icli_session_error_printf(handle, "%% IPv4 not supported.\n");
                                break;

                            case ICLI_NET_ADDR_TYPE_IPV6:
                                (void)vtss_icli_session_error_printf(handle, "%% IPv6 not supported.\n");
                                break;

                            case ICLI_NET_ADDR_TYPE_DOMAIN_NAME:
                                (void)vtss_icli_session_error_printf(handle, "%% Domain name not supported.\n");
                                break;
            ...
            /* get match_type */
#if 1 /* CP, Bugzilla#14843 - ambiguous between <uint> and <port_type> */
            match_type = _match_type_get(type, w, &(p->value), (icli_rc_t)rc);
#else
            match_type = _match_type_get(type, (icli_rc_t)rc);
#endif
        }
        break;
    }
    ...
    case ICLI_MATCH_TYPE_INCOMPLETE :
        if ( *para == NULL ) {
            if ( node->type == ICLI_VARIABLE_CWORD && match_type != ICLI_MATCH_TYPE_ERR) {
                p->word_id    = node->word_id;
                p->match_node = node;
...
)
{
    ...
    for ( d = 0; d < ICLI_RANDOM_OPTIONAL_DEPTH; ++d ) {
        ...
        */
        ...
        if ( node->type == ICLI_VARIABLE_PORT_TYPE && ___LOOP_BEGIN(node) ) {
            if ( handle->runtime_data.last_port_type == ICLI_PORT_TYPE_NONE ) {
                if ( handle->runtime_data.cmd_var ) {
...
#if ICLI_LOOP_SYNTAX
static BOOL _in_port_type_list_loop(
    IN icli_parameter_t        *last,
    IN icli_parsing_node_t     *node
)
{
    ...
    if ( last->match_node->type == ICLI_VARIABLE_PORT_TYPE &&
         last->match_node->loop_begin == 1 &&
         node->type == ICLI_VARIABLE_PORT_TYPE_LIST &&
         node->loop_head == last->match_node) {
        return TRUE;
    }

    if ( last->match_node->type == ICLI_VARIABLE_PORT_TYPE_LIST &&
         last->match_node->loop_number == 0 &&
         last->match_node == node ) {
        return TRUE;
    }
...
static BOOL _loop_number_allowed(
    IN  icli_session_handle_t   *handle,
    IN  icli_parsing_node_t     *loop_end
)
{
    icli_parsing_node_t     *loop_head;
    u32                     loop_count;
    icli_parameter_t        *p;

    if (loop_end->loop_head == NULL) {
        return FALSE;
    }

    /* infinite loop */
    if (loop_end->loop_number == 0) {
        if ( handle->runtime_data.cmd_var ) {
            // get last var
            _CMD_VAR_WALK( p );
            // check if in port list loop
            if ( _in_port_type_list_loop(p, loop_end) ) {
                return TRUE;
            }
        }
        return FALSE;
    ...
    loop_count = 0;
    for (loop_head = loop_end->loop_head; loop_head; ___SIBLING(loop_head)) {
        for ( p = handle->runtime_data.cmd_var; p; ___NEXT(p) ) {
            if (p->match_node == loop_head) {
                ++loop_count;
            }
        }
    }

    if (loop_count < loop_end->loop_number) {
        return TRUE;
    }
...
#if ICLI_LOOP_SYNTAX
        if (_loop_number_allowed(handle, node)) {
            rc = _match_single_node(handle, node->loop_head, w, b_enable_chk, FALSE, FALSE, TRUE);
            switch ( rc ) {
            case ICLI_RC_ERR_REDO:
            default:
                break;
...
static void _last_port_type_get(
    IN icli_session_handle_t    *handle
)
{
    icli_parameter_t        *p;
    ...
    // get last node
    for ( p = handle->runtime_data.cmd_var; p->next; p = p->next );

    // get port type if port type
    if ( p->value.type == ICLI_VARIABLE_PORT_TYPE ) {
...
)
{
    ...
    if ( node->loop_head == NULL ) {
        return FALSE;
    }

    _last_port_type_get( handle );
    ...
    case ICLI_PORT_TYPE_25_GIGABIT_ETHERNET:
    case ICLI_PORT_TYPE_CPU:
        return TRUE;
...
)
{
    ...
    /* check if in loop */
    i = 0;
    loop_head = handle->runtime_data.cmd_var;
    for ( p = handle->runtime_data.cmd_var, prev = NULL; p; prev = p, ___NEXT(p) ) {
        if ( p->match_node->loop_begin ) {
            ++i;
            loop_head = p;
        }
        if ( p->match_node->loop_head ) {
    ...
    /* check if port list */
    if ( _in_port_type_list_loop(prev, tree) == FALSE ) {
        if ( i ) {
            handle->runtime_data.b_in_loop = TRUE;
        }
        if ( tree->optional_begin ) {
            handle->runtime_data.b_in_loop = FALSE;
        }
    }

    /* not in loop, find from the begin */
    if ( tree->loop_number == 0 || tree->loop_begin ) {
        loop_head = handle->runtime_data.cmd_var;
    }
#endif
...
static void _invalid_error_display(
    IN  icli_session_handle_t   *handle,
    IN  char                    *w,
    IN  const char              *err_descr
)
{
    ...
    if ( handle->runtime_data.cmd_var ) {
        ...
        if ( p->match_node->type == ICLI_VARIABLE_KEYWORD && vtss_icli_str_sub(p->match_node->word, "interface", 0, NULL) == 0 ) {
            memset( str, 0, sizeof(str) );
            icli_snprintf( str, sizeof(str), "%% No such interface type: %s\n", w);
            _error_display(handle, NULL, str);
            return;
        }
    }

    _error_display(handle, w, err_descr);
}

#if 1 /* CP, 2014-10-28 14:12, Bugzilla#16112 */
static void _ambiguous_visible_chk(
    IN  icli_session_handle_t   *handle
)
{
    icli_parameter_t    *p;
    ...
    visible_cnt = 0;
    for ( p = handle->runtime_data.match_para; p; ___NEXT(p) ) {
        if ( vtss_icli_exec_visible_chk(handle, p) ) {
            ++visible_cnt;
        }
    }
...
#if 1 /* Bugzilla#12915 - invalid hidden interface entry command, then back to global-config mode */
static void _back_to_global_chk(
    IN  icli_session_handle_t   *handle
)
{
    icli_parameter_t        *p;
    ...
    // if current mode <= GLOBAL-CONFIG mode, then do nothing
    if ( handle->runtime_data.mode_level <= 1 ) {
        return;
    }
    ...
    handle->runtime_data.b_back_to_global = TRUE;
}
#endif
...
)
{
...
#if 0 /* CP, 02/17/2014 12:47, Bugzilla#14126, comment processing */
    u32                     i;
    BOOL                    b_comment_redo;
    BOOL                    b_current_b;
#endif
    ...
    if ( handle->runtime_data.b_in_parsing ) {
        tree = vtss_icli_parsing_tree_get( (icli_cmd_mode_t)(handle->runtime_data.current_parsing_mode) );
    } else {
        tree = vtss_icli_parsing_tree_get( (icli_cmd_mode_t)(handle->runtime_data.mode_para[handle->runtime_data.mode_level].mode) );
    }
...
#if 1 /* CP, 02/17/2014 12:47, Bugzilla#14126, comment processing */
...
_CMD_WALK_REDO:
...
#if ICLI_LOOP_SYNTAX
            if (_loop_number_allowed(handle, tree)) {
                rc = _match_single_node(handle, tree->loop_head, w, TRUE, FALSE, FALSE, TRUE);
                switch ( rc ) {
                case ICLI_RC_OK:
                    break;
                case ICLI_RC_ERR_REDO:
        ...
        case 0 : /* no match */
            ...
            if ( handle->runtime_data.rc == ICLI_RC_ERR_INCOMPLETE ) {
...
#if 0 /* CP, 02/17/2014 12:47, Bugzilla#14126, comment processing */
                    /*
                        for the case of "terminal length 50!123", where 50!123
                        will not be matched at the first time, then go into here
                        to check if 50!123 contains comment. If yes, then remove
                        comment char, 50!123 to be 50, and walk again.
                    */
                ...
                _invalid_error_display(handle, w, "Invalid");

#if 1 /* Bugzilla#12915 - invalid hidden interface entry command, then back to global-config mode */
                _back_to_global_chk( handle );
#endif
            }
            _handle_para_free( handle );
            return ICLI_RC_ERR_MATCH;
...
#if 1 /* CP, 2014-10-28 14:12, Bugzilla#16112 */
            _ambiguous_visible_chk( handle );
#endif

            // pre-process match_para
            if ( handle->runtime_data.exactly_cnt == 0 ) {
                for ( p = handle->runtime_data.match_para; p; ___NEXT(p) ) {
                    if ( vtss_icli_variable_data_string_type_get((icli_variable_type_t)(p->value.type)) ) {
                        p->match_type = ICLI_MATCH_TYPE_EXACTLY;
                        ++( handle->runtime_data.exactly_cnt );
                    }
                }
...
)
{
    ...
    *c = ICLI_EOS;
    ...
    // indicate it is copied command
    handle->runtime_data.b_cmd_copy = TRUE;
}
...
)
{
    ...
    if (para == NULL || para->match_node->execution == NULL || para->match_node->execution->cmd_cb == NULL) {
        return;
    }
...
)
{
        ...
        case ICLI_VARIABLE_RANGE_STRING:
            if ( ICLI_IS_(EOS, *(handle->runtime_data.cmd_walk + 1)) ) {
                ...
                if ( ICLI_IS_(STRING_END, *c) ) {
                    _cr_get(handle, prev_node, p, FALSE, FALSE);
                    if ( prev_node ) {
                        prev_node->next = NULL;
                        vtss_icli_exec_parameter_free( p );
                        p = prev_node;
    ...
    if ( handle->runtime_data.b_comment ) {
        w = handle->runtime_data.cmd_walk;
    } else {
    ...
    if ( b ) {
        ...
        if ( _must_child_allowed(handle, tree) ) {
            ...
            rc = _match_node_find(handle, tree->child, empty_string, b_enable_chk, TRUE);
#else
            rc = _match_node_find(handle, tree->child, empty_string, FALSE, TRUE);
#endif
            switch ( rc ) {
            case ICLI_RC_ERR_REDO:
            default:
        ...
        if (_loop_number_allowed(handle, tree)) {
            char empty_string[1] = {0};
#if 1 /* CP, 2014-10-28 14:12, Bugzilla#16112 */
            rc = _match_single_node(handle, tree->loop_head, empty_string, b_enable_chk, FALSE, FALSE, TRUE);
#else
            rc = _match_single_node(handle, tree->loop_head, empty_string, FALSE, FALSE, FALSE, TRUE);
#endif
            switch ( rc ) {
            case ICLI_RC_ERR_REDO:
            default:
...
#if 1 /* CP, 2014-10-28 14:12, Bugzilla#16112 */
        char empty_string[1] = {0};
        rc = _random_optional_match(handle, tree, empty_string, b_enable_chk, FALSE, -1);
#else
        rc = _random_optional_match(handle, tree, empty_string, FALSE, FALSE, -1);
#endif
...
#if 1 /* CP, 2014-10-28 14:12, Bugzilla#16112 */
        char empty_string[1] = {0};
        rc = _match_node_find(handle, tree, empty_string, b_enable_chk, TRUE);
#else
        rc = _match_node_find(handle, tree, empty_string, FALSE, TRUE);
#endif
        switch ( rc ) {
        case ICLI_RC_ERR_REDO:
        default:
    ...
    for ( p = handle->runtime_data.match_para; p != NULL; p = n ) {
        n = p->next;
#if 1 /* CP, 2014-10-28 14:12, Bugzilla#16112 */
        match_type = _node_match(handle, p->match_node, w, b_enable_chk, &p);
#else
        match_type = _node_match(handle, p->match_node, w, FALSE, &p);
#endif
        switch ( match_type ) {
...
{
    r->usid        = icli_platform_isid2usid( r->isid );
    r->begin_uport = icli_platform_iport2uport( r->begin_iport );
    r->switch_id   = icli_platform_usid2switchid( r->usid );
}
...
)
{
    ...
    if (handle->runtime_data.current_parsing_mode == ICLI_CMD_MODE_MULTILINE) {
        if (icli_multiline_parsing_complete(handle->session_id, handle->runtime_data.cmd)) {
    ...
    rc_authorize = icli_platform_cmd_authorize(handle,
                                               vtss_icli_exec_full_cmd_str_get(handle),
                                               vtss_icli_exec_highest_cmd_priv_get(handle),
                                               TRUE);
    ...
    for ( cmd_var = handle->runtime_data.cmd_var; cmd_var; ___NEXT(cmd_var) ) {
        if ( cmd_var->value.type == ICLI_VARIABLE_PORT_TYPE                          &&
             ...
             ___LOOP_BEGIN(cmd_var->match_node)                                     &&
             ( cmd_var->next == NULL || cmd_var->next->match_node->loop_head != cmd_var->match_node ) ) {
            /*
    ...
    case ICLI_EXEC_TYPE_CMD:
                        ...
                        if ( (u32)cmd_mode == node_prop->cmd_property->mode_destroy ) {
                            ...
                            if ( vtss_icli_str_cmp(mode_var->value.u.u_range_word, cmd_var->value.u.u_range_word) == 0 ) {
                                ...
                                case ICLI_CMD_MODE_APS:
                                    str = "Automatic Protection Switching";
                                    break;

                                case ICLI_CMD_MODE_ERPS:
                                    str = "Ethernet Ring Protection Switching";
                                    break;

                                case ICLI_CMD_MODE_IEC_MRP:
                                    str = "Media Redundancy Protocol";
                                    break;
                                ...
                                orig_err_display_mode = (icli_err_display_mode_t)(handle->runtime_data.err_display_mode);
                                handle->runtime_data.err_display_mode = ICLI_ERR_DISPLAY_MODE_PRINT;
        ...
        if ( rc_authorize == ICLI_RC_ERR_AAA_REJECT ) {
            _error_display(handle, NULL, "%% Command authorization failed.\n");
        ...
        handle->runtime_data.exec_rc = (*cmd_cb)(session_id, mode_var, cmd_var, usr_opt);
    ...
    case ICLI_EXEC_TYPE_PARSING:
        if ( handle->runtime_data.b_in_parsing ) {
            for ( node_prop = &(p->match_node->node_property); node_prop; ___NEXT(node_prop) ) {
                if ( node_prop->cmd_property->cmd_id == p->match_node->execution->cmd_id ) {
                    if ( node_prop->cmd_property->mode_goto != ICLI_CMD_MODE_MAX ) {
                        if (node_prop->cmd_property->mode_goto == ICLI_CMD_MODE_MULTILINE) {
                            if (icli_multiline_parsing_begin(handle->session_id, last_cmd_var->value.u.u_line)) {
                        ...
                        // Bugzilla#20208, #20921 - device crash when running into syntax check mode on icli
                        // Save the current parsing mode variable into parsing session
                        for (mode_var = handle->runtime_data.cmd_var; mode_var && mode_var->next; ___NEXT(mode_var));
                        if (mode_var) {
                            handle->runtime_data.current_parsing_mode_var = *mode_var;
                        }
                    }
                    break;
                }
...
)
{
    if ( handle->runtime_data.err_buf[0] != ICLI_EOS ) {
        ...
        memset(handle->runtime_data.err_buf, 0, sizeof(handle->runtime_data.err_buf));
    }
}
#endif
...
)
{
    ...
    if ( vtss_icli_str_len(handle->runtime_data.cmd_walk) == 0 ) {
        // empty then get first possible node
        if ( handle->runtime_data.b_in_parsing ) {
            node = vtss_icli_parsing_tree_get( (icli_cmd_mode_t)(handle->runtime_data.current_parsing_mode) );
        } else {
            node = vtss_icli_parsing_tree_get( (icli_cmd_mode_t)(handle->runtime_data.mode_para[handle->runtime_data.mode_level].mode) );
        }
        b_next = TRUE;
    } else {
        // execute command
...
#if 1 /* Bugzilla#14129 - remove debug level */
        if ( _present_chk(handle, node) && _privilege_chk(handle, node) && _visible_chk(handle, node) ) {
            for ( node_prop = &(node->node_property); node_prop; ___NEXT(node_prop) ) {
                if ( _node_prop_present(handle, node_prop)                                  &&
                    ...
                    if ( n < ICLI_CMD_CNT ) {
                        if ( _sort_insert(vtss_icli_register_var_cmd_get(node_prop->cmd_property->cmd_id)) ) {
                            ++n;
                        }
                    } else {
...
)
{
    ...
    i32                         i;
    icli_cmd_mode_t             original_parsing_mode = ICLI_CMD_MODE_GLOBAL_CONFIG;
...
#if 1 /* Bugzilla#12915 - invalid hidden interface entry command, then back to global-config mode */
    handle->runtime_data.b_back_to_global = FALSE;
#endif

#if 1 /* CP, 01/24/2014 14:19, Bugzilla#13919 - give return code of callback */
    handle->runtime_data.exec_rc = ICLI_RC_OK;
#endif

    // no command copy
    handle->runtime_data.b_cmd_copy = FALSE;

    switch ( handle->runtime_data.exec_type ) {
    case ICLI_EXEC_TYPE_PARSING:
    case ICLI_EXEC_TYPE_CMD: {
        if ( (handle->runtime_data.exec_type == ICLI_EXEC_TYPE_PARSING &&
              (( handle->runtime_data.b_in_parsing && handle->runtime_data.current_parsing_mode <= ICLI_CMD_MODE_GLOBAL_CONFIG) ||
               (!handle->runtime_data.b_in_parsing && handle->runtime_data.mode_level <= ICLI_CMD_MODE_GLOBAL_CONFIG)))
             ||
             ( handle->runtime_data.exec_type == ICLI_EXEC_TYPE_CMD &&
               handle->runtime_data.mode_para[handle->runtime_data.mode_level].mode <= ICLI_CMD_MODE_GLOBAL_CONFIG)) {
            /* Execute command in 'global-config' mode. */
            rc = (icli_rc_t)_exec_cmd( handle );
        } else {
            /* Execute command in current mode.
             *
             * 1. Look up the input command in current mode first.
             * 2. If it is failed in in current mode, try to look up again in 'global-config' mode silently.
             *    The look up result:
             *    2.a. Failed  -> Display the previous error message in step 1.
             *    2.b. Success -> Execute the command and then switch to 'global-config' mode.
             */

            // store original error display mode
            original_err_display_mode = (icli_err_display_mode_t)(handle->runtime_data.err_display_mode);
            ...
            /* 1. Look up the input command in current mode first. */
            if ( (rc = (icli_rc_t)_exec_cmd( handle ) ) != ICLI_RC_OK ) {
                if ( rc == ICLI_RC_ERR_AMBIGUOUS ) {
                    /* display previous error message in current mode */
                    _exec_err_display( handle );
                } else {
                    /* All failed cases in current mode will continue look up
                     * the 'global-config' mode silently
                     */
                    ...
                    if ( handle->runtime_data.exec_type == ICLI_EXEC_TYPE_PARSING ) {
                        // store original parsing level
                        original_parsing_mode = (icli_cmd_mode_t)(handle->runtime_data.current_parsing_mode);
                        handle->runtime_data.current_parsing_mode = ICLI_CMD_MODE_GLOBAL_CONFIG;

                        if ( handle->runtime_data.b_in_parsing ) {
                            // store original mode level
                            original_mode_level = ICLI_CMD_MODE_GLOBAL_CONFIG;
                        }
                    }

                    // Switch to 'global-config' mode
                    handle->runtime_data.mode_level = ICLI_CMD_MODE_GLOBAL_CONFIG;
                    ...
                    } else { // return ICLI_RC_OK
                        if ( handle->runtime_data.exec_type == ICLI_EXEC_TYPE_CMD ) {
                            // Automatically go to 'global-config' mode.
                            // Exit until we are there in order to free all mode
                            // variables properly.

                            // First restore original mode.
                            T_D("original_mode_level = %d. Current level = %d", original_mode_level, handle->runtime_data.mode_level);
                            handle->runtime_data.mode_level = original_mode_level;

                            // Then iterate until we reach config mode.
                            while (handle->runtime_data.mode_level > ICLI_CMD_MODE_GLOBAL_CONFIG) {
                                (void)vtss_icli_session_mode_exit(handle->session_id);
                            }
                        } else { // ICLI_EXEC_TYPE_PARSING
                            handle->runtime_data.mode_level = handle->runtime_data.current_parsing_mode = ICLI_CMD_MODE_GLOBAL_CONFIG;
                        }
                    }
                }
            }
    ...
    case ICLI_EXEC_TYPE_TAB:
        rc = (icli_rc_t)_exec_tab( handle );
        break;

    case ICLI_EXEC_TYPE_QUESTION:
        rc = (icli_rc_t)_exec_question( handle );
        break;

#if 1 /* CP, 08/13/2013 13:00, Bugzilla#12423 - show full command syntax */
    case ICLI_EXEC_TYPE_FULL_CMD:
        rc = (icli_rc_t)_exec_full_cmd( handle );
        break;
#endif
...
)
{
    ...
    orig_current_parsing_mode = (icli_cmd_mode_t)(handle->runtime_data.current_parsing_mode);
...
#if 1 /* Bugzilla#14129 - remove debug level */
    handle->runtime_data.b_priv_cmd           = TRUE;
#endif
...
#if 1 /* Bugzilla#14129 - remove debug level */
    handle->runtime_data.b_priv_cmd           = FALSE;
#endif
...
)
{
    ...
    switch (p->value.type) {
    case ICLI_VARIABLE_KEYWORD:
        (void)icli_sprintf(c, "%s", p->match_node->word);
        break;

    /* mesa_mac_t */
    case ICLI_VARIABLE_MAC_ADDR:
    case ICLI_VARIABLE_MAC_UCAST:
    case ICLI_VARIABLE_MAC_MCAST:
        (void)vtss_icli_mac_to_str(p->value.u.u_mac_addr.addr, c);
        break;

    /* mesa_ip_t */
    case ICLI_VARIABLE_IPV4_ADDR:
    case ICLI_VARIABLE_IPV4_NETMASK:
    case ICLI_VARIABLE_IPV4_WILDCARD:
    case ICLI_VARIABLE_IPV4_UCAST:
    case ICLI_VARIABLE_IPV4_MCAST:
    case ICLI_VARIABLE_IPV4_NMCAST:
    case ICLI_VARIABLE_IPV4_ABC:
        (void)vtss_icli_ipv4_to_str(p->value.u.u_ipv4_addr, c);
        break;

    /* icli_ipv4_subnet_t */
    case ICLI_VARIABLE_IPV4_SUBNET:
        (void)vtss_icli_ipv4_to_str(p->value.u.u_ipv4_subnet.ip, c);
        c = c + vtss_icli_str_len(c);
        (void)icli_sprintf(c, "/");
        ++c;
        (void)vtss_icli_ipv4_to_str(p->value.u.u_ipv4_subnet.netmask, c);
        break;

    /* u32 */
    case ICLI_VARIABLE_IPV4_PREFIX:
    case ICLI_VARIABLE_IPV6_PREFIX:
    case ICLI_VARIABLE_UINT:
    case ICLI_VARIABLE_VLAN_ID:
    case ICLI_VARIABLE_SWITCH_ID:
    case ICLI_VARIABLE_RANGE_UINT:
        (void)icli_sprintf(c, "%u", p->value.u.u_uint);
        break;

    /* mesa_ipv6_t */
    case ICLI_VARIABLE_IPV6_ADDR:
    case ICLI_VARIABLE_IPV6_NETMASK:
    case ICLI_VARIABLE_IPV6_UCAST:
    case ICLI_VARIABLE_IPV6_MCAST:
    case ICLI_VARIABLE_IPV6_LINKLOCAL:
        (void)vtss_icli_ipv6_to_str(p->value.u.u_ipv6_addr, c);
        break;

    /* mesa_ipv6_network_t */
    case ICLI_VARIABLE_IPV6_SUBNET:
        (void)vtss_icli_ipv6_to_str(p->value.u.u_ipv6_subnet.address, c);
        c = c + vtss_icli_str_len(c);
        (void)icli_sprintf(c, "/");
        ++c;
        (void)icli_sprintf(c, "%u", p->value.u.u_ipv6_subnet.prefix_size);
        break;

    /* i8 */
    case ICLI_VARIABLE_INT8:
        (void)icli_sprintf(c, "%d", p->value.u.u_int8);
        break;

    /* i16 */
    case ICLI_VARIABLE_INT16:
        (void)icli_sprintf(c, "%d", p->value.u.u_int16);
        break;

    /* i32 */
    case ICLI_VARIABLE_INT:
    case ICLI_VARIABLE_RANGE_INT:
        (void)icli_sprintf(c, "%d", p->value.u.u_int);
        break;

    /* u8 */
    case ICLI_VARIABLE_UINT8:
    case ICLI_VARIABLE_DSCP:
    case ICLI_VARIABLE_DPL:
        (void)icli_sprintf(c, "%u", p->value.u.u_uint8);
        break;

    /* u16 */
    case ICLI_VARIABLE_UINT16:
        (void)icli_sprintf(c, "%u", p->value.u.u_uint16);
        break;

    /* icli_date_t */
    case ICLI_VARIABLE_DATE:
        (void)icli_sprintf(c, "%u", p->value.u.u_date.year);
        c = c + vtss_icli_str_len(c);
        (void)icli_sprintf(c, "/");
        ++c;
        (void)icli_sprintf(c, "%u", p->value.u.u_date.month);
        c = c + vtss_icli_str_len(c);
        (void)icli_sprintf(c, "/");
        ++c;
        (void)icli_sprintf(c, "%u", p->value.u.u_date.day);
        break;

    /* icli_time_t */
    case ICLI_VARIABLE_TIME:
        (void)icli_sprintf(c, "%u", p->value.u.u_time.hour);
        c = c + vtss_icli_str_len(c);
        (void)icli_sprintf(c, ":");
        ++c;
        (void)icli_sprintf(c, "%u", p->value.u.u_time.min);
        c = c + vtss_icli_str_len(c);
        (void)icli_sprintf(c, ":");
        ++c;
        (void)icli_sprintf(c, "%u", p->value.u.u_time.sec);
        break;

    /* icli_time_t */
    case ICLI_VARIABLE_HHMM:
        (void)icli_sprintf(c, "%u", p->value.u.u_time.hour);
        c = c + vtss_icli_str_len(c);
        (void)icli_sprintf(c, ":");
        ++c;
    ...
    case ICLI_VARIABLE_LINE:
    case ICLI_VARIABLE_GREP_STRING:
    case ICLI_VARIABLE_HOST_NAME:
    case ICLI_VARIABLE_VWORD:
    case ICLI_VARIABLE_DOMAIN_NAME:
    case ICLI_VARIABLE_URL:
    case ICLI_VARIABLE_FILE:
    case ICLI_VARIABLE_URL_FILE:
    case ICLI_VARIABLE_RANGE_WORD:
    case ICLI_VARIABLE_RANGE_KWORD:
    case ICLI_VARIABLE_RANGE_DWORD:
    case ICLI_VARIABLE_RANGE_FWORD:
    case ICLI_VARIABLE_RANGE_LINE:
    case ICLI_VARIABLE_RANGE_VWORD:
    case ICLI_VARIABLE_RANGE_HOST_NAME:
    case ICLI_VARIABLE_RANGE_DOMAIN_NAME:
    case ICLI_VARIABLE_RANGE_URL:
    case ICLI_VARIABLE_RANGE_FILE:
    case ICLI_VARIABLE_RANGE_URL_FILE:
        (void)vtss_icli_str_cpy(c, p->value.u.u_word);
        break;

    case ICLI_VARIABLE_STRING:
    case ICLI_VARIABLE_RANGE_STRING:
        (void)icli_sprintf(c, "\"%s\"", p->value.u.u_word);
        break;

    /* icli_switch_port_range_t */
    case ICLI_VARIABLE_PORT_TYPE_ID:
        (void)vtss_icli_switch_port_range_to_str(&(p->value.u.u_port_id), c);
        break;

    /* icli_stack_port_range_t */
    case ICLI_VARIABLE_PORT_TYPE_LIST:
        (void)vtss_icli_stack_port_range_to_str(&(p->value.u.u_port_list), c);
        break;
    ...
    case ICLI_VARIABLE_SWITCH_LIST:
    case ICLI_VARIABLE_RANGE_UINT_LIST:
        (void)vtss_icli_unsigned_range_to_str(&(p->value.u.u_vlan_list), c);
        break;

    /* icli_signed_range_t */
    case ICLI_VARIABLE_RANGE_INT_LIST:
        (void)vtss_icli_signed_range_to_str(&(p->value.u.u_range_int_list), c);
        break;

    /* icli_range_t */
    case ICLI_VARIABLE_RANGE_LIST:
        (void)vtss_icli_range_to_str(&(p->value.u.u_range_list), c);
        break;

    /* icli_oui_t */
    case ICLI_VARIABLE_OUI:
        (void)vtss_icli_oui_to_str(&(p->value.u.u_oui), c);
        break;

    /* icli_clock_id_t */
    case ICLI_VARIABLE_CLOCK_ID:
        (void)vtss_icli_clock_id_to_str(&(p->value.u.u_clock_id), c);
        break;

    /* icli_vcap_vr_t */
    case ICLI_VARIABLE_VCAP_VR:
        (void)vtss_icli_vcap_vr_to_str(&(p->value.u.u_vcap_vr), c);
        break;

    /* icli_hexval_t */
    case ICLI_VARIABLE_HEXVAL:
    case ICLI_VARIABLE_RANGE_HEXVAL:
        (void)vtss_icli_hexval_to_str(&(p->value.u.u_hexval), c);
        break;

    /* mesa_ip_t */
    case ICLI_VARIABLE_AREA_ID:
        (void)vtss_icli_ipv4_to_str(p->value.u.u_area_id, c);
        break;
    ...
    case ICLI_VARIABLE_GREP_BEGIN:
        (void)icli_sprintf(c, "begin");
        break;

    /* BOOL */
    case ICLI_VARIABLE_GREP_INCLUDE:
        (void)icli_sprintf(c, "include");
        break;

    /* BOOL */
    case ICLI_VARIABLE_GREP_EXCLUDE:
        (void)icli_sprintf(c, "exclude");
        break;
...
)
{
    ...
    /*
        Well, we steal str_buf to pack full command string.
        It is somewhat dangerous anyway.
    */
    memset(handle->runtime_data.str_buf, 0, sizeof(handle->runtime_data.str_buf));

    b_get = FALSE;
    c = (char *)(handle->runtime_data.str_buf);
    for (p = handle->runtime_data.cmd_var; p; ___NEXT(p)) {
        if (vtss_icli_exec_para_word_get(p, c)) {
    ...
    return (char *)(handle->runtime_data.str_buf);
}
...
)
{
    ...
    // get the highest command privilege
    cmd_priv = ICLI_PRIVILEGE_0;
    for (node_prop = &(p->match_node->node_property); node_prop; ___NEXT(node_prop)) {
        if (node_prop->cmd_property->privilege > cmd_priv) {
            cmd_priv = node_prop->cmd_property->privilege;
        }
    }
...
)
{
    ...
    // find the last parameter
    for (p = handle->runtime_data.cmd_var; p && p->next; ___NEXT(p)) {
        ;
    }
    ...
    // get the highest command privilege by parameter
    return vtss_icli_exec_highest_para_priv_get(p);
}

/*
    check the matched node is visible or invisible
...
{
    return _visible_chk( handle, para->match_node );
}
