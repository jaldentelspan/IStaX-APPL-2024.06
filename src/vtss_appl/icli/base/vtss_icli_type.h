...
typedef enum {
    ...
    ICLI_RC_ERR_AAA_REJECT  = -983,
    ICLI_RC_ERR_AAA_IGNORE  = -982,
    ICLI_RC_ERR_PARSING_END = -981,
} icli_rc_t;

/* ICLI error return macro */
#define ICLI_RC(expr) { icli_rc_t _rc_ = (expr); if (_rc_ != ICLI_RC_OK) return _rc_; }
...
typedef enum {
    ...
    ICLI_PORT_TYPE_25_GIGABIT_ETHERNET,
    ICLI_PORT_TYPE_CPU,
...
typedef struct {
    icli_port_type_t    port_type;      /* icli_port_type_t                             */
...
typedef enum {
    ICLI_NET_ADDR_TYPE_IPV4,
    ICLI_NET_ADDR_TYPE_IPV6,
    ICLI_NET_ADDR_TYPE_DOMAIN_NAME,
    //------- add below
    ICLI_NET_ADDR_TYPE_MAX
} icli_net_addr_type_t;

typedef struct {
    union {
        mesa_ip_t           u_ipv4_addr;
        mesa_ipv6_t         u_ipv6_addr;
        char                u_domain_name[ICLI_VALUE_STR_MAX_LEN + 4];
    } u;

    // put here because if alignment
    icli_net_addr_type_t    type;
} icli_net_addr_t;
...
typedef enum {
    ...
    ICLI_VARIABLE_DOMAIN_NAME,
    ICLI_VARIABLE_URL,
    ICLI_VARIABLE_FILE,
    ICLI_VARIABLE_URL_FILE,
    ICLI_VARIABLE_NET_ADDR, // 60
    ICLI_VARIABLE_AREA_ID,
    ...
    ICLI_VARIABLE_RANGE_HOST_NAME,
    ICLI_VARIABLE_RANGE_DOMAIN_NAME,
    ICLI_VARIABLE_RANGE_URL,
    ICLI_VARIABLE_RANGE_FILE,
    ICLI_VARIABLE_RANGE_URL_FILE,
...
typedef struct {
    union {
        mesa_mac_t                  u_mac_addr;         // ICLI_VARIABLE_MAC_ADDR
        mesa_mac_t                  u_mac_ucast;        // ICLI_VARIABLE_MAC_UCAST
        mesa_mac_t                  u_mac_mcast;        // ICLI_VARIABLE_MAC_MCAST
        mesa_ip_t                   u_ipv4_addr;        // ICLI_VARIABLE_IPV4_ADDR
        mesa_ip_t                   u_ipv4_netmask;     // ICLI_VARIABLE_IPV4_NETMASK
        mesa_ip_t                   u_ipv4_wildcard;    // ICLI_VARIABLE_IPV4_WILDCARD
        mesa_ip_t                   u_ipv4_ucast;       // ICLI_VARIABLE_IPV4_UCAST
        mesa_ip_t                   u_ipv4_mcast;       // ICLI_VARIABLE_IPV4_MCAST
        mesa_ip_t                   u_ipv4_nmcast;      // ICLI_VARIABLE_IPV4_NMCAST
        mesa_ip_t                   u_ipv4_abc;         // ICLI_VARIABLE_IPV4_ABC
        icli_ipv4_subnet_t          u_ipv4_subnet;      // ICLI_VARIABLE_IPV4_SUBNET
        u32                         u_ipv4_prefix;      // ICLI_VARIABLE_IPV4_PREFIX
        mesa_ipv6_t                 u_ipv6_addr;        // ICLI_VARIABLE_IPV6_ADDR
        mesa_ipv6_t                 u_ipv6_netmask;     // ICLI_VARIABLE_IPV6_NETMASK
        mesa_ipv6_t                 u_ipv6_ucast;       // ICLI_VARIABLE_IPV6_UCAST
        mesa_ipv6_t                 u_ipv6_mcast;       // ICLI_VARIABLE_IPV6_MCAST
        mesa_ipv6_t                 u_ipv6_linklocal;   // ICLI_VARIABLE_IPV6_LINKLOCAL
        mesa_ipv6_network_t         u_ipv6_subnet;      // ICLI_VARIABLE_IPV6_SUBNET
        ...
        icli_unsigned_range_t       u_vlink_list;       // ICLI_VARIABLE_VLINK_LIST
        ...
        char                        u_host_name[ICLI_VALUE_STR_MAX_LEN + 4]; // ICLI_VARIABLE_HOST_NAME
        ...
        char                        u_domain_name[ICLI_VALUE_STR_MAX_LEN + 4]; // ICLI_VARIABLE_DOMAIN_NAME
        char                        u_url[ICLI_URL_MAX_LEN + 4]; // ICLI_VARIABLE_URL
        char                        u_file[ICLI_FILE_NAME_MAX_LEN + 4]; // ICLI_VARIABLE_FILE
        char                        u_url_file[ICLI_URL_MAX_LEN + 4]; // ICLI_VARIABLE_URL_FILE
        icli_net_addr_t             u_net_addr;         // ICLI_VARIABLE_NET_ADDR
        mesa_ip_t                   u_area_id;          // ICLI_VARIABLE_AREA_ID
        ...
        char                        u_range_host_name[ICLI_VALUE_STR_MAX_LEN + 4];   // ICLI_VARIABLE_RANGE_HOST_NAME
        char                        u_range_domain_name[ICLI_VALUE_STR_MAX_LEN + 4]; // ICLI_VARIABLE_RANGE_DOMAIN_NAME
        char                        u_range_url[ICLI_URL_MAX_LEN + 4]; // ICLI_VARIABLE_RANGE_URL
        char                        u_range_file[ICLI_FILE_NAME_MAX_LEN + 4]; // ICLI_VARIABLE_RANGE_FILE
        char                        u_range_url_file[ICLI_URL_MAX_LEN + 4]; // ICLI_VARIABLE_RANGE_URL_FILE
    } u;
...
typedef enum {
    ICLI_URL_PROTOCOL_TFTP,
    ICLI_URL_PROTOCOL_FTP,
    ICLI_URL_PROTOCOL_HTTP,
    ICLI_URL_PROTOCOL_HTTPS,
    ICLI_URL_PROTOCOL_FILE,
    ICLI_URL_PROTOCOL_FLASH,
    ICLI_URL_PROTOCOL_SFTP,
    ICLI_URL_PROTOCOL_SCP,
    ICLI_URL_PROTOCOL_USB,

    //-- add above
    ICLI_URL_PROTOCOL_MAX,
} icli_url_proto_t;
...
typedef union {
    ...
    BOOL                        url_not_support[ICLI_URL_PROTOCOL_MAX];
    BOOL                        net_addr_not_support[ICLI_NET_ADDR_TYPE_MAX];
} icli_runtime_t;
...
/*
    ...
    RETURN
        TRUE
            ...
            ICLI_ASK_RANGE      : use runtime.range
        ...
        FALSE
            ...
            ICLI_ASK_RANGE      : use original one in *.icli
...
struct _icli_parsing_node {
    ...
    // command property
    icli_node_property_t        node_property;
    ...
    */
    ...
    /*
        ...
        but the beginning node of the previous back
        "a b c d", d->parent = c
    ...
    */
    ...
    /* pointer to loop head */
    icli_parsing_node_t         *loop_head;

#if ICLI_LOOP_SYNTAX
    /* indicate if the node is begin node of loop, 0: no, 1: yes */
    u32                         loop_begin;
...
typedef mesa_rc (*icli_multiline_callback_t)(char *);

typedef struct {
    // The multiline buffer for the session.
    char buffer[ICLI_MULTILINE_BUFFER_MAX_LEN];
    // The delimiter that marks the start and end of the buffer.
    char delimiter;
    // The callback function responsible for processing each line input
    // of the multiline buffer. Not the first, that is taken care by the
    // respective multiline "enter" function.
    icli_multiline_callback_t callback;
} icli_session_multiline_data_t;
...
typedef struct {
    ...
    */
    ...
    // Hack hack hack....
    // This is not pretty!
    // But I need a consisten way to pass the file descriptor from the
    // ssh-accept loop all the way to the icli_session_engine, and this is only
    // suppose to be used if it is created by ssh!
#define CLIENT_NEED_CLI_IOLAYER_IN_THREAD_DATA_MAGIC 0x1756A24F
    u32                         client_fd;
    u32                         client_need_cli_iolayer_in_thread_data;

    /* Storage containers for multiline mode */
    icli_session_multiline_data_t multiline_data;
...
typedef struct {
    ...
    char     mode_prompt[ICLI_CMD_MODE_MAX][ICLI_MODE_PROMPT_MAX_LEN + 4];
