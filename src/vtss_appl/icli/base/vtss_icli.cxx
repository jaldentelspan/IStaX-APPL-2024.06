...
)
{
    ...
    // device name
    (void)vtss_icli_str_ncpy(conf->dev_name, ICLI_DEFAULT_DEVICE_NAME, ICLI_DEV_NAME_MAX_LEN);

    // mode name
    for ( i = 0; i < ICLI_CMD_MODE_MAX; ++i ) {
        (void)vtss_icli_str_ncpy(conf->mode_prompt[i], icli_platform_mode_prompt_get((icli_cmd_mode_t)i), ICLI_MODE_PROMPT_MAX_LEN);
    }
...
)
{
    ...
    conf = (icli_conf_data_t *)icli_malloc( sizeof(icli_conf_data_t) );
    if ( conf == NULL ) {
...
)
{
    ...
    (void)vtss_icli_str_ncpy(g_conf_data.mode_prompt[mode], prompt, ICLI_MODE_PROMPT_MAX_LEN);
    g_conf_data.mode_prompt[mode][ICLI_MODE_PROMPT_MAX_LEN] = 0;
...
)
{
    ...
    T_I("Not in range, end_uport:%d", end_uport);
    /* not in range */
    return FALSE;
}
...
)
{
    ...
    for ( i = 0; i < src->cnt; i++ ) {
        if ( _switch_range_get((icli_port_type_t)(src->switch_range[i].port_type), &(src->switch_range[i]), dst, NULL, NULL) == FALSE ) {
...
)
{
    ...
    if ( g_conf_data.b_enable_secret[priv] ) {
        // get mac
        icli_platform_hmac_md5(g_enable_key, g_enable_key_len, (u8 *)clear_password, vtss_icli_str_len(clear_password), mac);
