...
typedef struct {
    char                        mode_prompt[ICLI_MODE_PROMPT_MAX_LEN + 1];
...
static icli_node_property_t     *g_free_node_property = NULL;
static i32                      g_free_node_cnt = 0;
...
static icli_node_property_t *_free_node_property_get(
    IN i32  word_id
)
{
    icli_node_property_t    *new_property;

    //Pool is empty, allocate new pool with 1024 entries
    if ( g_free_node_cnt <= 0 ) {
        g_free_node_property = (icli_node_property_t *)icli_calloc(1024, sizeof(icli_node_property_t));
        if (g_free_node_property != NULL) {
            g_free_node_cnt = 1024;
        } else {
            T_E("memory is used out\n");
            return NULL;
        }
    }
    new_property = g_free_node_property + (1024 - g_free_node_cnt--);
...
)
{
    ...
    i32                     w_type;
#if ICLI_RANDOM_OPTIONAL
    i32                     prev_type = ICLI_WORD_TYPE_KEYWORD;
    i32                     first_optional_id = -1;
#endif
    i32                     prev_begin_id = 0;
    i32                     prev_end_id = 0;
    i32                     current_begin_id;
    i32                     current_end_id;
    i32                     level;
    char                    *w;
    char                    *sub_cmd;
...
#if ICLI_LOOP_SYNTAX
            /* mark loop begin */
            for (node = m_tree; node; ___SIBLING(node)) {
                node->loop_begin = 1;
            }

            /* mark loop end to last children*/
            for ( i = current_begin_id; i <= current_end_id; ++i ) {
                if ( g_cmd_node[i].child == NULL ) {
                    g_cmd_node[i].loop_head = m_tree;
                    g_cmd_node[i].loop_number = g_cmd_node[current_end_id].loop_number;
                }
            }

            /* mark loop end to random optional */
            for ( i = 0; i < ICLI_RANDOM_OPTIONAL_DEPTH; ++i ) {
                if ( g_cmd_node[current_end_id].random_optional[i] ) {
                    node = g_cmd_node[current_end_id].random_optional[i];
                    random_optional_level = g_cmd_node[current_end_id].random_optional_level[i];
                    for (j = node->word_id; j < current_end_id; ++j) {
                        node = &(g_cmd_node[j]);
                        if ( ___BIT_MASK_GET(node->optional_end, random_optional_level) ) {
                            // find
                            node->loop_head = m_tree;
                            node->loop_number = g_cmd_node[current_end_id].loop_number;
                        }
                    }
                }
            }
...
)
{
    ...
    u32                     word_id;
    icli_node_property_t    *new_node_property;
    icli_parsing_node_t     *ot, *nt, *pre_ot, *last_sibling;
    u32                     i, same_node_cnt = 0;
    ...
    for ( pre_ot = old_tree->child->child, ot = old_tree->child->child, nt = new_tree->child->child; (ot || nt); pre_ot = ot, ___CHILD(ot), ___CHILD(nt) ) {
        if (ot && nt && _node_is_same(ot, nt)) {
            same_node_cnt++;
            continue;
        } else if (ot) {
            // Add the new node as the sibling of old node
            if (ot->sibling) {
                // Find the last existing sibling in old node
                last_sibling = ot;
                while ( last_sibling->sibling ) {
                    last_sibling = last_sibling->sibling;
                }
                if (nt) {
                    nt->parent = last_sibling->parent;
                }
                last_sibling->sibling = nt;
            } else if (nt) {
                // Set the new node as the sibling of old node
                nt->parent = ot->parent;
                ot->sibling = nt;
            }
        } else if (pre_ot) {
            // Set the new node as the child of old node
            nt->parent = pre_ot;
            pre_ot->child = nt;
        }
        break;
    ...
    for ( i = 0, ot = old_tree->child->child; i < same_node_cnt; i++, ___CHILD(ot) ) {
        ++word_id;
        _free_parsing_node_add( &(g_cmd_node[word_id]) );
        tree = ot;
        ___APPEND_NODE_PROPERTY();
