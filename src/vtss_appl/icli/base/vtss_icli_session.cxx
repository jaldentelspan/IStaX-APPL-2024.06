...
VTSS_ENUM_INC(icli_port_type_t);
#endif
...
i32 icli_prompt_interpreted_get(char *interpreted_prompt, size_t len);
...
)
{
    ...
    char                *mode_prompt;
    char                interpreted_prompt[ICLI_PROMPT_INTERPRETED_MAX_LEN + 1];
    // The full prompt has a maximum size of:
    //   ICLI_PROMPT_INTERPRETED_MAX_LEN for interpreted prompt
    //   + 1                             for opening parenthesis
    //   + ICLI_MODE_PROMPT_MAX_LEN      for mode (e.g. 'config')
    //   + 1                             for closing parenthesis
    //   + 1                             for '#' or '>'
    //   + 1                             for space
    //   + 1                             for terminating NULL
    char                full_prompt[ICLI_PROMPT_INTERPRETED_MAX_LEN + ICLI_MODE_PROMPT_MAX_LEN + 5];
    ...
    switch_info.b_primary = TRUE;
    switch_info.usid      = 0;

    // get switch information
    icli_platform_switch_info_get( &switch_info );
#endif
    ...
    if (switch_info.b_primary) {
        if (icli_prompt_interpreted_get(interpreted_prompt, sizeof(interpreted_prompt)) != ICLI_RC_OK) {
            strncpy(interpreted_prompt, "<Error in prompt>", sizeof(interpreted_prompt) - 1);
            interpreted_prompt[sizeof(interpreted_prompt) - 1] = '\0';
        }

        if (vtss_icli_str_len(mode_prompt)) {
            (void)icli_snprintf(full_prompt, sizeof(full_prompt), "%s(%s)%c ", interpreted_prompt, mode_prompt, handle->runtime_data.privilege >= handle->config_data->privileged_level ? '#' : '>');
        } else {
            (void)icli_snprintf(full_prompt, sizeof(full_prompt), "%s%c ", interpreted_prompt, handle->runtime_data.privilege >= handle->config_data->privileged_level ? '#' : '>');
        }
    } else {
        if ( vtss_icli_str_len(mode_prompt) ) {
            (void)icli_snprintf(full_prompt, sizeof(full_prompt), "Secondary_%u(%s)%c ", switch_info.usid, mode_prompt, handle->runtime_data.privilege >= handle->config_data->privileged_level ? '#' : '>');
        } else {
            (void)icli_snprintf(full_prompt, sizeof(full_prompt), "Secondary_%u%c ", switch_info.usid, handle->runtime_data.privilege >= handle->config_data->privileged_level ? '#' : '>');
        }
    }

    vtss_icli_session_str_put(handle, full_prompt);
    handle->runtime_data.prompt_len = vtss_icli_str_len(full_prompt);
}
...
)
{
    ...
    case ICLI_VARIABLE_RANGE_UINT_LIST:
        ...
        if ( runtime_cb ) {
            ...
            if ( b ) {
                ...
                memset(byword, 0, sizeof(handle->runtime_data.runtime.byword));
                (void)icli_sprintf(byword, "<");

                switch ( range.type ) {
                case ICLI_RANGE_TYPE_SIGNED:
                    sr = &( range.u.sr );
                    for ( i = 0; i < sr->cnt; ++i ) {
                        len = vtss_icli_str_len( byword );
                        if ( i ) {
                            (void)icli_snprintf(byword + len, ICLI_BYWORD_MAX_LEN - len, ",");
                            len = vtss_icli_str_len( byword );
                        }
                        if ( sr->range[i].min == sr->range[i].max ) {
                            (void)icli_snprintf(byword + len, ICLI_BYWORD_MAX_LEN - len, "%d", sr->range[i].min);
                        } else {
                            (void)icli_snprintf(byword + len, ICLI_BYWORD_MAX_LEN - len, "%d%c%d", sr->range[i].min, delimiter, sr->range[i].max);
                        }
                    }
                    break;
                ...
                case ICLI_RANGE_TYPE_UNSIGNED:
                    ...
                    for ( i = 0; i < ur->cnt; ++i ) {
                        ...
                        if ( ur->range[i].min == ur->range[i].max ) {
                            (void)icli_snprintf(byword + len, ICLI_BYWORD_MAX_LEN - len, "%u", ur->range[i].min);
                        } else {
                            (void)icli_snprintf(byword + len, ICLI_BYWORD_MAX_LEN - len, "%u%c%u", ur->range[i].min, delimiter, ur->range[i].max);
                        }
                    }
                    break;
                ...
                len = vtss_icli_str_len( byword );
                if ( len == ICLI_BYWORD_MAX_LEN ) {
                    byword[ICLI_BYWORD_MAX_LEN - 1] = '>';
                } else {
                    (void)icli_snprintf(byword + len, ICLI_BYWORD_MAX_LEN - len, ">");
                }
                byword[ICLI_BYWORD_MAX_LEN] = 0;
                return byword;
            }
        }
        ...
        bw = &(node->node_property.cmd_property->byword[node->word_id]);
        byword = bw->word;
        if ( vtss_icli_str_len(byword) && bw->para_cnt ) {
            if ( bw->para_cnt != -1 ) {
                /*
                    this byword does not need to be freed because it is linked
                    into node_property of the parsing tree and it can be used
    ...
    case ICLI_VARIABLE_DPL:
        byword = handle->runtime_data.runtime.byword;
        memset(byword, 0, sizeof(handle->runtime_data.runtime.byword));
        (void)icli_sprintf(byword, "<%u-%u>", ICLI_DPL_MIN, ICLI_DPL_MAX);
        return byword;

    case ICLI_VARIABLE_GREP:
        ICLI_RETURN_NON_CONST_CHAR_PTR(s_pipe, "|");

    case ICLI_VARIABLE_GREP_BEGIN:
        ICLI_RETURN_NON_CONST_CHAR_PTR(s_begin, "begin");

    case ICLI_VARIABLE_GREP_INCLUDE:
        ICLI_RETURN_NON_CONST_CHAR_PTR(s_include, "include");

    case ICLI_VARIABLE_GREP_EXCLUDE:
        ICLI_RETURN_NON_CONST_CHAR_PTR(s_exclude, "exclude");

    case ICLI_VARIABLE_GREP_STRING:
        ICLI_RETURN_NON_CONST_CHAR_PTR(s_line, "<line>");
...
static const char *_cword_prefix_get(
    IN  icli_session_handle_t   *handle,
    IN  icli_parameter_t        *p,
    OUT u32                     *prefix_num
)
...
/*
    hook for pressing Tab and ?, Step 1
        The static API is to check if each word in match_sort_list is allowed
        or not by AAA. If not allowed, then remove it from match_sort_list
        and free it. So that _tab_display() and _question_display() will
        display only valid words.
*/
static BOOL _aaa_match_list(
    IN  icli_session_handle_t   *handle
)
{
    icli_parameter_t    *p;
    icli_parameter_t    *prev_p;
    char                *c;
    char                *command;
    BOOL                b_space;
    BOOL                b_match_list;

    b_space = ICLI_IS_(SPACE, (handle->runtime_data.cmd[handle->runtime_data.cmd_len - 1]));
    b_match_list = (handle->runtime_data.match_sort_list != NULL);

    if (b_space && handle->runtime_data.b_cr) {
        if (icli_platform_cmd_authorize(handle,
                                        vtss_icli_exec_full_cmd_str_get(handle),
                                        vtss_icli_exec_highest_cmd_priv_get(handle),
                                        FALSE) == ICLI_RC_ERR_AAA_REJECT) {
            handle->runtime_data.b_cr = FALSE;
        }
    }

_aaa_match_list_restart:
    for (prev_p = NULL, p = handle->runtime_data.match_sort_list; p; prev_p = p, ___NEXT(p)) {
        // get full command from cmd_var
        command = vtss_icli_exec_full_cmd_str_get(handle);

        // append space
        c = command;
        if (vtss_icli_str_len(c)) {
            c += vtss_icli_str_len(c);
            (void)icli_sprintf(c, " ");
            ++c;
        }

        // append next possible word
        (void)icli_sprintf(c, "%s", p->match_node->word);

        // AAA
        switch (icli_platform_cmd_authorize(handle, command, vtss_icli_exec_highest_para_priv_get(p), FALSE)) {
        case ICLI_RC_ERR_AAA_REJECT:
            // check cr
            if ((b_space == FALSE) && handle->runtime_data.b_cr) {
                handle->runtime_data.b_cr = FALSE;
            }

            /* not allow, remove this one */
            if (prev_p) {
                prev_p->next = p->next;
                vtss_icli_exec_parameter_free(p);
                p = prev_p;
            } else {
                handle->runtime_data.match_sort_list = p->next;
                vtss_icli_exec_parameter_free(p);
                goto _aaa_match_list_restart;
            }
            break;

        case ICLI_RC_ERR_AAA_IGNORE:
        case ICLI_RC_OK:
        default:
            break;
        }
    ...
    if (handle->runtime_data.b_cr == FALSE && b_match_list && handle->runtime_data.match_sort_list == NULL) {
        if (handle->runtime_data.exec_type == ICLI_EXEC_TYPE_TAB) {
            vtss_icli_session_str_put(handle, "\n");
        }
        vtss_icli_session_str_put(handle, "% Command authorization failed.\n");
        return FALSE;
    }
...
static void _match_sort_list_visible_chk(
    IN  icli_session_handle_t   *handle
)
{
    icli_parameter_t    *p;
    ...
    while ( p ) {
        if ( vtss_icli_exec_visible_chk(handle, p) ) {
            ...
            vtss_icli_exec_parameter_free( q );
        }
    }
}
#endif
...
)
{
    ...
    _match_sort_list_visible_chk( handle );
#endif

    if (_aaa_match_list(handle) == FALSE) {
        return;
    }
    ...
    if ( keyword ) {
        ...
        case ICLI_VARIABLE_PORT_TYPE:
            if ( p->value.type == ICLI_VARIABLE_PORT_TYPE ) {
                ...
                (void)vtss_icli_str_cpy(w, vtss_icli_variable_port_type_get_name((icli_port_type_t)(handle->runtime_data.tab_port_type)));
            }
            break;
...
)
{
    ...
    if ( handle->runtime_data.last_port_type == ICLI_PORT_TYPE_ALL ) {
        ...
        (void)_port_list_str_get( (icli_port_type_t)(handle->runtime_data.last_port_type), s );
    }
...
)
{
    ...
    if ( handle->runtime_data.b_comment && handle->runtime_data.b_cr ) {
        _session_printf(handle, "    ");
...
static int __cli_io_vprintf(cli_iolayer_t *pIO, const char *fmt, va_list ap)
{
    return vdprintf(pIO->fd, fmt, ap);
}

static void __cli_io_putchar(cli_iolayer_t *pIO, char ch)
{
    write(pIO->fd, &ch, 1);
}

static void __cli_io_puts(cli_iolayer_t *pIO, const char *s)
{
    write(pIO->fd, s, strlen(s));
}

static mesa_rc __cli_io_getch(cli_iolayer_t *pIO, int secs, char *ch)
{
    fd_set fds;
    struct timeval tv;
    ...
    int rc = select(pIO->fd + 1, &fds, NULL, NULL, &tv);
...
static void __cli_io_dummy(cli_iolayer_t *pIO) {}

static i32 add_to_multiline(i32 session_id, const char *s)
{
    i32 rc;
    _ICLI_SEMA_GIVE();
    rc = icli_multiline_process_line(session_id, s);
    _ICLI_SEMA_TAKE();
    return rc;
}
...
)
{
    ...
    if (handle->open_data.client_need_cli_iolayer_in_thread_data == CLIENT_NEED_CLI_IOLAYER_IN_THREAD_DATA_MAGIC) {
        memset(&handle->cli_iolayer, 0, sizeof(handle->cli_iolayer));
        handle->cli_iolayer.fd = handle->open_data.client_fd;
        handle->cli_iolayer.cli_vprintf = __cli_io_vprintf;
        handle->cli_iolayer.cli_putchar = __cli_io_putchar;
        handle->cli_iolayer.cli_puts = __cli_io_puts;
        handle->cli_iolayer.cli_flush = __cli_io_dummy;
        handle->cli_iolayer.cli_init = __cli_io_dummy;
        handle->cli_iolayer.cli_close = __cli_io_dummy;  // lifetime is handled by iCli
        handle->cli_iolayer.cli_getch = __cli_io_getch;
        cli_set_io_handle(&handle->cli_iolayer);
    }
    ...
    handle->runtime_data.thread_id = icli_os_thread_id_get();
    ...
    case ICLI_SESSION_WAY_THREAD_SSH:
        ...
        do_auth = FALSE;
        /* without auth */
        if ( ! icli_platform_has_user_auth() ) {
            handle->runtime_data.privilege = 15;
        }
        break;
    ...
    while ( handle->runtime_data.alive ) {
        ...
        if ( do_auth ) {
            ...
            case ICLI_SESSION_WAY_TELNET:
                ...
                rc = icli_platform_user_auth(handle->open_data.way, NULL, username, password, &handle->runtime_data.privilege, &handle->runtime_data.agent_id);
                if ( rc != ICLI_RC_OK ) {
        ...
        if ( handle->runtime_data.connect_time == 0 ) {
            handle->runtime_data.connect_time = icli_os_current_time_get();
        }
        ...
        (void) vtss_icli_session_mode_get(session_id, &mode);
        ...
        switch ( rc ) {
        case ICLI_RC_OK:
            // If in multiline mode, don't process line as ICLI command, but
            // pass the buffer on to the multiline code and be done.
            if (mode == ICLI_CMD_MODE_MULTILINE) {
                rc = add_to_multiline(session_id, handle->runtime_data.cmd);
                break;
            }
        ...
        case ICLI_RC_ERR_EMPTY:
            // This is fragile: IF the user entered a command that took us into multiline mode,
            // then the below 'if' would lead to two calls total to add_to_multiline. We therefore
            // have to record the mode *prior* to execution of the user command.
            //
            // With that out of the way: Empty lines must be passed to the multiline buffer as well,
            // instead of being skipped, which is what ICLI would normally do.
            //
            // Well, of course, with the exception of the user pressing '^Z' to abort out of all
            // config modes (multiline, global config). In that case _usr_cmd_get() will set the
            // "after_cmd_act(ion)" to indicate that we should jump out of the current mode.
            //
            // Hey, presto!
            if (mode == ICLI_CMD_MODE_MULTILINE &&
                handle->runtime_data.after_cmd_act != ICLI_AFTER_CMD_ACT_GOTO_EXEC_MODE) {
                rc = add_to_multiline(session_id, "");
                break;
            }
        ...
        case ICLI_SESSION_WAY_SSH:
            if ( handle->runtime_data.alive == FALSE ) {
                rc = icli_platform_user_logout(handle);
            }
            break;
...
/*
    ...
    COMMENT
        n/a
*/
void vtss_icli_session_close_by_username(
    IN char     *username
)
{
    u32     i;

    if ( username == NULL || vtss_icli_str_len(username) == 0 ) {
        return;
    }

    for ( i = 0; i < g_max_sessions; ++i ) {
        if ( vtss_icli_str_cmp(username, g_session_handle[i].runtime_data.user_name) == 0 ) {
...
)
{
    ...
    /* check level, -1 because of later ++ */
...
)
{
    ...
    if ( handle->runtime_data.alive ) {
        data->elapsed_time = (icli_os_current_time_get() - handle->runtime_data.connect_time) / 1000;
    } else {
        data->elapsed_time = 0;
    }

    if ( handle->runtime_data.alive ) {
        data->idle_time = (icli_os_current_time_get() - handle->runtime_data.idle_time) / 1000;
    } else {
        data->idle_time = 0;
    }

    data->privilege      = (icli_privilege_t)(handle->runtime_data.privilege);
    data->width          = handle->config_data->width;
    data->lines          = handle->config_data->lines;
    data->wait_time      = handle->config_data->wait_time;
    data->line_mode      = (icli_line_mode_t)(handle->runtime_data.line_mode);
    ...
    data->privileged_level = (icli_privilege_t)(handle->config_data->privileged_level);
#endif
...
)
{
    ...
    if ( handle->runtime_data.b_cmd_copy ) {
        len = 0;
    } else {
...
mesa_rc vtss_icli_session_multiline_data_get(u32 session_id, icli_session_multiline_data_t *const data)
{
    ...
    if (handle) {
        ...
        strncpy(data->buffer, handle->open_data.multiline_data.buffer, ICLI_MULTILINE_BUFFER_MAX_LEN);
        data->delimiter = handle->open_data.multiline_data.delimiter;
        data->callback = handle->open_data.multiline_data.callback;
    } else {
        T_E("Failed to fetch session data for session_id %d.\n", session_id);
        return VTSS_RC_ERROR;
    }
    return VTSS_RC_OK;
}

mesa_rc vtss_icli_session_multiline_data_set(u32 session_id, const icli_session_multiline_data_t *data)
{
    icli_session_handle_t *handle = vtss_icli_session_handle_get(session_id);
    ...
    if (handle) {
        ...
        strncpy(handle->open_data.multiline_data.buffer, data->buffer, ICLI_MULTILINE_BUFFER_MAX_LEN);
        handle->open_data.multiline_data.delimiter = data->delimiter;
        handle->open_data.multiline_data.callback = data->callback;
    } else {
        T_E("Failed to fetch session data for session_id %d.\n", session_id);
        return VTSS_RC_ERROR;
    }
...
void vtss_icli_session_multiline_data_clear(u32 session_id)
{
    icli_session_handle_t *handle = vtss_icli_session_handle_get(session_id);

    if (handle) {
        memset(handle->open_data.multiline_data.buffer, 0, sizeof(handle->open_data.multiline_data.buffer));
        handle->open_data.multiline_data.delimiter = 0;
        handle->open_data.multiline_data.callback = NULL;
    } else {
        T_E("Failed to fetch session data for session_id %d.\n"
            "Thus, the multiline data of the session were not cleared.\n", session_id);
    }
    return;
}
...
{
    u32     i;
    u32     thread_id = icli_os_thread_id_get();
...
/*
    ...
    COMMENT
        n/a
*/
void vtss_icli_session_debug_cmd_allow_set(
    IN  icli_session_handle_t   *handle,
    IN  BOOL                    b_allow
)
{
    ...
    handle->runtime_data.b_debug_cmd_allow = b_allow ? TRUE : FALSE;

    if ( b_allow && handle->runtime_data.b_debug_cmd_allow_second == FALSE ) {
        handle->runtime_data.b_debug_cmd_allow_second = TRUE;
    }
}
...
/*
    ...
    COMMENT
        n/a
*/
BOOL vtss_icli_session_debug_cmd_allow_get(
    IN  icli_session_handle_t   *handle
)
{
    if ( handle == FALSE ) {
    ...
    return (BOOL)(handle->runtime_data.b_debug_cmd_allow);
}
...
/*
    ...
    COMMENT
        n/a
*/
BOOL vtss_icli_session_debug_cmd_allow_first_time_get(
    IN  icli_session_handle_t   *handle
)
{
    if ( handle == FALSE ) {
    ...
    return (BOOL)( handle->runtime_data.b_debug_cmd_allow_second == FALSE );
}
#endif
...
*/
i32 vtss_icli_session_mode_para_get(
    IN  u32                     session_id,
    OUT icli_cmd_mode_t         *mode,
...
)
{
    ...
    /* In syntax checking, we get data from parsing session */
    if (handle->runtime_data.b_in_parsing) {
        // Get current parsing mode
        if (mode) {
            *mode = (icli_cmd_mode_t) handle->runtime_data.current_parsing_mode;
        }

        // Get current parsing mode variable
        if (value) {
            *value = handle->runtime_data.current_parsing_mode_var.value;
        }
    ...
    /* check level */
    if (handle->runtime_data.mode_level >= ICLI_MODE_MAX_LEVEL) {
        return ICLI_RC_ERR_PARAMETER;
    }
    ...
    if (value) {
        for (cmd_var = mode_para->cmd_var; cmd_var && cmd_var->next; ___NEXT(cmd_var));
        if (cmd_var) {
            *value = cmd_var->value;
        } else {
            T_D("Cannot find mode parameter value");
            return ICLI_RC_ERR_PARAMETER;
        }
    }
