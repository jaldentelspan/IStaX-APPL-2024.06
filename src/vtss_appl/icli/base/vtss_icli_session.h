...
typedef struct {
    ...
    icli_parameter_t            current_parsing_mode_var; // For saving the current parsing mode variable
    ...
    */
    ...
    /*
        it is icli_rc_t
        result code returned by command callback after executing 'succesfully'
    */
    i32                         exec_rc;
#endif

#if 1 /* Bugzilla#14129 - remove debug level */
    /* indicate if debug command is allowed or denied */
    u32                         b_debug_cmd_allow;

    /*
        if this allow is the second time for deciding to display warning
        message or not
    */
    u32                         b_debug_cmd_allow_second;

    /*
        indicate if the session is currently setting command privilege or not
        if yes then b_debug_cmd_allow should be ignored
    */
    u32                         b_priv_cmd;
#endif

#if 1 /* Bugzilla#12915 - invalid hidden interface entry command, then back to global-config mode */
    u32                         b_back_to_global;
#endif
...
mesa_rc vtss_icli_session_multiline_data_get(u32 session_id,
                                             icli_session_multiline_data_t *const data);

mesa_rc vtss_icli_session_multiline_data_set(u32 session_id,
                                             const icli_session_multiline_data_t *data);
