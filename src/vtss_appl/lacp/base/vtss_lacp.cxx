...
#define OFFSET_IN(x)  (x + VTSS_PORT_NO_START - 1)
#define OFFSET_OUT(x) (x - VTSS_PORT_NO_START + 1)
...
static int markerpdu_send(vtss_lacp_port_vars_t *pp)
{
    ...
    markerpdu = vtss_os_alloc_xmit(OFFSET_IN(pp->actor_port_number), sizeof(vtss_lacp_markerpdu_t));
    if (markerpdu == NULL) {
    ...
    return vtss_os_xmit(OFFSET_IN(pp->actor_port_number), markerpdu, sizeof(vtss_lacp_markerpdu_t));
}
#endif /* VTSS_LACP_USE_MARKER */
...
{
    if (pp->hw_aggregator) {
        VTSS_LACP_TRACE(VTSS_LACP_TRLVL_DEBUG, ("Disable hwaggr %u port %s\n",
                                                (unsigned)pp->hw_aggregator->aggregator_identifier,
                                                l2port2str((unsigned)pp->actor_port_number)));
#ifdef VTSS_LACP_USE_MARKER
        if (pp->mark_reply_timer_counter == 0) {
            pp->hw_aggregator->last_change = LACP->ticks_since_start;
            vtss_os_clear_hwaggr(OFFSET_IN(pp->hw_aggregator->aggregator_identifier), OFFSET_IN(pp->actor_port_number));
        } else {
            VTSS_LACP_TRACE(VTSS_LACP_TRLVL_DEBUG, ("Disable hwaggr %u port %s canceling outstanding enable\n",
                                                    (unsigned)pp->hw_aggregator->aggregator_identifier,
                                                    l2port2str((unsigned)pp->actor_port_number - 1)));
        ...
        pp->hw_aggregator->last_change = LACP->ticks_since_start;
        vtss_os_clear_hwaggr(OFFSET_IN(pp->hw_aggregator->aggregator_identifier), OFFSET_IN(pp->actor_port_number));
        pp->hw_aggregator->num_of_hw_ports--;
#endif /* VTSS_LACP_USE_MARKER */
...
static void got_mark_reply_or_tmo(vtss_lacp_port_vars_t *pp, vtss_common_bool_t got_reply)
{
    ...
    if (got_reply || (pp->mark_reply_timer_counter && --pp->mark_reply_timer_counter == 0)) {
        VTSS_LACP_TRACE(got_reply == VTSS_COMMON_BOOL_TRUE ? VTSS_LACP_TRLVL_DEBUG : VTSS_LACP_TRLVL_WARNING,
                        ("Delayed enable hwaggr %u port %s: %s\n",
                         (unsigned)pp->hw_aggregator->aggregator_identifier,
                         l2port2str((unsigned)pp->actor_port_number - 1),
                         because[got_reply]));
        pp->mark_reply_timer_counter = 0;
        pp->hw_aggregator->last_change = LACP->ticks_since_start;
        vtss_os_set_hwaggr(OFFSET_IN(pp->hw_aggregator->aggregator_identifier), OFFSET_IN(pp->actor_port_number));
        vtss_os_set_fwdstate(OFFSET_IN(pp->actor_port_number), VTSS_COMMON_FWDSTATE_ENABLED);
    }
}
#endif /* VTSS_LACP_USE_MARKER */
...
#ifdef VTSS_LACP_USE_MARKER
    if (pp->mark_reply_timer_counter == 0) {
        VTSS_LACP_TRACE(VTSS_LACP_TRLVL_DEBUG, ("Enable hwaggr %u port %s timer started\n",
                                                (unsigned)pp->hw_aggregator->aggregator_identifier,
                                                l2port2str((unsigned)pp->actor_port_number - 1)));
        vtss_os_set_fwdstate(OFFSET_IN(pp->actor_port_number), VTSS_COMMON_FWDSTATE_DISABLED); /* Silence while we switch over */
...
#define partnerinfo ((const vtss_lacp_info_t *)lacpdu->partner_info)
    ...
    if (vtss_os_translate_port(OFFSET_IN(UNAL_NET2HOSTS(partnerinfo->port)), 0) != pp->actor_port_number ||
...
//        VTSS_COMMON_MACADDR_CMP(partnerinfo->system_macaddr, pp->port_macaddr.macaddr) ||
    ...
    if (pp->collmaxdelay)
        VTSS_LACP_TRACE(VTSS_LACP_TRLVL_NOISE, ("port %s: coll max delay = %u\n",
                                                l2port2str((unsigned)pp->actor_port_number - 1),
...
#define partnerinfo ((vtss_lacp_info_t *)lacpdu->partner_info)
    ...
    lacpdu = (vtss_lacp_lacpdu_t *)vtss_os_alloc_xmit(OFFSET_IN(pp->actor_port_number), sizeof(vtss_lacp_lacpdu_t));
    if (lacpdu == NULL) {
        return VTSS_COMMON_CC_GENERR;
    }
    ...
    VTSS_COMMON_UNALIGNED_PUT_2B(actorinfo->port, HOST2NETS(vtss_os_translate_port(OFFSET_IN(pp->actor_port_number), 1)));
    ...
    VTSS_LACP_TRACE(VTSS_LACP_TRLVL_NOISE, ("Sent LACPDU on port %s\n",
                                            l2port2str((int)pp->actor_port_number)));
    VTSS_LACP_TRACE(VTSS_LACP_TRLVL_NOISE, ("actor system %s key 0x%p port state = 0x%x %s\n",
                                            vtss_common_str_macaddr((const vtss_common_macaddr_t VTSS_COMMON_PTR_ATTRIB *)actorinfo->system_macaddr),
                                            actorinfo->key, (unsigned)actorinfo->state,
                                            vtss_lacp_str_portstate(actorinfo->state)));
    VTSS_LACP_TRACE(VTSS_LACP_TRLVL_NOISE, ("partner system %s key 0x%p port state = 0x%x %s\n",
                                            vtss_common_str_macaddr((const vtss_common_macaddr_t VTSS_COMMON_PTR_ATTRIB *)partnerinfo->system_macaddr),
                                            partnerinfo->key, (unsigned)partnerinfo->state,
                                            vtss_lacp_str_portstate(partnerinfo->state)));
    pp->stats.lacp_frame_xmits++;
    return vtss_os_xmit(OFFSET_IN(pp->actor_port_number), lacpdu, sizeof(vtss_lacp_lacpdu_t));
#undef partnerinfo
#undef actorinfo
}
...
#define frm_head ((vtss_lacp_frame_header_t *)mpdu->frame_header)
    ...
    case VTSS_LACP_TVLTYPE_MARKER_INFO :
        ...
        mpdu = (vtss_lacp_markerpdu_t *)vtss_os_alloc_xmit(OFFSET_IN(pp->actor_port_number), sizeof(vtss_lacp_markerpdu_t));
        if (mpdu == NULL) {
            return;
        }
        ...
        vtss_os_xmit(OFFSET_IN(pp->actor_port_number), mpdu, sizeof(vtss_lacp_markerpdu_t));
        break;
...
static void rx_stev(vtss_lacp_port_vars_t *pp, const vtss_lacp_lacpdu_t *lacpdu)
{
    ...
    if (pp->sm_rx_state != last_state || lacpdu) {
        VTSS_LACP_TRACE(VTSS_LACP_TRLVL_NOISE, ("Rx Machine: Port=%s, Last State=%u, Curr State=%u smvars 0x%x %s\n",
                                                l2port2str((unsigned)pp->actor_port_number - 1), (unsigned)last_state,
        ...
        case VTSS_LACP_RXSTATE_CURRENT :
            ...
            if (!VTSS_COMMON_MACADDR_CMP(((const vtss_lacp_info_t *)lacpdu->actor_info)->system_macaddr, LACP->system_config.system_id.macaddr)) {
                ...
                VTSS_LACP_TRACE(VTSS_LACP_TRLVL_WARNING, ("An illegal loopback occurred on port %s\n", l2port2str((unsigned)pp->actor_port_number - 1)));
...
static void periodic_stev(vtss_lacp_port_vars_t *pp)
{
    ...
    if (pp->sm_periodic_state != last_state) {
        VTSS_LACP_TRACE(VTSS_LACP_TRLVL_NOISE, ("Periodic Machine: Port=%s, Last State=%u, Curr State=%u\n",
                                                l2port2str((unsigned)pp->actor_port_number - 1), (unsigned)last_state, (unsigned)pp->sm_periodic_state));
        switch (pp->sm_periodic_state) {
        case VTSS_LACP_PERIODICSTATE_NONE :
...
static unsigned int max_port_bundle(vtss_lacp_port_vars_t *pp)
{
    if (pp->port_config.port_key == 0) {
        return VTSS_LACP_MAX_PORTS_IN_AGGR;
    } else {
        return LACP->key_config[pp->port_config.port_key - 1].max_bundle;
    }
}
...
static void port_selection_logic(vtss_lacp_port_vars_t *pp)
{
    ...
    BOOL found = FALSE;
    unsigned int tmp_prio = 0xffff, tmp_port;

    /* if the port is already Selected, do nothing */
    if (pp->sm_vars & VTSS_LACP_PORT_SELECTED) {
        /* If there is not room for this port in the aggr group then put it to Standby */
        if ((pp->aggregator->num_of_hw_ports >= max_port_bundle(pp)) &&
            (pp->sm_mux_state == (VTSS_LACP_MUXSTATE_DETACHED | VTSS_LACP_MUXSTATE_WAITING)) &&
            (pp->partner_oper_port_state == (VTSS_LACP_PORTSTATE_LACP_ACTIVITY | VTSS_LACP_PORTSTATE_LACP_TIMEOUT | VTSS_LACP_PORTSTATE_AGGREGATION))) {
            pp->sm_vars |= VTSS_LACP_PORT_STANDBY;
            pp->sm_vars &= ~VTSS_LACP_PORT_SELECTED;
        }
        return;
        ...
        if (!local_priority(&pp->aggregator->partner_system_priority, &pp->partner_oper_system)) {
            ...
            if (tmp_port != pp->partner_oper_port_number) {
                ...
                if ((pp->aggregator->num_of_ports < max_port_bundle(pp)) &&
                    (pp->partner_oper_port_state & VTSS_LACP_PORTSTATE_SYNCHRONIZATION)) {
                    pp->sm_vars &= ~VTSS_LACP_PORT_STANDBY;
                    pp->sm_vars |= VTSS_LACP_PORT_SELECTED;
            ...
            if (pp->aggregator->num_of_ports < max_port_bundle(pp)) {
                ...
                /* Check if other standby ports have higher prio than this */
                for (next_pp = pp->aggregator->lag_ports; next_pp; next_pp = next_pp->next_lag_port) {
                    if (!(next_pp->sm_vars & VTSS_LACP_PORT_STANDBY)) {
                        continue;
                    }
                    if (pp->port_config.port_prio < next_pp->port_config.port_prio) {
                        continue;
                    } else if (pp->port_config.port_prio > next_pp->port_config.port_prio) {
                ...
                if (tmp_port == pp->actor_port_number) {
                    pp->sm_vars &= ~VTSS_LACP_PORT_STANDBY;
                    pp->sm_vars |= VTSS_LACP_PORT_SELECTED;
                    pp->aggregator->num_of_ports++;
                }
    ...
    for (aggregator = &LACP->aggregators[0]; aggregator < &LACP->aggregators[fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT)]; aggregator++) {
        ...
        if ((aggregator->actor_port_speed == pp->port_speed && /* if all parameters match AND */
            ...
            if (pp->aggregator->num_of_ports >= max_port_bundle(pp)) {
                if ((pp->port_config.port_key > 0) && LACP->key_config[pp->port_config.port_key - 1].revertive) {
                    ...
                    if (tmp_port != pp->actor_port_number) {
                        /* This port has higher priorty than other, put this port to Selected */
                        for (next_pp = aggregator->lag_ports; next_pp; next_pp = next_pp->next_lag_port) {
...
static void tx_stev(vtss_lacp_port_vars_t *pp)
{
    ...
    if (pp->ntt && (pp->sm_vars & VTSS_LACP_PORT_LACP_ENABLED) &&
        ...
        if (pp->sm_tx_timer_counter) {
            ...
            VTSS_LACP_TRACE(VTSS_LACP_TRLVL_DEBUG, ("tx_stev: Port=%s dropped xmit for timelimit\n",
                                                    l2port2str((unsigned)pp->actor_port_number - 1)));
    }
}
    ...
    if (pp->sm_pc_timer_counter && !(--pp->sm_pc_timer_counter)) {
        if (pp->sm_partner_churn_state == VTSS_LACP_CHURNSTATE_MONITOR) {
            if (VTSS_COMMON_MACADDR_CMP(pp->partner_oper_system.macaddr, VTSS_COMMON_ZEROMAC)) {
                VTSS_LACP_TRACE(VTSS_LACP_TRLVL_DEBUG, ("Port %s to partner %s (port %u) is churning\n",
                                                        l2port2str((unsigned)pp->actor_port_number - 1),
...
static volatile vtss_common_port_t next_work_port = 0xffffffff;

void vtss_lacp_more_work(void)
{
    vtss_lacp_port_vars_t *pp;

    if (next_work_port >= fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT)) { /* No more work to do */
        return;
    }
...
static void finish_work(void)
{
    while (next_work_port < fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT)) {
        vtss_lacp_more_work();
    }
}
...
static void lacp_sw_init(void)
{
    ...
    for (aix = 0; aix < fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT); aix++) {
        LACP->aggregators[aix].aggregator_identifier = aix + 1;
        LACP->key_config[aix].revertive = VTSS_COMMON_BOOL_TRUE;
        LACP->key_config[aix].max_bundle = VTSS_LACP_MAX_PORTS_IN_AGGR;
    }
    pp = &LACP->ports[0];
    for (pix = 0; pix < fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT); pp++) {
...
{
    pp->actor_oper_port_key = vtss_os_make_key(OFFSET_IN(pp->actor_port_number), pp->port_config.port_key);
}

static void new_port_state(vtss_lacp_port_vars_t *pp, vtss_common_linkstate_t new_state)
{
    pp->port_up = new_state;
    pp->duplex_mode = vtss_os_get_linkduplex(OFFSET_IN(pp->actor_port_number));
    pp->port_speed = vtss_os_get_linkspeed(OFFSET_IN(pp->actor_port_number));
    new_port_key(pp);
    restart_port(pp);
}
...
static void lacp_hw_init(void)
{
    ...
    pp = &LACP->ports[0];
    for (pix = 1; pix <= fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT); pix++, pp++) {
        vtss_os_get_portmac(OFFSET_IN(pix), &pp->port_macaddr);
        new_port_state(pp, vtss_os_get_linkstate(OFFSET_IN(pix)));
    }
}
...
void vtss_lacp_init(void)
{
    ...
    vtss_printf("LACP Total bytes: %u: %u ports @ %u bytes, %u aggregations @ %u bytes\n",
                (unsigned)sizeof(vtss_lacp_system_vars_t), (unsigned)fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT),
                (unsigned)sizeof(vtss_lacp_port_vars_t), (unsigned)fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT),
...
void vtss_lacp_set_config(const vtss_lacp_system_config_t *system_config)
{
    ...
    for (size_t i = 0; i < LACP->ports.size(); ++i) {
        restart_port(&LACP->ports[i]);
    }
}
                              ...
                              const vtss_lacp_port_config_t *port_config)
{
    ...
    portno = OFFSET_OUT(portno); /* First port is always 1 */
    VTSS_LACP_ASSERT(portno > 0 && portno <= fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT));
    LACP_INIT;
...
void vtss_lacp_set_groupconf(vtss_lacp_key_t key,
                             const vtss_lacp_groupconfig_t *config)
{
    vtss_lacp_port_vars_t *pp;

    VTSS_LACP_ASSERT(key > 0 && key <= VTSS_LACP_MAX_AGGR_);
    LACP->key_config[key - 1].revertive  = config->revertive;
    LACP->key_config[key - 1].max_bundle = config->max_bundle;

    for (size_t i = 0; i < LACP->ports.size(); ++i) {
        pp = &LACP->ports[i];
        if (pp->actor_oper_port_key == key) {
            restart_port(pp);
        }
    }
}

void vtss_lacp_get_groupconf(vtss_lacp_key_t key,
                             vtss_lacp_groupconfig_t *config)
{
    VTSS_LACP_ASSERT(key > 0 && key <= VTSS_LACP_MAX_AGGR_);
    config->revertive = LACP->key_config[key - 1].revertive;
    config->max_bundle = LACP->key_config[key - 1].max_bundle;
}
...
vtss_common_bool_t vtss_lacp_get_aggr_status(vtss_lacp_agid_t aggrid, vtss_lacp_aggregatorstatus_t *aggr_status)
{
    ...
    BOOL not_defaulted = VTSS_COMMON_BOOL_FALSE;

    aggrid = OFFSET_OUT(aggrid); /* First id is always 1 */
    VTSS_LACP_ASSERT(aggrid > 0 && aggrid <= VTSS_LACP_MAX_AGGR_);
    LACP_INIT;
    cap = &LACP->aggregators[aggrid - 1];
    VTSS_LACP_ASSERT(aggrid == cap->aggregator_identifier);
    if (aggr_status) {
        vtss_clear(*aggr_status);
        aggr_status->aggrid = OFFSET_IN(aggrid);
        ...
        for (ccp = cap->lag_ports; ccp; ccp = ccp->next_lag_port) {
            ...
            if (!(ccp->actor_oper_port_state & VTSS_LACP_PORTSTATE_DEFAULTED)) {
                not_defaulted = TRUE;
            }
        }
    }
...
void vtss_lacp_get_port_status(vtss_common_port_t portno, vtss_lacp_portstatus_t *port_status)
{
    ...
    port_status->port_number = portno + VTSS_PORT_NO_START;
    port_status->port_enabled = (ccp->sm_vars & VTSS_LACP_PORT_LACP_ENABLED) ? VTSS_COMMON_BOOL_TRUE : VTSS_COMMON_BOOL_FALSE;
    port_status->port_forwarding = (vtss_common_stpstate_t)(VTSS_COMMON_STPSTATE_FORWARDING == vtss_os_get_stpstate(OFFSET_IN(portno)));
    port_status->actor_oper_port_key = ccp->actor_oper_port_key;
    port_status->actor_admin_port_key = ccp->port_config.port_key;
    port_status->actor_port_aggregator_identifier = ccp->aggregator ? (OFFSET_IN(ccp->aggregator->aggregator_identifier)) : 0;
    ...
    port_status->actor_lacp_state = ccp->actor_oper_port_state;
    port_status->partner_lacp_state = ccp->partner_oper_port_state;
}
...
void vtss_lacp_dump(void)
{
    ...
    for (cap = &LACP->aggregators[0]; cap < &LACP->aggregators[fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT)]; cap++) {
        ...
        vtss_printf("Aggr %u: %u ports (max %u in one group)\n",
                    ...
                    (unsigned)cap->num_of_ports, AGGR_MGMT_LAG_PORTS_MAX_);
        for (cpp = cap->lag_ports; cpp; cpp = cpp->next_lag_port) {
            vtss_printf("  Port %u (LACP:%s): operkey:0x%x fwd:%u ",
                        ...
                        (unsigned)(VTSS_COMMON_STPSTATE_FORWARDING == vtss_os_get_stpstate(OFFSET_IN(cpp->actor_port_number))));
...
void vtss_lacp_linkstate_changed(vtss_common_port_t iportno, vtss_common_linkstate_t new_state)
{
    vtss_lacp_port_vars_t *pp;
    vtss_common_port_t uportno;

    uportno = OFFSET_OUT(iportno); /* First port is always 1 */
    VTSS_LACP_ASSERT(uportno > 0 && uportno <= fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT));
    if (LACP_IS_UNINITED) {
        return;
    }
    pp = &LACP->ports[uportno - 1];

#if defined(VTSS_LACP_DELAYED_PORTMAC_ADDRESS)
    /* If port is UP - get MAC address now! */
    if (new_state == VTSS_COMMON_LINKSTATE_UP) {
        vtss_os_get_portmac(OFFSET_IN(uportno), &pp->port_macaddr);
    }
#endif /* defined(VTSS_LACP_DELAYED_PORTMAC_ADDRESS) */

    VTSS_LACP_TRACE(VTSS_LACP_TRLVL_DEBUG, ("Port %s changed state from %s to %s (duplex %s)\n",
                                            l2port2str((unsigned)iportno),
                                            vtss_common_str_linkstate(pp->port_up),
                                            vtss_common_str_linkstate(new_state),
                                            vtss_common_str_linkduplex(vtss_os_get_linkduplex(OFFSET_IN(uportno)))));
    finish_work();
...
void vtss_lacp_tick(void)
{
    ...
    if (!(LACP->ticks_since_start % VTSS_LACP_TICKS_PER_SEC)) {
        for (pix = 0; pix < fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT); pix++) { /* Allow more transmits */
...
#define frmhead ((const vtss_lacp_frame_header_t *)frame)
    from_port = OFFSET_OUT(from_port); /* First port is always 1 */
    VTSS_LACP_ASSERT(from_port > 0 && from_port <= fast_cap(MEBA_CAP_BOARD_PORT_MAP_COUNT));
    ...
    if (len >= SIZEOF_VTSS_LACP_LACPDU) {
        ...
        case VTSS_LACP_SUBTYPE_LACP :
            VTSS_LACP_TRACE(VTSS_LACP_TRLVL_NOISE, ("Received LACPDU on port %s\n",
                                                    l2port2str((unsigned)from_port)));
            pp->stats.lacp_frame_recvs++;
            rx_stev(pp, (const vtss_lacp_lacpdu_t *)frame);
            tx_stev(pp);
            break;
        case VTSS_LACP_SUBTYPE_MARK :
            VTSS_LACP_TRACE(VTSS_LACP_TRLVL_DEBUG, ("Received MARKERPDU on port %s\n",
                                                    l2port2str((unsigned)from_port)));
            handle_markerframe(pp, (const vtss_lacp_markerpdu_t *)frame);
            break;
        default :
            pp->stats.unknown_frame_recvs++;
            VTSS_LACP_TRACE(VTSS_LACP_TRLVL_DEBUG, ("Ignoring xxPDU frame from port %s with unknown subtype (0x%x)\n",
                                                    l2port2str((unsigned)from_port), (unsigned)frmhead->subtype));
            break;
        }
    } else {
        pp->stats.illegal_frame_recvs++;
        VTSS_LACP_TRACE(VTSS_LACP_TRLVL_DEBUG, ("Ignoring xxPDU frame from port %s with short length (%u)\n",
                                                l2port2str((unsigned)from_port), (unsigned)len));
    }
#undef frmhead
}
