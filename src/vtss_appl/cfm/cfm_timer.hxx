...
#ifndef _CFM_TIMER_HXX_
#define _CFM_TIMER_HXX_

#include "main_types.h"

struct cfm_timer_s;
typedef void (*cfm_timer_callback_t)(struct cfm_timer_s &timer, void *context);

typedef struct cfm_timer_s {
    // Function to call upon timeout
    cfm_timer_callback_t callback;

    // User-defined context passed back in the callback
    void *context;

    // A name for this timer (only used in trace output and 'debug cfm timers')
    const char *name;
    ...
    struct cfm_timer_s *prev;

    // Points to next active timer. NULL if last or this timer is not active.
    struct cfm_timer_s *next;
} cfm_timer_t;

// User operations.
// CFM mutex must be taken upon calls to these functions, and it is already
// taken upon callback.
