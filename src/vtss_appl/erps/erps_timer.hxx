...
#ifndef _ERPS_TIMER_HXX_
#define _ERPS_TIMER_HXX_

#include "main_types.h"

struct erps_timer_s;
typedef void (*erps_timer_callback_t)(struct erps_timer_s &timer, void *context);

typedef struct erps_timer_s {
    // Function to call upon timeout
    erps_timer_callback_t callback;

    // User-defined context passed back in the callback
    void *context;

    // A name for this timer (only used in trace output and 'debug erps timers')
    const char *name;
    ...
    struct erps_timer_s *prev;

    // Points to next active timer. NULL if last or this timer is not active.
    struct erps_timer_s *next;
} erps_timer_t;

// User operations.
// ERPS mutex must be taken upon calls to these functions, and it is already
// taken upon callback.
