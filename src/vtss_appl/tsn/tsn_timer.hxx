...
#ifndef _TSN_TIMER_HXX_
#define _TSN_TIMER_HXX_

#include "main_types.h"

struct tsn_timer_s;
typedef void (*tsn_timer_callback_t)(struct tsn_timer_s &timer, void *context);

typedef struct tsn_timer_s {
    // Function to call upon timeout
    tsn_timer_callback_t callback;

    // User-defined context passed back in the callback
    void *context;

    // A name for this timer (only used in trace output and 'debug show tsn
    // timers')
    const char *name;
    ...
    struct tsn_timer_s *prev;

    // Points to next active timer. NULL if last or this timer is not active.
    struct tsn_timer_s *next;
} tsn_timer_t;

// User operations.
// TSN mutex must be taken upon calls to these functions, and it is already
// taken upon callback.
