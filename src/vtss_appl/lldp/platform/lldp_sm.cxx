...
SM_STEP(TX)
{
    T_RG_PORT(TRACE_GRP_TX, sm->port_no, "sm->initialize:%d, sm->portEnabled:%d, sm->tx.state:%d",
              sm->initialize, sm->portEnabled, sm->tx.state);
    ...
    case TX_LLDP_INITIALIZE:
        ...
        T_NG_PORT(TRACE_GRP_TX, sm->port_no, "sm->adminStatus:%d", sm->adminStatus);

        if ((sm->adminStatus == VTSS_APPL_LLDP_ENABLED_RX_TX) || (sm->adminStatus == VTSS_APPL_LLDP_ENABLED_TX_ONLY)) {
            SM_ENTER(TX, TX_IDLE);
    ...
    case TX_IDLE:
        ...
        if (lldpmed_medFastStart_timer_get(sm->port_no) != 0) {
            ...
            // even though we do a fast start.a
            if (sm->timers.txDelayWhile > 1) {
                T_NG_PORT(TRACE_GRP_TX, (mesa_port_no_t)sm->port_no, "%s", "Forceing DelayWhile to 1");
                sm->timers.txDelayWhile = 1; // Section 11.2.2, TIA1075
            }
        }
#endif

#ifdef VTSS_SW_OPTION_POE
        // Check if PoE has changed in order to transmit new LLDP information within 10 sec. (IEEE 802.3at-2009, section 33.6.2)
        if (lldp_poe_fast_start(sm->port_no, TRUE)) {
            T_IG_PORT(TRACE_GRP_POE, (mesa_port_no_t)sm->port_no, "%s", "Fast PoE start");
            sm->timers.txTTR = 0;
        }
#endif


        T_DG_PORT(TRACE_GRP_TX, (mesa_port_no_t)sm->port_no,
        ...
        if ((sm->adminStatus == VTSS_APPL_LLDP_DISABLED) || (sm->adminStatus == VTSS_APPL_LLDP_ENABLED_RX_ONLY)) {
...
#endif
            T_NG_PORT(TRACE_GRP_TX, (mesa_port_no_t)sm->port_no, "%s", "Next state = TX_INFO_FRAME");
    ...
    case TX_INFO_FRAME:
        T_NG_PORT(TRACE_GRP_TX, sm->port_no, "Entering TX_INFO case, port = %d", (unsigned) sm->port_no);
        /* UCT */
        SM_ENTER(TX, TX_IDLE);
        break;

    default:
        T_DG_PORT(TRACE_GRP_TX, sm->port_no, "Unhandled TX state %u", (unsigned)sm->tx.state);
        break;
    }
}
...
SM_STEP(RX)
{
    ...
    case RX_LLDP_INITIALIZE:
        sm->adminStatus = lldp_os_get_admin_status(sm->port_no);
        if ((sm->adminStatus == VTSS_APPL_LLDP_ENABLED_RX_TX) ||
            (sm->adminStatus == VTSS_APPL_LLDP_ENABLED_RX_ONLY)) {
    ...
    case RX_WAIT_FOR_FRAME:
        ...
        } else if ((sm->adminStatus == VTSS_APPL_LLDP_DISABLED) ||
                   (sm->adminStatus == VTSS_APPL_LLDP_ENABLED_TX_ONLY)) {
            SM_ENTER(RX, RX_LLDP_INITIALIZE);
        }
        break;
...
void lldp_sm_step (lldp_sm_t *sm, BOOL rx_only)
{
    ...
    do {
        ...
        SM_STEP_RUN(RX);
        T_RG_PORT(TRACE_GRP_TX, sm->port_no, "portEnabled:%d, rx_only:%d", sm->portEnabled, rx_only);
...
void lldp_sm_init (lldp_sm_t   *sm, lldp_port_t port)
{
    ...
    T_DG_PORT(TRACE_GRP_CONF, port, "initializing, admin_status:%d", sm->adminStatus);
