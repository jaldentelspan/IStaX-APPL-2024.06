 ...
 * \return error code.
 **/
mesa_rc lldpmed_icli_civic_addr(i32 session_id, BOOL has_country, BOOL has_state, BOOL has_county, BOOL has_city, BOOL has_district,
 ...
 * \return error code.
 **/
mesa_rc lldpmed_icli_fast_start(i32 session, u32 value, BOOL no);
 ...
 * \param has_voice                 [IN] TRUE if policy is a voice policy.
 * \param has_voice_signaling       [IN] TRUE if policy is a voice signaling policy.
 * \param has_guest_voice_signaling [IN] TRUE if policy is a guest voice signaling policy.
 * \param has_guest_voice           [IN] TRUE if policy is a guest voice policy.
 * \param has_softphone_voice       [IN] TRUE if policy is a softphone policy.
 * \param has_video_conferencing    [IN] TRUE if policy is a video conferencing policy.
 * \param has_streaming_video       [IN] TRUE if policy is a streaming video policy.
 * \param has_video_signaling       [IN] TRUE if policy is a video signaling policy.
 ...
 * \return error code.
 **/
mesa_rc lldpmed_icli_media_vlan_policy(i32 session_id, u32 index, BOOL has_voice, BOOL has_voice_signaling, BOOL has_guest_voice_signaling,
 ...
 * \return error code.
 **/
mesa_rc lldpmed_icli_assign_policy(i32 session_id, icli_stack_port_range_t *plist, icli_range_t *policy_list, BOOL no);

/*
 * \brief Function for setting LLDP MED operation mode
 *
 * \param session_id [IN] The session id.
 * \param plist      [IN] List of port to assign the policies to.
 * \has_connectivity [IN] TRUE to set operation mode as connectivity.
 * \has_end_point    [IN] TRUE to set operation mode as end-point.
 * \param no         [IN] TRUE to set datum to the default value.
 * \return error code.
 **/
...
/*
 * \brief Function for getting if PoE is supported
 *
 * \param session_id [IN] The session id.
 * \param ask        [IN] runtime ask
 * \param runtime    [IN] Answer
 **/
