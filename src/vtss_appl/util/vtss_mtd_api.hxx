...
typedef struct {
    ...
    int devno;                  /* mtd device number */
    char dev[64];               /* mtd device name */
    const char *name;		/* Name from /proc/mtd */
 ...
 * \param length [IN] program length bytes.
 *
 * \param pos [IN] program at position.
 *
 * \return VTSS_RC_OK if the operation succeeded.
 *         VTSS_RC_ERROR if the operation failed.
 */
...
// Small helpers for nand/nor architecture support
const char* vtss_mtd_type(const char *mtd_name);
bool vtss_mtd_is_nor(const char *mtd_name);
bool vtss_mtd_rootfs_is_nor();
bool vtss_mtd_is_uboot();
