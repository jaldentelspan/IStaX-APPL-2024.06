...
)
{
    ...
    walk_node = root;
    while ( walk_node ) {
        r = (vtss_avl_tree_cmp_result_t)((*(ptree->cmp_func))(pdata, walk_node->user_data));
        switch ( r ) {
...
)
{
    ...
    *pdata = NULL;
    ...
    while ( *insert_point ) {
        ...
        r = (vtss_avl_tree_cmp_result_t)((*ptree->cmp_func)(pwalknode->user_data, pnewdata));
        switch ( r ) {
        case VTSS_AVL_TREE_CMP_RESULT_A_B_SAME:
            // recover height
...
/**
  ...
  *                           if *pdata is NULL and get is GET_PREV, then get will be GET_LAST
  *                           if *pdata is NULL and get is GET_NEXT, then get will be GET_FIRST
...
)
{
    ...
    case VTSS_AVL_TREE_GET_PREV:
        if ( *pdata == NULL ) {
            get = VTSS_AVL_TREE_GET_LAST;
        }
        break;

    case VTSS_AVL_TREE_GET_NEXT:
        if ( *pdata == NULL ) {
            get = VTSS_AVL_TREE_GET_FIRST;
        }
        break;
