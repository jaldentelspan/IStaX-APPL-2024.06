...
KNOWN_SECTIONS = [KnownSec.new("user exec",                                 0, nil,                          'ICLI_CMD_MODE_EXEC'),
                  ...
                  KnownSec.new("interface tunnel-tp <tunnel_tp_num:uint>", 15, ['tunnel_tp_num'],            'ICLI_CMD_MODE_INTERFACE_TP_TUNNEL'),
...
class IcliGenerator
  ...
  def generate_one_icli_cmd(cmd, privilege, mode, mode_var, no_form, default_form)
    ...
    props = []
    props << 'ICLI_CMD_PROP_GREP'    if (cmd.tokens[0] == 'show') || (cmd.tokens[0] == 'debug')
