...
#ifndef __VTSS_BASICS_EXPOSE_JSON_FUNCTION_EXPORTER_SET_HXX__
#define __VTSS_BASICS_EXPOSE_JSON_FUNCTION_EXPORTER_SET_HXX__
...
struct FunctionExporterSet : public FunctionExporterAbstract {
    FunctionExporterSet(SUBJECT *subject, NamespaceNode *ns, const char *n,
                        const char *d)
    ...
    vtss::json::Result::Code exec(const Request *req, ostreamBuf *os) {
        ...
        mesa_rc rc = args.call_as_set(subject_);
        if (rc != MESA_RC_OK) {
            error_code = rc;
...
#endif  // __VTSS_BASICS_EXPOSE_JSON_FUNCTION_EXPORTER_SET_HXX__
