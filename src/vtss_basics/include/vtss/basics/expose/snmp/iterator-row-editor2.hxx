...
#ifndef __VTSS_BASICS_EXPOSE_SNMP_ITERATOR_ROW_EDITOR2_HXX__
#define __VTSS_BASICS_EXPOSE_SNMP_ITERATOR_ROW_EDITOR2_HXX__
...
struct IteratorRowEditor2 : public IteratorRowEditorBase,
                            public IteratorCommon {
  public:
    typedef typename INTERFACE::P::add_t AddPtr;
    typedef typename INTERFACE::P::def_t SetDefPtr;

    typedef typename Interface2Any<ParamTuple, INTERFACE>::type ParamTuple_;

    IteratorRowEditor2(AddPtr a, SetDefPtr df, ParamTuple_ &v, uint32_t &state)
    ...
    uint32_t get_external_state() {
        return value.template get_value<
                PARAM_ACTION<RowEditorState2>>();
    }

    void set_external_state(uint32_t s) {
        value.template get_value<PARAM_ACTION<RowEditorState2>>() =
                s;
    }
...
#endif  // __VTSS_BASICS_EXPOSE_SNMP_ITERATOR_ROW_EDITOR2_HXX__
